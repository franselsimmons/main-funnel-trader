...
function namespacedShortKey(key, fallback) {
     ...
     if (safe.startsWith('LONG:') || safe.includes(`${SHORT_KEY_PREFIX}LONG:`)) {
         throw new Error('SHORT_DISCORD_KEY_REJECTED_LONG_NAMESPACE');
     }
...
function discordColorForSide(payload = {}) {
    return isShortPayload(payload) ? 0x16a34a : 0x64748b;
}
...
const initialSl = safeNumber(entry.initialSl ?? entry.sl ?? entry.stopLoss, 0);
const activeSl = safeNumber(entry.activeSl ?? entry.currentSl ?? initialSl, initialSl);
const tp = safeNumber(entry.tp ?? entry.takeProfit, 0);
const currentPrice = safeNumber(entry.currentPrice ?? entry.markPrice ?? entry.price, 0);
const exitPrice = safeNumber(entry.exitPrice ?? entry.closedPrice, 0);
const riskDistance = initialSl > entryPrice ? initialSl - entryPrice : 0;
const validShortGeometry = entryPrice > 0 && tp > 0 && initialSl > 0 && tp < entryPrice && entryPrice < initialSl;
const shortGrossR = riskDistance > 0 && exitPrice > 0
? (entryPrice - exitPrice) / riskDistance
: null;
const shortCurrentR = riskDistance > 0 && currentPrice > 0
...
shortTpHit: currentPrice > 0 && tp > 0 ? currentPrice <= tp : false,
shortSlHit: currentPrice > 0 && activeSl > 0 ? currentPrice >= activeSl : false,
shortGrossR,
shortCurrentR,
grossR: Number.isFinite(shortGrossR) ? shortGrossR : safeNumber(entry.grossR, 0),
currentR: Number.isFinite(shortCurrentR) ? shortCurrentR : safeNumber(entry.currentR, 0),
riskGeometryRule: 'SHORT: tp < entry < sl',
tpHitRule: 'SHORT: price <= tp',
slHitRule: 'SHORT: price >= activeSl',
...
'ALLOWED'].includes(upper(payload.currentFitStatus))
    ...
    const fit = getShortCurrentFit(payload);
...
|| null,
    ...
    currentR: payload.shortCurrentR ?? risk.shortCurrentR ?? payload.currentR ??
null,
...
function entrySkipReason(entry = {}) {
    ...
    if (!hasValidShortTradeShape(entry)) return 'ENTRY_INVALID_SHORT_TRADE_SHAPE_SL_ENTRY_TP_REQUIRED';
    ...
    const decision = normalizedEntryDecisionSnapshot(entry);
    if (decision.wouldPublishWithoutTemporal === false) return 'ENTRY_NON_TEMPORAL_DECISION_SNAPSHOT_BLOCKED';
    ...
    embeds: [
         {
             ...
             fields: [
               ...
               field('SL rule', 'SHORT: price >= initialSl', true),
               field('Current fit', shortCurrentFit === null ? 'NA' :
fmtNumber(shortCurrentFit, 2), true),
...
const result = {
     ...transportResult,
     entryPublicationResultVersion: 'SHORT_ENTRY_PUBLICATION_RESULT_V1',
        ...
        embeds: [
             {
                 ...
                 fields: [
                   ...
                   field('Gross R', fmtR(outcome.shortGrossR ?? risk.shortGrossR ??
outcome.grossR), true),
                   field('Current R', fmtR(outcome.shortCurrentR ?? risk.shortCurrentR ??
...
'NA', true)
  ...
  const transportResult = await postDiscord(content);
    const result = {
         ...transportResult,
         exitPublicationResultVersion: 'SHORT_EXIT_PUBLICATION_RESULT_V1',
