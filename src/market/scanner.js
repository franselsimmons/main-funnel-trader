...
'SHORT_SCAN_MARKET_UNIVERSE_AND_MARKET_WEATHER_KEYS_ONLY';
...
throw new Error('SHORT_SCANNER_KEY_REJECTED_LONG_NAMESPACE');
}
...
1,
Math.floor(safeNumber(CONFIG.short?.trade?.positionTimeStopMin ??
...
const shortHit = (
text.includes('MICRO_SHORT_') || text.includes('TRADESIDE=SHORT') || text.includes('TRADE_SIDE=SHORT') ||
text.includes('POSITION_SIDE=SHORT') || text.includes('POSITIONSIDE=SHORT') || text.includes('SIDE=SHORT') ||
text.includes('SIDE=BEAR') || text.includes('DIRECTION=SHORT') || text.includes('DIRECTION=BEAR') ||
text.includes('SIDE=SELL') || text.includes('DIRECTION=SELL') || text.startsWith('SHORT_') ||
text.includes('_SHORT_') || text.endsWith('_SHORT') || text.startsWith('BEAR_') ||
text.includes('_BEAR_') || text.endsWith('_BEAR') || text.startsWith('SELL_') ||
text.includes('_SELL_') || text.endsWith('_SELL')
);
const longHit = (
text.includes('MICRO_LONG_') || text.includes('TRADESIDE=LONG') || text.includes('TRADE_SIDE=LONG') ||
text.includes('POSITION_SIDE=LONG') || text.includes('POSITIONSIDE=LONG') || text.includes('SIDE=LONG') ||
text.includes('SIDE=BULL') || text.includes('DIRECTION=LONG') || text.includes('DIRECTION=BULL') ||
text.includes('SIDE=BUY') || text.includes('DIRECTION=BUY') || text.startsWith('LONG_') ||
text.includes('_LONG_') || text.endsWith('_LONG') || text.startsWith('BULL_') ||
text.includes('_BULL_') || text.endsWith('_BULL') || text.startsWith('BUY_') ||
text.includes('_BUY_') || text.endsWith('_BUY')
);
if (shortHit && !longHit) return TARGET_TRADE_SIDE;
if (longHit && !shortHit) return OPPOSITE_TRADE_SIDE;
if (shortHit && longHit) {
if (text.includes('TRADE_SIDE=SHORT') || text.includes('TRADESIDE=SHORT') || text.includes('MICRO_SHORT_')) return TARGET_TRADE_SIDE;
if (text.includes('TRADE_SIDE=LONG') || text.includes('TRADESIDE=LONG') || text.includes('MICRO_LONG_')) return OPPOSITE_TRADE_SIDE;
}
return 'UNKNOWN';
}
...
const btc = upper(btcState);
const bearishBtc = ['BEARISH', 'STRONG_BEARISH', 'BEAR', 'STRONG_BEAR', 'DOWN'].includes(btc);
const bullishBtc = ['BULLISH', 'STRONG_BULLISH', 'BULL', 'STRONG_BULL', 'UP'].includes(btc);
if (ch1 < 0 || ch24 < 0 || bearishBtc) return TARGET_SCANNER_SIDE;
if (ch1 > 0 && ch24 > 0 && bullishBtc) return 'long_disabled';
return 'unknown';
}
...
const fakeState = fakeBreakdown
? 'FAKE_BREAKDOWN'
: fakeBreakdownRisk
? 'FAKE_RISK'
: 'FAKE_CLEAN';
const breakdown = safeToken(breakdownType, 'NO_BREAKDOWN');
...
scannerFingerprintVersion: 'short_scanner_v5_metadata_only_market_universe',
scannerFingerprintRole: 'METADATA_ONLY',
scannerFingerprintsMetadataOnly: true,
scannerFingerprintsUsedAsLearningFamily: false,
...
if (fake?.breakdownType === 'VALID_BREAKDOWN') return 'SHORT_VALID_BREAKDOWN';
...
breakoutType: 'NONE',
fakeBreakdown: Boolean(fake.fakeBreakdown),
fakeBreakdownRisk: Boolean(fake.fakeBreakdownRisk),
fakeBreakdownReason: fake.fakeBreakdownReason || null,
breakdownType: fake.breakdownType || 'UNKNOWN',
breakdownValid: Boolean(fake.breakdownValid),
shortContinuation: Boolean(fake.shortContinuation),
...
scannerBucket: fake.scannerBucket || null,
setupTypeHint: fake.setupTypeHint || null,
regimeBucketHint: fake.regimeBucketHint || null,
confirmationProfileHint: fake.confirmationProfileHint || null
};
}
...
const quoteVolume24h = Math.max(0, safeNumber(ticker.quoteVolume24h, 0));
const downsideScore = Math.max(0, -change24h) * 120;
const volumeScore = Math.log10(Math.max(1, quoteVolume24h));
return downsideScore + volumeScore;
}
...
const ch1 = safeNumber(change1h, 0);
const ch24 = safeNumber(change24h, 0);
const down1h = Math.max(0, -ch1);
const down24h = Math.max(0, -ch24);
const passesMoveFilter = down1h >= minAbsChange1h() || down24h >= minAbsChange24h();
...
const hardBlockedByDirection = blockNoDirectionEnabled() && (!hasDirectionalSide || !fallingMove || risingMove);
const hardBlockedByMove = blockSmallMoveEnabled() && !passesMoveFilter;
const hardBlockedByFake = blockFakeBreakoutEnabled() && Boolean(fake.fakeBreakdown);
const hardBlocked = hardBlockedByDirection || hardBlockedByMove || hardBlockedByFake;
const scannerGatePassed = !hardBlocked && fallingMove && !risingMove && passesMoveFilter && hasDirectionalSide && !fake.fakeBreakdown;
const analyzeEligible = !hardBlocked && fallingMove && !risingMove && hasDirectionalSide;
...
cryptoBreadthOnly: snapshot.cryptoBreadthOnly === true,
nonCryptoInstrumentsExcluded: snapshot.nonCryptoInstrumentsExcluded === true,
...
cryptoBreadthOnly: true,
nonCryptoInstrumentsExcluded: true,
marketWeatherInput: true,
...
const snapshotId = String(options.snapshotIdOverride || options.snapshotId || randomId('scan_short'));
...
historicalReplayProviderVersion: historicalMode ? 'SHORT_SCANNER_HISTORICAL_PROVIDER_STORAGE_FREE_V2' : null,
...sideFlags(),
...scopeFlags(),
