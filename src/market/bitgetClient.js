...
'SHORT_ONLY_VIRTUAL_75_TRUE_MICRO_MARKET_DATA_V1'
},
...
const baseDelayMs = safeNumber((CONFIG.short?.bitget || CONFIG.bitget ||
{})?.retryDelayMs, 250);
...
const bids = parseBookSide(root?.bids);
const asks = parseBookSide(root?.asks);
const bestBid = safeNumber(bids[0]?.[0], 0);
const bestAsk = safeNumber(asks[0]?.[0], 0);
const mid = bestBid > 0 && bestAsk > 0
? (bestBid + bestAsk) / 2
: safeNumber(root?.mid ?? root?.price ?? root?.last, 0);
