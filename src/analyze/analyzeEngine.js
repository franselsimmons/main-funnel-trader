...
function namespacedShortAnalyzeKey(key, fallback = '') {
    let raw = String(key || fallback || '').trim();
    if (!raw) throw new Error('SHORT_ANALYZE_KEY_REQUIRED');
    if (raw.startsWith(SHORT_KEY_PREFIX)) return raw;
    if (raw.startsWith('LONG:') || raw.includes(`${SHORT_KEY_PREFIX}LONG:`)) {
        throw new Error('SHORT_ANALYZE_KEY_REJECTED_LONG_NAMESPACE');
    }
...
function resolveAnalyzeKeyFunction(name) {
    const shortFn = KEYS.short?.analyze?.[name];
    const genericFn = KEYS.analyze?.[name];
    if (typeof shortFn === 'function') return shortFn;
...
const ANALYZE_KEYS = Object.freeze({
    obsLast: (...args) => namespacedShortAnalyzeKey(
         resolveAnalyzeKeyFunction('obsLast')(...args)
    ),
    weekMicros: (...args) => namespacedShortAnalyzeKey(
         resolveAnalyzeKeyFunction('weekMicros')(...args)
    ),
    weekParents: (...args) => namespacedShortAnalyzeKey(
         resolveAnalyzeKeyFunction('weekParents')(...args)
    ),
    microStats: (...args) => namespacedShortAnalyzeKey(
         resolveAnalyzeKeyFunction('microStats')(...args)
    ),
    parentStats: (...args) => namespacedShortAnalyzeKey(
         resolveAnalyzeKeyFunction('parentStats')(...args)
    ),
    microOutcomes: (...args) => namespacedShortAnalyzeKey(
...
const WRITE_SCOPE =
    KEYS.scopes?.SHORT_ANALYZE_PARTIAL ||
    KEYS.scopes?.ANALYZE_SHORT_PARTIAL ||
    null;
...
export async function getMicroOutcomeHistory(microFamilyId) {
    ...
    if (!normalized || !validLearningId(normalized) || !isSelectableShortTrueMicroFamilyId(normalized)) {
        return [];
    }
    const redis = getDurableRedis();
