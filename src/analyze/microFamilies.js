...
const LONG_TOKENS = new Set([
  'LONG', 'BULL', 'BULLISH', 'BUY', 'BID', 'UP', 'UPSIDE', 'GREEN'
]);
...
function coarseFlow(flow) {
    ...
    if (['TREND', 'IMPULSE', 'DUMP', 'SELL_IMPULSE', 'BEAR_FLOW'].includes(f)) return
'TREND';
...
function detectFlowAligned(metrics = {}) {
    ...
    return Boolean(
        metrics.flowAlign || metrics.flowAligned || metrics.momentumAlign ||
        metrics.askFlowAlign || metrics.bearFlow || metrics.sellFlow
    ) || flow === 'TREND' || flow === 'BUILDING' || obRelation === 'WITH' ||
        btcRel === 'BTC_WITH' ||
...
function detectHardContra(metrics = {}) {
    ...
    const btcRel = btcRelation(TARGET_TRADE_SIDE, metrics.btcState);
    return Boolean(
        metrics.fakeBreakout || metrics.fakeBreakoutRisk || metrics.avoidShort ||
        metrics.doNotShort || metrics.weakContra || metrics.contraSignal ||
        metrics.bullishDivergence
    ) || obRelation === 'AGAINST' || btcRel === 'BTC_AGAINST' ||
        hasLongSignal(metrics.flow) || hasLongSignal(metrics.flowDirection) ||
        hasLongSignal(metrics.orderFlow) || hasLongSignal(metrics.marketFlow) ||
