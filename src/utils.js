...
export const WEEKEND_MODE = 'RUNTIME_CONTROLLED';
export const SESSION_MODE = 'RUNTIME_CONTROLLED';
...
const rawTs = source.entryTs ?? source.entryCreatedAt ?? source.openedAt ??
source.createdAt ?? source.contextTs ?? fallbackTs;
const context = buildTemporalContextUtc(rawTs);
const entryIsWeekend = source.entryIsWeekend ?? source.isWeekend ??
context.isWeekend;
return {
temporalContextVersion: source.temporalContextVersion ||
context.temporalContextVersion,
temporalPolicyVersion: source.temporalPolicyVersion ||
context.temporalPolicyVersion,
temporalGenerationSchemaVersion: source.temporalGenerationSchemaVersion ||
context.temporalGenerationSchemaVersion,
weekendPolicyVersion: source.weekendPolicyVersion ||
context.weekendPolicyVersion,
sessionPolicyVersion: source.sessionPolicyVersion ||
context.sessionPolicyVersion,
weekendMode: WEEKEND_MODE,
sessionMode: SESSION_MODE,
entryTs: normalizeTemporalTimestamp(rawTs),
entryHourUtc: source.entryHourUtc ?? source.hourUtc ?? context.hourUtc,
entryDayOfWeekUtc: source.entryDayOfWeekUtc || source.dayOfWeekUtc ||
context.dayOfWeekUtc,
...
? [...source.sessionTags]
: [...context.sessionTags],
entrySessionBucket: source.entrySessionBucket || source.primarySessionBucket
|| context.primarySessionBucket,
entrySessionOverlap: source.entrySessionOverlap ?? source.sessionOverlap ??
context.sessionOverlap,
...
return {
temporalContextVersion: context.temporalContextVersion,
temporalPolicyVersion: context.temporalPolicyVersion,
temporalGenerationSchemaVersion: context.temporalGenerationSchemaVersion,
weekendPolicyVersion: context.weekendPolicyVersion,
sessionPolicyVersion: context.sessionPolicyVersion,
weekendMode: context.weekendMode,
...
export function deriveShortLearningStatus(completedValue, avgRValue = 0) {
...
// In deze SHORT-root is LONG expliciet uitgeschakeld.
...
// In deze SHORT-root betekent dit: non-SHORT blokkeren.
...
export function buildShortChildTrueMicroFamilyId(setup, regime,
confirmationProfile) {
...
if (!raw) return raw;
if (isSelectableShortTrueMicroFamilyId(raw) ||
isShortParentTrueMicroFamilyId(raw)) {
return upper(raw);
}
const tokens = [
row.symbol,
...
if (ch24 > 1.5 && ch1 > 0.5) return 'STRONG_BULLISH';
if (ch24 < -1.5 && ch1 < -0.5) return 'STRONG_BEARISH';
