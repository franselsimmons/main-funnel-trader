...
return {
minRR: safeNumber(CONFIG.short?.trade?.minRR ?? CONFIG.trade?.shortMinRR ??

CONFIG.trade?.minRR, 0.5),
defaultRR: safeNumber(CONFIG.short?.trade?.defaultRR ??
CONFIG.trade?.shortDefaultRR ?? CONFIG.trade?.defaultRR, 1.5),
maxSpreadPct: safeNumber(CONFIG.short?.trade?.maxSpreadPct ??
CONFIG.trade?.shortMaxSpreadPct ?? CONFIG.trade?.maxSpreadPct, 0.015),
minRiskPct: safeNumber(CONFIG.short?.trade?.minRiskPct ??
CONFIG.trade?.shortMinRiskPct ?? CONFIG.trade?.minRiskPct, 0.004),
maxRiskPct: safeNumber(CONFIG.short?.trade?.maxRiskPct ??
CONFIG.trade?.shortMaxRiskPct ?? CONFIG.trade?.maxRiskPct, 0.025),
fallbackRiskPct: safeNumber(CONFIG.short?.trade?.fallbackRiskPct ??
CONFIG.trade?.shortFallbackRiskPct ?? CONFIG.trade?.fallbackRiskPct, 0.005),
atrRiskMult: safeNumber(CONFIG.short?.trade?.atrRiskMult ??
CONFIG.trade?.shortAtrRiskMult ?? CONFIG.trade?.atrRiskMult, 1.2),
spreadRiskMult: safeNumber(CONFIG.short?.trade?.spreadRiskMult ??
CONFIG.trade?.shortSpreadRiskMult ?? CONFIG.trade?.spreadRiskMult, 5),
...
if (hasLongSignal(reason) && !hasShortSignal(reason)) return
OPPOSITE_TRADE_SIDE;
...
if (tradeSide === TARGET_TRADE_SIDE && ['BEARISH', 'STRONG_BEAR', 'BEAR',
'DOWN'].includes(btc)) {
return 'BTC_WITH';
}
