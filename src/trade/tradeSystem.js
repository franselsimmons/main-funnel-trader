...
const TRADE_RUN_ROUTE_VERSION =
  'SHORT_API_TRADE_RUN_V15_ROUTE_TIMING_DEADLINE_FIX';
...
function routeTimeoutResponse({
  ...
  lock,
  routeTimings = {},
  preTradeDurationMs = 0,
  tradeDeadlineAt = null,
  deadlineBudgetAtStartMs = 0
}) {
  return {
    ok: false,
    tradeOk: false,
...
export default async function handler(
  ...
  function markPhase(name) {
    routeTimings[name] = Date.now() - routeStartedAt;
  }

  markPhase('ROUTE_START');
  ...
  try {
    ...
    // ===== Deadline berekenen vlak voor tradeSystem =====
    const tradeStartedAt = now();
    markPhase('TRADE_CALL_START');
    ...
    // maxRuntimeMs is ingesteld in runOptions, maar we kunnen het hier ook gebruiken
    const tradeMaxRuntimeMs = DEFAULT_TRADE_RUNTIME_MS; // of uit runOptions halen

    const tradeDeadlineAt = Math.min(
      tradeStartedAt + tradeMaxRuntimeMs,
      absoluteRouteDeadlineAt
    );

    const remainingTradeBudgetMs =
      tradeDeadlineAt - Date.now();

    if (remainingTradeBudgetMs < 1000) {
      // Niet genoeg tijd meer, geef een fout terug
      return res.status(200).json({
        ok: false,
        skipped: true,
        reason: 'INSUFFICIENT_ROUTE_BUDGET_BEFORE_TRADE_SYSTEM',
        remainingTradeBudgetMs,
        preTradeDurationMs: Date.now() - routeStartedAt,
        routeTimings,
        tradeDeadlineAt,
        tradeStartedAt
      });
    ...
    // Voeg tradeStartedAt toe voor debug
    runOptions.tradeStartedAt = tradeStartedAt;
    ...
    // Voeg routeTimings en preTradeDurationMs toe aan response
    const preTradeDurationMs =
      tradeStartedAt - routeStartedAt;
    ...
    return res
      ...
      .json({
        ...
        // run: debug ? payload : undefined, // verwijderd om dubbele payload te voorkomen

        ...payload,

        routeTimings,
        preTradeDurationMs,
        tradeDeadlineAt: runOptions.deadlineAt,
        tradeStartedAt: runOptions.tradeStartedAt,
        deadlineBudgetAtStartMs: remainingTradeBudgetMs,
