...
// Volledig bestand met alle helpers, deadline-safe wrapper, logging en fixes.
// ReferenceError 'deadline is not defined' opgelost.
...
/*
 * De route mag langer bestaan, maar tradeSystem stopt bewust eerder.
 * Zo blijft tijd over voor Redis, lock-release en JSON-serialisatie.
 */
const DEFAULT_MAX_RUNTIME_MS = 17_500;
const HARD_MAX_RUNTIME_MS = 21_000;
const DEFAULT_HARD_RETURN_RESERVE_MS = 3_500;
...
// Globale variabelen voor phase logging (worden in runTradeSystem geïnitialiseerd)
...
function timeoutPayload(label, timeoutMs) {
  return new Promise((resolve) => {
    ...
    }, Math.max(1, int(timeoutMs, 1, 1, 60_000)));
...
function timeoutInsideDeadline(ms, deadline, reserveMs = DEFAULT_PHASE_RESERVE_MS, minMs = 80) {
  ...
  return Math.max(minMs, Math.min(int(ms, minMs, minMs, 60_000), budget));
}

async function withDeadline(promiseFactory, {
  deadline,
  timeoutMs,
  reserveMs = DEFAULT_PHASE_RESERVE_MS,
  label,
  fallback
}) {
  ...
  if (ms <= 0) {
    return typeof fallback === 'function'
      ? fallback({ __deadline: true, label })
      : fallback;
  }
  let timer;
  try {
...
async function readSnapshotCursor(snapshotId, cfg) {
  ...
  return Math.max(0, int(value.nextIndex ?? value.cursor ?? value.index, 0, 0, 100_000));
}

async function persistSnapshotCursor({
  snapshotId,
  nextIndex,
  totalCandidates,
  complete,
  ...
  return {
    ...
    maxEntriesPerRun: int(
      first(
        ...
        CONFIG.short?.trade?.maxEntriesPerRun
      ),
    ...
    selectionTimeoutMs: int(
      first(
        ...
        CONFIG.short?.trade?.selectionTimeoutMs
      ),
    ...
    analyzeImportTimeoutMs: int(
      first(
        options.analyzeImportTimeoutMs,
        CONFIG.short?.trade?.analyzeImportTimeoutMs
      ),
    ...
    positionImportTimeoutMs: int(
      first(
        options.positionImportTimeoutMs,
        CONFIG.short?.trade?.positionImportTimeoutMs
      ),
    ...
    discordImportTimeoutMs: int(
      first(
        options.discordImportTimeoutMs,
        CONFIG.short?.trade?.discordImportTimeoutMs
      ),
    ...
    sizingImportTimeoutMs: int(
      first(
        options.sizingImportTimeoutMs,
        CONFIG.short?.trade?.sizingImportTimeoutMs
      ),
...
function normalizeBaseSymbolSafe(value) {
  if (typeof Utils.normalizeBaseSymbol === 'function') {
    try { return Utils.normalizeBaseSymbol(value); } catch { /* fallback */ }
  }
  return String(value || '')
    .trim()
    .toUpperCase()
...
function normalizeContractSymbolSafe(value) {
  if (typeof Utils.normalizeContractSymbol === 'function') {
    try { return Utils.normalizeContractSymbol(value); } catch { /* fallback */ }
  }
...
function randomIdSafe(prefix = 'id') {
  if (typeof Utils.randomId === 'function') {
    try { return Utils.randomId(prefix); } catch { /* fallback */ }
  }
...
function hasCompletePositiveGeometry(row = {}) {
  ...
  return {
    ...
    sl,
    tp,
    valid: complete && tp < entry && entry < sl
  };
}
...
async function saveVirtualPositionFast(
  entry,
  cfg,
  deadline,
  runtimeWarnings,
...
async function safeRiskFractionForEntry(
  input = {},
  sizing = sizingConfig(),
  deadline,
  cfg,
...
async function getActiveRotationDirect(cfg, runtimeWarnings, deadline, options = {}) {
  ...
  if (deadlineExceeded(deadline, cfg.hardReturnReserveMs + 1_200)) {
...
async function maybeSendDiscordEntryAlert(
  entry = {},
  cfg = tradeConfig(),
  deadline = null,
  runtimeWarnings = [],
...
export async function runTradeSystem(options = {}) {
  ...
  try {
    ...
    // ----- DEADLINE WORDT HIER GEDECLAREERD -----
    const deadline = createDeadline(startedAt, cfg, options);

    // ----- recordPhase WORDT HIER GEDEFINIEERD (na deadline) -----
    const recordPhase = (name, extra = {}) => {
      recordTradeSystemPhase(name, extra, runId, startedAt, deadline);
    };

    // Nu kunnen we recordPhase veilig aanroepen
    recordPhase('TRADE_START');

    // Controleer of de deadline al verstreken is
