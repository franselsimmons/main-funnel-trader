...
const TRADE_SYSTEM_VERSION = 'SHORT_TRADE_SYSTEM_V6_ABORT_DEADLINE_SAFE';
...
// ===== NIEUWE HELPERS VOOR ABORT EN DEADLINE =====
...
function operationTimeoutMs(options = {}, configuredMs, reserveMs = 100) {
  const deadline = Number.isFinite(options.deadlineAt) ? options.deadlineAt : Infinity;
  const nowMs = now();
  const budget = deadline - nowMs - reserveMs;
  if (budget <= 0) return 0;
  return Math.max(1, Math.min(configuredMs, budget));
}
...
function createDeadline(startedAt, cfg, options = {}) {
  ...
  // Gebruik de externe deadline als die korter is
...
function createDeadline(startedAt, cfg) {
  const maxRuntimeMs = int(
    cfg.maxRuntimeMs,
    DEFAULT_MAX_RUNTIME_MS,
    8000,
...
function snapshotCursorKey(snapshotId) {
  return `${SHORT_KEY_PREFIX}TRADE:SNAPSHOT:CURSOR:${hashText(
    snapshotId,
    16
  )}`;
}
...
async function deleteKeySafe(redis, key) {
  if (!redis || !key || typeof redis.del !== 'function') {
    return false;
  }
  try {
    await redis.del(key);
...
async function readSnapshotCursor(snapshotId, cfg) {
  ...
  const value = await withTimeout(
    getJsonSafe(
      safeGetDurableRedis(),
      snapshotCursorKey(snapshotId),
      null
    ),
    Math.min(220, cfg.redisWriteTimeoutMs),
    'READ_SNAPSHOT_CURSOR'
  );
  if (isTimeoutResult(value) || !value) {
    return 0;
  }
...
async function persistSnapshotCursor({
  ...
  // Controleer abort/deadline voordat we schrijven
  try {
    throwIfAborted(options);
  } catch {
    // Als we geaborteerd zijn, overslaan (cursor blijft staan voor volgende run)
    return false;
  }
  const redis = safeGetDurableRedis();
  const key = snapshotCursorKey(snapshotId);
  if (complete) {
    return deleteKeySafe(redis, key);
  }
    ...
    for (const key of directKeys) {
      ...
      if (parsed.known) {
        return {
          ...parsed,
          source,
          sourceReason: 'KNOWN_MARKET_WEATHER_KEY'
        };
      }
    }
  }
  ...
  for (const source of sources) {
    ...
    if (parsed.known) {
      return {
        ...parsed,
        source,
        sourceReason: 'KNOWN_MARKET_WEATHER_FIELDS'
      };
    }
  }
...
function hasCompletePositiveGeometry(row = {}) {
  const entry = finiteOrNull(
    first(row.entry, row.entryPrice)
  );
  const sl = finiteOrNull(
    first(
      row.sl,
      row.initialSl,
      row.stopLoss
...
function policyBlockedGate(row = {}) {
  ...
  if (geometry.complete && !geometry.valid) {
    reasons.push(
      'INVALID_SHORT_GEOMETRY_TP_LT_ENTRY_LT_SL_REQUIRED'
    );
  }
...
async function lazyImportPositionSizing(deadline, cfg, warnings) {
  ...
  if (!result) {
    warnings.push(
      'POSITION_SIZING_IMPORT_TIMEOUT_OR_FAILED_RISK_ZERO'
    );
    return null;
  }
...
async function lazyImportDiscord(deadline, cfg, warnings) {
  ...
  if (!result) {
    warnings.push(
      'DISCORD_IMPORT_TIMEOUT_OR_FAILED_ALERT_SKIPPED'
    );
    return null;
  }
...
async function getActiveRotationDirect(
  cfg,
  runtimeWarnings,
  deadline,
  options = {}
  ...
  if (
    deadlineExceeded(
      deadline,
      cfg.hardReturnReserveMs + 1_200
    )
  ) {
    runtimeWarnings.push(
      'DIRECT_SELECTION_READ_SKIPPED_DEADLINE_NEAR'
    );
    return null;
  }
  const durableRedis = safeGetDurableRedis();
  for (const key of activeSelectionKeys()) {
    ...
    if (!result) continue;
    runtimeWarnings.push(
      `DIRECT_ACTIVE_SELECTION_READ_FROM:${key}`
    );
    return {
      ...(typeof result === 'object' ? result : {}),
      directSelectionKey: key,
      directSelectionReaderVersion:
        DIRECT_SELECTION_READER_VERSION
    };
  }
  runtimeWarnings.push(
    'DIRECT_ACTIVE_SELECTION_EMPTY'
  );
  return null;
}
...
async function analyzeCandidatesBatchSafe(liveRows, options, cfg, runtimeWarnings, deadline, runOptions = {}) {
  ...
  if (!AnalyzeEngine) {
    ...
    return {
      rows: liveRows,
      analyzeError: 'ANALYZE_ENGINE_UNAVAILABLE',
  ...
  if (deadlineExceeded(deadline, cfg.hardReturnReserveMs + 500)) {
    ...
    return {
      rows: liveRows,
      analyzeError: 'ANALYZE_SKIPPED_DEADLINE_NEAR',
      analyzeFallbackUsed: true
    };
  }
  ...
  if (typeof AnalyzeEngine.analyzeCandidatesBatch !== 'function') {
    ...
    return {
      rows: liveRows,
      analyzeError: 'ANALYZE_CANDIDATES_FUNCTION_MISSING',
      analyzeFallbackUsed: true
    };
  }
...
export async function runTradeSystem(options = {}) {
  ...
  try {
    ...
    // Controleer abort/deadline bij start
    throwIfAborted(options);
    ...
    let openPositions = await loadOpenPositionsFast(cfg, runtimeWarnings, deadline, options);
    throwIfAborted(options);
    ...
    } else if (!deadlineExceeded(deadline, cfg.hardReturnReserveMs + 700)) {
    ...
    const storedCursor = forceProcessSnapshot
      ? 0
      : await readSnapshotCursor(
          snapshot.snapshotId,
          cfg
        );
    throwIfAborted(options);
    const chunkStart = Math.min(
      storedCursor,
      allTargetCandidates.length
    );
    const chunkEndExclusive = Math.min(
      allTargetCandidates.length,
      chunkStart + cfg.candidateChunkSize
    );
    const chunkCandidates = allTargetCandidates.slice(
      chunkStart,
      chunkEndExclusive
    );
    const chunk = {
      ...
      complete:
        chunkEndExclusive >= allTargetCandidates.length
    };
    runtimeWarnings.push(
      `SNAPSHOT_CHUNK:${chunk.start}-${chunk.endExclusive}/${chunk.total}`
    );
    ...
    for (let index = 0; index < candidates.length; index++) {
      // Controleer abort/deadline
      throwIfAborted(options);
      // Geef event-loop vrij elke 3 iteraties
      ...
      processed.push(await safeProcessCandidate(candidates[index], marketContext));
    }
    ...
    for (const rawRow of analyzedRows) {
      ...
      // Controleer abort/deadline voor elke kandidaat
      throwIfAborted(options);
      // Geef event-loop vrij elke 3 iteraties
      if (counters.entryLoopAttempts > 0 && counters.entryLoopAttempts % 3 === 0) {
        await yieldToEventLoop();
        throwIfAborted(options);
      }
      ...
      if (discordAlertEligible) {
        counters.discordAlertEligibleRows += 1;
      }
      ...
      try {
        const saveResult = await saveVirtualPositionFast(
          entry,
          cfg,
          deadline,
          runtimeWarnings,
        ...
        const discordResult = await maybeSendDiscordEntryAlert(
          entry,
          cfg,
          deadline,
          runtimeWarnings,
    ...
    // Cursor-persistentie alleen als niet geaborteerd en er tijd is
    let cursorPersisted = false;
    try {
      throwIfAborted(options);
      const canAdvanceCursor =
        analyzeResult.analyzeFallbackUsed !== true &&
        processed.length === candidates.length;
      const effectiveNextIndex = canAdvanceCursor
        ? chunk.nextIndex
        : chunk.start;
      const effectiveChunkComplete =
        canAdvanceCursor &&
        effectiveNextIndex >= chunk.total;
      ...
      if (!cursorPersisted) {
        runtimeWarnings.push(
          'SNAPSHOT_CURSOR_PERSIST_FAILED_OR_TIMEOUT'
        );
      }
      if (!effectiveChunkComplete) {
        runtimeWarnings.push(
          `SNAPSHOT_PARTIAL_NEXT_CRON_CONTINUES_AT:${effectiveNextIndex}`
        );
      }
      // Update resultaten voor de response
    ...
    const baseResult = {
      ...
      snapshotChunkNextIndex: chunk.nextIndex, // default, wordt later overschreven
      snapshotChunkComplete: chunk.complete,   // default, wordt later overschreven
