...
const ENTRY_MARKET_WEATHER_CAPTURE_VERSION = 'SHORT_ENTRY_MARKET_WEATHER_CAPTURE_V4_CURRENT_CONFIRMED_SYNC_NO_STALE_OUTPUT';
const MARKET_WEATHER_FEATURE_FLAGS_VERSION = 'SHORT_MARKET_WEATHER_FEATURE_FLAGS_V2_CURRENT_CONFIRMED_SYNC';
const MARKET_WEATHER_AGGREGATION_VERSION = 'SHORT_MARKET_WEATHER_AGGREGATION_V1_REGIME_REGIMETREND';

const MICRO_MICRO_VERSION = 'SHORT_PARENT_15_MICRO_75_MICRO_MICRO_ONLY_SELECTION_V4_WEATHER_AWARE_IMPORT_SAFE';
const RISK_PLAN_VERSION = 'SHORT_ADAPTIVE_RR_TP_SL_V3_WEATHER_AWARE_PLAYBOOK';
const COST_MODEL_VERSION = 'POSITION_ENGINE_SHORT_NET_COST_V16_MARKET_WEATHER_PRESERVED_EMPIRICAL_VETO';
const MEASUREMENT_FIX_VERSION = 'SHORT_MEASUREMENT_FIX_CANDLE_FIRST_TOUCH_MICRO_MICRO_V2_WEATHER_PRESERVED';
const OBSERVATION_DEDUPE_VERSION = 'SHORT_OBS_DEDUPE_SNAPSHOT_SYMBOL_MICRO_ENTRY_V4_MARKET_WEATHER';
const OUTCOME_DEDUPE_VERSION = 'SHORT_OUTCOME_DEDUPE_CLOSED_POSITION_V8_MARKET_WEATHER_PRESERVED_EMPIRICAL_VETO';
const HARD_TIME_STOP_CLEANUP_VERSION = 'SHORT_POSITION_ENGINE_HARD_TIME_STOP_PRE_PRICE_EXIT_V1';

const OUTCOME_IDENTITY_LOCK_VERSION = 'SHORT_OUTCOME_LEARNING_FAMILY_EQUALS_MICRO_MICRO_V2_WEATHER_PRESERVED';
const EXIT_ALERT_RUNTIME_GATE_VERSION = 'SHORT_EXIT_ALERT_RUNTIME_GATE_APPROVED_ONLY_V4_MM_STATUS_PASSED_WEATHER_KNOWN';

const MICRO_MICRO_RUNTIME_GATE_VERSION = 'SHORT_MM_RUNTIME_STATUS_GATE_V3_WEATHER_EMPIRICAL_VETO';
...
// --- Aangepaste limieten voor Vercel timeout (60 sec) ---
...
const DEFAULT_MONITOR_POSITION_LIMIT = 20;
const DEFAULT_MONITOR_BATCH_SIZE = 10;
const DEFAULT_MONITOR_CONCURRENCY = 2;
const DEFAULT_MONITOR_RUNTIME_MS = 5000;
const DEFAULT_MONITOR_ONE_POSITION_TIMEOUT_MS = 3000;
const DEFAULT_PRICE_FETCH_TIMEOUT_MS = 200;
const DEFAULT_CANDLE_FETCH_TIMEOUT_MS = 700;
const DEFAULT_RECORD_OUTCOME_TIMEOUT_MS = 2000;
const DEFAULT_DISCORD_EXIT_TIMEOUT_MS = 500;
// ----------------------------------------------------
...
function safeParseMarketWeatherKey(key = '') {
  try {
    const parsed = parseMarketWeatherKey(key);

    if (parsed && typeof parsed === 'object') {
      const resolvedKey = parsed.key || key || UNKNOWN_MARKET_WEATHER_KEY;
      const regime = normalizeMarketWeatherRegime(parsed.regime || parsed.marketWeatherRegime || 'UNKNOWN');
      const trendSide = normalizeMarketWeatherTrendSide(parsed.trendSide || parsed.marketWeatherTrendSide || 'UNKNOWN');

      return {
        valid:
          resolvedKey !== UNKNOWN_MARKET_WEATHER_KEY &&
          regime !== 'UNKNOWN' &&
          trendSide !== 'UNKNOWN',
        key:
          resolvedKey !== UNKNOWN_MARKET_WEATHER_KEY &&
  ...
  if (!raw || raw === UNKNOWN_MARKET_WEATHER_KEY || raw.includes('[OBJECT OBJECT]')) {
    return {
      valid: false,
  ...
  if (parts.length >= 2) {
    ...
    const valid = built !== UNKNOWN_MARKET_WEATHER_KEY && regime !== 'UNKNOWN' && trendSide !== 'UNKNOWN';

    return {
      valid,
      key: valid ? built : UNKNOWN_MARKET_WEATHER_KEY,
...
function isUnknownMarketWeatherKey(key = '') {
  const parsed = safeParseMarketWeatherKey(key);

  return (
    !parsed.valid ||
    !parsed.key ||
    parsed.key === UNKNOWN_MARKET_WEATHER_KEY ||
    parsed.regime === 'UNKNOWN' ||
    parsed.trendSide === 'UNKNOWN'
  );
}

function knownParsedMarketWeather(value) {
  const parsed = safeParseMarketWeatherKey(value);
...
function firstKnownMarketWeatherKey(...values) {
  for (const value of values) {
    const parsed = knownParsedMarketWeather(value);
...
function firstKnownMarketWeatherFields(...objects) {
  for (const source of objects) {
    if (!source || typeof source !== 'object') continue;

    const keyParsed = firstKnownMarketWeatherKey(
    ...
    const built = buildEntryMarketWeatherKey(regime, trendSide);
    const parsed = knownParsedMarketWeather(built);
...
function resolveConsistentMarketWeatherSnapshot(row = {}, fallback = {}, timestamp = now()) {
  const entryParsed = firstKnownMarketWeatherKey(
    row.entryMarketWeatherKey,
    fallback.entryMarketWeatherKey,
    row.confirmedMarketWeatherKey,
    fallback.confirmedMarketWeatherKey,
    row.currentMarketWeatherKey,
    fallback.currentMarketWeatherKey,
    row.marketWeatherKey,
    fallback.marketWeatherKey
  ) || firstKnownMarketWeatherFields(row, fallback);

  const confirmedParsed = firstKnownMarketWeatherKey(
    row.confirmedMarketWeatherKey,
    fallback.confirmedMarketWeatherKey,
    entryParsed?.key,
    row.marketWeatherKey,
  ...
  const currentParsed = firstKnownMarketWeatherKey(
    confirmedParsed?.key,
    entryParsed?.key
  );

  const entryKnown = Boolean(entryParsed);
  const confirmedKnown = Boolean(confirmedParsed);
  const currentKnown = Boolean(currentParsed);

  const rawCurrent = knownParsedMarketWeather(row.currentMarketWeatherKey);
  const staleCurrentRepaired = Boolean(
    rawCurrent &&
    currentParsed &&
    rawCurrent.key !== currentParsed.key
  );
  ...
  return {
    ...
    capturedAt: firstValue(
      row.entryMarketWeatherCapturedAt,
      fallback.entryMarketWeatherCapturedAt,
      row.confirmedMarketWeatherAt,
      fallback.confirmedMarketWeatherAt,
      row.confirmedAt,
...
function consistentMarketWeatherFields(row = {}, fallback = {}, timestamp = now()) {
  const resolved = resolveConsistentMarketWeatherSnapshot(row, fallback, timestamp);
  const entry = resolved.entry;
  const confirmed = resolved.confirmed;
  const current = resolved.current;

  return {
    entryMarketWeatherKey: entry.key,
    entryMarketWeatherKeyVersion: MARKET_WEATHER_KEY_VERSION,
    entryMarketWeatherRegime: entry.regime,
    entryMarketWeatherTrendSide: entry.trendSide,
    entryMarketWeatherCapturedAt: resolved.capturedAt,
    entryMarketWeatherRaw:
      row.entryMarketWeatherRaw ||
      row.entryMarketWeather ||
      fallback.entryMarketWeatherRaw ||
      fallback.entryMarketWeather ||
      null,
    entryMarketWeatherRawAvailableFields: Array.isArray(row.entryMarketWeatherRawAvailableFields)
      ...
      : Object.keys({
          ...fallback,
          ...row
        }).filter((field) => field.toLowerCase().includes('weather')).slice(0, 80),
    entryMarketWeatherImmutable: true,
    entryMarketWeatherNeverRecomputedAtExit: true,
    entryMarketWeatherIsUnknown: !resolved.entryKnown,

    confirmedMarketWeatherKey: confirmed.key,
    confirmedMarketWeatherKeyVersion: MARKET_WEATHER_KEY_VERSION,
    confirmedMarketWeatherRegime: confirmed.regime,
    confirmedMarketWeatherTrendSide: confirmed.trendSide,
    confirmedMarketWeatherKnown: resolved.confirmedKnown,

    currentMarketWeatherKey: current.key,
    currentMarketWeatherKeyVersion: MARKET_WEATHER_KEY_VERSION,
    currentMarketWeatherRegime: current.regime,
    currentMarketWeatherTrendSide: current.trendSide,
    currentMarketWeatherKnown: resolved.currentKnown,

    candidateMarketWeatherKey: entry.key,
    candidateMarketWeatherRegime: entry.regime,
    candidateMarketWeatherTrendSide: entry.trendSide,

    marketWeatherKey: current.key,
    marketWeatherRegime: current.regime,
    marketWeatherTrendSide: current.trendSide,
    marketWeatherKnown: resolved.entryKnown,
    marketWeatherUnknown: !resolved.entryKnown,

    positionEngineCurrentMarketWeatherOutputSynced: true,
    currentMarketWeatherStaleOutputRepaired: resolved.staleCurrentRepaired,
    currentMarketWeatherOutputRule: 'OPEN_POSITION_CURRENT_WEATHER_OUTPUT_EQUALS_CONFIRMED_OR_ENTRY_WHEN_NO_FRESH_CONTEXT',
    currentMarketWeatherOutputSource: resolved.confirmedKnown
      ? 'CONFIRMED_MARKET_WEATHER'
      : resolved.entryKnown
        ? 'ENTRY_MARKET_WEATHER'
        : 'UNKNOWN'
  };
}

function localBuildEntryMarketWeatherSnapshot(row = {}, timestamp = now()) {
  ...
  const parsedDirect = safeParseMarketWeatherKey(directKey);

  const regime = parsedDirect.valid
    ? parsedDirect.regime
    : normalizeMarketWeatherRegime(firstValue(
  ...
  const trendSide = parsedDirect.valid
    ? parsedDirect.trendSide
    : normalizeMarketWeatherTrendSide(firstValue(
  ...
  return {
    ...
    entryMarketWeatherCapturedAt: firstValue(
      row.entryMarketWeatherCapturedAt,
      row.confirmedMarketWeatherAt,
      row.confirmedAt,
      row.createdAt,
    ...
    entryMarketWeatherRaw: row.entryMarketWeatherRaw || row.entryMarketWeather || row.currentMarketWeather || null,
    entryMarketWeatherRawAvailableFields: Array.isArray(row.entryMarketWeatherRawAvailableFields)
      ? row.entryMarketWeatherRawAvailableFields
      : Object.keys(row || {}).filter((field) => field.toLowerCase().includes('weather')).slice(0, 50),
    entryMarketWeatherImmutable: true,
    entryMarketWeatherNeverRecomputedAtExit: true,
    entryMarketWeatherIsUnknown: isUnknownMarketWeatherKey(parsed.key)
  };
}

function attachEntryMarketWeatherSnapshot(row = {}, context = {}, timestamp = now()) {
  const seed = {
    ...context,
    ...row
  };
  ...
  try {
    snapshot = buildEntryMarketWeatherSnapshot(seed, timestamp);
  } catch {
    snapshot = localBuildEntryMarketWeatherSnapshot(seed, timestamp);
  }
  ...
  return {
    ...base,
    ...consistentMarketWeatherFields(base, context, timestamp),
    entryMarketWeatherImmutable: true,
    entryMarketWeatherNeverRecomputedAtExit: true
  };
}

function preserveEntryMarketWeatherSnapshot(position = {}, fallback = {}) {
  const timestamp = position.entryMarketWeatherCapturedAt ||
    position.openedAt ||
    position.createdAt ||
    fallback.entryMarketWeatherCapturedAt ||
    fallback.openedAt ||
  ...
  const preserved = attachEntryMarketWeatherSnapshot(seed, fallback, timestamp);

  return {
    ...position,
    ...preserved,
    ...consistentMarketWeatherFields(preserved, fallback, timestamp)
  };
}
...
function marketWeatherFeatureFlags() {
  return {
    ...
    unknownMarketWeatherSignalType: SIGNAL_TYPE_OBSERVE_ONLY,
    unknownMarketWeatherRiskFractionForEntry: 0,
    ...
    noTradeIndicatorsImport: true,
    positionEngineDoesNotImportSrcTradeIndicators: true,
    importSafe: true,

    currentMarketWeatherOutputSynced: true,
    staleCurrentMarketWeatherOutputRepairEnabled: true,
    openPositionCurrentWeatherFallsBackToConfirmedOrEntry: true,
    currentMarketWeatherNotPreservedFromOldOpenPositionRows: true
  };
}

function marketWeatherTradeReadinessGate(row = {}) {
  const preserved = preserveEntryMarketWeatherSnapshot(row, row);
  const key = preserved.entryMarketWeatherKey || UNKNOWN_MARKET_WEATHER_KEY;
  const parsed = safeParseMarketWeatherKey(key);
  const unknown = isUnknownMarketWeatherKey(key);

  return {
    version: 'SHORT_MARKET_WEATHER_TRADE_READINESS_GATE_V4_CURRENT_CONFIRMED_SYNC',
    marketWeatherUnknown: unknown,
    marketWeatherKnown: !unknown,
    entryMarketWeatherKey: parsed.key,
    entryMarketWeatherKeyVersion: preserved.entryMarketWeatherKeyVersion || MARKET_WEATHER_KEY_VERSION,
    entryMarketWeatherRegime: preserved.entryMarketWeatherRegime || parsed.regime,
    entryMarketWeatherTrendSide: preserved.entryMarketWeatherTrendSide || parsed.trendSide,
    confirmedMarketWeatherKey: preserved.confirmedMarketWeatherKey || parsed.key,
    currentMarketWeatherKey: preserved.currentMarketWeatherKey || parsed.key,
    currentMarketWeatherStaleOutputRepaired: Boolean(preserved.currentMarketWeatherStaleOutputRepaired),
    tradeReadyAllowed: !unknown,
    riskFractionForEntry: unknown ? 0 : null,
    signalType: unknown ? SIGNAL_TYPE_OBSERVE_ONLY : null,
    reason: unknown ? 'MARKET_WEATHER_UNKNOWN' : 'MARKET_WEATHER_KNOWN',
    unknownMarketWeatherNeverTradeReady: true
  };
}
...
function attachEntryMarketWeather(row = {}, timestamp = now()) {
  ...
  const consistent = consistentMarketWeatherFields(locked, row, timestamp);
  const merged = {
    ...locked,
    ...consistent
  };

  const gate = marketWeatherTradeReadinessGate(merged);

  return {
    ...merged,

    entryMarketWeather: merged.entryMarketWeatherRaw || row.entryMarketWeather || null,

    entryMarketWeatherCaptureVersion: ENTRY_MARKET_WEATHER_CAPTURE_VERSION,
    entryMarketWeatherKeyVersion: merged.entryMarketWeatherKeyVersion || MARKET_WEATHER_KEY_VERSION,
    entryMarketWeatherImmutable: true,
    entryMarketWeatherNeverRecomputedAtExit: true,
...
function preserveLockedEntryWeather(position = {}, fallback = {}) {
  ...
  const consistent = consistentMarketWeatherFields(preserved, fallback);
  const merged = {
    ...preserved,
    ...consistent
  };
...
function isExecutionFingerprintId(id = '') {
  ...
  if (isExactShortMicroMicroId(value)) return false;
...
function normalizeTradeSide(value) {
  ...
  if (shortHit && longHit) {
    ...
    if (normalized.includes('MICRO_SHORT_') || normalized.includes('MM_SHORT_')) return TARGET_TRADE_SIDE;
...
function normalizeMicroIdentity(row = {}) {
  ...
  return {
    ...
    base75ChildTrueMicroFamilyId: microFamilyId,
    baseTrueMicroFamilyId: microFamilyId,
    trueMicro75FamilyId: microFamilyId,
...
function microMicroRuntimeGateConfig() {
  return {
    ...
    statuses: {
      ...
      passed: 'completed >= 35 && LCB95(avgR)>0 && positive net-edge => Discord allowed only when selected and weather playbook is known',
...
function microMicroRuntimeGate(row = {}) {
  ...
  return {
    ...
    unknownMarketWeatherHandledBySignalAndRisk: true
  };
}
...
function normalizeRiskDecision(row = {}, runtimeStatusGate = microMicroRuntimeGate(row)) {
  ...
  const marketWeatherUnknown = marketWeatherGate.marketWeatherUnknown;
  ...
  const observeOnly =
    ...
    rawSignalType === 'OBSERVATION_ONLY';
  ...
  const derivedSignalType =
    hardBlocked || rawBlocked
      ? SIGNAL_TYPE_BLOCKED
      : marketWeatherUnknown || observeOnly
        ? SIGNAL_TYPE_OBSERVE_ONLY
        : runtimeStatusGate.status === MICRO_MICRO_STATUS_PASSED && riskFractionForEntry > 0 && shrunkLCB95AvgR > 0
  ...
  return {
    ...
    signalTypeIsDerivedOnly: true,
    signalTypeIsActionLabelOnly: false,
    signalTypeDerivationOrder: 'policy_veto_weather_proof_lcb95_then_riskFraction_then_signalType',
    ...
    riskDecisionVersion: 'SHORT_POSITION_ENGINE_RISK_INPUT_BRIDGE_V3_MARKET_WEATHER_OBSERVE_ONLY',

    riskBlockedReason: hardBlocked
      ? runtimeStatusGate.reason || 'POLICY_OR_EMPIRICAL_VETO'
      : rawBlocked
        ? 'RAW_SIGNAL_BLOCKED'
        : marketWeatherUnknown
...
function identityFlags() {
  return {
    ...
    exitAlertRequiresKnownMarketWeather: true,
...
function buildVirtualFlags(row = {}) {
  ...
  return {
    ...
    liveEligible: false,
    discordAlertEligible: Boolean(row.discordAlertEligible && riskDecision.signalType === SIGNAL_TYPE_TRADE_READY),
    selectedMicroFamilyAlert: Boolean(row.selectedMicroFamilyAlert && riskDecision.signalType === SIGNAL_TYPE_TRADE_READY),
    selectedForDiscord: Boolean(
      riskDecision.signalType === SIGNAL_TYPE_TRADE_READY &&
      (row.selectedForDiscord || row.discordAlertEligible || row.selectedMicroFamilyAlert)
    ),
...
function compactWeeklyStats(value = null) {
  ...
  return {
    ...
    exactMicroMicroFamilyId: value.exactMicroMicroFamilyId || value.microMicroFamilyId || null,
    ...
    marketWeatherGate: marketWeatherTradeReadinessGate(value),
...
function compactOpenPositionRow(row = {}) {
  const withWeather = preserveLockedEntryWeather(row, row);
...
export async function getOpenPositions(options = {}) {
  ...
  const validHydratedRows = readRows
    ...
    .map((row) => forceShortPositionFields({
      ...preserveLockedEntryWeather(row, row),
...
export async function getOpenPosition(symbol) {
  ...
  return forceShortPositionFields(preserveLockedEntryWeather(row, row));
}
...
export async function saveOpenPosition(position) {
  ...
  const normalized = forceShortPositionFields(
    position.entryMarketWeatherKey
      ? preserveLockedEntryWeather(position, position)
      : attachEntryMarketWeather(position, position.openedAt || position.createdAt || now())
  );
...
export function updatePathMetrics(position, price) {
  ...
  Object.assign(position, forceShortPositionFields(preserveLockedEntryWeather(position, position)));
...
async function markPriceFetchFailed(position, { persist = false } = {}) {
  ...
  if (persist) {
    await saveOpenPosition(forceShortPositionFields(preserveLockedEntryWeather(position, position))).catch(() => null);
  }
...
function exitAlertRuntimeGateStatus(position = {}, outcome = {}, options = {}) {
  ...
  if (isUnknownMarketWeatherKey(position.entryMarketWeatherKey || outcome.entryMarketWeatherKey)) {
    return {
      approved: false,
      required: true,
      version: EXIT_ALERT_RUNTIME_GATE_VERSION,
...
async function monitorOnePosition({
  ...
  try {
    const cfg = tradeConfig(options);
    position = forceShortPositionFields(preserveLockedEntryWeather(position, position));
...
export async function monitorOpenPositions(options = {}) {
  ...
  const allPositions = (Array.isArray(openPositions) ? openPositions : [])
    ...
    .map((row) => forceShortPositionFields(preserveLockedEntryWeather(row, row)))
    .sort(sortOpenPositions);
