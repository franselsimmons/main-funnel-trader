...
function resolveMarketEventClusterId(row = {}, entryTs = now()) {
  ...
  return `SHORT_EVENT_${Math.floor(normalizeTimestampMs(entryTs, now()) / 3_600_000)}`;
}
...
function temporalPolicyForContext(context = buildTemporalContext()) {
  ...
  return Object.freeze({
    ...
    enforceMissingRootGenerationBehavior:
      'BLOCK_ALL_NEW_SHORT_DISCORD_ENTRIES_KEEP_LEARNING_AND_EXITS_ACTIVE',
...
function stripSymbolTokensFromFamilyId(id = '', row = {}) {
    ...
    if (parseFixedShortTaxonomyId(raw)) {
        return upper(raw);
    }
...
function getShortRiskGeometry(row = {}) {
  ...
  return {
         ...
         shortRiskShapeValid: validShortGeometry,



         shortTpHit: validShortGeometry && priceForHit !== null ? priceForHit <= tp :
false,
         shortSlHit: validShortGeometry && priceForHit !== null ? priceForHit >=
initialSl : false,
...
function grossR(row = {}) {
    ...
    if (geometry.validShortGeometry && geometry.shortGrossR !== null) {
           return geometry.shortGrossR;
    }
...
uniqueStrings(entries.map(getParentTrueMicroFamilyId)),
          ...
          exact75ChildIds:
uniqueStrings(entries.map(getTrueMicroFamilyId)).filter(isExactShortChildTrueMicroId),
          parent15Ids:
uniqueStrings(entries.map(getParentTrueMicroFamilyId)).filter(isParentShortMicroId),
...
row.selectedForDiscord)).length,
         ...
         exact75ChildIds:
uniqueStrings(exits.map(getTrueMicroFamilyId)).filter(isExactShortChildTrueMicroId),
         parent15Ids:
uniqueStrings(exits.map(getParentTrueMicroFamilyId)).filter(isParentShortMicroId),
...
function buildExitFlags(row = {}) {
  ...
  return {
      ...
      validShortGeometry: riskValid,
      shortValidGeometry: riskValid,
...
Number.isFinite(Number(row.currentR))
      ...
      shortCurrentR: row.shortCurrentR !== undefined &&
Number.isFinite(Number(row.shortCurrentR))
        ? Number(row.shortCurrentR)
        : calcShortCurrentR(row),
...
uniqueStrings(positions.map(getCoarseMicroFamilyId)),
         ...
         exact75ChildIds:
uniqueStrings(positions.map(getTrueMicroFamilyId)).filter(isExactShortChildTrueMicroId),
         parent15Ids:
uniqueStrings(positions.map(getParentTrueMicroFamilyId)).filter(isParentShortMicroId),
...
|| row.selectedForDiscord)).length,
         ...
         invalidShortRiskShape: debugRows.filter((row) =>
!row.shortRiskShapeValid).length,
