...
export function normalizeMicroFamilyId(raw) {
  ...
  if (/^(LONG|SHORT)_\d+/.test(value)) return null;
...
export function isCoreMicroFamilyId(raw) {
  ...
  return Boolean(
    id &&
      id.includes(`_${MICRO_FAMILY_SCHEMA_VERSION}_`) &&
      normalizeAnalyzeFamilyId(extractParentFamilyIdFromMicroId(id))
  );
}
...
function getSignalCoreMicroFamilyIds(signal = {}, maxFamilyIdsChecked = DEFAULT_MAX_FAMILY_IDS_CHECKED) {
...
export function extractRotationMicroFamilyIds(rotation = {}) {
  if (!rotation || typeof rotation !== "object") return [];
  ...
  return uniqCoreMicroIds(values);
}
...
export function attachMicroRotationKeys(signal = {}, options = {}) {
  ...
  const analyzeFamilyId = extractAnalyzeFamilyIdFromSignal(row);

  const sideCompatible =
    !analyzeFamilyId ||
    tradeSide === "UNKNOWN" ||
    (tradeSide === "LONG" && analyzeFamilyId.startsWith("LONG_")) ||
    (tradeSide === "SHORT" && analyzeFamilyId.startsWith("SHORT_"));

  const cleanAnalyzeFamilyId = sideCompatible ? analyzeFamilyId : null;

  const explicitCoreId = getSignalCoreMicroFamilyIds(row, 1)[0] || null;
  const coreFromAnalyze = buildCoreMicroIdFromAnalyzeFamilyId(cleanAnalyzeFamilyId);
  const primaryMicroFamilyId = coreFromAnalyze || explicitCoreId || null;

  const parentFamilyId =
    cleanAnalyzeFamilyId ||
    normalizeAnalyzeFamilyId(extractParentFamilyIdFromMicroId(primaryMicroFamilyId)) ||
    null;

  const microFamilyIds = primaryMicroFamilyId
    ? [primaryMicroFamilyId]
    : [];

  const familyIds = parentFamilyId
    ? [parentFamilyId]
    : [];
  ...
  return {
    ...
    // Belangrijk: familyId blijft LONG_1..SHORT_50, niet MICRO_*.
    familyId: parentFamilyId,
    familyIds,
    families: familyIds,
...
function normalizeExternalGateResult(result = {}) {
...
export async function checkTradeSignalAgainstRotation(signal = {}, options = {}) {
  ...
  const explicitActiveIds = extractRotationMicroFamilyIds(explicitRotation);
  ...
  if (!rotation || activeMicroFamilyIds.length === 0) {
    externalGateResult = normalizeExternalGateResult(
      (await evaluateExternalLiveGateSafe(attachedSignal, options)) || {}
    );

    const externalIds = extractRotationMicroFamilyIds(externalGateResult);

    if (!rotation && externalGateResult && Object.keys(externalGateResult).length > 0) {
...
export async function microRotationGate(action = {}, options = {}) {
  ...
  return {
    // Legacy behavior: gate call itself succeeded, but trade is blocked.
    ok: true,
    allow: false,
    allowed: false,
    pass: false,
