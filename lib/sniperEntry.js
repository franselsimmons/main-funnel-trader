...
export function getSniperEntry(c, ob, rsiSignal) {
  ...
  // 🔥 ELITE institutionele traps
  if (dir === 1 && absorption.absorbingBids && walls.bidWallStrong && rsiOK) {
    return { valid: true, type: "INSTITUTIONAL_LONG", score: 95 };
  }
  if (dir === -1 && absorption.absorbingAsks && walls.askWallStrong && rsiOK) {
    return { valid: true, type: "INSTITUTIONAL_SHORT", score: 95 };
  }
  ...
  score += confluence * 0.5;  // 0-50 punten

  if (flow === "TREND") score += 10;
  if (flow === "BUILDING") score += 5;

  // Orderbook bias alignment
  ...
  // Bepaal validiteit op basis van score en extra condities
  let valid = false;
  ...
  return {
    ...
    score,
    type:
      score >= 85 ? "SNIPER_ELITE" :
      score >= 75 ? "SNIPER_STRONG" :
      score >= 65 ? "SNIPER_OK" : "SNIPER_WEAK"
  };
}