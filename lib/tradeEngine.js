...
export async function executeTrade(mode, coin, opts = {}) {
  ...
  /* 🔥 CRITICAL FIX: gebruik scanner thresholds */
  ...
  if (ob.spreadPct > spreadMax) {
    return retryReject(m, symbol, `spread_${ob.spreadPct.toFixed(2)}`);
  }

  if (ob.depthMinUsd1p < depthMin) {
    return retryReject(m, symbol, `depth_${Math.round(ob.depthMinUsd1p)}`);
  }
