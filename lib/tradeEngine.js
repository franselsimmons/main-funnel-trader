...
export async function executeTrade(mode, coin, opts = {}) {
  ...
  if (!entry || !price) return { ok: false };

  const ob = await fetchOrderbook(symbol + "USDT");
  if (!ob?.valid) return retryReject(m, symbol, "ob_missing");

  if (ob.spreadPct > 2)
    return retryReject(m, symbol, "spread_high");

  if (ob.depthMinUsd1p < 800)
    return retryReject(m, symbol, "depth_low");

  const account = await getAccount();
  const riskPct = 0.02; // 2% per trade
