// ================= confluenceEngine.js (FINAL PRO VERSION) =================
export function calculateConfluence(
  ...
  if(liquidation?.clusters?.length){
    ...
    for(const cl of liquidation.clusters){
      ...
      // Protection
      if(isBull && clPrice < price && longRatio > 0.6) liqScore += 4;
      if(!isBull && clPrice > price && shortRatio > 0.6) liqScore += 4;
    }
  }

  // ✅ cap voorkomt overdominantie
  score += Math.min(liqScore, 20);

  // ✅ FIX: slimme fallback i.p.v. blind +5
  if(!hasLiquidation){
    if(c.flow === "TREND" && c.moveScore >= 75){
      score += 5;
    } else {
      score += 2;
    }
  ...
  if(rsiCtx?.valid){
    ...
    // geen data = kleine neutrale boost
    score += 3;
  }
