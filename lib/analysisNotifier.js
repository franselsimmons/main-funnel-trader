...
function getWebhookSignatureValue() {
  return safeWebhookString(ANALYSIS_WEBHOOK_SECRET || "", "").trim();
}
...
function buildAnalysisWebhookEvent(action, meta = {}) {
  ...
  const rawPayload = {
    ...
    tradeWebhookSecret: signature,
    analysisWebhookSecret: signature,

    "x-signature": signature,
    "x-webhook-signature": signature,
    "x-trade-webhook-signature": signature,
    "x-analysis-webhook-signature": signature,
  ...
  const symbol = safeWebhookString(
    payload.symbol || action?.symbol || "UNKNOWN",
    "UNKNOWN"
  );

  const side = safeWebhookString(
    payload.side || action?.side || "unknown",
    "unknown"
  );
...
function buildStrictFlatAnalysisWebhookBody(event) {
  ...
  const base = {
    ...
    payloadJson: truncateWebhookString(
      payload,
  ...
  const bodyObj = sanitizeWebhookPayload(
    base,
    ANALYSIS_WEBHOOK_PAYLOAD_MAX_CHARS
  );
...
function buildAnalysisWebhookHeaders() {
  ...
  headers["x-tradesystem-secret"] = signature;
  ...
  headers["x-tradesystem-signature"] = signature;

  // Common auth fallbacks
...
async function sendAnalysisWebhookEvent(event) {
  ...
  if (process.env.TS_DEBUG_ANALYSIS_WEBHOOK === "true") {
    console.log("TS_ANALYSIS_WEBHOOK_BODY_TYPES:", JSON.stringify({
      ...
      keys: Object.keys(bodyObj).slice(0, 100)
    }));
  }
