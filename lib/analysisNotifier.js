// ================= ANALYSIS NOTIFIER (FIXED VERSION) =================

// FIX: We gebruiken nu functies (Lazy Loading) in plaats van hardcoded 'const' bovenaan.
// Dit garandeert dat de .env variabelen altijd succesvol worden uitgelezen, 
// ongeacht in welke volgorde je bestanden opstarten.
...
const DEFAULT_TIMEOUT_MS = Number(process.env.ANALYSIS_WEBHOOK_TIMEOUT_MS || 12_000);

// FIX: Kleine batches. Geen 25 meer, maar max 4.
// Dit voorkomt 504 Vercel timeouts en "This operation was aborted".
const MAX_BATCH_SIZE = Number(process.env.ANALYSIS_WEBHOOK_MAX_BATCH_SIZE || 4);
...
async function postJson(url, payload, timeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  try {
    ...
    if (webhookSecret) {
      ...
      headers["x-analysis-webhook-secret"] = webhookSecret;
