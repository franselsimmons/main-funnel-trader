...
export function getAdvancedRSIContext(candles){
  ...
  // 🔥 RSI (zoals TradingView) - smooth aangepast naar 30
  const rsiRaw = rsiCalc(closes, 14);
  const rsiSmooth = ema(rsiRaw, 30); // WAS 5
...
// ================= TYPE 1 ENTRY FILTER (NIEUW: nooit blokkeren, margins +3) =================
export function isType1RSIEntry(rsiCtx, side){

  if(!rsiCtx?.valid) return true; // 🔥 nooit blokkeren
