...
/**
 * Bitget Spot orderbook → normalized snapshot
 */
export async function fetchOrderbook(symbol) {
  ...
  try {
    ...
    if (!r.ok) return invalid(`http_${r.status}`);
 ...
 * Default values nu gelijk aan execution defaults
 */
export function orderbookPass(ob, thresholds = {}) {
  if (!ob || !ob.valid) return false;

  const spreadMax = n(thresholds?.spreadMaxPct ?? 2, 2);
  const depthMin = n(thresholds?.depthMinUsd ?? 800, 800);
