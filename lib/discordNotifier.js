// ================= CONFIG =================
// Zorg dat deze variabelen in je omgeving (env) staan of vul ze hier tijdelijk in
...
function compactNumber(value) {
  ...
  // Voor crypto/forex gebruiken we 6 decimalen, voor RR is 2 vaak genoeg
  // maar we houden 6 aan voor de consistentie met jouw entry/tp/sl
  return n.toFixed(6).replace(/\.?0+$/, ""); // Verwijdert overbodige nullen aan het eind
}
...
function formatEntryMessage(t) {
  ...
  const rr = compactNumber(t.rr); // Gefixt: geen lange decimalen meer

  // De variabele 'cb' zorgt dat de code-blokken altijd correct worden verzonden
  const cb = "```"; 
