...
function buildMainSignalId(t, symbol, side, grade) {
  ...
  // Uniek per echt signaal. Voorkomt dat identieke prijslevels later als duplicate blokkeren.
...
export async function sendEntry(t) {
  ...
  // Discord blijft exact als normale signal-card.
  // Geen extra execution-status bericht.
  if (webhook) {
  ...
  return {
    ok: executeMainBot ? Boolean(mainBotResult?.ok) : true,
