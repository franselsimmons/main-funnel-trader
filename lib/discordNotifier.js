...
function buildEntryFields(t) {
  ...
  return [
    makeField("Trade plan", [
      kv("Entry", entry, compactPrice),
      kv("SL", sl, compactPrice),
      kv("TP", tp, compactPrice),
      kv("RR", rr, value => compactNumber(value, 3)),
      kv("Base RR", baseRR, value => compactNumber(value, 3)),
      kv("TP multiplier", tpRewardMultiplier, value => compactNumber(value, 3))
    ], true),

    makeField("Signal quality", [
      kv("Score", score, compactScore),
      kv("Confluence", confluence, compactScore),
      kv("Raw confluence", rawConfluence, compactScore),
      kv("Sniper", sniperScore, compactScore),
      kv("Flow", flow, toUpperText),
      kv("BTC", btcState, toUpperText),
      kv("Regime", regime, toUpperText)
    ], true),

    makeField("RSI context", [
      kv("RSI", rsi, value => compactNumber(value, 2)),
      kv("RSI HTF", rsiHTF, value => compactNumber(value, 2)),
      kv("Zone", rsiZone, toUpperText),
      kv("Edge", rsiEdge, toUpperText),
      kv("Edge rank", rsiEdgeRank, value => compactNumber(value, 0)),
      kv("Continuation", rsiContinuationOk, formatBool)
    ], true),

    makeField("Orderbook", [
      kv("OB bias", obBias, toUpperText),
      kv("Spread", spreadPct, formatBps),
      kv("Depth 1%", depthMinUsd1p, compactDepthUsd),
      kv("Spoof", spoof, formatBool)
    ], true),

    makeField("Market context", [
      kv("Funding", funding, value => formatFractionPct(value, 4)),
      kv("1h move", change1h, formatMovePct),
      kv("24h move", change24, formatMovePct),
      kv("Liquidation data", hasLiquidationData, formatBool)
    ], true),

    makeField("Structure", [
      kv("Pullback", pullbackConfirmed, formatBool),
      kv("Sweep", sweepConfirmed, formatBool),
      kv("Retest", retestConfirmed, formatBool),
      kv("Distance from high", distanceFromLocalHighPct, value => formatFractionPct(value, 3))
    ], true),

    makeField("Special flags", [
      kv("BTC bullish short exception", btcBullishBearException),
      kv("Exception reason", special?.btcBullishBearException?.reason || t?.btcBullishBearExceptionReason),
      kv("Bullish mid trend probe", bullishMidTrendProbe),
      kv("Probe reason", special?.bullishMidTrendProbe?.reason || t?.bullishMidTrendProbeReason)
    ], false)
  ];
}
...
function buildExitFields(t) {
  ...
  return [
    makeField("Result", [
      kv("Reason", reason),
      kv("Exit R", t?.exitR, value => compactNumber(value, 3)),
      kv("Trigger R", t?.triggerR, value => compactNumber(value, 3)),
      kv("PnL", t?.pnlPct, value => `${compactNumber(value, 3)}%`),
      kv("Trigger PnL", t?.triggerPnlPct, value => `${compactNumber(value, 3)}%`),
      kv("Hold", t?.holdMinutes, value => `${compactNumber(value, 1)} min`)
    ], true),

    makeField("Prices", [
      kv("Entry", t?.entry, compactPrice),
      kv("Exit", t?.exit ?? t?.executionPrice, compactPrice),
      kv("Trigger", t?.triggerPrice, compactPrice),
      kv("SL", t?.sl, compactPrice),
      kv("Initial SL", t?.initialSl, compactPrice),
      kv("TP", t?.tp, compactPrice)
    ], true),

    makeField("Path metrics", [
      kv("MFE R", t?.mfeR, value => compactNumber(value, 3)),
      kv("MAE R", t?.maeR, value => compactNumber(value, 3)),
      kv("Current R", t?.currentR, value => compactNumber(value, 3)),
      kv("Max TP progress", t?.maxTpProgress, value => formatFractionPct(value, 1)),
      kv("Max SL progress", t?.maxSlProgress, value => formatFractionPct(value, 1)),
      kv("Direct to SL", t?.directToSL, formatBool)
    ], true),

    makeField("Giveback / BE", [
      kv("Reached 0.5R", t?.reachedHalfR, formatBool),
      kv("Reached 1R", t?.reachedOneR, formatBool),
      kv("Near TP seen", t?.nearTpSeen, formatBool),
      kv("SL after 0.5R", t?.slAfterHalfR, formatBool),
      kv("SL after 1R", t?.slAfterOneR, formatBool),
      kv("SL after near TP", t?.slAfterNearTp, formatBool),
      kv("BE active", t?.breakEvenActivated, formatBool),
      kv("BE stop", t?.breakEvenStop, formatBool)
    ], true),

    makeField("Timing", [
      kv("Ticks observed", t?.ticksObserved, value => compactNumber(value, 0)),
      kv("Favorable ticks", t?.favorableTicks, value => compactNumber(value, 0)),
      kv("Adverse ticks", t?.adverseTicks, value => compactNumber(value, 0)),
      kv("Ticks to MFE", t?.ticksToMfe, value => compactNumber(value, 0)),
      kv("Ticks to MAE", t?.ticksToMae, value => compactNumber(value, 0))
    ], true),

    makeField("Original signal", [
      kv("Setup", t?.setupClass, toUpperText),
      kv("Entry reason", t?.entryReason || t?.entryType, toUpperText),
      kv("Score", t?.score || t?.moveScore, compactScore),
      kv("Confluence", t?.confluence, compactScore),
      kv("Sniper", t?.sniperScore, compactScore),
      kv("RSI zone", t?.rsiZone, toUpperText),
      kv("OB bias", t?.obBias, toUpperText),
      kv("BTC", t?.btcState, toUpperText),
      kv("Spread", t?.spreadPct, formatBps),
      kv("Depth 1%", t?.depthMinUsd1p, compactDepthUsd)
    ], false)
  ];
}
