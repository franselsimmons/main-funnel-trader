...
function buildEntryEmbed(t) {
  ...
  const fields = [
    makeField("ID", makeKv([
      ["Trade ID", metrics.tradeId ? `\`${truncateMiddle(metrics.tradeId, 220)}\`` : "N/A"],
      ["Cohort", metrics.cohortKey ? `\`${truncateMiddle(metrics.cohortKey, 700)}\`` : "N/A"]
    ]), false),

    makeField("Prices", makeKv([
      ["Entry", entryPrice],
      ["TP", tpPrice],
      ["SL", slPrice]
    ]), true),

    makeField("Setup", makeKv([
      ["Grade", grade],
      ["Class", setupClass],
      ["Reason", metrics.setup.entryReason],
      ["Points", compactNumber(metrics.setup.gradePoints, 0)]
    ]), true),

    makeField("Scores", makeKv([
      ["Scanner", compactNumber(metrics.scores.score, 0)],
      ["Conf", `${compactNumber(metrics.scores.confluence, 0)} / raw ${compactNumber(metrics.scores.rawConfluence, 0)}`],
      ["Sniper", `${compactNumber(metrics.scores.sniperScore, 0)} / raw ${compactNumber(metrics.scores.rawSniperScore, 0)}`],
      ["Fallback sniper", compactNumber(metrics.scores.fallbackSniperScore, 0)]
    ]), true),

    makeField("RR", makeKv([
      ["Base", compactR(metrics.rr.baseRR)],
      ["Final", compactR(metrics.rr.finalRr)],
      ["Req", compactR(metrics.rr.requiredRR)],
      ["Final req", compactR(metrics.rr.finalRequiredRR)],
      ["TP mult", compactNumber(metrics.rr.tpRewardMultiplier, 3)]
    ]), true),

    makeField("RSI", makeKv([
      ["RSI", compactNumber(metrics.rsi.rsi, 2)],
      ["HTF", compactNumber(metrics.rsi.rsiHTF, 2)],
      ["Zone", metrics.rsi.rsiZone],
      ["Edge", metrics.rsi.rsiEdge],
      ["Rank", compactNumber(metrics.rsi.rsiEdgeRank, 0)],
      ["Cont", boolText(metrics.rsi.continuationOk)]
    ]), true),

    makeField("Orderbook", makeKv([
      ["Bias", metrics.ob.bias],
      ["Relation", metrics.ob.relation],
      ["Spread", `${compactNumber(metrics.ob.spreadBps, 2)} bps`],
      ["Spread bucket", metrics.ob.spreadBucket],
      ["Depth", `$${compactNumber(metrics.ob.depthMinUsd1p, 0)}`],
      ["Depth bucket", metrics.ob.depthBucket],
      ["Spoof", boolText(metrics.ob.spoof)]
    ]), true),

    makeField("Market", makeKv([
      ["BTC", metrics.market.btcState],
      ["Regime", metrics.market.regime],
      ["Flow", metrics.market.flow],
      ["TF", `${compactNumber(metrics.market.tfStrength, 2)} / ${metrics.market.tfAlignment}`],
      ["1h", compactPct(metrics.market.change1h)],
      ["24h", compactPct(metrics.market.change24)],
      ["Funding", `${compactNumber(metrics.market.funding, 5)} | ${metrics.market.fundingBucket}`]
    ]), true),

    makeField("Structure", makeKv([
      ["Pullback", boolText(metrics.structure.pullbackConfirmed)],
      ["Sweep", boolText(metrics.structure.sweepConfirmed)],
      ["Retest", boolText(metrics.structure.retestConfirmed)],
      ["Dist high", compactPct(metrics.structure.distanceFromLocalHighPct * 100, 3)]
    ]), true),

    makeField("Gates", makeKv([
      ["Quality", metrics.gates.qualityGateReason],
      ["Depth", metrics.gates.finalDepthReason],
      ["Confirm req", boolText(metrics.gates.confirmationRequired)],
      ["Confirm seen", boolText(metrics.gates.confirmationSeen)]
    ]), true),

    ...makeJsonFields("Analytics", metrics, MAX_JSON_FIELDS)
  ].filter(Boolean);

  return {
    title: `ENTRY: ${symbol} ${side} | ${setupClass}`,
    color,
    fields,
...
function buildExitEmbed(t) {
  ...
  const entrySnapshot = buildCompactEntrySnapshotForExit(t);
  ...
  const fields = [
    ...
    makeField("Outcome", makeKv([
      ["Reason", reason],
      ["Exit R", compactR(metrics.outcome.exitR, 3)],
      ["PnL", compactPct(metrics.outcome.pnlPct, 3)],
      ["Trigger R", compactR(metrics.outcome.triggerR, 3)],
      ["Trigger PnL", compactPct(metrics.outcome.triggerPnlPct, 3)],
      ["Hold", `${compactNumber(metrics.outcome.holdMinutes, 1)} min`]
    ]), true),

    makeField("Prices", makeKv([
      ["Entry", compactPrice(t?.entry)],
      ["Exit", compactPrice(t?.exit || t?.executionPrice)],
      ["Trigger", compactPrice(t?.triggerPrice)],
      ["TP", compactPrice(t?.tp)],
      ["SL", compactPrice(t?.sl)]
    ]), true),

    makeField("Path", makeKv([
      ["MFE", compactR(metrics.path.mfeR, 3)],
      ["MAE", compactR(metrics.path.maeR, 3)],
      ["Current", compactR(metrics.path.currentR, 3)],
      ["TP progress", compactPct(metrics.path.maxTpProgress * 100, 1)],
      ["SL progress", compactPct(metrics.path.maxSlProgress * 100, 1)]
    ]), true),

    makeField("Path Flags", makeKv([
      ["Direct SL", boolText(metrics.path.directToSL)],
      ["Half R", boolText(metrics.path.reachedHalfR)],
      ["One R", boolText(metrics.path.reachedOneR)],
      ["Near TP", boolText(metrics.path.nearTpSeen)],
      ["SL after 0.5R", boolText(metrics.path.slAfterHalfR)],
      ["SL after 1R", boolText(metrics.path.slAfterOneR)],
      ["SL after near TP", boolText(metrics.path.slAfterNearTp)]
    ]), true),

    makeField("Break Even", makeKv([
      ["Activated", boolText(metrics.be.breakEvenActivated)],
      ["BE stop", boolText(metrics.be.breakEvenStop)],
      ["BE SL", metrics.be.breakEvenSl ? compactPrice(metrics.be.breakEvenSl) : "N/A"]
    ]), true),

    ...makeJsonFields("Outcome Analytics", metrics, MAX_JSON_FIELDS),
    ...makeJsonFields("Entry Snapshot", entrySnapshot, 1)
  ].filter(Boolean);
