...
// ⏱️ ANTI-SPAM INSTELLING
// Aantal minuten dat het script wacht voordat dezelfde munt wéér gestuurd mag worden
const COOLDOWN_MINUTES = 60; 

// ================= STATE (GEHEUGEN) =================
// Hier onthoudt het script wanneer een munt voor het laatst is verstuurd
...
export async function sendEntry(t) {
  ...
  if (lastSent && (now - lastSent) < cooldownMs) {
    console.log(`⏳ Dubbel signaal voor ${symbol} genegeerd (zit nog in de cooldown).`);
    return; // Breekt de functie af, verstuurt niks naar Discord
  }

  // Sla de huidige tijd op voor deze munt
...
export async function sendExit(t) {
  ...
  // Haal de munt uit de anti-spam lijst, want de trade is nu klaar
  recentSignals.delete(symbol);
