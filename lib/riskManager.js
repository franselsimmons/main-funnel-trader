...
export async function calculateRisk(c, ob = {}, liquidity = null, liquidation = null){
  ...
  if(isBull){
    ...
    const bullStopCandidates = [
      ...
      { price: below ? Number(below) * (1 - slBufferPct) : null, source: "liquidation cluster below" }
    ];
    ...
    const bullTargetCandidates = [
      ...
      { price: above ? Number(above) * (1 - tpBufferPct) : null, source: "short liquidation target" }
    ];
    ...
    const bearStopCandidates = [
      ...
      { price: above ? Number(above) * (1 + slBufferPct) : null, source: "liquidation cluster above" }
    ];
    ...
    const bearTargetCandidates = [
      ...
      { price: below ? Number(below) * (1 + tpBufferPct) : null, source: "long liquidation target" }
    ];
