...
export async function calculateRisk(c, ob = {}, liquidity = null, liquidation = null){
  ...
  // Geef de trade meer ademruimte om ruis (wicks/sweeps) te overleven
  const atrDist = price * Math.max(atrPct * 1.85, spread * 4.0, 0.0050); // ATR multiplier omhoog van 1.35 naar 1.85
  const minRiskDist = Math.max(atrDist, price * 0.0065); // Minimum risico-afstand van 0.42% naar 0.65%
  const maxRiskDist = Math.max(atrDist * 3.50, price * 0.045); 

  // De buffer die hij onder een support/swing low plaatst was veel te krap
  const slBufferPct = Math.max(spread * 3.0, atrPct * 0.65, 0.0025); // Van 0.22 ATR naar 0.65 ATR
  const tpBufferPct = Math.max(spread * 1.5, atrPct * 0.25, 0.0015);
  ...
  const aboveMajor = liquidation?.majorAbove
    ? Number(liquidation.majorAbove)
    : null;

  const belowMajor = liquidation?.majorBelow
    ? Number(liquidation.majorBelow)
    : null;
  ...
  if(isBull){
    ...
    const bullTargetCandidates = [
      ...
      { price: aboveNearest ? Number(aboveNearest) * (1 - tpBufferPct) : null, source: "long liquidation target (nearest)" }
    ];
    ...
    const bearTargetCandidates = [
      ...
      { price: belowNearest ? Number(belowNearest) * (1 + tpBufferPct) : null, source: "short liquidation target (nearest)" }
    ];
