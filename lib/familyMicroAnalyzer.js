...
const DEFAULT_ALLOWLIST_LIMIT = readPositiveNumberEnv(
  "FAMILY_MICRO_ALLOWLIST_LIMIT",
  25
);

const DEFAULT_ROTATION_ALLOWLIST_LIMIT = readPositiveNumberEnv(
  "FAMILY_MICRO_ROTATION_ALLOWLIST_LIMIT",
  250
);

const DEFAULT_FALLBACK_MIN_CLOSED = readPositiveNumberEnv(
  "FAMILY_MICRO_FALLBACK_MIN_CLOSED",
  1
);

const DEFAULT_FALLBACK_MIN_OBSERVED = readPositiveNumberEnv(
  "FAMILY_MICRO_FALLBACK_MIN_OBSERVED",
  1
);

const DEFAULT_ENABLE_COLLECTING_FALLBACK = readBooleanEnv(
  "FAMILY_MICRO_ENABLE_COLLECTING_FALLBACK",
  true
);

const DEFAULT_ENABLE_OBSERVED_FALLBACK = readBooleanEnv(
  "FAMILY_MICRO_ENABLE_OBSERVED_FALLBACK",
  true
);
...
function normalizeSide(value) {
  ...
  if (key.includes("BULL")) return "LONG";
  if (key.includes("BEAR")) return "SHORT";
...
function getEntryTs(event, fallback = Date.now()) {
  return normalizeTs(
    event?.openedAt ??
      event?.entryAt ??
      event?.entryTs ??
...
function getExitTs(event, fallback = Date.now()) {
  return normalizeTs(
    event?.closedAt ??
      event?.exitAt ??
      event?.exitTs ??
...
function getTradeId(event) {
  const id =
    ...
    event?.orderId ||
    event?.clientOrderId ||
    event?.analyzeTradeId ||
    event?.analyzePositionId ||
...
function hasEntrySignal(event) {
  if (!event || typeof event !== "object") return false;

  if (event.entry !== undefined && event.entry !== null) return true;
  if (event.entryPrice !== undefined && event.entryPrice !== null) return true;
  if (event.openedAt || event.entryAt || event.entryTs) return true;

  const status = toKey(event?.status || event?.action || event?.reason, "");
...
// ================= TRADE EVENT CONSOLIDATION =================

function getFeatureRichnessScore(event) {
  if (!event || typeof event !== "object") return 0;

  let score = 0;

  for (const path of FEATURE_RICHNESS_PATHS) {
    const value = getAt(event, path);
  ...
  if (hasEntrySignal(event)) score += 8;
  if (getDefinitionText(event)) score += 8;
  if (hasNumericOutcome(event)) score -= 2;

  return score;
}

function pickFeatureEvent(events) {
  return [...safeArray(events)]
    .sort((a, b) => {
      const entryDiff = Number(hasEntrySignal(b)) - Number(hasEntrySignal(a));
      if (entryDiff !== 0) return entryDiff;

      const richDiff = getFeatureRichnessScore(b) - getFeatureRichnessScore(a);
      if (richDiff !== 0) return richDiff;

      return getEntryTs(a) - getEntryTs(b);
    })[0] || null;
}

function pickOutcomeEvent(events) {
  const sorted = [...safeArray(events)].sort((a, b) => getExitTs(b) - getExitTs(a));

  return (
    sorted.find(event => hasNumericOutcome(event) && hasExitSignal(event)) ||
    sorted.find(hasNumericOutcome) ||
    sorted.find(hasExitSignal) ||
    null
  );
}

function copyOutcomeFields(target, outcomeEvent) {
  if (!target || !outcomeEvent) return target;

  const rawR = getRawR(outcomeEvent);
  const rawPnlPct = getRawPnlPct(outcomeEvent);

  if (rawR !== null) {
    target.realizedR = rawR;
    target.pnlR = rawR;
    target.closedR = rawR;
    target.resultR = rawR;
  }
  ...
  if (rawPnlPct !== null) {
    target.pnlPct = rawPnlPct;
    target.realizedPnlPct = rawPnlPct;
    target.closedPnlPct = rawPnlPct;
    target.resultPnlPct = rawPnlPct;
  }

  target.exitPrice = outcomeEvent.exitPrice ?? outcomeEvent.exit?.price ?? target.exitPrice;
  target.exit = outcomeEvent.exit ?? target.exit;
  target.exitReason = outcomeEvent.exitReason ?? outcomeEvent.reason ?? outcomeEvent.status ?? target.exitReason;
  target.closedAt = outcomeEvent.closedAt ?? outcomeEvent.exitAt ?? outcomeEvent.exitTs ?? target.closedAt;
  target.exitAt = outcomeEvent.exitAt ?? outcomeEvent.closedAt ?? outcomeEvent.exitTs ?? target.exitAt;
  target.exitTs = outcomeEvent.exitTs ?? outcomeEvent.closedAt ?? outcomeEvent.exitAt ?? target.exitTs;

  const outcomeIsClosed = hasExitSignal(outcomeEvent) && hasNumericOutcome(outcomeEvent);

  target.closed = outcomeIsClosed;
  target.isClosed = outcomeIsClosed;

  return target;
}

function mergeTradeEventGroup(tradeId, events) {
  const sorted = [...safeArray(events)].sort((a, b) => getEventTs(a) - getEventTs(b));

  if (sorted.length <= 1) return sorted[0];

  const featureEvent = pickFeatureEvent(sorted) || sorted[0];
  const outcomeEvent = pickOutcomeEvent(sorted);

  if (!outcomeEvent) {
    return {
      ...featureEvent,
      tradeId: tradeId || getTradeId(featureEvent),
      analyzeKind: featureEvent.analyzeKind || "TRADE_RECORD",
      mergedTradeRecord: true,
      sourceEventCount: sorted.length,
      sourceEntryEventCount: sorted.filter(hasEntrySignal).length,
      sourceExitEventCount: sorted.filter(hasExitSignal).length
    };
  }

  const entryTs = getEntryTs(featureEvent, getEventTs(featureEvent));
  const exitTs = getExitTs(outcomeEvent, getEventTs(outcomeEvent));

  const merged = {
    ...outcomeEvent,
    ...featureEvent,

    tradeId: tradeId || getTradeId(featureEvent) || getTradeId(outcomeEvent),
    positionId: featureEvent.positionId ?? outcomeEvent.positionId,
    orderId: featureEvent.orderId ?? outcomeEvent.orderId,
    ...
    entryTs,
    analyzeUpdatedAt:
      featureEvent.analyzeUpdatedAt ??
      featureEvent.openedAt ??
      featureEvent.entryAt ??
      featureEvent.entryTs ??
      featureEvent.createdAt ??
      entryTs,

    latestEventTs: Math.max(...sorted.map(event => getEventTs(event, 0))),
    outcomeEventTs: exitTs,

    entrySourceAction: featureEvent.action ?? featureEvent.status ?? null,
    outcomeSourceAction: outcomeEvent.action ?? outcomeEvent.status ?? null
  };

  return copyOutcomeFields(merged, outcomeEvent);
}

function consolidateTradeEvents(events) {
  ...
  for (const event of safeArray(events)) {
    ...
    if (!groups.has(tradeId)) groups.set(tradeId, []);
    groups.get(tradeId).push(event);
  }

  const merged = [...passthrough];

  for (const [tradeId, groupedEvents] of groups.entries()) {
    const row = mergeTradeEventGroup(tradeId, groupedEvents);

    if (row) merged.push(row);
  }

  return merged.sort((a, b) => getEventTs(a) - getEventTs(b));
}
...
function normalizeSession(event) {
  ...
  const hour = new Date(getEntryTs(event)).getUTCHours();
...
function mapAllowlistRow(row, extra = {}) {
  return {
    id: row.id,
    ...
    trades: row.trades,
    open: row.open,
    pendingOutcome: row.pendingOutcome,
...
function balancedRows(rows, options = {}) {
  const sorted = safeArray(rows).sort(sortRows);

  const limit = Math.max(1, Math.round(safeNumber(options.limit, DEFAULT_ALLOWLIST_LIMIT)));
  const longLimit = Math.max(0, Math.round(safeNumber(options.familyCountLong, limit)));
  const shortLimit = Math.max(0, Math.round(safeNumber(options.familyCountShort, limit)));

  const selected = [];
  const selectedIds = new Set();

  const addRows = side => {
    const sideLimit = side === "LONG" ? longLimit : shortLimit;

    for (const row of sorted) {
      if (selected.length >= limit) return;
      if (selected.filter(item => item.side === side).length >= sideLimit) return;
  ...
  return selected.sort(sortRows);
}

function selectAllowlistRows(analysis, options = {}) {
  const level = options.level || "micro";
  const rows = getRowsForLevel(analysis, level);

  return balancedRows(
    safeArray(rows).filter(row => isAllowedRow(row, options)),
    options
  );
}

function selectCollectingFallbackRows(analysis, options = {}) {
  const level = options.level || "micro";
  const rows = getRowsForLevel(analysis, level);
  const minClosed = Math.max(1, Math.round(safeNumber(options.minClosed, DEFAULT_FALLBACK_MIN_CLOSED)));
...
function selectObservedFallbackRows(analysis, options = {}) {
  const level = options.level || "micro";
  const rows = getRowsForLevel(analysis, level);
  const minObserved = Math.max(1, Math.round(safeNumber(options.minObserved, DEFAULT_FALLBACK_MIN_OBSERVED)));

  return balancedRows(
    safeArray(rows)
      .filter(row => safeNumber(row.observed, 0) >= minObserved)
      .filter(row => row.status !== "BAD")
      .filter(row => row.status !== "EMPTY"),
    options
  );
...
function familyIdsFromRows(rows) {
  return uniqueCleanTokens(
    safeArray(rows).map(row => row.familyId || row.id)
  );
}

function buildMicroRotationAllowlist(analysis, options = {}) {
  const limit = Math.max(
    1,
    Math.round(safeNumber(options.limit, DEFAULT_ROTATION_ALLOWLIST_LIMIT))
  );

  const familyCountLong = Math.max(
    0,
    Math.round(safeNumber(options.familyCountLong, analysis?.config?.familyCountLong ?? 50))
  );

  const familyCountShort = Math.max(
    0,
    Math.round(safeNumber(options.familyCountShort, analysis?.config?.familyCountShort ?? 50))
  );

  const strictRowsRaw = selectAllowlistRows(analysis, {
    ...options,
    level: "micro",
    minStatus: options.minStatus || "STABLE",
    minClosed: options.minClosed ?? analysis?.config?.minMicroClosed ?? DEFAULT_MIN_MICRO_CLOSED,
    limit,
    familyCountLong,
    familyCountShort
  });

  const strictRows = strictRowsRaw.map(row => mapAllowlistRow(row, {
    allowlistMode: "STRICT_STABLE"
  }));

  if (strictRows.length > 0) {
    const activeMicroFamilyIds = familyIdsFromRows(strictRows);
    ...
    return {
      ...
      longCount: strictRows.filter(row => row.side === "LONG").length,
      shortCount: strictRows.filter(row => row.side === "SHORT").length
    };
  }

  const enableCollectingFallback = options.enableCollectingFallback ?? DEFAULT_ENABLE_COLLECTING_FALLBACK;

  if (enableCollectingFallback) {
    const fallbackRowsRaw = selectCollectingFallbackRows(analysis, {
      ...options,
      level: "micro",
      minClosed: options.fallbackMinClosed ?? DEFAULT_FALLBACK_MIN_CLOSED,
      limit,
      familyCountLong,
      familyCountShort
    });

    const fallbackRows = fallbackRowsRaw.map(row => mapAllowlistRow(row, {
      allowlistMode: "COLLECTING_CLOSED_FALLBACK"
    }));

    if (fallbackRows.length > 0) {
      const activeMicroFamilyIds = familyIdsFromRows(fallbackRows);

      return {
        ok: true,
        usable: true,
        enabled: true,
        mode: "COLLECTING_CLOSED_FALLBACK",
        bootstrap: true,
        rows: fallbackRows,
        strictRows,
        fallbackRows,
        activeMicroFamilyIds,
        allowedMicroFamilyIds: activeMicroFamilyIds,
        longCount: fallbackRows.filter(row => row.side === "LONG").length,
        shortCount: fallbackRows.filter(row => row.side === "SHORT").length
      };
    }
  }

  const enableObservedFallback = options.enableObservedFallback ?? DEFAULT_ENABLE_OBSERVED_FALLBACK;

  if (enableObservedFallback) {
    const observedRowsRaw = selectObservedFallbackRows(analysis, {
      ...options,
      level: "micro",
      minObserved: options.fallbackMinObserved ?? DEFAULT_FALLBACK_MIN_OBSERVED,
      limit,
      familyCountLong,
      familyCountShort
    });

    const observedRows = observedRowsRaw.map(row => mapAllowlistRow(row, {
      allowlistMode: "OBSERVED_BOOTSTRAP_FALLBACK"
    }));

    if (observedRows.length > 0) {
      const activeMicroFamilyIds = familyIdsFromRows(observedRows);

      return {
        ok: true,
        usable: true,
        enabled: true,
        mode: "OBSERVED_BOOTSTRAP_FALLBACK",
        bootstrap: true,
        rows: observedRows,
        strictRows,
        fallbackRows: observedRows,
        activeMicroFamilyIds,
        allowedMicroFamilyIds: activeMicroFamilyIds,
        longCount: observedRows.filter(row => row.side === "LONG").length,
        shortCount: observedRows.filter(row => row.side === "SHORT").length
      };
    }
  }
...
export function buildMainDiscordAllowlist(analysis, options = {}) {
  ...
  const limit = Math.max(1, safeNumber(options.limit, DEFAULT_ALLOWLIST_LIMIT));

  return selectAllowlistRows(analysis, {
    ...options,
    level,
    minClosed,
    minStatus: options.minStatus || "STABLE",
    limit
  }).map(row => mapAllowlistRow(row));
}
...
export function buildMainFamilyMicroAnalysis(events, options = {}) {
  ...
  const rawTradeEvents = rawEvents.filter(isTradeLike);
  const selectedEvents = consolidateTradeEvents(rawTradeEvents);
  ...
  const familyCountLong = Math.max(
    1,
    Math.round(safeNumber(options.familyCountLong, 50))
  );

  const familyCountShort = Math.max(
    1,
    Math.round(safeNumber(options.familyCountShort, 50))
  );
  ...
  const analysis = {
    ...
    config: {
      ...
      allowlistLimit: safeNumber(options.allowlistLimit, DEFAULT_ALLOWLIST_LIMIT),
      rotationAllowlistLimit: safeNumber(options.rotationAllowlistLimit, DEFAULT_ROTATION_ALLOWLIST_LIMIT),
      fallbackMinClosed: safeNumber(options.fallbackMinClosed, DEFAULT_FALLBACK_MIN_CLOSED),
      fallbackMinObserved: safeNumber(options.fallbackMinObserved, DEFAULT_FALLBACK_MIN_OBSERVED),
      enableCollectingFallback: options.enableCollectingFallback ?? DEFAULT_ENABLE_COLLECTING_FALLBACK,
      enableObservedFallback: options.enableObservedFallback ?? DEFAULT_ENABLE_OBSERVED_FALLBACK,
      ...
      note: "Events worden eerst per tradeId samengevoegd zodat entry-known microfeatures gekoppeld blijven aan exit/outcome. Microfamilies gebruiken entry-known velden; outcome-data wordt alleen gebruikt voor fair-winrate ranking/statistiek. avgR/PF zijn tie-breaks, niet primair."
    },
    ...
    summary: {
      ...
      consolidatedTrades: selectedEvents.length,
      mergedTradeRecords: selectedEvents.filter(event => event?.mergedTradeRecord).length,
  ...
  analysis.allowlists.parent = buildMainDiscordAllowlist(analysis, {
    ...
    minStatus: "STABLE",
    limit: safeNumber(options.allowlistLimit, DEFAULT_ALLOWLIST_LIMIT),
    familyCountLong,
    familyCountShort
  });
  ...
  const microRotation = buildMicroRotationAllowlist(analysis, {
    level: "micro",
    minClosed: minMicroClosed,
    minStatus: "STABLE",
    limit: safeNumber(options.rotationAllowlistLimit, DEFAULT_ROTATION_ALLOWLIST_LIMIT),
    familyCountLong,
    familyCountShort,
    fallbackMinClosed: options.fallbackMinClosed,
    fallbackMinObserved: options.fallbackMinObserved,
    enableCollectingFallback: options.enableCollectingFallback,
    enableObservedFallback: options.enableObservedFallback
  });

  analysis.allowlists.micro = microRotation.rows;
  analysis.allowlists.microStrict = microRotation.strictRows;
  analysis.allowlists.microFallback = microRotation.fallbackRows;
  analysis.allowlists.meta = {
    level: "micro",
    mode: microRotation.mode,
    bootstrap: microRotation.bootstrap,
    usable: microRotation.usable,
    enabled: microRotation.enabled,
    activeMicroFamilyIds: microRotation.activeMicroFamilyIds.length,
    allowedMicroFamilyIds: microRotation.allowedMicroFamilyIds.length,
    longCount: microRotation.longCount,
    shortCount: microRotation.shortCount
  };

  analysis.activeMicroFamilyIds = microRotation.activeMicroFamilyIds;
  analysis.allowedMicroFamilyIds = microRotation.allowedMicroFamilyIds;
  analysis.microFamilyIds = microRotation.activeMicroFamilyIds;
  analysis.activeFamilyIds = microRotation.activeMicroFamilyIds;
  analysis.allowedFamilyIds = microRotation.allowedMicroFamilyIds;
  ...
  analysis.rotation = {
    ...
    usable: microRotation.usable,
    bootstrap: microRotation.bootstrap,
    source: "familyMicroAnalyzer",
    mode: microRotation.mode,
    rankingMode: "FAIR_WINRATE_WILSON_BAYES",
    rankingMetric: "FAIR_WINRATE_WILSON_BAYES",
    generatedAt: analysis.generatedAt,

    activeMicroFamilyIds: microRotation.activeMicroFamilyIds,
    allowedMicroFamilyIds: microRotation.allowedMicroFamilyIds,
    microFamilyIds: microRotation.activeMicroFamilyIds,

    activeFamilyIds: microRotation.activeMicroFamilyIds,
    allowedFamilyIds: microRotation.allowedMicroFamilyIds,
    familyIds: microRotation.activeMicroFamilyIds,

    allowlist: microRotation.rows,
    activeFamilies: microRotation.rows,
    allowedFamilies: microRotation.rows,
    topFamilies: microRotation.rows,

    longCount: microRotation.longCount,
    shortCount: microRotation.shortCount,

    meta: {
      strictRows: microRotation.strictRows.length,
      fallbackRows: microRotation.fallbackRows.length,
      parentFamilies: parentFamilies.length,
      subFamilies: subFamilies.length,
      microFamilies: microFamilies.length,
      rawTradeEvents: rawTradeEvents.length,
  ...
  analysis.weeklyRotation = analysis.rotation;

  analysis.enabled = true;
  analysis.usable = microRotation.usable;
  analysis.bootstrap = microRotation.bootstrap;
