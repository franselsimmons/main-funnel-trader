...
function isClosed(event) {
  ...
  const status = toKey(event?.status || event?.action || event?.exitReason, "");

  if (status.includes("CLOSED")) return true;
  if (status.includes("EXIT")) return true;
  if (status.includes("TP")) return true;
  if (status.includes("SL")) return true;

  return event?.exitPrice !== undefined && event?.exitPrice !== null;
}
...
function getRealizedR(event) {
  const value = firstDefined(event, [
    "realizedR",
    "pnlR",
    "resultR",
    "outcomeR",
...
function classifyStatus(row, minClosed) {
  ...
  const pf = safeNumber(row.profitFactor, 0);
  ...
  if (avgR >= 0.75 && winrateNum >= 70 && pf >= 2.25) {
    status = "ELITE";
  } else if (avgR >= 0.45 && winrateNum >= 60 && pf >= 1.75) {
    status = "HOT";
  } else if (avgR >= 0.25 && winrateNum >= 55 && pf >= 1.35) {
    status = "GOOD";
  } else if (avgR >= 0.05 && winrateNum >= 45 && pf >= 1.05) {
    status = "STABLE";
  } else if (avgR > 0 && pf >= 1) {
    status = "CANDIDATE";
  }
...
function isAllowedRow(row, options = {}) {
  ...
  const minAvgR = safeNumber(options.minAvgR, 0);
  const minProfitFactor = safeNumber(options.minProfitFactor, 1.05);
...
export function getBestMainLongShort(analysis, options = {}) {
  ...
  return {
    ...
    bestLong: bestRow(rows, "LONG", {
      ...
      minAvgR: options.minAvgR ?? 0,
      minProfitFactor: options.minProfitFactor ?? 1.05
    }),
