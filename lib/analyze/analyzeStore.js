...
function normalizeAnalyzeEvent(event, fallbackTs = Date.now()) {
  ...
  return cleanObject({
    ...
    closed: action === "EXIT" ? true : Boolean(event.closed || event.isClosed),
    closedAt,
...
export async function appendAnalyzeEvents(events, context = {}) {
  ...
  for (const rawEvent of received) {
    ...
    if (action === "ENTRY") {
      ...
      indexes.entryByTradeId.set(tradeId, event);
      indexes.openByTradeId.set(tradeId, event);
  ...
  const closed = finalRecords.filter(row => row.closed === true).length;
