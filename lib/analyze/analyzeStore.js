...
function getMemoryStore() {
  ...
  if (Array.isArray(current)) {
    return normalizeStorePayload(current, "memory_legacy_array");
  }
...
function buildFilterSnapshot(event) {
  ...
  const cleanPicked = Object.fromEntries(
...
function normalizeEntryRecord(event, meta = {}) {
  ...
  return {
    ...
    familyDefinition: event?.familyDefinition || event?.definition || "",
...
function normalizeExitPatch(event, meta = {}) {
  ...
  return {
    ...
    exit: nullableNumber(event?.exit ?? event?.exitPrice ?? event?.price),
    exitPrice: nullableNumber(event?.exitPrice ?? event?.exit ?? event?.price),
...
function applyExitPatch(existing, patch) {
  ...
  return {
    ...
    analyzeFamilyId: existing.analyzeFamilyId || existing.familyId || "",
    familyFrozen: existing.familyFrozen,
    familyFrozenAt: existing.familyFrozenAt,
...
function normalizeTradeRecord(record) {
  ...
  const tradeId = extractTradeId(record) || safeString(record?.analyzeEventKey).trim();
  if (!tradeId) return null;

  const symbol = normalizeSymbol(record?.symbol);
  const side = normalizeSide(record?.side || record?.direction);
  ...
  return {
    ...
    entry: nullableNumber(record?.entry ?? record?.entryPrice),
    ...
    exit: nullableNumber(record?.exit ?? record?.exitPrice),
    exitPrice: nullableNumber(record?.exitPrice ?? record?.exit),
    exitReason: safeString(record?.exitReason ?? record?.reason),
    realizedR: nullableNumber(record?.realizedR ?? record?.pnlR ?? record?.resultR),
    pnlR: nullableNumber(record?.pnlR ?? record?.realizedR ?? record?.resultR),
    pnlPct: nullableNumber(record?.pnlPct ?? record?.pnlPercent),
    ...
    for (const trade of safeArray(store.trades)) {
      ...
      if (nextTs >= prevTs) {
        tradeMap.set(normalized.tradeId, {
          ...
          familyId: prev.familyId || normalized.familyId || "",
          analyzeFamilyId: prev.analyzeFamilyId || normalized.analyzeFamilyId || "",
          familyFrozen: prev.familyFrozen || normalized.familyFrozen,
...
export async function appendAnalyzeEvents(
  ...
  return {
    ...
    unmatchedExits: stats.unmatchedExits,
