...
function resolveActiveRotation(status) {
  ...
  if (status.activeRotation) return status.activeRotation;
  if (status.rotation) return status.rotation;
  if (status.active) return status.active;
  if (status.currentRotation) return status.currentRotation;
...
function hasUsableRotation(rotation) {
  ...
  if (rotation.status && rotation.status !== 'ACTIVE') return false;
...
export async function checkTradeSignalAgainstRotation(signal, options = {}) {
  ...
  if (!loaded.ok) {
    const decision = {
      ...
      candidateFamilyId: collectCandidateFamilyIds(signal)[0] || null,
      candidateFamilyIds: collectCandidateFamilyIds(signal),
