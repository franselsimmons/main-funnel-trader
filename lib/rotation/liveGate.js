  ...
  for (const path of paths) {
    ...
    for (const value of Object.values(map)) {
      ...
      if (value && typeof value === "object") {
        values.push(collectIdsFromFamilyRows([value]));
      }
    }
  }
...
function normalizeGate(rawGate = {}, options = {}) {
  ...
  const explicitEnabled = firstDefined(gate, ["enabled"]);
  const explicitOk = firstDefined(gate, ["ok"]);

  const enabled = parseBoolean(explicitEnabled, true);
  ...
  return {
    ...
    weekKey:
      options.weekKey ||
      firstDefined(gate, ["weekKey", "targetWeekKey", "activeWeekKey"]) ||
      null,
...
function buildResult({
  ...
  const exposedGate = buildExposedGate(gate, side, activeIds);
  ...
  return {
    ...
    weekKey: gate.weekKey ?? gate.targetWeekKey ?? null,
    targetWeekKey: gate.targetWeekKey ?? gate.weekKey ?? null,
