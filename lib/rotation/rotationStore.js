...
function inferLevelFromFamilyId(familyId, fallbackLevel = 'MICRO') {
  ...
  if (id === 'LONG_UNKNOWN' || id === 'SHORT_UNKNOWN') return 'PARENT';
...
export async function loadActiveRotationStatus({
  ...
  const rawRotation = await loadActiveRotation();
  const rotation = normalizeLoadedRotation(rawRotation);
...
function isAnalyzerFamilyRow(value) {
  ...
  const familyIdUpper = familyId.toUpperCase();

  const hasFamilyId =
    familyIdUpper.startsWith('MICRO_') ||
    familyIdUpper.startsWith('SUB_') ||
    familyIdUpper.includes('_LONG_') ||
    familyIdUpper.includes('_SHORT_');
...
export function isMicroFamilyActive(rotation, microFamilyId, side = null) {
  ...
  const normalizedMicroFamilyId = String(microFamilyId || '').trim();
  const normalizedSide = side ? parseSide(side) : null;

  return rotation.allowlist.map(normalizeAllowlistItem).some(item => {
    if (item.microFamilyId !== normalizedMicroFamilyId) return false;
...
export function isEntryAllowedByRotation(rotation, entry = {}) {
  ...
  const side = requireSideMatch
    ? parseSide(entry.side || entry.direction, inferredSide || 'LONG')
    : null;
