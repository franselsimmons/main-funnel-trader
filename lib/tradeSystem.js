...
// ================= STRATEGY VERSION =================
const STRATEGY_VERSION = "TS_V11_5_RSI_EDGE_RELAXED";
...
const MID_BULL_MAX_SPREAD_PCT = 0.00154;
...
const A_ENTRY_MIN_RR = 1.05;
const B_ENTRY_MIN_RR = 1.00;
const GOD_ENTRY_MIN_RR = 1.25;

const A_MIN_SNIPER = 48;
const A_MIN_CONFLUENCE = 60;
const B_MIN_SNIPER = 40;
const B_MIN_CONFLUENCE = 58;

const GOD_MIN_SNIPER = 75;
const GOD_MIN_CONFLUENCE = 75;

const MID_RSI_MIN_CONFLUENCE = 58;
const TREND_CONTINUATION_MIN_CONFLUENCE = 58;
const TREND_CONTINUATION_MIN_SNIPER = 55;

const A_FINAL_MIN_RR = 1.10;
...
const GLOBAL_MIN_CONFLUENCE = 52;
...
const NEUTRAL_OB_A_EXCEPTION_MIN_CONFLUENCE = 74;
const NEUTRAL_OB_A_EXCEPTION_MIN_RR = 1.15;
const NEUTRAL_OB_A_EXCEPTION_MIN_SNIPER = 68;
const NEUTRAL_OB_B_EXCEPTION_MIN_CONFLUENCE = 76;
const NEUTRAL_OB_B_EXCEPTION_MIN_RR = 1.08;
const NEUTRAL_OB_B_EXCEPTION_MIN_SNIPER = 72;
const NEUTRAL_OB_B_EXCEPTION_MIN_SCORE = 75;
...
// cleanup disabled optimizer chunks (eenmalig)
...
function buildCommonPayload(c, flow, sniper, funding, ob) {
  return {
    ...
    rsiEntryEdge: c._debugRsiEntryEdge || null,
    rsiConfluenceBoost: c._debugRsiConfluenceBoost ?? 0,
...
// ---- NIEUWE RSI EDGE HELPERS ----
function getRsiEntryEdgeMeta({ side, rsiZone, rsiSignal }) {
  const s = String(side || "").toLowerCase();
  const zone = String(rsiZone || "MID").toUpperCase();

  const longMap = {
    LOWER_3: { label: "EXTREME_PULLBACK_LONG", confluenceBoost: 14, gradePoints: 3, rrDiscount: 0.16, sniperDiscount: 4 },
    LOWER_2: { label: "DEEP_PULLBACK_LONG", confluenceBoost: 10, gradePoints: 2, rrDiscount: 0.11, sniperDiscount: 3 },
    LOWER_1: { label: "PULLBACK_LONG", confluenceBoost: 6, gradePoints: 1, rrDiscount: 0.06, sniperDiscount: 2 },
    MID: { label: "MID_LONG", confluenceBoost: 0, gradePoints: 0, rrDiscount: 0, sniperDiscount: 0 },
    UPPER_1: { label: "EXTENDED_LONG", confluenceBoost: -8, gradePoints: -2, rrDiscount: 0, sniperDiscount: 0 },
    UPPER_2: { label: "OVEREXTENDED_LONG", confluenceBoost: -14, gradePoints: -3, rrDiscount: 0, sniperDiscount: 0 },
    UPPER_3: { label: "EXTREME_EXTENSION_LONG", confluenceBoost: -20, gradePoints: -4, rrDiscount: 0, sniperDiscount: 0 }
  };

  const shortMap = {
    UPPER_3: { label: "EXTREME_PULLBACK_SHORT", confluenceBoost: 14, gradePoints: 3, rrDiscount: 0.16, sniperDiscount: 4 },
    UPPER_2: { label: "DEEP_PULLBACK_SHORT", confluenceBoost: 10, gradePoints: 2, rrDiscount: 0.11, sniperDiscount: 3 },
    UPPER_1: { label: "PULLBACK_SHORT", confluenceBoost: 6, gradePoints: 1, rrDiscount: 0.06, sniperDiscount: 2 },
    MID: { label: "MID_SHORT", confluenceBoost: 0, gradePoints: 0, rrDiscount: 0, sniperDiscount: 0 },
    LOWER_1: { label: "EXTENDED_SHORT", confluenceBoost: -8, gradePoints: -2, rrDiscount: 0, sniperDiscount: 0 },
    LOWER_2: { label: "OVEREXTENDED_SHORT", confluenceBoost: -14, gradePoints: -3, rrDiscount: 0, sniperDiscount: 0 },
    LOWER_3: { label: "EXTREME_EXTENSION_SHORT", confluenceBoost: -20, gradePoints: -4, rrDiscount: 0, sniperDiscount: 0 }
  };
  ...
  const continuationBonus =
    zone === "MID" && Boolean(rsiSignal?.continuationOK)
      ? 4
      : 0;

  const reversalBonus =
    s === "bull"
      ? Boolean(rsiSignal?.rising || rsiSignal?.m15?.reclaimFromLower)
      : Boolean(rsiSignal?.falling || rsiSignal?.m15?.rejectionFromUpper);

  return {
    zone,
    label: continuationBonus > 0
      ? `${meta.label}_CONTINUATION`
      : meta.label,

    confluenceBoost: meta.confluenceBoost + continuationBonus + (reversalBonus ? 2 : 0),
    gradePoints: meta.gradePoints + (continuationBonus > 0 ? 1 : 0),
    rrDiscount: meta.rrDiscount + (continuationBonus > 0 ? 0.03 : 0),
    sniperDiscount: meta.sniperDiscount + (continuationBonus > 0 ? 1 : 0)
  };
}

function applyRsiEdgeToConfluence(confluence, rsiEdge) {
  return clamp(
    Number(confluence || 0) + Number(rsiEdge?.confluenceBoost || 0),
    0,
    100
  );
}

function getRsiAdjustedRequiredRR({ requiredRR, rsiEdge, setupClass }) {
  const raw = Number(requiredRR || 0);
  const discount = Number(rsiEdge?.rrDiscount || 0);
  const klass = String(setupClass || "").toUpperCase();

  let floor = 0.95;

  if (klass === "A") floor = 1.03;
  if (klass === "GOD") floor = 1.12;
  if (klass === "A_SHORT_EXCEPTION") floor = 1.10;
  if (klass === "B_TREND_PROBE") floor = 1.04;

  return Number(Math.max(floor, raw - discount).toFixed(2));
}

function getRsiAdjustedRequiredSniper({ requiredSniper, rsiEdge }) {
  return Math.max(
    35,
    Number(requiredSniper || 0) - Number(rsiEdge?.sniperDiscount || 0)
  );
}

function getRsiSetupRrCredit(rsiEdge) {
  return Math.min(0.10, Math.max(0, Number(rsiEdge?.rrDiscount || 0)));
}
...
function getSetupGrade({ c, ob, flow, sniper, confluence, rr, hasLiquidationData, isBull }) {
  ...
  // ---- RSI edge grade points ----
  const rsiEdge = c?._debugRsiEdgeMeta || getRsiEntryEdgeMeta({
    side: c?.side,
...
function buildCurrentTsMasterPreset() {
  return {
    ...
    NEUTRAL_OB_A_EXCEPTION_MIN_CONFLUENCE: 74,
    NEUTRAL_OB_A_EXCEPTION_MIN_RR: 1.15,
    NEUTRAL_OB_A_EXCEPTION_MIN_SNIPER: 68,

    NEUTRAL_OB_B_EXCEPTION_MIN_CONFLUENCE: 76,
    NEUTRAL_OB_B_EXCEPTION_MIN_RR: 1.08,
    NEUTRAL_OB_B_EXCEPTION_MIN_SNIPER: 72,
    NEUTRAL_OB_B_EXCEPTION_MIN_SCORE: 75,
    ...
    ENABLE_BULLISH_MID_TREND_PROBES,
    BULLISH_MID_TREND_PROBE_MIN_CONFLUENCE: 60,
    BULLISH_MID_TREND_PROBE_MIN_SNIPER: 42,
    BULLISH_MID_TREND_PROBE_MIN_RR: 1.12,
    BULLISH_MID_TREND_PROBE_MAX_SPREAD_PCT: 0.00110,
    BULLISH_MID_TREND_PROBE_MIN_DEPTH_USD_1P: 60_000,
    BULLISH_MID_TREND_PROBE_REQUIRE_BTC_BULLISH,
    BULLISH_MID_TREND_PROBE_MIN_1H_MOVE_PCT,
    BULLISH_MID_TREND_PROBE_MIN_24H_MOVE_PCT,
    BULLISH_MID_TREND_PROBE_MIN_PULLBACK_FROM_HIGH_PCT,
...
function normalizeRsiSignalSafe(raw, rsiData) {
  ...
  const hasM15 =
    ...
    (raw?.rsi !== undefined || m15?.valid === true);

  const hasH1 =
    Number.isFinite(Number(rsiHTF)) &&
    (raw?.rsiHTF !== undefined || h1?.valid === true);
...
async function getSafeRsiSignal({ c, rsiData, isBull, btcState }) {
  ...
  const value = await callFirstValid(
    ...
    [
      ...
      () => getRSISignal(rsiData, c.side)
    ],
    isUsableRsiSignal,
    fallback
  );

  return normalizeRsiSignalSafe(value, rsiData);
}

function buildFallbackSniper({ c, flow, ob, rsiZone, isBull }) {
  let score = 0;

  score += clamp(Number(c.moveScore || 0) * 0.45, 0, 40);

  if (String(c.stage || "").toLowerCase() === "entry") score += 10;
  if (String(c.stage || "").toLowerCase() === "almost") score += 6;

  if (flow?.type === "TREND") score += 10;
  if (flow?.type === "BUILDING") score += 6;

  if (isObWithSide(ob, isBull)) score += 10;
  if (isObAgainstSide(ob, isBull)) score -= 8;

  if (isBull && ["LOWER_1", "LOWER_2", "MID"].includes(rsiZone)) score += 8;
  if (!isBull && ["UPPER_1", "UPPER_2", "MID"].includes(rsiZone)) score += 8;
...
async function getSafeSniperEntry({ c, flow, ob, rsiSignal, rsiZone, isBull, btcState }) {
  ...
  const score = safeNumber(raw?.score ?? raw?.sniperScore, fallback.score);

  return {
    ...raw,
    type: raw?.type || raw?.signal || fallback.type,
    score,
    valid: raw?.valid !== false && score >= B_MIN_SNIPER
  };
}
...
function buildFallbackRiskGeometry({ c, ob, isBull }) {
  if (process.env.TS_ALLOW_FALLBACK_RISK !== "true") {
    return null;
  }
  ...
  const spread = normalizeSpread(ob?.spreadPct);
  const riskPct = clamp(0.006 + spread * 4, 0.004, 0.018);
...
function buildFallbackConfluence({ c, flow, sniper, ob, rr, rsiZone, isBull }) {
  let score = 0;

  score += clamp(Number(c.moveScore || 0) * 0.35, 0, 35);
  score += clamp(Number(sniper?.score || 0) * 0.30, 0, 30);

  if (flow?.type === "TREND") score += 12;
  if (flow?.type === "BUILDING") score += 7;

  if (isObWithSide(ob, isBull)) score += 10;
  if (isObAgainstSide(ob, isBull)) score -= 10;
  ...
  if (isBull && ["LOWER_1", "LOWER_2", "MID"].includes(rsiZone)) score += 5;
  if (!isBull && ["UPPER_1", "UPPER_2", "MID"].includes(rsiZone)) score += 5;
...
async function getSafeConfluence({
  ...
  return safeNumber(raw?.score ?? raw?.confluence ?? raw, fallback);
}
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    // Eenmalige cleanup van oude optimizer chunks
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        // RSI edge calculation
        const rsiEdge = getRsiEntryEdgeMeta({
          side: c.side,
          rsiZone,
          rsiSignal
        });
        ...
        c._debugRsiConfluenceBoost = rsiEdge.confluenceBoost;
        c._debugRsiRrDiscount = rsiEdge.rrDiscount;
        c._debugRsiSniperDiscount = rsiEdge.sniperDiscount;
        ...
        // Hard block only for UPPER_2/3 longs; UPPER_1 gets penalty via rsiEdge
        if (isBull && ["UPPER_2", "UPPER_3"].includes(rsiZone)) {
          const payload = buildWait(
            c,
            "RSI_LONG_TOO_HIGH",
            flow,
        ...
        const candidateForValidation = {
          ...
          rsiContinuationOK: Boolean(rsiSignal?.continuationOK),
          rsiContinuationScore: Number(rsiSignal?.continuationScore || 0),
        ...
        // ---- setup detection with RR credit ----
        ...
        const requiredRR = getRsiAdjustedRequiredRR({
          requiredRR: rawRequiredRR,
          rsiEdge,
        ...
        if (
          String(obData?.bias || "NEUTRAL").toUpperCase() === "NEUTRAL" &&
          ...
          const requiredNeutralConf =
            setup.setupClass === "A" || setup.setupClass === "GOD"
        ...
        const rawFinalRequiredRR =
          ["A", "GOD", "A_SHORT_EXCEPTION"].includes(setup.setupClass)
            ? Math.max(requiredRR, A_FINAL_MIN_RR)
            : requiredRR;

        const finalRequiredRR = getRsiAdjustedRequiredRR({
          requiredRR: rawFinalRequiredRR,
          rsiEdge,
          setupClass: setup.setupClass
        });
        ...
        console.log("TS_ENTRY_OPENED:", JSON.stringify({
          ...
          rsiConfluenceBoost: rsiEdge.confluenceBoost,
