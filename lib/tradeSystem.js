...
async function fetchCandles(symbol, timeframe = "1h", limit = 150){
  ...
  const url = `https://api.bitget.com/api/v2/mix/market/candles?symbol=${symbol}&granularity=${granularity}&limit=${limit}`;
...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}){
  ...
  await Promise.all(candidates.map(async (c) => {
    ...
    try{
      // 🔥 BITGET FIX: gebruik rawBitgetSymbol of standaard symbol+USDT
      const pair = c.rawBitgetSymbol || (symbol + "USDT");
