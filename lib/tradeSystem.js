...
const MICRO_ROTATION_USE_CURRENT_WEEK_BOOTSTRAP =
  process.env.MICRO_ROTATION_USE_CURRENT_WEEK_BOOTSTRAP === "false";
...
async function backfillMicroLearningFromRuntimeOutcomes({
  ...
  if (!force && activeIdsBefore.length > 0) {
    return {
      ok: true,
      skipped: true,
      reason: "ACTIVE_ROTATION_ALREADY_READY",
  ...
  if (!rows.length) {
    runtimeBackfill.lastAt = now;
    runtimeBackfill.lastRowsScanned = 0;
    ...
    runtimeBackfill.lastReason = "NO_RUNTIME_OUTCOME_ROWS";
    runtimeBackfill.lastSource = "RUNTIME";

    return {
      ok: false,
      skipped: true,
      reason: "NO_RUNTIME_OUTCOME_ROWS"
    };
  }

  const backfillWeek = createMicroWeekSummary(getIsoWeekKey(now));
  backfillWeek.source = "TRADE_SYSTEM_RUNTIME_OUTCOME_BACKFILL";
  ...
  state.activeRotation = activeIds.length
    ? {
        ...activeRotation,
        source: "TRADE_SYSTEM_RUNTIME_BACKFILL_ROTATION",
        sourceWeekKey: backfillWeek.weekKey,
        refreshedAt: now,
        bootstrap: false
      }
    : null;
  ...
  runtimeBackfill.lastAt = now;
  runtimeBackfill.lastRowsScanned = rows.length;
  ...
  runtimeBackfill.lastReason =
    activeIds.length > 0
      ? "RUNTIME_BACKFILL_ROTATION_READY"
      : familyCount === 0
