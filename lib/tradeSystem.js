...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    if(rsiData){
      ...
      if(isBull && rsi < 30) rsiStrength = 2;
      if(!isBull && rsi > 70) rsiStrength = 2;
    }
    ...
    // ================= STAGE FILTER (alleen entry + almost) =================
    ...
    // ================= ENTRY FILTERS (alleen nog cooldowns, geen harde condities meer) =================
    ...
    // ================= SETUP VALID (scheidt setup van timing) =================
    const setupIsValid =
      flow.type !== "NEUTRAL" &&
      confluence >= minConf;
    ...
    const setupScoreOk =
      setupGrade.grade === "A" ||
      (setupGrade.grade === "B" && confluence >= 65);

    const structureOk =
      !isObAgainstSide(ob, isBull) ||
      confluence >= 70;
    ...
    // ================= ENTRY READY (sniper of RSI timing) =================
    const sniperScore = getSniperScore(sniper);
    const sniperReady =
      sniper?.valid &&
      sniperScore >= 55;
    ...
    const entryReady =
      sniperReady || rsiReady;

    // ================= ENTRY BLOCK (nieuwe, vereenvoudigde logica) =================
    if(
      c.stage === "entry" &&
      setupValid &&
      entryReady &&
      ...
      const reasonEntry = sniperReady ? (sniper?.type || "SNIPER") : "RSI_TIMING";

      // Dynamische TP op basis van RSI strength
