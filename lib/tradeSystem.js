...
const DURABLE_LOCK_ATTEMPTS = Number(
  process.env.TRADE_SYSTEM_LOCK_ATTEMPTS || 2
);

const DURABLE_LOCK_RETRY_MS = Number(
  process.env.TRADE_SYSTEM_LOCK_RETRY_MS || 500
);
const DURABLE_LOCK_TTL_MS = Number(
  process.env.TRADE_SYSTEM_LOCK_TTL_MS || 45_000
);
...
async function acquireRuntimeLock(owner) {
  ...
  for (let attempt = 0; attempt < DURABLE_LOCK_ATTEMPTS; attempt++) {
    try {
      ...
      if (attempt === DURABLE_LOCK_ATTEMPTS - 1) {
...
export async function processTrades(input, options = {}) {
  ...
  if (!lockAcquired) {
    console.warn("TRADE_SYSTEM_RUN_SKIPPED_LOCK_BUSY:", JSON.stringify({
      runId,
    ...
    return {
      ok: true,
      skipped: true,
      reason: "TRADE_SYSTEM_LOCK_BUSY_PREVIOUS_RUN_ACTIVE",
      actions: [],
      candidatesCount: 0,
      strategyVersion: STRATEGY_VERSION,
      durableEnabled: true,
