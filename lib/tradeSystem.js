...
const MAX_SPREAD_PCT = 0.0025; // Verlaagd naar 0.25%
const MIN_DEPTH_USD_1P = 200000; // Verhoogd naar 200k
const MIN_RR_FLOOR = 1.05; // Minimale RR is nu 1.1

// ================= DYNAMIC RR FLOOR =================
const GRADE_A_MIN_RR_FLOOR = 1.0;  // Zelfs perfecte setups moeten 1:1 zijn
...
function getDynamicMinRrFloor({
  ...
  if(
    ...
    floor = Math.min(floor, 0.85); // Aangepast voor ultra elite
  }

  // De absoluut laagste RR is nu 1.0, ongeacht de perfectie van de setup
...
export async function processTrades(
  ...
  // NIEUW: haal 1u candles voor RSI
  const candles1hGlobal = market?.candles1h || [];
  ...
  for(const originalCoin of candidates){
    ...
    // NIEUW: RSI berekenen voor deze coin (op basis van 1u candles)
    const rsi1h = calculateRSI(candles1hGlobal, 14);
    const rsiZone = getRsiZone(rsi1h);
    ...
    // ================= 🔥 NIEUWE RSI ALIGNMENT CHECK =================
    const rsiAligned = isRsiAligned(isBull, rsiZone);
    if(!rsiAligned){
      const waitPayload = buildWait(
        c,
        "RSI_NOT_ALIGNED",
