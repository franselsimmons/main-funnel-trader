...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // Setup valid (zonder RSI als harde eis)
    const setupValid = stageOK && sniperOK && gradeOK && !ob.spoof && rr >= minRrFloor;

    // Elite entry met nieuwe RSI signal
    const eliteEntry = setupValid &&
                        rsiSignal.valid &&
                        rsiSignal.strength >= 1 &&
                        confluence >= 60;
    ...
    if (eliteEntry) {
      ...
      if (certaintyMode === "safe") {
        ...
        // RSI alleen voor TP tuning (gebruik de RSI uit het signaal)
        if (isBull) {
