// tradeSystem.js - Definitieve versie met robuuste RSI context check
// - RSI context wordt alleen gebruikt als rsi (number) en zones bestaan
// - Geen vertrouwen op .valid flag (bestaart niet altijd)
// - Alle eerdere fixes: MID filter na confluence, BTC neutral 70, earlyRSI 75, sniper?.score
// - Debug logs na riskBase/rr
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    if (isBull && !["LOWER_2","LOWER_3"].includes(rsiZone) && !earlyRSI) {
      const waitPayload = buildWait(c, "RSI_NOT_OVERSOLD_ENOUGH", flow, sniper, 0, 0, funding, ob, null, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    if (isBull && rsiZone === "LOWER_2" && c.change1h > -0.2) {
      const waitPayload = buildWait(c, "RSI_NOT_DEEP_ENOUGH", flow, sniper, 0, 0, funding, ob, null, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    if (!isBull && !["UPPER_2","UPPER_3"].includes(rsiZone) && !earlyRSI) {
      const waitPayload = buildWait(c, "RSI_NOT_OVERBOUGHT_ENOUGH", flow, sniper, 0, 0, funding, ob, null, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    if (!isBull && rsiZone === "UPPER_2" && c.change1h < 0.2) {
      const waitPayload = buildWait(c, "RSI_NOT_HIGH_ENOUGH", flow, sniper, 0, 0, funding, ob, null, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    // 🔥 ROBUUSTE RSI CONTEXT CHECK (geen .valid vlag)
    ...
    if (rsiZone === "MID" && confluence < 80) {
      const waitPayload = buildWait(c, "RSI_MID_NO_EDGE", flow, sniper, confluence, 0, funding, ob, null, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    // Debug log na riskBase (geen reference error)
    console.log(`📊 DATA: ${c.symbol} | hasRSI=${!!rsiContext} | conf=${confluence} | sniper=${sniperScore} | rr=${rr.toFixed(2)}`);
