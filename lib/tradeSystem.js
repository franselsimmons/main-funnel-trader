...
// Liquidation volledig uit (was 451)
...
const notifyState = new Map();      // eenvoudige flags voor notificaties
...
// ================= CANDLE FETCH (BITGET + retry + throttle) =================
...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  await Promise.all(candidates.map(async (c) => {
    ...
    const rsiContextM15 = candles15m.length ? getAdvancedRSIContext(candles15m) : { rsi: 50 };
    const rsiContextH1 = candles1h.length ? getAdvancedRSIContext(candles1h) : { rsi: 50 };
  ...
  for (const originalCoin of candidates) {
    ...
    // Elite entry drempels (gebruik eenvoudige variabelen)
    const eliteEntry = setupValid && confluence >= 60 && c.tfStrength >= 1 && getSniperScore(sniper) >= 60;
    ...
    if (prev) {
      ...
      if (hitTP || hitSL) {
        ...
        memory.delete(key);
        // cleanup notify states
        notifyState.delete(key);          // entry flag
      ...
      // HOLD: alleen notificatie als die nog niet gestuurd is
    ...
    // ========== ENTRY FILTERS (cooldowns, flow, rr, tf, etc.) ==========
