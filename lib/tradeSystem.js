...
const HOLD_LOG_INTERVAL_MS = 5 * 60 * 1000;
...
function buildWait(c, reason, flow, sniper, confluence, rr, funding, ob, risk, setupGrade){
  ...
  return {
    ...
    sl: risk?.sl ?? null,
    tp: risk?.tp ?? null,
    slSource: risk?.slSource || "risk_manager",
    tpSource: risk?.tpSource || "risk_manager",
...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    if(prev){
      ...
      if(
        shouldLog &&
        Date.now() - Number(state.holdLoggedAt || 0) >= HOLD_LOG_INTERVAL_MS
      ){
        await logAction(holdPayload, regimeLevel, btcState, true);
    ...
    if(
      ...
      const position = {
        ...
        slSource: riskBase.slSource || "risk_manager",
        tpSource: riskBase.tpSource || "risk_manager"
      };
