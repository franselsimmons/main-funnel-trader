...
const BULLISH_MID_TREND_PROBE_MIN_RR = 1.25;

const BULLISH_MID_TREND_PROBE_MAX_SPREAD_PCT = 0.00135;
...
const BULLISH_MID_TREND_PROBE_MIN_PULLBACK_FROM_HIGH_PCT = 0.0015;
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      if (obAgainst && confluence < 78 && !bullishMidTrendProbeOk) {
      ...
      if ((badSpread || badDepth) && confluence < 75 && !bullishMidTrendProbeOk) {
