...
export async function processTrades(input, options = {}) {
  ...
  const notifyResult = await sendExit({
    symbol: c.symbol,
    side: c.side,
    setupClass: pos.setupClass || "UNKNOWN",
    reason,
  ...
  if (notifyResult?.discordSent === true) {
    notifyState.set(exitKey, true);
  } else {
    console.warn("DISCORD_EXIT_NOT_CONFIRMED", JSON.stringify({
      symbol: c.symbol,
      side: c.side,
      setupClass: pos.setupClass || "UNKNOWN",
      grade: pos.grade,
  ...
  const notifyResult = await sendEntry({
    symbol: c.symbol,
    side: c.side,
    setupClass,
    entry: position.entry,
  ...
  if (notifyResult?.discordSent === true) {
    ...
    console.warn("DISCORD_ENTRY_NOT_CONFIRMED", JSON.stringify({
      symbol: c.symbol,
      side: c.side,
      setupClass,
      grade: position.grade,
