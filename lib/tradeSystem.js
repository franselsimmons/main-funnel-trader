// tradeSystem.js - Definitief met versoepelingen EN strengere entry eisen (geen C entry, MID RSI hard block)
// - isDeep niet meer nodig voor A/B entry
// - C entry verwijderd (was te slap)
// - B entry strenger: sniper>=60, conf>=65
// - A entry strenger: sniper>=70, conf>=75, rr>=1.1
// - MID RSI altijd blokkeren (ook bij trend continuation)
// - Momentum filter aangescherpt
...
const GRADE_B_MIN_RR_FLOOR = 1.05;     // verhoogd
const GRADE_C_MIN_RR_FLOOR = 1.05;     // C entries bestaan niet meer maar voor de vorm
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // ========== RSI-LOGICA (trend-continuation) - VERSTERKT ==========
    // Alleen LOWER_1 / UPPER_1 toegestaan, MID is altijd verboden
    ...
    // 🔥 HARD BLOK: MID RSI is nooit toegestaan, ook niet met trendContinuation
    if (rsiZone === "MID") {
