...
export async function processTrades(input, options = {}) {
  ...
  // 🔥 ACCEPTEER ZOWEL SCAN-OBJECT ALS DIRECTE ARRAY
  let candidatesRaw = [];
  let scanRegime = null;
  let scanBtc = null;

  if (Array.isArray(input)) {
    // Directe candidates (smart selector heeft al bepaald welke coins)
    candidatesRaw = input;
  } else {
    // Scan-object (oude fallback voor backward compatibility)
    candidatesRaw = [
  ...
  // ================= PREFILTER + DYNAMISCHE CAP (ALLEEN BIJ DIRECTE CANDIDATES) =================
  let candidates;
  if (Array.isArray(input)) {
    // 1. Filter zwakke scores en coins zonder stage (alleen 'entry' en 'almost' zijn relevant)
    ...
    // 2. Dynamische cap op basis van regime (meer coins in low vol, minder in high vol)
    let maxDeepAnalysis = 6; // default
    if (scanRegime === "LOW_VOL") maxDeepAnalysis = 10;
    if (scanRegime === "HIGH_VOL") maxDeepAnalysis = 4;

    // 3. Neem alleen de top maxDeepAnalysis (gesorteerd op score; smart selector sorteerde al, maar voor de zekerheid)
    preFiltered.sort((a, b) => Number(b.moveScore || 0) - Number(a.moveScore || 0));
    candidates = preFiltered.slice(0, maxDeepAnalysis);

    if (candidates.length === 0) {
      return { actions: [], message: "Geen coins na prefilter" };
    }
  } else {
    // Oude fallback: maximaal 8 coins, gesorteerd op score
    candidates = candidatesRaw
      .sort((a, b) => Number(b.moveScore || 0) - Number(a.moveScore || 0))
      .slice(0, 8);
    if (candidates.length === 0) {
      return { actions: [], message: "Geen entry/almost coins in scan" };
    }
  }
  ...
  // ========== FETCH ALLE DATA PER COIN (alleen voor de geselecteerde, beperkte candidate set) ==========
  ...
  await Promise.all(candidates.map(async (c) => {
    ...
    let funding = { rate: 0 };
    try {
      funding = await cachedFetch(`fund_${symbol}`, () => fetchFunding(symbol + "USDT"), 30000);
    } catch {}

    const candles5m = await cachedFetch(`c5_${cleanPair}`, () => fetchCandles(cleanPair, "5m", 100), 20000);
    ...
    const candles4h = await cachedFetch(`c4h_${cleanPair}`, () => fetchCandles(cleanPair, "4h", 100), 20000).catch(() => null);

    const mtfRsi = getMTFRSI({ m15: candles15m, h1: candles1h, h4: candles4h });
    const structureState = candles5m.length ? getStructureState(candles5m) : { trend: "NEUTRAL" };
  ...
  for (const originalCoin of candidates) {
    ...
    const setupValid = stageOK && finalSniperOK && gradeOK && !ob.spoof && rr >= minRrFloor;
    ...
    if (flow.type === "NEUTRAL" && confluence < 58) {
