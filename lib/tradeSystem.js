// tradeSystem.js - Definitief met alle aanscherpingen
// - C entry verwijderd
// - RR guard toegevoegd (fix 1)
// - Market quality minder agressief (fix 2)
// - Harde sniper filter (>=60) (fix 3)
// - Building filter aangescherpt (fix 5)
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // ========== FIX 2: SLIMME MARKET QUALITY FILTER ==========
    const badDepth = ob.depthMinUsd1p < 180000;
    const badSpread = normalizeSpread(ob.spreadPct) > 0.003;
    if ((badDepth || badSpread) && confluence < 65 && sniperScore < 70) {
    ...
    // MID filter
    if (rsiZone === "MID" && flow.type !== "TREND" && confluence < 65) {
    ...
    // Momentum filter
    const hasMomentum =
      Math.abs(Number(c.change1h || 0)) > 0.12 ||
    ...
    if (flow.type === "BUILDING" && sniperScore < 70) {
      const waitPayload = buildWait(c, "BUILDING_LOW_SNIPER", flow, sniper, confluence, rr, funding, ob, riskBase, null, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    const sniperOK = sniperScore >= 65;
    const sniperOK_legacy = (sniper?.valid && sniperScore >= 65) || confluence >= 85;
    ...
    // A en B entry (C entry verwijderd)
    ...
    const bEntry = !eliteEntry && bSetupValid && sniperScore >= 65 && confluence >= 68 && rr >= 1.05 && c.tfStrength >= 1;
    ...
    const spread = normalizeSpread(ob.spreadPct);
    const badSpreadOld = spread > MAX_SPREAD_PCT;
    const badDepthOld = Number(ob.depthMinUsd1p || 0) < MIN_DEPTH_USD_1P;
    let marketQualityConfluenceLimit = certaintyMode === "safe" ? 75 : 65;
    if ((badSpreadOld || badDepthOld) && confluence < marketQualityConfluenceLimit && isDeep) { actions.push(buildWait(c, "BAD_MARKET_QUALITY_OLD", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null)); continue; }
