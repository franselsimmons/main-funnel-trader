...
const ANALYSIS_WEBHOOK_CONCURRENCY = 8;
const ANALYSIS_WEBHOOK_TIMEOUT_MS = 6000;
...
async function sendAnalysisWebhookEvent(event) {
  if (!ENABLE_ANALYSIS_WEBHOOK) return { ok: false, skipped: true, reason: "disabled" };
  if (!ANALYSIS_WEBHOOK_URL) return { ok: false, skipped: true, reason: "missing_url" };

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), ANALYSIS_WEBHOOK_TIMEOUT_MS);
