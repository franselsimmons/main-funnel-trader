...
// CHANGE: webhook is now disabled by default, can be enabled via env var
const ENABLE_ANALYSIS_WEBHOOK =
  String(process.env.ENABLE_ANALYSIS_WEBHOOK || "false").toLowerCase() === "true";
...
function finalizeResult(actions, candidates, btcState, runId, options = {}) {
  ...
  // CHANGE: webhook call is now non-blocking and only runs if enabled
  if (ENABLE_ANALYSIS_WEBHOOK && ANALYSIS_WEBHOOK_URL) {
    setTimeout(() => {
      sendAnalysisActions(finalActions, {
        runId,
        btcState
      }).catch(e => {
