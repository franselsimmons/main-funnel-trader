// tradeSystem.js - Definitief met alle fixes
// - Confluence berekend vóór sniper (fix 1)
// - C_ENTRY verwijderd (fix 2)
// - Position management vóór entry guards (fix 3)
// - BTC/regime uit options (fix 4)
// - Liquidation fix (fix 5)
...
function buildWait(c, reason, flow, sniper, confluence, rr, funding, ob, risk, setupGrade, requiredConfluence, requiredRR) {
  ...
  console.log(`🚫 BLOCK: ${c.symbol} | ${reason} | rsiZone=${c._debugRsiZone || "?"} | sniper=${sniper?.score || 0} | conf=${confluence} | rr=${rr?.toFixed(2)} | fake=${c._debugFakeBreakout} | flow=${flow?.type}`);
  return base;
}
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // 🔥 Cooldown / open‑positie checks worden later uitgevoerd (na position management)
    ...
    if (btcState === "NEUTRAL" && c.moveScore < 70) continue;
    ...
    c.flow = flow.type;
    const rsiSignal = dataMap.get(c.symbol)?.rsiData?.mtf ? getRSISignal(dataMap.get(c.symbol).rsiData.mtf, c.side) : { valid: false, strength: 0 };
    const rsi = Number.isFinite(rsiSignal?.rsi) ? rsiSignal.rsi : null;
    if (rsi === null) {
      const waitPayload = buildWait(c, "RSI_DATA_INVALID", flow, null, 0, 0, funding, ob, null, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
    ...
    // ========== FIX 1 & 5: eerst confluence, dan liquidations, dan sniper ==========
    ...
    const { ob: obData, funding, rsiData, liquidation } = dataMap.get(c.symbol) || { ob: DEFAULT_OB, funding: { rate: 0 }, rsiData: null, liquidation: null };
    ...
    const rsiContext = rawRsiCtx && Number.isFinite(rawRsiCtx.rsi) && rawRsiCtx.zones
      ? { valid: true, rsi: rawRsiCtx.rsi, zones: rawRsiCtx.zones }
      : null;
    ...
    if (obData.fetchFailed) {
      const waitPayload = buildWait(c, "ORDERBOOK_FETCH_FAILED", flow, sniper, confluence, 0, funding, obData, null, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    const marketCtx = { candles15m: rsiData?.candles15m || [], candles1h: rsiData?.candles1h || [] };
    const riskBase = await calculateRisk(c, obData, liquidity, hasLiquidationData ? liquidation : null, marketCtx);
    ...
    console.log(`📊 DATA: ${c.symbol} | hasRSI=${!!rsiContext} | conf=${confluence} | sniper=${sniperScore} | rr=${rr.toFixed(2)} | fake=${isValidFakeBreakout}`);
    ...
    if (isBull) {
      ...
      if (!rsiOK) {
        const waitPayload = buildWait(c, "RSI_LONG_NO_EDGE", flow, sniper, confluence, rr, funding, obData, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
        actions.push(waitPayload);
        continue;
      }
    }
    ...
    if (!isBull) {
      const rsiOK =
        ...
        earlyRSI ||
        trendContinuationRSI ||
        ["LOWER_1"].includes(rsiZone);

      if (!rsiOK) {
        const waitPayload = buildWait(c, "RSI_SHORT_NO_EDGE", flow, sniper, confluence, rr, funding, obData, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
        actions.push(waitPayload);
        continue;
      }
    }

    if (isBull && rsiZone === "LOWER_2" && c.change1h > -0.05) {
      const waitPayload = buildWait(c, "RSI_NOT_DEEP_ENOUGH", flow, sniper, confluence, rr, funding, obData, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    if (!isBull && rsiZone === "UPPER_2" && c.change1h < 0.2) {
      const waitPayload = buildWait(c, "RSI_NOT_HIGH_ENOUGH", flow, sniper, confluence, rr, funding, obData, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    if (rsiZone === "MID" && !trendContinuationRSI && confluence < 70) {
      const waitPayload = buildWait(c, "RSI_MID_NO_EDGE", flow, sniper, confluence, rr, funding, obData, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    if ((isBull && c.structure === "BEARISH") || (!isBull && c.structure === "BULLISH")) {
      const waitPayload = buildWait(c, "STRUCTURE_AGAINST", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null);
      actions.push(waitPayload);
      continue;
    }

    const strongMomentum = Math.abs(Number(c.change1h||0)) > 0.25 && Math.abs(Number(c.change24||0)) > 2 && (flow.type === "TREND" || flow.type === "BUILDING");
    const softMomentum = Math.abs(Number(c.change1h||0)) > 0.15 && Math.abs(Number(c.change24||0)) > 1.5 && (flow.type === "TREND" || flow.type === "BUILDING");
    if (!strongMomentum && !softMomentum) {
      const waitPayload = buildWait(c, "NO_MOMENTUM", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null);
      actions.push(waitPayload);
      continue;
    }

    if (!isValidFakeBreakout && flow.type !== "TREND") {
      const waitPayload = buildWait(c, "NO_FAKE_BREAKOUT", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null);
      actions.push(waitPayload);
      continue;
    }

    // ========== POSITION MANAGEMENT (moet eerst) ==========
    if (prev) {
      const pos = { ...prev };
    ...
    // ========== ENTRY FILTERS (pas nu, na position management) ==========
    ...
    const sniperOK = sniperScore >= 55;
    const sniperOK_legacy = (sniper?.valid && sniperScore >= 55) || confluence >= 85;
    const finalSniperOK = sniperOK || sniperOK_legacy;

    const setupGrade = getSetupGrade({ c, ob: obData, flow, sniper, confluence, rr, hasLiquidationData, isBull });
    ...
    const aSetupValid = stageOK && finalSniperOK && setupGrade.grade === "A" && !obData.spoof && rr >= minRrFloor;
    const eliteEntry = aSetupValid && sniperScore >= 60 && confluence >= 70 && rr >= 1.1 && c.tfStrength >= 1;
    const bSetupValid = stageOK && setupGrade.grade === "B" && !obData.spoof && rr >= 1.05;
    const bEntry = !eliteEntry && bSetupValid && sniperScore >= 50 && confluence >= 60 && rr >= 1.05 && c.tfStrength >= 1;

    // ❌ C_ENTRY is verwijderd (fix 2)
    ...
    console.log(`🔍 ${c.symbol} (${c.analysisType || "DEEP"}): sniper=${sniperScore}, conf=${confluence}, rr=${rr.toFixed(2)}, grade=${setupGrade.grade}, elite=${eliteEntry}, b=${bEntry}, godmode=${godModeEntry}, rsiZone=${rsiZone}, fakeBreakout=${isValidFakeBreakout}`);

    const lowVolConfluenceLimit = certaintyMode === "safe" ? 60 : 55;
    if (vol === "LOW" && confluence < lowVolConfluenceLimit && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "LOW_VOL", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
      continue;
    }
    if (flow.type === "NEUTRAL" && confluence < 58 && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "NO_FLOW", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
      continue;
    }
    const rrOverride = confluence >= 85 && sniperScore >= 80;
    if (rr < minRrFloor && !rrOverride && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "LOW_RR", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, minRrFloor));
      continue;
    }
    if ((c.stage === "entry" && c.tfStrength < 1) || (c.stage === "almost" && c.tfStrength < 1) || (c.stage === "buildup" && c.tfStrength < BUILDUP_ELITE_MIN_TF)) {
      const reason = c.analysisType === "DEEP" ? "ENTRY_FILTERED" : "LIGHT_NO_ENTRY";
      actions.push(buildWait(c, reason, flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
      continue;
    }
    if (confluence < 55 && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "LOW_CONFLUENCE", flow, sniper, confluence, rr, funding, obData, riskBase, null, 55, null));
      continue;
    }
    const obAgainst = isObAgainstSide(obData, isBull);
    if (obAgainst && confluence < 75 && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "OB_AGAINST", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
      continue;
    }
    ...
    if ((badSpread || badDepth) && confluence < marketQualityConfluenceLimit && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "BAD_MARKET_QUALITY", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
      continue;
    }
    if (obData.bias === "NEUTRAL" && confluence < 50 && !isNeutralObEntryException({ c, flow, sniper, confluence, rr, setupGrade, counterTrend: (btcState === "BULLISH" && !isBull) || (btcState === "BEARISH" && isBull) }) && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "OB_NEUTRAL_LOW_CONF", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
      continue;
    }
    const fundingRate = Number(funding?.rate || 0);
    if (Math.abs(fundingRate) > 0.015 && confluence < 85 && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "EXTREME_FUNDING", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
      continue;
    }
    if (isBull && fundingRate > 0.012 && confluence < 85 && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "BULL_CROWDED_FUNDING", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
      continue;
    }
    if (!isBull && fundingRate < -0.012 && confluence < 85 && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "BEAR_CROWDED_FUNDING", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
      continue;
    }
    ...
    if (c.analysisType !== "DEEP" && !shouldEnter) {
      actions.push(buildWait(c, "LIGHT_MONITORING", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
    } else if (!shouldEnter) {
      actions.push(buildWait(c, "SETUP_NOT_READY", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null));
    }
  }
