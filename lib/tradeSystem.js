// tradeSystem.js - Definitief met OB-fix + exit-safe position management
// - obData/funding/rsiData/liquidation worden direct uit dataMap gehaald
// - Geen ob undefined bug meer
// - Confluence wordt vóór sniper berekend
// - C_ENTRY verwijderd
// - Open posities worden altijd eerst gemonitord op TP/SL
// - Open posities worden ook gemonitord als ze niet meer in de scanner-funnel zitten
// - Geen harde max-trade-cap in tradeSystem; DEEP/LIGHT is alleen analyse-label
...
const GRADE_B_MIN_RR_FLOOR = 1.0;
...
function appendOpenPositionCandidates(candidates) {
  const out = Array.isArray(candidates) ? [...candidates] : [];
  const seen = new Set(out.map(c => `${String(c.symbol || "").toUpperCase()}_${String(c.side || "").toLowerCase()}`));
  ...
  for (const [key, pos] of memory.entries()) {
    ...
    out.push({
      ...
      stageSource: pos.stageSource || "open_position",
      uiOnly: false,
      moveScore: Number(pos.score || pos.moveScore || 100),
      price: pos.entry,
      rawBitgetSymbol: pos.rawBitgetSymbol || symbol,
      analysisType: "OPEN_MONITOR"
    });
...
export async function processTrades(input, options = {}) {
  ...
  let candidates = dedupeCandidates(
    candidatesRaw.filter(c => {
      ...
      const stage = c?.stage;
      return score >= 50 && ["entry", "almost", "buildup"].includes(stage);
    })
  );
  ...
  candidates = candidates
    ...
    .map((c, index) => ({
      ...c,
      analysisType: c.analysisType || (index < 10 ? "DEEP" : "LIGHT")
    }));

  candidates = appendOpenPositionCandidates(candidates);
  ...
  if (candidates.length === 0) {
    ...
    return {
      ...
      candidatesCount: candidatesRaw.length
    };
  }
  ...
  const fetchCoinData = async (c) => {
    ...
    const cleanPair = normalizeBitgetSymbol(c.rawBitgetSymbol || symbol);
    ...
    const candles15m = await cachedFetch(
      `c15_${cleanPair}`,
      () => fetchCandles(cleanPair, "15m", 100),
      20000
    );

    const candles1h = await cachedFetch(
      `c1h_${cleanPair}`,
      () => fetchCandles(cleanPair, "1h", 100),
      20000
    );
  ...
  for (const originalCoin of candidates) {
    const c = {
      ...originalCoin,
      symbol: String(originalCoin.symbol || "").toUpperCase(),
      side: String(originalCoin.side || "").toLowerCase()
    };

    if (!c.symbol || !["bull", "bear"].includes(c.side)) {
      continue;
    }
    ...
    if (prev) {
      ...
      const hitTP = isBull
        ? Number(c.price) >= Number(pos.tp)
        : Number(c.price) <= Number(pos.tp);

      const hitSL = isBull
        ? Number(c.price) <= Number(pos.sl)
        : Number(c.price) >= Number(pos.sl);
      ...
      await logAction(holdPayload, pos.regime || "N/A", btcState, shouldLog);
    ...
    if (btcState === "BULLISH" && !isBull && Number(c.moveScore || 0) < 75) continue;
    if (btcState === "BEARISH" && isBull && Number(c.moveScore || 0) < 75) continue;
    if (btcState === "NEUTRAL" && Number(c.moveScore || 0) < 70) continue;
    ...
    const rsiContext =
      rawRsiCtx &&
      Number.isFinite(Number(rawRsiCtx.rsi)) &&
      rawRsiCtx.zones
        ? {
            valid: true,
            rsi: rawRsiCtx.rsi,
    ...
    if (candles15m.length >= 20) {
      const recentHigh = Math.max(...candles15m.slice(-20).map(candle => candle.high));
      const recentLow = Math.min(...candles15m.slice(-20).map(candle => candle.low));
    ...
    if (processingLocks.has(symbolLockKey)) {
      actions.push(
        buildWait(c, "DUPLICATE_PROCESSING_LOCK", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null)
      );
      continue;
    }
    ...
    if (Date.now() < (cooldownMap.get(key) || 0)) {
      actions.push(
        buildWait(c, "COOLDOWN", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null)
      );
      continue;
    }
    ...
    if (memory.has(`${c.symbol}_${isBull ? "bear" : "bull"}`)) {
      actions.push(
        buildWait(c, "OPPOSITE_POSITION_OPEN", flow, sniper, confluence, rr, funding, obData, riskBase, null, null, null)
      );
      continue;
    }
    ...
    const sniperOKLegacy = (sniper?.valid && sniperScore >= 55) || confluence >= 85;
    const finalSniperOK = sniperOK || sniperOKLegacy;
    ...
    if (
      ...
      actions.push(buildWait(c, reason, flow, sniper, confluence, rr, funding, obData, riskBase, setupGrade, null, null));
      continue;
    }

    if (confluence < 55 && c.analysisType === "DEEP") {
      actions.push(buildWait(c, "LOW_CONFLUENCE", flow, sniper, confluence, rr, funding, obData, riskBase, setupGrade, 55, null));
      continue;
    }
    ...
    const marketQualityConfluenceLimit = certaintyMode === "safe" ? 75 : 65;
    ...
    if (shouldEnter && !hasAnyOpenPositionForSymbol(c.symbol)) {
      ...
      const position = {
        ...
        sniperScore,
        regime: regimeLevel,
        rawBitgetSymbol: c.rawBitgetSymbol || c.symbol
      };
    ...
    if (c.analysisType !== "DEEP" && !shouldEnter) {
      actions.push(buildWait(c, "LIGHT_MONITORING", flow, sniper, confluence, rr, funding, obData, riskBase, setupGrade, null, null));
    } else if (!shouldEnter) {
