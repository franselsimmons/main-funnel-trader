...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // ================= RSI HARD ZONE FILTER (MID = NO TRADE) =================
    ...
    // ❌ BLOCK FOUTE KANT
    if (isBull && rsiZone.startsWith("UPPER")) {
      const waitPayload = buildWait(c, "RSI_TOO_HIGH_FOR_LONG", flow, rsiSignal, 0, 0, funding, ob, null, { grade: "C", points: 0, recommendedRisk: "watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    if (!isBull && rsiZone.startsWith("LOWER")) {
      const waitPayload = buildWait(c, "RSI_TOO_LOW_FOR_SHORT", flow, rsiSignal, 0, 0, funding, ob, null, { grade: "C", points: 0, recommendedRisk: "watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    // ================= EINDE RSI HARD FILTER =================
    ...
    console.log(`🔍 ${c.symbol} (${isDeep ? "DEEP" : "LIGHT"}): sniper=${sniper.score}, conf=${confluence}, rr=${rr.toFixed(2)}, grade=${setupGrade.grade}, elite=${eliteEntry}, b=${bEntry}, godmode=${godModeEntry}, rsiZone=${rsiZone}`);
