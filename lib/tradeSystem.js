...
const BULLISH_MID_TREND_PROBE_MIN_RR = 1.20;
...
// ================= FIX 1: VOORWAARDELIJK LOADEN VAN OPTIMIZER CHUNKS =================
...
// ================= FIX 4: OPGEFRUIEN VAN OPTIMIZER CHUNKS (EENMALIG) =================
...
// ================= FIX 2: TS_PREFILTER LOG ALLEEN BIJ DEBUG =================
...
async function updateShadowFeatureOutcomes() {
  // Shadow outcomes zijn uitgeschakeld via ENABLE_SHADOW_OUTCOMES
  return;
}
...
function normalizeRsiSignalSafe(raw, rsiData) {
  ...
  return {
    ...
    rsi,
    rsiHTF,
    zones: raw?.zones || getDefaultRsiZones(),
...
async function getSafeRsiSignal({ c, rsiData, isBull, btcState }) {
  ...
  return callFirstValid(
    ...
    [
      ...
      () => getRSISignal(c, rsiData)
    ],
    value => value !== null && value !== undefined,
    fallback
  ).then(value => normalizeRsiSignalSafe(value, rsiData));
}
...
function logOptimizerReport({ reasonTable, btcState, runId }) {
  // Optimizer is volledig uitgeschakeld via flag ENABLE_TS_OPTIMIZER = false
  return {
    tag: "TS_MASTER_BEST_AFSTELLING",
    strategyVersion: STRATEGY_VERSION,
    runId,
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    // Eenmalige cleanup van oude optimizer chunks (na eerste deploy mag deze call weg)
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        const requiredRR = Math.max(
          Number(setup.requiredRR || MIN_RR_FLOOR),
          Number(dynamicMinRr || MIN_RR_FLOOR)
        );

        if (Number(sniper?.score || 0) < Number(setup.requiredSniper || 0)) {
          const payload = buildWait(
            c,
            "LOW_SNIPER",
            flow,
