...
function buildCanonicalRotationDecision({
  ...
  const allowed =
    Boolean(rotationCheck?.allowed) ||
    Boolean(canonicalMatch?.allowed);
  ...
  const reason = allowed
    ? canonicalMatch.allowed && !rotationCheck.allowed
  ...
  const activeMicroFamilyIds =
    canonicalMatch.activeMicroFamilyIds?.length
      ? canonicalMatch.activeMicroFamilyIds
      : durableArray(rawDecision.activeMicroFamilyIds);

  const checkedMicroFamilyIds =
    canonicalMatch.checkedMicroFamilyIds?.length
      ? canonicalMatch.checkedMicroFamilyIds
      : durableArray(rawDecision.checkedMicroFamilyIds);
...
function microLearningHasAnyFamilies(state = {}) {
  return Boolean(
    Object.keys(state?.daily?.families || {}).length ||
    Object.keys(state?.week?.families || {}).length ||
    Object.keys(state?.lastCompletedWeek?.families || {}).length ||
    state?.activeRotation?.allowedMicroFamilyIds?.length ||
    state?.activeRotation?.activeMicroFamilyIds?.length
  );
}
