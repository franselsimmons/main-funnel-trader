...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // 🔥 Verbeterde B‑entry (losser dan A)
    const eliteEntry = setupValid && sniper.score >= 75 && confluence >= 75 && rr >= 1.1;
    const bEntry = setupValid && setupGrade.grade === "B" && sniper.score >= 60 && confluence >= 65 && rr >= 1.05;
      ...
      try {
        ...
        if (notify && !notifyState.get(key)) {
          await sendEntry({
            ...
            sniperScore: sniper.score  // ✅ correcte veldnaam
          });
