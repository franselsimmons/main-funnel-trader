...
// ================= STRATEGY VERSION =================
// Reset bewust: nieuwe entry/exit-logica + nieuwe durable runtime key.
const STRATEGY_VERSION = "TS_V10_A_BE_MID_SPREAD_TP130_RESET";
...
// A-coins blijven op base-RR gate 1.25.
// Planned/final RR krijgt aparte floor.
const GRADE_A_MIN_RR_FLOOR = 1.10;
const GRADE_B_MIN_RR_FLOOR = 1.05;
const GRADE_C_MIN_RR_FLOOR = 1.10;

const A_ENTRY_MIN_RR = 1.25;
const A_FINAL_MIN_RR = 1.60;

const B_ENTRY_MIN_RR = 1.05;
const GOD_ENTRY_MIN_RR = 1.45;

const A_MIN_SNIPER = 72;
const A_MIN_CONFLUENCE = 81;

const B_MIN_SNIPER = 62;
const B_MIN_CONFLUENCE = 68;

const GOD_MIN_SNIPER = 85;
const GOD_MIN_CONFLUENCE = 85;

const MID_RSI_MIN_CONFLUENCE = 72;
const TREND_CONTINUATION_MIN_CONFLUENCE = 65;
const TREND_CONTINUATION_MIN_SNIPER = 55;

const MID_RSI_LONG_MAX_SPREAD_PCT = 0.00175;
const MID_RSI_LONG_SPREAD_OVERRIDE_CONF = 88;
const MID_RSI_LONG_SPREAD_OVERRIDE_SNIPER = 80;

const BE_TRIGGER_R = 0.50;
const BE_OFFSET_R = 0.03;
...
const CURRENT_FILTER_VALUES = Object.freeze({
  ...
  MID_RSI_LONG_MAX_SPREAD_PCT,
  MID_RSI_LONG_SPREAD_OVERRIDE_CONF,
  MID_RSI_LONG_SPREAD_OVERRIDE_SNIPER,
  BE_TRIGGER_R,
  BE_OFFSET_R
});
...
function getTpRewardMultiplier({ setupClass, certaintyMode, sniperScore, rsi, isBull }) {
  ...
  if (setupClass === "GOD") multiplier = 1.50;
  else if (setupClass === "A") multiplier = 1.30;
  ...
  if (setupClass === "A") {
    return clamp(multiplier, 1.25, 1.50);
  }

  if (setupClass === "GOD") {
    return clamp(multiplier, 1.45, 1.80);
  }

  return clamp(multiplier, 0.95, 1.20);
}

function applyBreakEvenStop(position, isBull) {
  if (!position) return position;

  const setupClass = String(position.setupClass || "").toUpperCase();

  if (setupClass !== "A" && setupClass !== "GOD") return position;
  if (position.breakEvenApplied) return position;

  const currentR = Number(position.currentR || 0);
  if (currentR < BE_TRIGGER_R) return position;
  ...
  if (!entry || !initialSl || !currentSl) return position;

  const riskDist = Math.abs(entry - initialSl);
  if (!riskDist) return position;

  const newSl = isBull
    ? entry + riskDist * BE_OFFSET_R
    : entry - riskDist * BE_OFFSET_R;

  // Nooit SL losser maken.
  if (isBull && newSl <= currentSl) return position;
  if (!isBull && newSl >= currentSl) return position;

  position.sl = Number(newSl);
  position.breakEvenApplied = true;
  position.breakEvenAt = Date.now();
  position.breakEvenTriggerR = BE_TRIGGER_R;
  position.breakEvenOffsetR = BE_OFFSET_R;
...
function positionOrBreakEvenReason(pos, exitR) {
  if (pos?.breakEvenApplied && Number(exitR || 0) >= 0) {
    return "BE_SL";
  }
...
function buildLoserProfile() {
  const losers = (auditState.closedTrades || []).filter(t => Number(t.exitR || 0) < 0);
...
function buildWinnerProfile() {
  const winners = (auditState.closedTrades || []).filter(t => Number(t.exitR || 0) > 0);
...
function buildRecommendedFilterValues({ reasonTable }) {
  ...
  const aRR = evaluateMinThreshold(aRows, "plannedRR", A_FINAL_MIN_RR);
  ...
  if (aRows.length >= 5) {
    ...
    pushRecommendation({
      parameter: "A_FINAL_MIN_RR",
      currentValue: A_FINAL_MIN_RR,
      suggestedValue: aRR.suggestedValue,
      confidence: aRR.confidence,
      action: aRR.suggestedValue > A_FINAL_MIN_RR ? "RAISE" : "KEEP_OR_LOWER",
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      if (prev) {
        ...
        if (hitTP || hitSL) {
          const exitRPreview = calculateExitR(pos, c.price, isBull);

          const reason = hitTP
            ? "TP"
            : positionOrBreakEvenReason(pos, exitRPreview);
      ...
      const midRsiLongBadSpread =
        isBull &&
        rsiZone === "MID" &&
        spread > MID_RSI_LONG_MAX_SPREAD_PCT;

      const midRsiLongSpreadOverride =
        confluence >= MID_RSI_LONG_SPREAD_OVERRIDE_CONF &&
        sniperScore >= MID_RSI_LONG_SPREAD_OVERRIDE_SNIPER;

      if (midRsiLongBadSpread && !midRsiLongSpreadOverride) {
        actions.push(buildWait(
          c,
          "MID_RSI_LONG_BAD_SPREAD",
          flow,
          sniper,
          confluence,
          rr,
