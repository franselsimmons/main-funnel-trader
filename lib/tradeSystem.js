...
const ANALYSIS_WEBHOOK_BASE_URL =
  ...
  (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : "https://crypto-voorspeller.vercel.app");

function buildDefaultAnalysisWebhookUrl() {
  const base = String(ANALYSIS_WEBHOOK_BASE_URL || "").replace(/\/+$/, "");
  const path = String(ANALYSIS_WEBHOOK_PATH || "/api/webhooks/tradesystem").startsWith("/")
...
// Default AAN zodra er een URL is.
// Alleen uitzetten met ENABLE_ANALYSIS_WEBHOOK=false.
...
// Zet alleen true als je expliciet wilt aborten bij missende secret.
// Beter: laat endpoint 401 geven zodat je echte error ziet.
...
const ANALYSIS_WEBHOOK_CONCURRENCY = Number(
  process.env.ANALYSIS_WEBHOOK_CONCURRENCY || 8
);

const ANALYSIS_WEBHOOK_TIMEOUT_MS = Number(
  process.env.ANALYSIS_WEBHOOK_TIMEOUT_MS || 7000
);
...
// ================= STRICT FLAT STRING-ONLY WEBHOOK (NO WRAPPED) =================
...
function buildStrictFlatAnalysisWebhookBody(event) {
  ...
  const base = {
    ...
    strategyVersion: safeWebhookString(
      event?.strategyVersion || STRATEGY_VERSION,
      STRATEGY_VERSION
    ),
    ...
    // De volledige payload als JSON-string. Geen object.
  ...
  const bodyObj = sanitizeWebhookPayload(base);
...
function buildAnalysisWebhookHeaders() {
  ...
  headers["x-trade-webhook-secret"] = ANALYSIS_WEBHOOK_SECRET;
...
async function postAnalysisWebhookBody(bodyObj) {
  const flatBodyObj = sanitizeWebhookPayload(bodyObj);
...
async function sendAnalysisWebhookEvent(event) {
  ...
  if (REQUIRE_ANALYSIS_WEBHOOK_SECRET && !ANALYSIS_WEBHOOK_SECRET) {
    ...
    return { ok: false, skipped: true, reason: "missing_secret_required" };
  }
...
async function finalizeResult(actions, candidates, btcState, runId, options = {}) {
  ...
  // Webhook call now blocking and properly awaited
  const analysisRowsCount = finalActions.filter(action => {
    if (!action?.symbol || !action?.side) return false;

    const actionType = String(action?.action || "").toUpperCase();

    if (actionType === "HOLD" && process.env.ANALYSIS_WEBHOOK_SEND_HOLDS !== "true") {
      return false;
    }
  ...
  if (ENABLE_ANALYSIS_WEBHOOK && ANALYSIS_WEBHOOK_URL) {
    console.log("TS_ANALYSIS_WEBHOOK_ATTEMPT:", JSON.stringify({
      ...
      analysisRows: analysisRowsCount,
      entries: finalActions.filter(a => String(a.action || "").toUpperCase() === "ENTRY").length,
      exits: finalActions.filter(a => String(a.action || "").toUpperCase() === "EXIT").length,
      waits: finalActions.filter(a => String(a.action || "").toUpperCase() === "WAIT").length,
      holds: finalActions.filter(a => String(a.action || "").toUpperCase() === "HOLD").length,
      urlConfigured: Boolean(ANALYSIS_WEBHOOK_URL),
      secretConfigured: Boolean(ANALYSIS_WEBHOOK_SECRET)
    }));
    ...
    console.warn("TS_ANALYSIS_WEBHOOK_SKIPPED_CONFIG:", JSON.stringify({
      runId,
      enabled: ENABLE_ANALYSIS_WEBHOOK,
      urlConfigured: Boolean(ANALYSIS_WEBHOOK_URL),
      secretConfigured: Boolean(ANALYSIS_WEBHOOK_SECRET),
