...
const SHORT_RSI_HARD_CHASE_MAX = 42.0;
...
async function getSafeSniperEntry({ c, flow, ob, rsiSignal, rsiZone, isBull, btcState }) {
  ...
  // ================= PATCH 2: VERBETERDE CALLFIRSTVALID MET GEBRUIK VAN isUsableExternalScore =================
  const raw = await callFirstValid(
    "getSniperEntry",
    [
      () => getSniperEntry({
    ...
    value => isUsableExternalScore(value, [
      "score",
      "sniperScore",
      "confidence",
      "value"
  ...
  // DEBUG LOGGING FOR RAW SNIPER RESULT (guarded by env var)
...
async function getSafeConfluence({
  ...
  // ================= PATCH 3: VERBETERDE CALLFIRSTVALID MET GEBRUIK VAN isUsableExternalScore =================
  const raw = await callFirstValid(
    "calculateConfluence",
    [
      () => calculateConfluence({
  ...
  // DEBUG LOGGING FOR RAW CONFLUENCE RESULT (guarded by env var)
  if (process.env.TS_DEBUG_RAW_SIGNALS === "true") {
  ...
  // ================= V12.1 FALLBACK-SCORE CAP =================
  if (ENABLE_V12_FALLBACK_SCORE_CAP && externalCollapsed) {
...
// ================= MAX DEEP CANDIDATES (SIDE-BALANCED) =================
