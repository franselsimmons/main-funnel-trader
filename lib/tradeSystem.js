// tradeSystem.js - Definitieve versie met debug logging
// - Alle eerdere fixes (MID filter na confluence, BTC neutral 70, earlyRSI 75, sniper?.score)
// - Debug log bij elke WAIT: toont exacte reden en meetwaarden
...
function buildWait(c, reason, flow, sniper, confluence, rr, funding, ob, risk, setupGrade, requiredConfluence, requiredRR) {
  ...
  // 🔍 DEBUG LOG – zie exact waarom een coin wordt geweigerd
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    c._debugRsiZone = rsiZone; // voor debug log
    ...
    c._debugFakeBreakout = isValidFakeBreakout; // voor debug log
