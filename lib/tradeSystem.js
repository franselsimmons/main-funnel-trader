...
// ================= STRATEGY VERSION =================
const STRATEGY_VERSION = "TS_V12_3_COHORT_POLICY";
...
const GRADE_C_MIN_RR_FLOOR = 1.08;      // PATCHED

const A_ENTRY_MIN_RR = 1.10;          // was 1.05
const B_ENTRY_MIN_RR = 1.08;          // was 1.00
const GOD_ENTRY_MIN_RR = 1.25;

const A_MIN_SNIPER = 75;              // was 48
...
const A_FINAL_MIN_RR = 1.12;          // was 1.10
...
const A_MIN_DEPTH_USD_1P = 40_250;     // was 40_250
...
const BTC_BEARISH_LONG_MIN_SCORE = 85;
...
const NEUTRAL_OB_A_EXCEPTION_MIN_CONFLUENCE = 86;   // was 74
...
// ================= V12.1 FALLBACK CONFIDENCE CAP =================
const ENABLE_V12_FALLBACK_SCORE_CAP = true;
const FALLBACK_COLLAPSED_CONF_CAP = 72;
const FALLBACK_COLLAPSED_NEUTRAL_OB_CAP = 64;
const FALLBACK_COLLAPSED_LOW_DEPTH_CAP = 60;
...
const EARLY_FAILURE_MIN_AGE_SEC = 30;
const EARLY_FAILURE_MIN_MFE_R = 0.12;
const EARLY_FAILURE_MAX_MAE_R = -0.22;
const EARLY_FAILURE_MAX_CURRENT_R = -0.18;

const EARLY_SOFT_STOP_MIN_AGE_SEC = 20;
const EARLY_SOFT_STOP_MAX_CURRENT_R = -0.45;
const EARLY_SOFT_STOP_MAX_MFE_R = 0.15;

const EARLY_OB_FLIP_MIN_AGE_SEC = 60;
...
const COHORT_COLD_START_MODE = "ELITE_ONLY"; // "ELITE_ONLY" or "SHADOW_ONLY"

const COHORT_COLD_START_MIN_CONFLUENCE = 82;
const COHORT_COLD_START_MIN_SNIPER = 82;
const COHORT_COLD_START_MIN_RR = 1.12;
const COHORT_COLD_START_MAX_SPREAD_PCT = 0.0012;
const COHORT_COLD_START_MIN_DEPTH_USD_1P = 100000;
...
function getCandidateRsiZone(c) {
  return String(
    c?.rsiZone ||
    c?._debugRsiZone ||
    c?.rsiSignal?.zone ||
...
function getCandidateRsiValue(c) {
  const raw =
    c?.rsiValue ??
    c?.rsi15m ??
    c?.rsiM15 ??
    c?.scannerRsi ??
    c?.rsiSignal?.rsi ??
    (typeof c?.rsi === "object" ? c?.rsi?.rsi ?? c?.rsi?.value : c?.rsi);

  const n = Number(raw);
  return Number.isFinite(n) ? n : null;
}

function applyTsAwareCandidateScore(c, btcState) {
  const side = String(c?.side || "").toLowerCase();
  const stage = String(c?.stage || "").toLowerCase();
  const rsiZone = getCandidateRsiZone(c);
  const rsi = getCandidateRsiValue(c);
  const state = String(btcState || "NEUTRAL").toUpperCase();

  const originalScore = Number(c?.moveScore || 0);

  let penalty = 0;
  const reasons = [];

  if (side === "bear" && ["LOWER_2", "LOWER_3"].includes(rsiZone)) {
    penalty += 35;
    reasons.push("TS_SHORT_OVERSOLD_ZONE");
  }

  if (side === "bear" && rsiZone === "LOWER_1") {
    penalty += 18;
    reasons.push("TS_SHORT_LOWER1_CHASE");
  }

  if (side === "bear" && Number.isFinite(Number(rsi)) && Number(rsi) < 42) {
    penalty += 20;
    reasons.push("TS_SHORT_RSI_BELOW_42");
  }

  if (side === "bull" && ["UPPER_2", "UPPER_3"].includes(rsiZone)) {
    penalty += 30;
    reasons.push("TS_LONG_OVEREXTENDED");
  }

  if (
    side === "bull" &&
    ["BEARISH", "STRONG_BEAR"].includes(state)
  ) {
    penalty += stage === "entry" ? 22 : 28;
    reasons.push("TS_BTC_BEARISH_LONG_PENALTY");
  }

  const adjustedScore = clamp(Math.round(originalScore - penalty), 0, 100);

  return {
    ...c,
    originalMoveScore: originalScore,
    moveScore: adjustedScore,
    tsAwarePenalty: penalty,
    tsAwarePenaltyReasons: reasons
  };
}

function applyTsAwareCandidateScoring(candidates, btcState) {
  const rows = Array.isArray(candidates) ? candidates : [];

  const adjusted = rows
    .map(c => applyTsAwareCandidateScore(c, btcState))
    .filter(c => Number(c.moveScore || 0) >= CANDIDATE_MIN_SCORE);

  if (process.env.TS_DEBUG_PREFILTER === "true") {
    const rejected = rows.length - adjusted.length;

    const reasonCounts = {};

    for (const row of rows.map(c => applyTsAwareCandidateScore(c, btcState))) {
      if (Number(row.moveScore || 0) >= CANDIDATE_MIN_SCORE) continue;

      for (const reason of row.tsAwarePenaltyReasons || ["TS_AWARE_SCORE_TOO_LOW"]) {
        reasonCounts[reason] = Number(reasonCounts[reason] || 0) + 1;
      }
    }

    console.log("TS_AWARE_PREFILTER", JSON.stringify({
      input: rows.length,
      accepted: adjusted.length,
      rejected,
      btcState,
      reasonCounts,
      examples: adjusted.slice(0, 20).map(c => ({
        symbol: c.symbol,
        side: c.side,
        stage: c.stage,
        originalMoveScore: c.originalMoveScore,
        adjustedMoveScore: c.moveScore,
        penalty: c.tsAwarePenalty,
        reasons: c.tsAwarePenaltyReasons
      }))
    }));
  }
...
function normalizeRsiSignalSafe(raw, rsiData) {
  ...
  return {
    ...
    blocked: Boolean(raw?.blocked || raw?.htfBlocked)
  };
}
...
function mergeMtfRsi(primary, fallback) {
  ...
  const mergeNode = (a, b) => {
    const rsi = isValidRsiNumber(a?.rsi) ? Number(a.rsi) : Number(b?.rsi || 50);
...
function isUsableExternalScore(raw, keys = [], minScore = MIN_EXTERNAL_SCORE_HEALTH) {
  const score = extractNumericScore(raw, keys);
  const n = Number(score);

  return Number.isFinite(n) && n >= Number(minScore || 0);
}
...
async function getSafeSniperEntry({ c, flow, ob, rsiSignal, rsiZone, isBull, btcState }) {
  ...
  const raw = await callFirstValid(
    ...
    value => isUsableExternalScore(
      value,
      ["score", "sniperScore", "confidence", "value"],
      MIN_EXTERNAL_SNIPER_HEALTH
    ),
    null
  );

  if (process.env.TS_DEBUG_RAW_SIGNALS === "true") {
    console.log("TS_RAW_SNIPER_RESULT", JSON.stringify({
      symbol: c.symbol,
      side: c.side,
      raw,
      extracted: extractNumericScore(raw, [
        "score",
        "sniperScore",
        "confidence",
        "value"
...
function isUsableRiskGeometry(risk, isBull) {
  if (!isValidRiskGeometry(risk, isBull)) return false;

  const rr = calculateRRFromPrices(risk.entry, risk.sl, risk.tp, isBull);

  return rr >= MIN_ACCEPTABLE_PRIMARY_RR;
}
...
function buildFallbackRiskGeometry({ c, ob, isBull }) {
  ...
  const rewardMultiplier = isBull ? 1.42 : 1.35;
  const rewardPct = riskPct * rewardMultiplier;
...
async function getSafeRiskGeometry({
  ...
  const risk = await callFirstValid(
    ...
    value => isUsableRiskGeometry(value, isBull),
    null
  );
...
async function getSafeConfluence({
  ...
  const raw = await callFirstValid(
    ...
    value => isUsableExternalScore(
      value,
      ["score", "confluence", "totalScore", "finalScore", "value"],
      MIN_EXTERNAL_CONFLUENCE_HEALTH
    ),
    null
  );

  if (process.env.TS_DEBUG_RAW_SIGNALS === "true") {
    console.log("TS_RAW_CONFLUENCE_RESULT", JSON.stringify({
      symbol: c.symbol,
      side: c.side,
      raw,
      extracted: extractNumericScore(raw, [
  ...
  const hasHealthyExternal = externalScore >= MIN_EXTERNAL_CONFLUENCE_HEALTH;
  const externalCollapsed = externalScore > 0 && externalScore < MIN_EXTERNAL_SCORE_HEALTH;
  const externalMissing = externalScore <= 0;

  let finalScore = hasHealthyExternal
    ? Math.round((externalScore * 0.70) + (fallbackScore * 0.30))
    : fallbackScore;

  if (hasHealthyExternal && fallbackScore - externalScore > 20) {
    finalScore = Math.min(finalScore, externalScore + 14);
  }

  finalScore = clamp(finalScore, 0, 100);

  if (ENABLE_V12_FALLBACK_SCORE_CAP && !hasHealthyExternal) {
    const obBias = String(ob?.bias || "NEUTRAL").toUpperCase();
    const depth = Number(ob?.depthMinUsd1p || 0);

    let cap = FALLBACK_COLLAPSED_CONF_CAP;

    if (obBias === "NEUTRAL" || obBias === "UNKNOWN") {
      cap = Math.min(cap, FALLBACK_COLLAPSED_NEUTRAL_OB_CAP);
    }

    if (depth > 0 && depth < MIN_DEPTH_USD_1P) {
      cap = Math.min(cap, FALLBACK_COLLAPSED_LOW_DEPTH_CAP);
    }
  ...
  c._debugConfluenceSource = hasHealthyExternal
    ? finalScore === fallbackScore && fallbackScore > externalScore
      ? "FALLBACK_OVERRIDE_HEALTHY_EXTERNAL"
      : "HEALTHY_EXTERNAL"
    : "FALLBACK_NO_HEALTHY_EXTERNAL";
...
function getNoSetupReason({
  ...
  const minRR = ENABLE_B_ENTRIES
    ? Math.min(A_ENTRY_MIN_RR, B_ENTRY_MIN_RR)
    : A_ENTRY_MIN_RR;

  const minConf = ENABLE_B_ENTRIES
    ? Math.min(A_MIN_CONFLUENCE, B_MIN_CONFLUENCE)
    : A_MIN_CONFLUENCE;

  const minSniper = ENABLE_B_ENTRIES
    ? Math.min(A_MIN_SNIPER, B_MIN_SNIPER)
    : A_MIN_SNIPER;

  if (setupEvalRR < minRR) return "SETUP_RR_TOO_LOW";
  if (sniperScore < minSniper) return "SETUP_SNIPER_TOO_LOW";
  if (confluence < minConf) return "SETUP_CONFLUENCE_TOO_LOW";

  if (side === "bull" && String(rsiZone).startsWith("UPPER")) {
    return "SETUP_BULL_RSI_TOO_EXTENDED";
  }

  if (side === "bear" && String(rsiZone).startsWith("LOWER")) {
    return "SETUP_BEAR_RSI_TOO_EXTENDED";
  }

  return "LOW_ENTRY_QUALITY";
}
...
function buildEntryPosition({
  ...
  return initializePositionPathMetrics({
    ...
    liveCohortKey: liveCohortPolicy?.cohortKey || null,
    cohortDecision: liveCohortPolicy?.decision || null,
    cohortReason: liveCohortPolicy?.reason || null,
    cohortSample: Number(liveCohortPolicy?.sample || 0),
...
// ================= V12 SIDE EXPOSURE / LOSS CIRCUIT =================
// (Note: de oude versie van validateSideExposureGateV12 is verwijderd; alleen de onderstaande versies blijven bestaan)

function countOpenPositionsTotal() {
  return memory.size;
}

function countOpenPositionsBySide(side) {
  const s = String(side || "").toLowerCase();
  let count = 0;
  for (const pos of memory.values()) {
    if (String(pos?.side || "").toLowerCase() === s) count++;
  }
  return count;
}

function countRecentLossesBySide(side, lookbackMs = SIDE_LOSS_THROTTLE_LOOKBACK_MS) {
  const s = String(side || "").toLowerCase();
  const since = Date.now() - Number(lookbackMs || 0);
  return (auditState.closedTrades || []).filter(t => {
    const sameSide = String(t?.side || "").toLowerCase() === s;
    const recent = Number(t?.exitedAt || 0) >= since;
    const badLoss = Number(t?.exitR || 0) <= SIDE_LOSS_THROTTLE_MAX_EXIT_R;
    return sameSide && recent && badLoss;
  }).length;
}

function isEliteAQuality({ confluence, sniperScore }) {
  return (
    Number(sniperScore || 0) >= SIDE_LOSS_THROTTLE_ELITE_SNIPER &&
    Number(confluence || 0) >= SIDE_LOSS_THROTTLE_ELITE_CONFLUENCE
  );
}

function validateNumericRsiTimingGateV12(candidate) {
  if (!ENABLE_V12_SHORT_RSI_CHASE_GATE) {
    return { ok: true, reason: "RSI_TIMING_DISABLED" };
  }
  ...
  const obBias = String(candidate.obBias || "NEUTRAL").toUpperCase();
  const rsiSlope3 = Number(candidate.rsiSlope3 || 0);
  ...
  if (!Number.isFinite(rsi) || rsi <= 0) {
    return { ok: false, reason: "V12_RSI_NUMERIC_INVALID" };
  }

  // hard chase block
  if (side === "bear" && rsi < SHORT_RSI_HARD_CHASE_MAX) {
    return { ok: false, reason: "V12_SHORT_RSI_HARD_CHASE" };
  }

  // soft chase block unless elite
  if (
    side === "bear" &&
    rsi >= SHORT_RSI_HARD_CHASE_MAX &&
    rsi < SHORT_RSI_SOFT_CHASE_MAX &&
    (sniperScore < SHORT_SOFT_CHASE_MIN_SNIPER ||
      confluence < SHORT_SOFT_CHASE_MIN_CONFLUENCE ||
      !obAligned ||
      rsiSlope3 > 0.20)
  ) {
    return { ok: false, reason: "V12_SHORT_RSI_SOFT_CHASE_WEAK" };
  }

  // mid-range no man's land
  if (
    side === "bear" &&
    rsi > SHORT_MID_HIGH_RSI_MAX &&
    rsi < SHORT_OVERBOUGHT_RSI_MIN &&
    !(
      sniperScore >= SHORT_OVERBOUGHT_MIN_SNIPER &&
      confluence >= SHORT_OVERBOUGHT_MIN_CONFLUENCE &&
      obAligned
    )
  ) {
    return { ok: false, reason: "V12_SHORT_MID_RSI_NO_MANS_LAND" };
  }

  // overbought short: require rejection and elite stats
  if (
    side === "bear" &&
    rsi >= SHORT_OVERBOUGHT_RSI_MIN &&
    rsi <= SHORT_OVERBOUGHT_RSI_MAX &&
    (sniperScore < SHORT_OVERBOUGHT_MIN_SNIPER ||
      confluence < SHORT_OVERBOUGHT_MIN_CONFLUENCE ||
      !obAligned ||
      !["UPPER_1", "UPPER_2", "UPPER_3"].includes(rsiZone))
  ) {
    return { ok: false, reason: "V12_SHORT_OVERBOUGHT_WEAK_REJECTION" };
  }

  // extreme high RSI short without rejection
  if (side === "bear" && rsi > SHORT_OVERBOUGHT_RSI_MAX && !hasRejection) {
    return { ok: false, reason: "V12_SHORT_EXTREME_RSI_NO_REJECTION" };
  }

  // long chase block (unchanged)
  if (
    side === "bull" &&
    rsi > LONG_RSI_CHASE_MIN &&
    rsiZone === "MID" &&
    (sniperScore < LONG_RSI_CHASE_MIN_SNIPER ||
      confluence < LONG_RSI_CHASE_MIN_CONFLUENCE ||
      !obAligned)
  ) {
    return { ok: false, reason: "V12_LONG_RSI_CHASE_WEAK" };
  }

  return { ok: true, reason: "V12_RSI_TIMING_OK" };
}

// ================= V12.2 FULL SL CIRCUIT BREAKER =================
function countRecentFullStopLosses({ side = null, lookbackMs = FULL_SL_LOOKBACK_MS } = {}) {
  const since = Date.now() - Number(lookbackMs || 0);
  const requestedSide = side ? String(side).toLowerCase() : null;

  return (auditState.closedTrades || []).filter(t => {
    const recent = Number(t?.exitedAt || 0) >= since;
    const fullSL = Number(t?.exitR || 0) <= FULL_SL_EXIT_R;
    const sameSide = !requestedSide || String(t?.side || "").toLowerCase() === requestedSide;
    return recent && fullSL && sameSide;
  }).length;
}
...
function getOpenPositionCounts() {
  ...
  return {
    total: rows.length,
    bull: rows.filter(p => String(p?.side || "").toLowerCase() === "bull").length,
    bear: rows.filter(p => String(p?.side || "").toLowerCase() === "bear").length
  };
}

function countRecentSideLosses({ side, lookbackMs = SIDE_LOSS_THROTTLE_LOOKBACK_MS } = {}) {
  const requestedSide = String(side || "").toLowerCase();
  if (!requestedSide) return 0;
  const since = Date.now() - Number(lookbackMs || 0);
  return (auditState.closedTrades || []).filter(t => {
    return (
      Number(t?.exitedAt || 0) >= since &&
      String(t?.side || "").toLowerCase() === requestedSide &&
      Number(t?.exitR || 0) < 0
    );
  }).length;
}

function validateSideExposureGateV12(candidate) {
  const side = String(candidate.side || "").toLowerCase();
  const setupClass = String(candidate.setupClass || "").toUpperCase();
  const confluence = Number(candidate.confluence || 0);
  const sniperScore = Number(candidate.sniperScore || 0);
  ...
  const counts = getOpenPositionCounts();
  const openSameSide = side === "bull" ? counts.bull : counts.bear;
  const recentSideLosses = countRecentSideLosses({ side });
  ...
  const bypass =
    setupClass === "GOD" ||
    (sniperScore >= FULL_SL_BYPASS_MIN_SNIPER &&
      confluence >= FULL_SL_BYPASS_MIN_CONFLUENCE &&
      obAligned);

  if (counts.total >= MAX_OPEN_POSITIONS_TOTAL && !bypass) {
    return { ok: false, reason: "V12_MAX_TOTAL_EXPOSURE" };
  }

  if (openSameSide >= MAX_OPEN_POSITIONS_PER_SIDE && !bypass) {
    return { ok: false, reason: "V12_MAX_SIDE_EXPOSURE" };
  }

  if (recentSideLosses >= SIDE_LOSS_THROTTLE_MIN_LOSSES && !bypass) {
    return { ok: false, reason: "V12_SIDE_LOSS_THROTTLE" };
  }

  return { ok: true, reason: "V12_SIDE_EXPOSURE_OK" };
}

function validateFullSlCircuitBreakerV12(candidate) {
  if (!ENABLE_FULL_SL_CIRCUIT_BREAKER) {
    return { ok: true, reason: "FULL_SL_CIRCUIT_DISABLED" };
  }
  ...
  const totalFullSL = countRecentFullStopLosses();
  const sideFullSL = countRecentFullStopLosses({ side });
  const obAligned =
    (side === "bull" && obBias === "BULLISH") ||
    (side === "bear" && obBias === "BEARISH");
  ...
  if (totalFullSL >= FULL_SL_BLOCK_AFTER_TOTAL && !bypass) {
    return { ok: false, reason: "V12_FULL_SL_TOTAL_CIRCUIT_BREAKER" };
  }

  if (sideFullSL >= FULL_SL_BLOCK_AFTER_SIDE && !bypass) {
    return { ok: false, reason: "V12_FULL_SL_SIDE_CIRCUIT_BREAKER" };
  }

  return { ok: true, reason: "V12_FULL_SL_CIRCUIT_OK" };
}

// ================= V12.3 COHORT POLICY HELPERS =================
function getCompletedCohortOutcomeRows() {
  const rows = buildFeatureOutcomeRows().all || [];

  return rows
    .filter(row => row?.liveCohortKey)
    .filter(row => Boolean(row?.hasRiskGeometry))
    .filter(row => Boolean(row?.win) || Boolean(row?.loss))
    .filter(row => Number.isFinite(Number(row?.exitR)) || Number.isFinite(Number(row?.pnlPct)));
}

function classifyCohortStats(stats) {
  const completed = Number(stats?.completed || 0);
  const avgR = Number(stats?.avgR || 0);
  const winrateNum = Number(stats?.winrateNum || 0);
  const profitFactorR = Number(stats?.profitFactorR || 0);
  const directSLPct = parsePct(stats?.directSLPct);

  if (completed < COHORT_MIN_COMPLETED) {
    return {
      decision: "INSUFFICIENT_SAMPLE",
      reason: "COHORT_SAMPLE_TOO_SMALL"
    };
  }

  if (
    avgR >= COHORT_LIVE_MIN_AVG_R &&
    winrateNum >= COHORT_LIVE_MIN_WINRATE &&
    profitFactorR >= COHORT_LIVE_MIN_PROFIT_FACTOR_R &&
    directSLPct <= COHORT_LIVE_MAX_DIRECT_SL_PCT
  ) {
    return {
      decision: "LIVE_ALLOWED",
      reason: "COHORT_POSITIVE_EDGE"
    };
  }

  if (
    avgR <= COHORT_BLOCK_MAX_AVG_R ||
    directSLPct >= COHORT_BLOCK_MAX_DIRECT_SL_PCT
  ) {
    return {
      decision: "BLOCK",
      reason: "COHORT_NEGATIVE_EDGE"
    };
  }

  return {
    decision: "SHADOW_ONLY",
    reason: "COHORT_NOT_PROVEN"
  };
}

function buildCohortEdgeReport(rows = null, minSample = COHORT_REPORT_MIN_SAMPLE) {
  const sourceRows = Array.isArray(rows)
    ? rows
    : getCompletedCohortOutcomeRows();

  const completed = sourceRows
    .filter(row => row?.liveCohortKey)
    .filter(row => Boolean(row?.win) || Boolean(row?.loss));
  ...
  return Array.from(map.entries())
    .map(([cohortKey, cohortRows]) => {
      const stats = getOutcomeStats(cohortRows);
      const classification = classifyCohortStats(stats);
      ...
      return {
        ...
        sample: Number(stats.completed || 0),
    ...
    .sort((a, b) => {
      const decisionRank = decision => {
        if (decision === "LIVE_ALLOWED") return 3;
        if (decision === "SHADOW_ONLY") return 2;
        if (decision === "INSUFFICIENT_SAMPLE") return 1;
      ...
      return Number(b.avgR || 0) - Number(a.avgR || 0);
    });
}

function buildLiveCohortCandidateRow({
  c,
  btcState,
  flow,
  ob,
  ...
  const row = {
    ...
    setupClass: String(setup?.setupClass || "NONE").toUpperCase(),
    reason: String(setup?.entryReason || "UNKNOWN").toUpperCase(),
    entryReason: String(setup?.entryReason || "UNKNOWN").toUpperCase(),
    ...
    finalRr: Number(finalRr || plannedRR || 0),
    rr: Number(finalRr || plannedRR || 0),

    entry: Number(risk?.entry || 0),
    sl: Number(risk?.sl || 0),
    tp: Number(risk?.tp || 0),
    hasRiskGeometry:
      Number(risk?.entry || 0) > 0 &&
      Number(risk?.sl || 0) > 0 &&
      Number(risk?.tp || 0) > 0,
    ...
    externalConfluence: Number(c?._debugExternalConfluence || 0),
    fallbackConfluence: Number(c?._debugFallbackConfluence || 0),
    confluenceSource: c?._debugConfluenceSource || null,
    fallbackCollapsed: c?._debugConfluenceSource === "FALLBACK_EXTERNAL_COLLAPSED",
...
function evaluateColdStartCohortCandidate(row) {
  if (COHORT_COLD_START_MODE === "SHADOW_ONLY") {
    return {
      ok: false,
      decision: "SHADOW_ONLY",
      reason: "COHORT_COLD_START_SHADOW_ONLY"
    };
  }

  const setupClass = String(row?.setupClass || "NONE").toUpperCase();

  const elite =
    ["A", "GOD"].includes(setupClass) &&
    row.fallbackCollapsed !== true &&
    Number(row.confluence || 0) >= COHORT_COLD_START_MIN_CONFLUENCE &&
    Number(row.sniperScore || 0) >= COHORT_COLD_START_MIN_SNIPER &&
    Number(row.finalRr || row.plannedRR || 0) >= COHORT_COLD_START_MIN_RR &&
    normalizeSpread(row.spreadPct) <= COHORT_COLD_START_MAX_SPREAD_PCT &&
    Number(row.depthMinUsd1p || 0) >= COHORT_COLD_START_MIN_DEPTH_USD_1P &&
    Boolean(row.isObWithSide);

  if (!elite) {
    return {
      ok: false,
      decision: "SHADOW_ONLY",
      reason: "COHORT_COLD_START_NOT_ELITE"
    };
  }
  ...
  return {
    ok: true,
    decision: "LIVE_ALLOWED",
    reason: "COHORT_COLD_START_ELITE_ALLOW"
  };
}

function evaluateLiveCohortPolicy(row) {
  if (!ENABLE_COHORT_LIVE_POLICY) {
    return {
      ok: true,
      decision: "POLICY_DISABLED",
      reason: "COHORT_POLICY_DISABLED",
      cohortKey: row?.liveCohortKey || null,
      sample: 0,
      stats: null
    };
  }

  const cohortKey = row?.liveCohortKey;

  if (!cohortKey) {
    return {
      ok: false,
      decision: "SHADOW_ONLY",
      reason: "COHORT_KEY_MISSING",
      cohortKey: null,
      sample: 0,
      stats: null
    };
  ...
  const cohortRows = getCompletedCohortOutcomeRows()
    .filter(outcome => outcome.liveCohortKey === cohortKey);

  const stats = getOutcomeStats(cohortRows);
  const sample = Number(stats.completed || 0);

  if (sample < COHORT_MIN_COMPLETED) {
    const coldStart = evaluateColdStartCohortCandidate(row);
  ...
  const classified = classifyCohortStats(stats);

  if (classified.decision === "LIVE_ALLOWED") {
    return {
      ok: true,
...
function recordEntry(entryPayload, position) {
  ...
  auditState.recentEntries.push({
    ...
    liveCohortKey: position.liveCohortKey || null,
    cohortDecision: position.cohortDecision || null,
    cohortReason: position.cohortReason || null,
    cohortSample: Number(position.cohortSample || 0)
  });
...
function recordExit(exitPayload, pos, exitPrice, isBull) {
  ...
  const closedTrade = initializePostExitFields({
    ...
    liveCohortKey: pos.liveCohortKey || null,
    cohortDecision: pos.cohortDecision || null,
    cohortReason: pos.cohortReason || null,
    cohortSample: Number(pos.cohortSample || 0),
...
function logOptimizerReport({ reasonTable, btcState, runId }) {
  if (!ENABLE_FEATURE_STORE && !ENABLE_SHADOW_OUTCOMES) {
    return {
      ...
      enabled: false,
      skipped: true,
      reason: "FEATURE_STORE_AND_SHADOW_DISABLED"
    };
  }

  return buildTsMasterBestAfstellingLog({
    btcState,
    runId
  });
}
...
function buildAnalyzePayload({ auditResult, candidates, actions, btcState, runId }) {
  ...
  return {
    ...
    cohortPolicy: {
      enabled: ENABLE_COHORT_LIVE_POLICY,
      minCompleted: COHORT_MIN_COMPLETED,
      coldStartMode: COHORT_COLD_START_MODE,
      liveCriteria: {
        minWinrate: COHORT_LIVE_MIN_WINRATE,
        minAvgR: COHORT_LIVE_MIN_AVG_R,
        minProfitFactorR: COHORT_LIVE_MIN_PROFIT_FACTOR_R,
        maxDirectSLPct: COHORT_LIVE_MAX_DIRECT_SL_PCT
      }
    },

    cohortEdgeReport: buildCohortEdgeReport(
      buildFeatureOutcomeRows().all,
      COHORT_REPORT_MIN_SAMPLE
    ),
...
// ================= MAX DEEP CANDIDATES =================
// Upgraded side‑balanced cap: more candidates, fair share for bulls & bears
const MAX_DEEP_CANDIDATES = 96;
const MAX_DEEP_PER_SIDE = 48;
...
function capDeepCandidates(candidates) {
  ...
  const rank = row => {
    const stageBoost = String(row.stage || "").toLowerCase() === "entry" ? 1000 : 0;
    const score = Number(row.moveScore || 0);
    return stageBoost + score;
  };

  const bulls = fresh
    .filter(c => String(c.side || "").toLowerCase() === "bull")
    .sort((a, b) => rank(b) - rank(a))
    .slice(0, MAX_DEEP_PER_SIDE);

  const bears = fresh
    .filter(c => String(c.side || "").toLowerCase() === "bear")
    .sort((a, b) => rank(b) - rank(a))
    .slice(0, MAX_DEEP_PER_SIDE);

  const cappedFresh = [...bulls, ...bears]
    .sort((a, b) => rank(b) - rank(a))
...
function requiresEntryConfirmation(candidate) {
  ...
  if (
    SHORT_OVERBOUGHT_REQUIRE_CONFIRMATION &&
    side === "bear" &&
    rsi >= SHORT_OVERBOUGHT_RSI_MIN
  ) {
    return true;
...
function evaluateEarlyFailureExit(position, ob) {
  ...
  if (
    ageSec >= EARLY_SOFT_STOP_MIN_AGE_SEC &&
    currentR <= EARLY_SOFT_STOP_MAX_CURRENT_R &&
    mfeR < EARLY_SOFT_STOP_MAX_MFE_R
  ) {
    return {
      exit: true,
      reason: "EARLY_SOFT_STOP_NO_MFE"
    };
  }
...
const tsAwareCandidates = applyTsAwareCandidateScoring(allCandidates, btcState);
const candidates = capDeepCandidates(tsAwareCandidates);
  ...
  return finalizeResult([], [], btcState, runId, options);
    ...
    for (const originalCoin of candidates) {
      ...
      if (prev) {
        ...
        // early failure check BEFORE hard TP/SL
        const earlyFailure = evaluateEarlyFailureExit(pos, obData);
        if (!hitTP && !hitSL && earlyFailure.exit) {
          ...
          if (shouldLog) {
            const exitResult = Number(exitPayload.exitR || 0) > 0 ? "WIN" : Number(exitPayload.exitR || 0) < 0 ? "LOSS" : "FLAT";
            await logTrade({
              symbol: c.symbol,
              side: c.side,
              setupClass: pos.setupClass || "UNKNOWN",
          ...
          if (notify && !notifyState.get(exitKey)) {
            ...
            if (discordOk) notifyState.set(exitKey, true);
          }
      ...
      if (hasAnyOpenPositionForSymbol(c.symbol)) {
        ...
        actions.push(buildWait(c, "SYMBOL_ALREADY_OPEN", flow, null, 0, 0, funding, obData, null, null, null, null));
        console.log("TS_ENTRY_BLOCKED_SYMBOL_ALREADY_OPEN:", JSON.stringify({ symbol: c.symbol, side: c.side, openSide }));
        continue;
      }

      if (cooldownMap.has(key)) {
        actions.push(buildWait(c, "COOLDOWN_ACTIVE", flow, null, 0, 0, funding, obData, null, null, null, null));
        continue;
      }

      if (symbolCooldownMap.has(c.symbol)) {
        actions.push(buildWait(c, "SYMBOL_REENTRY_COOLDOWN_ACTIVE", flow, null, 0, 0, funding, obData, null, null, null, null));
        continue;
      }

      if (processingLocks.has(symbolLockKey)) {
        actions.push(buildWait(c, "PROCESSING_LOCK_ACTIVE", flow, null, 0, 0, funding, obData, null, null, null, null));
        continue;
      }
      ...
      try {
        ...
        if (rsiSignal.blocked === true) {
          const payload = buildWait(c, "RSI_HTF_BLOCKED", flow, null, 0, 0, funding, obData, null, null, null, null);
        ...
        if (rsiSignal.valid === false) {
          const payload = buildWait(c, "RSI_INVALID", flow, null, 0, 0, funding, obData, null, null, null, null);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }
        ...
        if (rsiEdge.hardBlock) {
          const payload = buildWait(c, isBull ? "RSI_LONG_EXTREME_TOO_HIGH" : "RSI_SHORT_EXTREME_TOO_LOW", flow, null, 0, 0, funding, obData, null, null, null, null);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
        ...
        if (isBull && ["UPPER_2", "UPPER_3"].includes(rsiZone) && !rsiEdge.continuationOk) {
          const payload = buildWait(c, "RSI_LONG_TOO_HIGH", flow, null, 0, 0, funding, obData, null, null, null, null);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }
        ...
        if (!isBull && SHORT_BLOCKED_RSI_ZONES.includes(rsiZone)) {
          const payload = buildWait(c, "RSI_SHORT_TOO_LOW", flow, null, 0, 0, funding, obData, null, null, null, null);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }
        ...
        if (c.side === "bull" && ["BEARISH", "STRONG_BEAR"].includes(String(btcState).toUpperCase())) {
          ...
          if (score < BTC_BEARISH_LONG_MIN_SCORE) {
            actions.push(buildWait(c, "BTC_BEARISH_LONG_LOW_SCORE", flow, null, 0, 0, funding, obData, null, null, null, null));
            continue;
          }
        }
        ...
        const sniper = await getSafeSniperEntry({ c, flow, ob: obData, rsiSignal, rsiZone, isBull, btcState });
        ...
        const liquidity = await cachedFetch(`liq_${contractSymbol}_${c.side}`, () => getSafeLiquidityZones({ contractSymbol, c }), 20000).catch(() => null);
        const risk = await getSafeRiskGeometry({ c, isBull, liquidity, liquidation, ob: obData, flow, sniper, rsiSignal });
        if (!isValidRiskGeometry(risk, isBull)) {
          const payload = buildWait(c, "RISK_INVALID", flow, sniper, 0, 0, funding, obData, risk, null, null, null);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }

        const baseRR = safeNumber(risk.rr || calculateRRFromPrices(risk.entry, risk.sl, risk.tp, isBull), 0);
        const hasLiquidationData = Boolean(liquidation && (Array.isArray(liquidation?.zones) || Array.isArray(liquidation?.longs) || Array.isArray(liquidation?.shorts) || Object.keys(liquidation || {}).length > 0));

        const rawConfluence = await getSafeConfluence({ c, flow, sniper, ob: obData, risk, rr: baseRR, funding, rsiSignal, rsiZone, btcState, regimeLevel, liquidation, isBull });
        ...
        if (confluence < GLOBAL_MIN_CONFLUENCE) {
          const payload = buildWait(c, "LOW_GLOBAL_CONFLUENCE", flow, sniper, confluence, baseRR, funding, obData, risk, null, GLOBAL_MIN_CONFLUENCE, null);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }

        const bullPullbackMeta = getBullPullbackMeta({ c, candles15m: rsiData?.candles15m || [], fakeBreakoutConfirmed: Boolean(c.fakeBreakoutConfirmed || c.fakeBreakout) });
        ...
        const candidateForValidation = {
          ...
          rsiHTF,
          rsiZone,
          rsiSlope3: Number(rsiSignal?.slope3 || rsiEdge?.slope3 || 0),
          rsiContinuationOK: Boolean(rsiSignal?.continuationOK || rsiSignal?.continuationOk || rsiEdge.continuationOk),
        ...
        // ----- NUMERIC RSI TIMING GATE (V12.2) -----
        const numericRsiTimingGate = validateNumericRsiTimingGateV12(candidateForValidation);
        if (!numericRsiTimingGate.ok) {
          const payload = buildWait(
            c,
            numericRsiTimingGate.reason,
            flow,
            sniper,
            confluence,
            baseRR,
          ...
          payload.rsiEdge = rsiEdge.label;
          payload.rsiSlope3 = rsiSlope3;
          payload.plannedRR = baseRR;
          payload.qualityGate = "V12_NUMERIC_RSI_TIMING";
          actions.push(payload);
          continue;
        }
        ...
        const btcBullishBearException = validateBtcBullishShortException(candidateForValidation);
        c._debugBtcBullishBearException = btcBullishBearException.exception;
        c._debugBtcBullishBearExceptionReason = btcBullishBearException.reason;
        c._debugBtcBullishBearExceptionChecks = btcBullishBearException.checks;

        if (c.side === "bear" && isBullMarket && !btcBullishBearException.ok) {
          const payload = buildWait(c, btcBullishBearException.reason || "BTC_BULLISH_WEAK_SHORT", flow, sniper, confluence, baseRR, funding, obData, risk, null, BTC_BULLISH_BEAR_EXCEPTION_MIN_CONF, BTC_BULLISH_BEAR_EXCEPTION_MIN_RR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }

        const setupGrade = getSetupGrade({ c, ob: obData, flow, sniper, confluence, rr: baseRR, hasLiquidationData, isBull });
        const counterTrend = getCounterTrend(c, btcState);
        const dynamicMinRr = getDynamicMinRrFloor({ c, setupGrade, flow, sniper, confluence, counterTrend });
        c.minRrFloor = dynamicMinRr;
        const setupEvalRR = Number((baseRR + getRsiSetupRrCredit(rsiEdge)).toFixed(2));
        c._debugSetupEvalRR = setupEvalRR;

        // Record scan observation for EVERY candidate (for later replay)
        ...
        if (!setup) {
          const noSetupReason = getNoSetupReason({
            setupEvalRR,
            confluence,
            sniperScore: Number(sniper?.score || 0),
            rsiZone,
          ...
          const payload = buildWait(
            ...
            risk,
            setupGrade,
            ENABLE_B_ENTRIES ? Math.min(A_MIN_CONFLUENCE, B_MIN_CONFLUENCE) : A_MIN_CONFLUENCE,
            ENABLE_B_ENTRIES ? Math.min(A_ENTRY_MIN_RR, B_ENTRY_MIN_RR) : A_ENTRY_MIN_RR
          );
          ...
          payload.rsiZone = rsiZone;
          payload.plannedRR = baseRR;
          payload.setupEvalRR = setupEvalRR;
          payload.minSetupRR = ENABLE_B_ENTRIES
            ? Math.min(A_ENTRY_MIN_RR, B_ENTRY_MIN_RR)
            : A_ENTRY_MIN_RR;
        ...
        const rawRequiredRR = Math.max(Number(setup.requiredRR || MIN_RR_FLOOR), Number(dynamicMinRr || MIN_RR_FLOOR));
        const requiredRR = applyRsiEdgeToRequiredRR(rawRequiredRR, rsiEdge);
        const requiredSniper = getRsiAdjustedRequiredSniper({ requiredSniper: setup.requiredSniper, rsiEdge });

        if (Number(sniper?.score || 0) < requiredSniper) {
          const payload = buildWait(c, "LOW_SNIPER", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }
        ...
        if (confluence < Number(setup.requiredConfluence || 0)) {
          const payload = buildWait(c, "LOW_CONFLUENCE", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }
        ...
        if (baseRR < requiredRR) {
          const payload = buildWait(c, "LOW_RR", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }

        const maxSpread = getMaxSpreadForCandidate({ c, rsiZone, setupClass: setup.setupClass });
        const spread = normalizeSpread(obData?.spreadPct);
        if (spread > maxSpread) {
          const midBullSpreadException = c.side === "bull" && rsiZone === "MID" && confluence >= MID_BULL_SPREAD_EXCEPTION_MIN_CONFLUENCE && Number(sniper?.score || 0) >= MID_BULL_SPREAD_EXCEPTION_MIN_SNIPER;
          if (!midBullSpreadException) {
            const payload = buildWait(c, c.side === "bull" && rsiZone === "MID" ? "MID_BULL_SPREAD_TOO_WIDE" : "SPREAD_TOO_WIDE", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
            payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
        ...
        const finalDepthValidation = validateFinalDepth({ ...candidateForValidation, setupClass: setup.setupClass, flow: flow.type });
        if (!finalDepthValidation.ok) {
          const payload = buildWait(c, finalDepthValidation.reason, flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }

        const bullAntiChase = validateBullAntiChase({ ...candidateForValidation, setupClass: setup.setupClass, bullishMidTrendProbeOk: bullishMidTrendProbe.ok });
        if (!bullAntiChase.ok) {
          const payload = buildWait(c, bullAntiChase.reason, flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }
        ...
        if (Math.abs(Number(funding?.rate || 0)) > EXTREME_FUNDING_ABS_MAX) {
          const payload = buildWait(c, "EXTREME_FUNDING", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }

        if (c.side === "bull" && Number(funding?.rate || 0) > BULL_CROWDED_FUNDING_MAX && confluence < CROWDED_FUNDING_MIN_CONFLUENCE) {
          actions.push(buildWait(c, "BULL_CROWDED_FUNDING", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, CROWDED_FUNDING_MIN_CONFLUENCE, requiredRR));
          continue;
        }

        if (c.side === "bear" && Number(funding?.rate || 0) < BEAR_CROWDED_FUNDING_MIN && confluence < CROWDED_FUNDING_MIN_CONFLUENCE) {
          actions.push(buildWait(c, "BEAR_CROWDED_FUNDING", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, CROWDED_FUNDING_MIN_CONFLUENCE, requiredRR));
          continue;
        }
        ...
        if (!entryQualityGate.ok) {
          const payload = buildWait(c, entryQualityGate.reason, flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          payload.rsiEdge = rsiEdge.label;
          payload.plannedRR = baseRR;
        ...
        // ----- SIDE EXPOSURE GATE (V12.2) -----
        const sideExposureGate = validateSideExposureGateV12({
          ...candidateForValidation,
          setupClass: setup.setupClass,
          rsi,
          rsiHTF,
        ...
        if (!sideExposureGate.ok) {
          const payload = buildWait(c, sideExposureGate.reason, flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          payload.rsiEdge = rsiEdge.label;
          payload.plannedRR = baseRR;
          payload.baseRR = baseRR;
          payload.openPositionCounts = getOpenPositionCounts();
          payload.recentSideLosses = countRecentSideLosses({ side: c.side });
          payload.qualityGate = "V12_SIDE_EXPOSURE";
          actions.push(payload);
          continue;
        }

        // ----- FULL SL CIRCUIT BREAKER (V12.2) -----
        const fullSlCircuit = validateFullSlCircuitBreakerV12({
          ...candidateForValidation,
          setupClass: setup.setupClass,
          rsi,
          rsiHTF,
        ...
        if (!fullSlCircuit.ok) {
          const payload = buildWait(c, fullSlCircuit.reason, flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          payload.rsiEdge = rsiEdge.label;
          payload.plannedRR = baseRR;
          payload.baseRR = baseRR;
          payload.recentFullSLTotal = countRecentFullStopLosses();
          payload.recentFullSLSameSide = countRecentFullStopLosses({ side: c.side });
          payload.qualityGate = "V12_FULL_SL_CIRCUIT";
          actions.push(payload);
          continue;
        }

        if (isObAgainstSide(obData, isBull) && confluence < OB_AGAINST_MIN_CONFLUENCE) {
          const payload = buildWait(c, "OB_AGAINST_SIDE", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, OB_AGAINST_MIN_CONFLUENCE, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          actions.push(payload);
          continue;
        }

        if (String(obData?.bias || "NEUTRAL").toUpperCase() === "NEUTRAL" && !isNeutralObEntryException({ c, flow, sniper, confluence, rr: baseRR, setupGrade, counterTrend })) {
          const requiredNeutralConf = (setup.setupClass === "A" || setup.setupClass === "GOD") ? NEUTRAL_OB_A_EXCEPTION_MIN_CONFLUENCE : NEUTRAL_OB_B_EXCEPTION_MIN_CONFLUENCE;
          if (confluence < requiredNeutralConf) {
            const payload = buildWait(c, "NEUTRAL_OB_LOW_CONFLUENCE", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, requiredNeutralConf, requiredRR);
            payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
            actions.push(payload);
            continue;
          }
        ...
        const tpRewardMultiplier = getTpRewardMultiplier({ setupClass: setup.setupClass, certaintyMode, sniperScore: Number(sniper?.score || 0), rsi, isBull });
        const finalTp = buildAdjustedTp(risk.entry, risk.tp, tpRewardMultiplier, isBull);
        const finalRr = calculateRRFromPrices(risk.entry, risk.sl, finalTp, isBull);
        const rawFinalRequiredRR = ["A", "GOD", "A_SHORT_EXCEPTION"].includes(setup.setupClass) ? Math.max(requiredRR, A_FINAL_MIN_RR) : requiredRR;
        const finalRequiredRR = applyRsiEdgeToRequiredRR(rawFinalRequiredRR, rsiEdge);
        if (finalRr < finalRequiredRR) {
          const payload = buildWait(c, "LOW_FINAL_RR", flow, sniper, confluence, finalRr, funding, obData, { ...risk, tp: finalTp }, setupGrade, setup.requiredConfluence, finalRequiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          payload.baseRR = baseRR;
        ...
        // ================= V12.3 COHORT LIVE POLICY GATE =================
        const liveCohortRow = buildLiveCohortCandidateRow({
          c,
          btcState,
          flow,
          ob: obData,
        ...
        const liveCohortPolicy = evaluateLiveCohortPolicy(liveCohortRow);

        if (!liveCohortPolicy.ok) {
          const payload = buildWait(
            c,
            liveCohortPolicy.reason,
            flow,
            sniper,
            confluence,
            baseRR,
          ...
          payload.entryReason = setup.entryReason;

          payload.plannedRR = Number(baseRR);
          payload.baseRR = Number(baseRR);
          payload.finalRr = Number(finalRr);
          payload.effectiveRR = Number(finalRr);
          payload.tpRewardMultiplier = Number(tpRewardMultiplier);

          payload.liveCohortKey = liveCohortPolicy.cohortKey;
          payload.cohortDecision = liveCohortPolicy.decision;
          payload.cohortReason = liveCohortPolicy.reason;
          payload.cohortSample = liveCohortPolicy.sample;
          payload.cohortStats = liveCohortPolicy.stats;

          payload.rawConfluence = Number(rawConfluence || 0);
          payload.externalConfluence = c._debugExternalConfluence;
          payload.fallbackConfluence = c._debugFallbackConfluence;
          payload.confluenceSource = c._debugConfluenceSource;

          payload.qualityGate = "V12_3_COHORT_POLICY";
        ...
        if (requiresEntryConfirmation(confirmationCandidate) && !hasRecentEntryConfirmation(key)) {
          ...
          const payload = buildWait(c, "V12_ENTRY_CONFIRMATION_PENDING", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, setup.requiredConfluence, requiredRR);
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          payload.rsiEdge = rsiEdge.label;
          payload.plannedRR = baseRR;
          payload.setupClass = setup.setupClass;
        ...
        const entryPayload = {
          ...
          liveCohortKey: liveCohortPolicy.cohortKey,
          cohortDecision: liveCohortPolicy.decision,
          cohortReason: liveCohortPolicy.reason,
          cohortSample: liveCohortPolicy.sample,
          cohortStats: liveCohortPolicy.stats,
        ...
        if (notify && !notifyState.get(key)) {
          const discordOk = await notifyEntrySafe({ ...entryPayload, flow: position.flow, stage: c.stage, scannerStage: c.scannerStage || c.stage });
          if (discordOk) notifyState.set(key, true);
        }
