...
import { getLiquidationZones } from "./liquidationEngine.js"; // ✅ FIX
...
export async function processTrades(coins){
  ...
  for(const originalCoin of coins){
    ...
    const fallbackLiq = getLiquidityZones(c, ob);
    ...
    const nearestAbove = liquidation?.nearestAbove || null;
    const nearestBelow = liquidation?.nearestBelow || null;

    // ================= FAKE BREAKOUT =================
    let fakeBreakout = false;

    // alleen als echt door zone heen
    if(isBull && nearestAbove && c.price > nearestAbove * 1.002){
      fakeBreakout = true;
    }

    if(!isBull && nearestBelow && c.price < nearestBelow * 0.998){
      fakeBreakout = true;
    }
