...
// - C entry verwijderd (geen fallback meer voor lage sniper scores)
// - Momentum filter aangescherpt
// - LOWER_2 drempel -0.05
// - LIGHT mag entries doen (behouden)
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // ========== MARKT KWALITEIT (aangescherpt) ==========
    if (ob.depthMinUsd1p < 250000 && confluence < 75) {
      const waitPayload = buildWait(c, "BAD_MARKET_QUALITY", flow, sniper, confluence, 0, funding, ob, null, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    const sniperOK = sniperScore >= 65;              // aangescherpt van 55 naar 65
    ...
    // Flow NEUTRAL filter aangescherpt: nu altijd blokkeren bij confluence < 65
