// tradeSystem.js - Definitieve versie met correcte volgorde
// 1. data ophalen
// 2. confluence, risk, rr, fake breakout berekenen
// 3. dan pas RSI-filters en entry logic
...
export async function processTrades(input, options = {}) {
  ...
  // ========== VERWERK COINS (met juiste volgorde) ==========
  for (const originalCoin of candidates) {
    ...
    // BTC dominance filters (blijven vooraan)
    ...
    // Flow en RSI basis (geen filters, alleen variabelen bepalen)
    ...
    // ROBUUSTE RSI CONTEXT voor calculateConfluence
    ...
    // 4️⃣ DEBUG LOG (nu met echte waarden)
    console.log(`📊 DATA: ${c.symbol} | hasRSI=${!!rsiContext} | conf=${confluence} | sniper=${sniperScore} | rr=${rr.toFixed(2)} | fake=${isValidFakeBreakout}`);

    // ========== NU PAS DE RSI-FILTERS (die kunnen continue veroorzaken) ==========
    if (isBull && !["LOWER_2","LOWER_3"].includes(rsiZone) && !earlyRSI) {
      const waitPayload = buildWait(c, "RSI_NOT_OVERSOLD_ENOUGH", flow, sniper, confluence, rr, funding, ob, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (!isBull && !["UPPER_2","UPPER_3"].includes(rsiZone) && !earlyRSI) {
      const waitPayload = buildWait(c, "RSI_NOT_OVERBOUGHT_ENOUGH", flow, sniper, confluence, rr, funding, ob, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    // Fake breakout check (na berekening)
    ...
    // Verdere entry filters (stage, sniperOK, grade, RR, enz.)
    ...
    // ========== POSITION MANAGEMENT ==========
    if (prev) {
      // ... (zelfde als eerder, blijft ongewijzigd)
      const pos = { ...prev };
