...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // Benodigde variabelen voor confluence
    ...
    console.log(c.symbol, {
      sniper: sniper.score,
      conf: confluence,
      rr: (await (async () => { const risk = await calculateRisk(c, ob, liquidity, null, { candles15m: [], candles1h: [] }); return risk?.rr; })()),
      grade: (() => {
        const rrTemp = Number(riskBase?.rr) || 0;
        const setupGradeTemp = getSetupGrade({ c, ob, flow, sniper, confluence, rr: rrTemp, hasLiquidationData, isBull });
        return setupGradeTemp.grade;
      })()
    });
    // Bovenstaande debug is complex, maar we blijven het simpel: we loggen later na rr berekening.
    // We herstructureren: bereken eerst riskBase, rr, setupGrade, daarna log.
    ...
    const momentumShift = Math.abs(Number(c.change1h || 0)) > 0.2 && Math.abs(Number(c.change24 || 0)) > 2;
    if (!momentumShift) {
    ...
    const sniperOK = sniper.score >= 70;
    ...
    // Setup valid alleen voor DEEP
    const setupValid = isDeep
      ? (stageOK && finalSniperOK && gradeOK && !ob.spoof && rr >= minRrFloor)
      : false;

    // 🔥 Elite entry: strenger (sniper >= 75, rr >= 1.1)
    const eliteEntry = setupValid && sniper.score >= 75 && rr >= 1.1;

    // 🔥 God mode entry (voor position sizing later)
    const godModeEntry = eliteEntry && sniper.score >= 85 && confluence >= 85 && rr >= 1.2;

    // Debug log (eenmalig per coin)
    console.log(`🔍 ${c.symbol} (${isDeep ? "DEEP" : "LIGHT"}): sniper=${sniper.score}, conf=${confluence}, rr=${rr.toFixed(2)}, grade=${setupGrade.grade}, elite=${eliteEntry}, godmode=${godModeEntry}`);
    ...
    if (prev) {
      ...
      const holdKey = `${key}_hold`;
      if (notify && !notifyState.get(holdKey)) {
    ...
    // ========== ENTRY FILTERS (alleen voor DEEP) ==========
    ...
    if (fakeBreakout && (confluence < 82 || !sniper?.valid) && isDeep) {
      actions.push(buildWait(c, "FAKE_BREAKOUT", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }

    if (btcState === "BULLISH" && !isBull && c.moveScore < 70 && isDeep) {
      actions.push(buildWait(c, "BTC_BULL_BLOCK_SHORT", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }
    if (btcState === "BEARISH" && isBull && c.moveScore < 70 && isDeep) {
      actions.push(buildWait(c, "BTC_BEAR_BLOCK_LONG", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }

    const minConfDynamic = getDynamicMinConf({ c, ob, flow, vol, isBull });
    const minConf = Math.max(50, minConfDynamic);
    ...
    // ========== DIRECT ENTRY (alleen voor DEEP) ==========
    if (eliteEntry) {
      const reasonEntry = godModeEntry ? "GOD_MODE" : "ELITE_ENTRY";
      let finalTp = riskBase.tp;
      if (certaintyMode === "safe") {
        finalTp = finalTp * 0.95;
      } else {
        // TP optimalisatie obv sniper score
        if (sniper.score >= 90) {
          finalTp *= 0.9;
        } else if (sniper.score >= 80) {
          finalTp *= 0.95;
        }
        // RSI tuning
        if (isBull) {
      ...
      const position = {
        ...
        rsiZone: rsiSignal.zones ? (rsi <= rsiSignal.zones.L3 ? "LOWER_3" : rsi <= rsiSignal.zones.L2 ? "LOWER_2" : rsi <= rsiSignal.zones.L1 ? "LOWER_1" : rsi >= rsiSignal.zones.U3 ? "UPPER_3" : rsi >= rsiSignal.zones.U2 ? "UPPER_2" : rsi >= rsiSignal.zones.U1 ? "UPPER_1" : "MID") : "MID"
      };
    ...
    // Geen entry – WAIT voor zowel DEEP als LIGHT
    if (!isDeep) {
  ...
  // Fallback als er geen acties zijn
