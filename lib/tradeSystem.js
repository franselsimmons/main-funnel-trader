...
const DATA_LAKE_RETENTION_DAYS = Number(process.env.TS_DATA_LAKE_RETENTION_DAYS || 30);
const DATA_LAKE_ANALYZE_LOOKBACK_DAYS = Number(process.env.TS_ANALYZE_LOOKBACK_DAYS || 14);
const DATA_LAKE_MAX_LOAD_ROWS = Number(process.env.TS_DATA_LAKE_MAX_LOAD_ROWS || 10000);
const DATA_LAKE_RAW_SCAN_MAX_PER_RUN = Number(process.env.TS_DATA_LAKE_RAW_SCAN_MAX_PER_RUN || 300);
...
function recordRawScanRowsToDataLake({ rows, runId, btcState, scanRegime }) {
  ...
  for (const raw of rows.slice(0, DATA_LAKE_RAW_SCAN_MAX_PER_RUN)) {
    queueDataLakeAppend({
      type: "SCAN_ROW",
      profile: DATA_LAKE_PROFILE,
      scanStage: "RAW_SCAN",
      ...
      side: raw?.side,
      stage: raw?.stage,
      scannerStage: raw?.scannerStage || raw?.stage,
      score: raw?.moveScore,
      price: raw?.price || raw?.lastPrice,

      flow: raw?.flow,
      scannerFlow: raw?.scannerFlow || raw?.flow,
      btcState,
      regime: scanRegime,
