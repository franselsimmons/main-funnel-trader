...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    const confluence = calculateConfluence(
      ...
      regimeForConfluence,
      hasLiquidationData ? liquidation : null,
      rsiData?.m15 // gebruik 15m context voor boost (of kan ook h1 zijn, maar blijf consistent)
    );
    ...
    const rsiOk = rsiData
      ? (
          isType1RSIEntry(rsiData.m15, c.side) &&   // entry timing
          isType1RSIEntry(rsiData.h1, c.side)       // trend alignment
        )
      : false;
