...
import { getLiquidationZones } from "./liquidationEngine.js"; // ✅ weer ingeschakeld
...
function getSetupGrade({ c, ob, flow, sniper, confluence, rr, hasLiquidationData, isBull }) {
  ...
  if (points >= 10) { grade = "A"; recommendedRisk = "normal"; }
  else if (points >= 8) { grade = "B"; recommendedRisk = "small"; }
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    if (isBull && !["LOWER_2", "LOWER_3"].includes(rsiZone)) {
    ...
    // Extra pro: LOWER_2 alleen als prijs nog daalt
    ...
    if (!strongMomentum) {
      const waitPayload = buildWait(c, "NO_STRONG_MOMENTUM", flow, sniper, confluence, rr, funding, ob, riskBase, null, null, null);
      actions.push(waitPayload);
      continue;
    }

    // ================= FAKE BREAKOUT – ORIGINELE (op basis van liquidation) =================
    ...
    // ================= FAKE BREAKOUT – NIEUW (candle-gebaseerde liquidity grab) =================
    ...
    // Combineer: als een van beiden aangeeft dat het een fake breakout is, beschouwen we het als geldig.
    const isValidFakeBreakout = fakeBreakout || candleFakeBreakout;

    if (!isValidFakeBreakout) {
