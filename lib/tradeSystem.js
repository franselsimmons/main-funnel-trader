...
// Liquidation volledig uitgeschakeld (was 451)
...
export async function processTrades(input, options = {}) {
  ...
  const fetchCoinData = async (c) => {
    ...
    dataMap.set(symbol, { ob, funding, rsiData });
  };
  ...
  for (const originalCoin of candidates) {
    ...
    const { ob, funding, rsiData } = dataMap.get(c.symbol) || { ob: DEFAULT_OB, funding: { rate: 0 }, rsiData: null };
    ...
    // LONG alleen bij LOWER_2 of LOWER_3
    ...
    // SHORT alleen bij UPPER_2 of UPPER_3
    ...
    const liquidation = null;
    const hasLiquidationData = false;
    ...
    // ================= STRONG MOMENTUM (vervangen oude momentum) =================
    ...
    // ================= EINDE STRONG MOMENTUM =================

    // ================= FAKE BREAKOUT DETECTIE (vervangt oude fake breakout) =================
    ...
    if (candles15m.length >= 20) {
      ...
      if (isBull) {
        // LONG: prijs zit net boven recente low (bounce na dump)
        if (c.price > recentLow && c.price < recentLow * 1.01) {
          fakeBreakout = true;
        }
      } else {
        // SHORT: prijs zit net onder recente high (bounce na pump)
        if (c.price < recentHigh && c.price > recentHigh * 0.99) {
          fakeBreakout = true;
        }
      }
    }
    ...
    console.log(`🔍 ${c.symbol} (${isDeep ? "DEEP" : "LIGHT"}): sniper=${sniper.score}, conf=${confluence}, rr=${rr.toFixed(2)}, grade=${setupGrade.grade}, elite=${eliteEntry}, b=${bEntry}, godmode=${godModeEntry}, rsiZone=${rsiZone}, fakeBreakout=${fakeBreakout}`);
    ...
    if (prev) {
      ...
      if (pos.entries && pos.entries.length < pos.maxEntries && timeSinceLastEntry > 5 * 60 * 1000) {
        ...
        if (betterPrice) {
          ...
          // geen Discord notificatie voor scaling
          continue;
        }
      }
