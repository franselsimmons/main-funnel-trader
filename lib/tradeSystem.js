...
// ⚠️ liquidationEngine wordt niet meer gebruikt (uitgeschakeld)
...
function getDynamicMinRrFloor({ c, setupGrade, flow, sniper, confluence, counterTrend }) {
  ...
  // 🔥 VERSOEPELD: minimale RR nu 0.85 i.p.v. 0.95
  return clamp(floor, 0.85, 1.50);
}
...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  // 🔥 MAX 4 COINS (voorkomt rate limit)
  ...
  const dataMap = new Map(); // symbol -> { ob, funding, rsiData }
  ...
  await Promise.all(candidates.map(async (c) => {
    ...
    // RSI + structure (fallback bij lege candles)
  ...
  for (const originalCoin of candidates) {
    ...
    // Markt context voor risk manager (candles uit rsiData)
    const marketCtx = {
    ...
    // STRUCTURE FILTER (alleen tegen BEARISH/BULLISH, NEUTRAL mag door)
    ...
    // MOMENTUM SHIFT (iets versoepeld: 0.2% / 2%)
    ...
    if (!isBull && btcState === "BEARISH") minRrFloor = Math.max(0.80, minRrFloor - 0.05);
    ...
    if (prev) {
      ...
      const action = "HOLD", reason = "RUNNING";
    ...
    if (hasLiquidationData) {
      // liquidation is altijd null, dus deze code wordt nooit uitgevoerd
    ...
    const obAgainst = isObAgainstSide(ob, isBull);
    const hasLiquidationRoom = true; // geen liquidation data, dus altijd voldoende ruimte
    ...
    // 🔥 FASE LOGICA MET VERSOEPELDE RSI
    const rsiAligned = (isBull && rsi <= 48) || (!isBull && rsi >= 52);
    ...
    if (state.phase === "WAIT") {
      const goodLongTiming = isBull && rsi <= 48;
      const goodShortTiming = !isBull && rsi >= 52;
    ...
    // 🔥 ELITE ENTRY MET VERSOEPELDE CONFLUENCE (65 i.p.v. 75)
    const elite = confluence >= 65 && (sniper?.score || 0) >= 70 && c.tfStrength >= 1.2;
    if (state.phase === "READY" && elite && !ob.spoof && rr >= minRrFloor) {
      ...
      if (certaintyMode === "safe") {
        ...
        if (isBull) { if (rsi < 30) finalTp *= 1.10; else if (rsi < 45) finalTp *= 1.05; }
        if (!isBull) { if (rsi > 70) finalTp *= 0.90; else if (rsi > 55) finalTp *= 0.95; }
      }
      const position = {
