...
// ================= CANDLE FETCH (met retry + throttle) =================
...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  await Promise.all(candidates.map(async (c) => {
    ...
    try {
      ...
      const structureState = c5m.length ? getStructureState(c5m) : { trend: "UNKNOWN" };
      rsiMap[symbol] = {
        m15: rsiContextM15,
        h1: rsiContextH1,
