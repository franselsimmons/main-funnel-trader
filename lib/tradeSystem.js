// tradeSystem.js - Definitief met versoepelde RSI & fake breakout filters
// Fixes:
// 1. trendContinuationRSI: confluence >= 65 (ipv 70)
// 2. Short staat LOWER_1 toe (momentum continuation)
// 3. allowWithoutFakeBreakout: TREND + confluence >= 65 (geen sniper eis)
// 4. MID filter: confluence < 70 (ipv 80)
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // FIX 1: trendContinuationRSI met confluence >= 65 (was 70)
    ...
    if (!isBull) {
      // FIX 2: Short ook LOWER_1 toestaan (momentum continuation)
      const rsiOK =
    ...
    // MID filter: FIX 4 → confluence < 70 (was 80)
    ...
    // FAKE BREAKOUT: FIX 3 → allowWithoutFakeBreakout versoepeld
    const allowWithoutFakeBreakout = flow.type === "TREND" && confluence >= 65;
