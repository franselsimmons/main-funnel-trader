...
const STRATEGY_VERSION = "TS_V8_A_RR_HIGHER_NO_SCALE";
...
const GRADE_A_MIN_RR_FLOOR = 1.20;
...
const TRADE_SYSTEM_FILTERS = Object.freeze([
  ...
  },
  {
    phase: "ENTRY",
    filter: "A/B/GOD ONLY",
    rule: "GOD, A or B setup only. No C_ENTRY. No scaling.",
...
function getSniperAdjustedRR(sniper, baseRR) {
  ...
  if (score >= 90) return Math.max(1.15, baseRR - 0.05);
  if (score >= 80) return Math.max(1.18, baseRR);
  if (score >= 70) return Math.max(1.20, baseRR + 0.03);
...
function buildActionHints({ setupClassStats, loserProfile, winnerProfile, reasonTable, completed, avgR, winrateNum }) {
  ...
  if (completed < 10) {
    hints.push({
      priority: 1,
      target: "SAMPLE",
      change: "keep running until at least 20 closed trades",
      why: "sample too small for hard tuning"
    });
  ...
  if (a && a.exits >= 5 && a.avgR < 0 && parsePct(a.winrate) < 50) {
    hints.push({
      ...
      change: "raise A minimum confluence +2 and A sniper +2",
  ...
  if (a && a.exits >= 5 && a.avgR > 0 && parsePct(a.winrate) >= 45) {
    hints.push({
      priority: 2,
      target: "A_RR",
      change: "A RR increase is active in V8; keep collecting sample before further tightening",
  ...
  if (avgR < 0 && winrateNum >= 50) {
    hints.push({
      ...
      change: "increase TP distance or reduce SL distance slightly",
...
function buildOptimizerReport({ reasonTable, btcState, runId }) {
  ...
  return {
    ...
    closedTradeSample: closed.slice(-20)
  };
}
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    if (certaintyMode === "safe") {
      ...
      if (sniperScore >= 90) finalTp *= 1.00;
      else if (sniperScore >= 80) finalTp *= 1.00;
    ...
    const finalRisk = Math.abs(c.price - riskBase.sl);
    const finalReward = Math.abs(finalTp - c.price);
    const finalRR = finalRisk > 0 ? finalReward / finalRisk : rr;
