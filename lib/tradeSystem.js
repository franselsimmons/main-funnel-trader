// tradeSystem.js - Definitief met alle versoepelingen
// - isDeep niet meer nodig voor A/B/C entry
// - C entry fallback voor lage sniper scores
// - Momentum filter losser
// - LOWER_2 drempel -0.05
// - LIGHT mag entries doen
...
// DEBUG: bouw een WAIT en log meteen de reden
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // RSI context voor calculateConfluence
    ...
    if (ob.fetchFailed) {
      const waitPayload = buildWait(c, "ORDERBOOK_FETCH_FAILED", flow, sniper, confluence, 0, funding, ob, null, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (isBull) {
      ...
      if (!rsiOK) {
        const waitPayload = buildWait(c, "RSI_LONG_NO_EDGE", flow, sniper, confluence, rr, funding, ob, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
        actions.push(waitPayload);
        continue;
      }
    }
    ...
    if (!isBull) {
      ...
      if (!rsiOK) {
        const waitPayload = buildWait(c, "RSI_SHORT_NO_EDGE", flow, sniper, confluence, rr, funding, ob, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
        actions.push(waitPayload);
        continue;
      }
    }

    // Extra check: LOWER_2 / UPPER_2 (aangepast)
    if (isBull && rsiZone === "LOWER_2" && c.change1h > -0.05) {
      const waitPayload = buildWait(c, "RSI_NOT_DEEP_ENOUGH", flow, sniper, confluence, rr, funding, ob, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    if (!isBull && rsiZone === "UPPER_2" && c.change1h < 0.2) {
      const waitPayload = buildWait(c, "RSI_NOT_HIGH_ENOUGH", flow, sniper, confluence, rr, funding, ob, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (rsiZone === "MID" && !trendContinuationRSI && confluence < 70) {
      const waitPayload = buildWait(c, "RSI_MID_NO_EDGE", flow, sniper, confluence, rr, funding, ob, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    // Fake breakout (alleen blokkeren als geen trend)
    ...
    // 🔥 A, B en C entry (isDeep verwijderd uit A)
    const aSetupValid = stageOK && finalSniperOK && setupGrade.grade === "A" && !ob.spoof && rr >= minRrFloor;
    ...
    // C entry fallback (lage sniper maar nog steeds kwaliteit)
    const cEntry = confluence >= 65 && sniperScore >= 25 && rr >= 1.0 && flow.type !== "NEUTRAL";

    const godModeEntry = eliteEntry && sniperScore >= 85 && confluence >= 85 && rr >= 1.2;
    const shouldEnter = eliteEntry || bEntry || cEntry;
    const reasonEntry = godModeEntry ? "GOD_MODE" : eliteEntry ? "ELITE_ENTRY" : bEntry ? "B_ENTRY" : cEntry ? "C_ENTRY" : "NONE";

    console.log(`🔍 ${c.symbol} (${isDeep?"DEEP":"LIGHT"}): sniper=${sniperScore}, conf=${confluence}, rr=${rr.toFixed(2)}, grade=${setupGrade.grade}, elite=${eliteEntry}, b=${bEntry}, c=${cEntry}, godmode=${godModeEntry}, rsiZone=${rsiZone}, fakeBreakout=${isValidFakeBreakout}`);
    ...
    if (confluence < 55 && isDeep) {
      actions.push(buildWait(c, "LOW_CONFLUENCE", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, 55, null));
      continue;
    }
    const obAgainst = isObAgainstSide(ob, isBull);
    ...
    // LIGHT monitoring alleen als geen entry
    if (!isDeep && !shouldEnter) {
