...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    // =====================================================
    // MANAGE EXISTING POSITION FIRST (zelfde als eerder)
    // =====================================================
    if(prev){
      const pos = { ...prev };
    ...
    if(ob.bias === "NEUTRAL" && confluence < 50 && !isNeutralObEntryException({c, flow, sniper, confluence, rr, setupGrade, counterTrend: (btcState === "BULLISH" && !isBull) || (btcState === "BEARISH" && isBull)})){
    ...
    // ================= FASE LOGICA =================
    // 🔥 NIEUW: RSI ALIGNED (long RSI <=45, short RSI >=55)
    const rsiAligned =
      (isBull && rsi <= 45) ||
      (!isBull && rsi >= 55);
    ...
    const setupValid =
      ...
      !ob.spoof &&
      rr >= minRrFloor &&
      rsiAligned;   // 🔥 TOEGEVOEGD

    // === FASE 1: IDLE -> WAIT
    if(setupValid && state.phase === "IDLE"){
      state.phase = "WAIT";
      notifyState.set(key, state);
      const waitPayload = buildWait(c, "WAIT_FOR_RSI_PULLBACK", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    if(state.phase === "WAIT"){
      // 🔥 DREMPELS AANGEPAST: long ≤42, short ≥58
      const goodLongTiming = isBull && rsi <= 42;
      const goodShortTiming = !isBull && rsi >= 58;
      const timingOk = goodLongTiming || goodShortTiming;

      if(!timingOk){
        const waitPayload = buildWait(c, "WAIT_RSI_NOT_READY", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null);
        actions.push(waitPayload);
        continue;
      }
    ...
    const sniperStrong = (sniper?.score || 0) >= 70;   // 🔥 NIEUW

    if(
      state.phase === "READY" &&
      setupValid &&
      sniperStrong &&          // 🔥 TOEGEVOEGD
      !ob.spoof &&
      rr >= minRrFloor
    ){
      const reasonEntry = "PHASE_READY";

      // 🔥 TP AANPASSING (RSI based) - VERVANGEN
      ...
      if(certaintyMode === "safe"){
        ...
        if(isBull){
          ...
          else if(rsi < 40) finalTp *= 1.05;
        }
        if(!isBull){
          if(rsi > 70) finalTp *= 0.90;
          else if(rsi > 60) finalTp *= 0.95;
        }
      }
    ...
    if(state.phase === "WAIT"){
      ...
      const waitPayload = buildWait(c, "SETUP_NOT_READY", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null);
      actions.push(waitPayload);
    }
  }
