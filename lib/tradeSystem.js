...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // Confluence berekenen (voordat sniper wordt aangeroepen, want sniper gebruikt c.confluence)
    const confluence = calculateConfluence(c, ob, liquidity, funding, regimeForConfluence, hasLiquidationData ? liquidation : null, null);
    c.confluence = confluence;   // 🔥 koppel aan coin object zodat sniper het kan gebruiken
    ...
    // 👇 ADAPTIEVE RR op basis van sniper score
    ...
    // Elite entry: sniper valid + score >= 70 (RSI niet meer verplicht)
