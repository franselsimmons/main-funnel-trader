...
function getSetupGrade({
  ...
  const tfScore = Number(c?.tfScore || multiTFScore(c) || 0);
  ...
  if(tfScore >= 5) points += 2;
  else if(tfScore >= 2) points += 1;
  else if(tfScore <= -1) points -= 2;
...
function buildWait(c, reason, flow, sniper, confluence, rr, funding, ob, risk, setupGrade){
  return {
    ...
    tfAlignment: c?.tfContext?.alignment || "UNKNOWN"
  };
}
...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    const tfContext = buildTimeframeContext(c);
    c.tfContext = tfContext;
    c.tfScore = tfContext.score;
    c.atrPct15m = tfContext.atrPct15m;
    c.atrPct1h = tfContext.atrPct1h;
    c.atrPct4h = tfContext.atrPct4h;
    c.atrPct24h = tfContext.atrPct24h;
    ...
    if(
      (c.stage === "entry" && c.tfScore < 1) ||
      (c.stage === "almost" && c.tfScore < 2)
    ){
