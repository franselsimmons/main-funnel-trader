...
const MIN_DEPTH_USD_1P = 200000; // 200k
const MIN_RR_FLOOR = 1.1; // Minimale RR is nu 1.1
...
// ================= NEUTRAL OB TUNING (EXTREME WINRATE) =================
...
const BUILDUP_ELITE_MIN_TF = 3; // Keiharde eis: meerdere TF's moeten perfect in sync zijn
...
function getSetupGrade({
  ...
  // Verhoogde eisen voor de grades (winrate focus)
  if(points >= 10){ // was 9
    grade = "A";
    recommendedRisk = "normal";
  }else if(points >= 7){ // was 6
    grade = "B";
...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    // EXTREME WINRATE EIS: Timeframe sterkte MOET nu 2 zijn voor entry, niet 1.
    if(
    ...
    if(fakeBreakout && confluence < 80){ // was 78
    ...
    if(btcState === "BULLISH" && !isBull && c.moveScore < 80){ // was 70
    ...
    if(btcState === "BEARISH" && isBull && c.moveScore < 80){ // was 70
    ...
    // EXTREME WINRATE EIS: Confluence absolute bodem is 70.
    const minConf = 70;
    ...
    if(obAgainst && confluence < 82){ // was 75
    ...
    if(!hasLiquidationRoom && confluence < 82){ // was 75
    ...
    if((badSpread || badDepth) && confluence < 88){ // was 85
    ...
    if(Math.abs(fundingRate) > 0.015 && confluence < 88){ // was 85
    ...
    const almostOK =
      ...
      (
        (
          confluence >= 85 && // was 82
          sniper?.valid &&
          getSniperScore(sniper) >= 78 && // was 72
          c.tfStrength >= 2 && // was 1
          flow.type !== "NEUTRAL" &&
          ob.bias !== "NEUTRAL"
        ) ||
        neutralObAlmostException
    ...
    const sniperOK =
      (
        sniper?.valid &&
        getSniperScore(sniper) >= 75 // was 65
      ) ||
      confluence >= 88; // was 85
    ...
    const gradeOK =
      ...
      (
        setupGrade.grade === "B" &&
        !counterTrend &&
        confluence >= 80 && // was 74
        (
          ob.bias !== "NEUTRAL" ||
          neutralObEntryException
        ) &&
        getSniperScore(sniper) >= 75 && // was 68
        c.tfStrength >= 2 && // was 1
        c.stage !== "buildup"
      );
    ...
    if(counterTrend){
      ...
      if(
        setupGrade.grade !== "A" ||
        confluence < 88 || // was 85
        getSniperScore(sniper) < 85 || // was 80
        !obRequired
      ){
