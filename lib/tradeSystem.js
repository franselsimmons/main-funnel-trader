...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // ========== DIRECT ENTRY MODE (geen phase WAIT/READY meer) ==========
    // SetupValid: RSI is geen blocker meer (alleen bonus voor TP)
    const setupValid = stageOK && sniperOK && gradeOK && !ob.spoof && rr >= minRrFloor;
    
    // Elite entry drempels (lager om entries te krijgen)
    ...
    // 🔥 Flow NEUTRAL alleen blokkeren als confluence te laag
    ...
    if (hasLiquidationData) {
      // liquidation is altijd null, deze code wordt niet uitgevoerd
    ...
    const minConf = Math.max(50, minConfDynamic); // versoepeld
    ...
    if (eliteEntry) {
      ...
      if (certaintyMode === "safe") {
        ...
        // 🔥 RSI alleen voor TP tuning (bonus)
        if (isBull) {
