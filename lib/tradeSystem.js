...
const MAX_SPREAD_PCT = 0.0025; // 0.25%
const MIN_DEPTH_USD_1P = 200000; // 200k USD
const MIN_RR_FLOOR = 1.05;

// ================= DYNAMIC RR FLOOR =================
const GRADE_A_MIN_RR_FLOOR = 0.70;
const GRADE_B_MIN_RR_FLOOR = 0.80;
const GRADE_C_MIN_RR_FLOOR = 0.90;
...
export async function processTrades(
  ...
  const obMap = {};
  const fundingMap = {};
  const rsiMap = {}; // 🔥 1. Cache voor RSI context

  // 🔥 1. Preload orderbook, funding én RSI in parallel
  await Promise.all(
    candidates.map(async (c) => {
      ...
      // 🔥 RSI preload (1 enkele fetch per symbool per run)
      try {
        const candles = await fetchCandles(symbol + "USDT", "1h", 150);
        rsiMap[symbol] = getAdvancedRSIContext(candles);
      } catch {
        rsiMap[symbol] = null;
      }
    })
  ...
  for (const originalCoin of candidates) {
    ...
    try {
      liquidation = await getLiquidationZones(c.symbol + "USDT", c.price);
    } catch { liquidation = null; }
    ...
    // 🔥 2. Haal RSI context uit cache en geef mee aan calculateConfluence
    const rsiCtx = rsiMap[c.symbol];
    const confluence = calculateConfluence(
      ...
      rsiCtx   // 🔥 MEEST BELANGRIJKE FIX: RSI beïnvloedt nu confluence score
    );

    // Quality gate
    const qualityFails = [];
    if (ob.bias !== "NEUTRAL" && !isObWithSide(ob, isBull)) qualityFails.push("OB");
    if (flow.type === "NEUTRAL") qualityFails.push("FLOW");
    if (!sniper?.valid || getSniperScore(sniper) < 70) qualityFails.push("SNIPER");
    if (c.tfStrength < 1) qualityFails.push("TF");
    if (qualityFails.length >= 3) {
      const waitPayload = buildWait(c, "QUALITY_FAIL_" + qualityFails.join("_"), flow, sniper, confluence, rr, funding, ob, riskBase, { grade: "C", points: 0, recommendedRisk: "watch" });
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (prev) {
      ...
      if (hitTP || hitSL) {
        ...
        if (shouldLog) {
          await logTrade({
            symbol: c.symbol, side: c.side, entry: pos.entry, exit: c.price, sl: pos.sl, tp: pos.tp,
            result: hitTP ? "WIN" : "LOSS", reason, rr: pos.rr, grade: pos.grade || "N/A",
            gradePoints: pos.gradePoints || 0, recommendedRisk: pos.recommendedRisk || "N/A",
            confluence, score: c.moveScore, flow: flow.type, sniper: sniper?.type || "NONE",
            sniperScore: sniper?.score || 0, obBias: ob.bias, funding: funding.rate || 0,
            slSource: pos.slSource || "N/A", tpSource: pos.tpSource || "N/A",
            regime: regimeLevel, btcState
          });
          await logAction(exitPayload, regimeLevel, btcState, true);
        }
        if (notify && !state.exit) {
          await sendExit({
            symbol: c.symbol, side: c.side, reason, rr: Number(pos.rr || 0).toFixed(2),
            grade: pos.grade || "N/A", recommendedRisk: pos.recommendedRisk || "N/A",
            slSource: pos.slSource || "N/A", tpSource: pos.tpSource || "N/A"
          });
          state.exit = true;
        }
      ...
      if (notify && !state.hold) {
        await sendHold({
          symbol: c.symbol, side: c.side, flow: flow.type, score: c.moveScore,
          rr: Number(pos.rr || 0).toFixed(2), grade: pos.grade || "N/A",
          recommendedRisk: pos.recommendedRisk || "N/A",
          slSource: pos.slSource || "N/A", tpSource: pos.tpSource || "N/A"
        });
        state.hold = true;
    ...
    if (hasAnyOpenPositionForSymbol(c.symbol)) {
      ...
      const waitPayload = buildWait(c, `SYMBOL_ALREADY_OPEN_${openSide}`, flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (Date.now() < symbolCooldownUntil) {
      const waitPayload = buildWait(c, "SYMBOL_COOLDOWN", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (processingLocks.has(symbolLockKey)) {
      const waitPayload = buildWait(c, "DUPLICATE_PROCESSING_LOCK", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (Date.now() < cooldownUntil) {
      const waitPayload = buildWait(c, "COOLDOWN", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (memory.has(oppositeKey)) {
      const waitPayload = buildWait(c, "OPPOSITE_POSITION_OPEN", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (vol === "LOW") {
      const waitPayload = buildWait(c, "LOW_VOL", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (flow.type === "NEUTRAL") {
      const waitPayload = buildWait(c, "NO_FLOW", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (rr < minRrFloor && confluence < 80) {
      const waitPayload = buildWait(c, "LOW_RR", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if ((c.stage === "entry" && c.tfStrength < 1) ||
        (c.stage === "almost" && c.tfStrength < 1) ||
        (c.stage === "buildup" && c.tfStrength < BUILDUP_ELITE_MIN_TF)) {
      const waitPayload = buildWait(c, "ENTRY_FILTERED", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (fakeBreakout && confluence < 78) {
      const waitPayload = buildWait(c, "FAKE_BREAKOUT", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (btcState === "BULLISH" && !isBull && c.moveScore < 70) {
      const waitPayload = buildWait(c, "BTC_BULL_BLOCK_SHORT", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (btcState === "BEARISH" && isBull && c.moveScore < 70) {
      const waitPayload = buildWait(c, "BTC_BEAR_BLOCK_LONG", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }

    // 🔥 Dynamische confluence drempel (aangepast met RSI bonuses)
    let minConf = 49;
    if (ob.bias === "NEUTRAL") minConf += 2;
    if (flow.type === "BUILDING") minConf += 2;
    if (c.tfStrength < 2) minConf += 1;
    if (vol === "HIGH") minConf += 1;
    if (flow.type === "TREND" && isObWithSide(ob, isBull) && sniper?.valid && getSniperScore(sniper) >= 75) minConf -= 10;

    // 🔥 RSI verlaagt threshold (optioneel maar sterk aangeraden)
    if (rsiCtx?.valid) {
      const isType1 = isType1RSIEntry(rsiCtx, c.side);
      if (isType1) minConf -= 6;
      // later kan hier divergence check komen: if (rsiCtx.divergence) minConf -= 3;
    }

    if (confluence < minConf) {
      const waitPayload = buildWait(c, "LOW_CONFLUENCE", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (obAgainst && confluence < 75) {
      const waitPayload = buildWait(c, "OB_AGAINST", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (!hasLiquidationRoom && confluence < 75) {
      const waitPayload = buildWait(c, "NO_LIQUIDATION_ROOM", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if ((badSpread || badDepth) && confluence < 85) {
      const waitPayload = buildWait(c, "BAD_MARKET_QUALITY", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (ob.bias === "NEUTRAL" && !neutralObEntryException && !neutralObAlmostException && c.stage !== "buildup") {
      const waitPayload = buildWait(c, "OB_NEUTRAL_LOW_CONF", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (Math.abs(fundingRate) > 0.015 && confluence < 85) {
      const waitPayload = buildWait(c, "EXTREME_FUNDING", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (isBull && fundingRate > 0.012 && confluence < 85) {
      const waitPayload = buildWait(c, "BULL_CROWDED_FUNDING", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    if (!isBull && fundingRate < -0.012 && confluence < 85) {
      const waitPayload = buildWait(c, "BEAR_CROWDED_FUNDING", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if (counterTrend) {
      ...
      if (setupGrade.grade !== "A" || confluence < 85 || getSniperScore(sniper) < 80 || !obRequired) {
        const waitPayload = buildWait(c, "COUNTERTREND_NOT_ELITE", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
        await logAction(waitPayload, regimeLevel, btcState, shouldLog);
        actions.push(waitPayload);
        continue;
      }
    ...
    // 🔥 3. Gebruik gecachte RSI context – géén extra fetch meer
    const rsiOk = rsiCtx ? isType1RSIEntry(rsiCtx, c.side) : false;

    if (stageOK && sniperOK && gradeOK && !ob.spoof && rr >= minRrFloor && rsiOk) {
      ...
      try {
        ...
        if (notify && !state.entry) {
          await sendEntry({
            symbol: c.symbol, side: c.side, entry: position.entry, sl: position.sl, tp: position.tp,
            rr: Number(position.rr).toFixed(2), sniper: reasonEntry, grade: position.grade,
            gradePoints: position.gradePoints, recommendedRisk: position.recommendedRisk,
