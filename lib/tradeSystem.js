...
export async function processTrades(input, options = {}) {
  ...
  // ================= PREFILTER + DYNAMISCHE CAP =================
  let candidates;
  if (Array.isArray(input)) {
    // Alleen coins met score >= 50 en entry/almost stage
    ...
    // Dynamische cap: max 5 coins voor diepe analyse (harde limiet om API calls te beperken)
    const MAX_DEEP_ANALYSIS = 5;
    candidates = preFiltered.slice(0, MAX_DEEP_ANALYSIS);

    if (candidates.length === 0) {
      return { actions: [], message: "Geen coins na prefilter" };
    }
  } else {
    // Oude scan-object fallback: max 8 coins
  ...
  // ========== DATA FETCH (GEBATCHED, MAX 3 CONCURRENT) ==========
  const dataMap = new Map();

  // Helper om data voor 1 coin te fetchen
  const fetchCoinData = async (c) => {
    ...
    // --- Candles (alleen 15m + 1h; 4h alleen als tfStrength hoog genoeg) ---
