...
export async function processTrades(input, options = {}) {
  ...
  if (Array.isArray(input)) {
    ...
    // 🔥 ADAPTIVE PIPELINE: split in DEEP (max 5) en LIGHT (max 20)
    const MAX_DEEP = 5;
    const MAX_LIGHT = 20;
    ...
    // Markeer analyse type
    for (const c of candidates) {
      c.analysisType = deepCandidates.includes(c) ? "DEEP" : "LIGHT";
    }

    if (candidates.length === 0) {
      // Geen prefiltered coins → fallback WAIT op basis van raw input
    ...
    candidates = candidatesRaw
      ...
      .slice(0, 8);
    if (candidates.length === 0) {
      return { actions: [], candidatesCount: 0 };
    }
    // Markeer als DEEP (oude fallback)
  ...
  // ========== DATA FETCH (alleen voor candidates) ==========
  ...
  for (const originalCoin of candidates) {
    ...
    // 🔥 Setup valid alleen voor DEEP coins (LIGHT worden ALTIJD WAIT)
    const setupValid = isDeep
      ? (stageOK && finalSniperOK && gradeOK && !ob.spoof && rr >= minRrFloor)
      : false;

    // 🔥 VERZOEPELDE ELITE ENTRY (meer kans op entry, maar nog steeds kwaliteit)
    const eliteEntry = setupValid && sniper.valid && sniper.score >= 75 && (rsiSignal.valid || confluence >= 80);

    // ========== POSITION MANAGEMENT (alleen voor bestaande posities) ==========
    if (prev) {
      const pos = { ...prev };
    ...
    if ((c.stage === "entry" && c.tfStrength < 1) || (c.stage === "almost" && c.tfStrength < 1) || (c.stage === "buildup" && c.tfStrength < BUILDUP_ELITE_MIN_TF)) {
      if (isDeep) {
        ...
        // LIGHT: we moeten een WAIT toevoegen om inzicht te geven
    ...
    if (eliteEntry) {
      const reasonEntry = "DIRECT_ENTRY";
    ...
    // Als geen van de bovenstaande filters een WAIT heeft geproduceerd, voeg dan een generieke WAIT toe.
    // Dit zorgt dat LIGHT coins altijd zichtbaar worden.
    if (!isDeep) {
  ...
  // 🔥 FALLBACK: als er GEEN acties zijn gegenereerd (alleen diep maar alles gefilterd)
