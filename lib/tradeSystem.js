...
const lastSignalMap = new Map();    // 🔥 extra harde cooldown per symbool (1 uur)
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // ========== DIRECT ENTRY ==========
    if (shouldEnter && !memory.has(key) && !hasAnyOpenPositionForSymbol(c.symbol)) {
      ...
      try {
        ...
        symbolCooldownMap.set(c.symbol, Date.now() + SYMBOL_REENTRY_COOLDOWN_MS);
        cooldownMap.set(key, Date.now() + COOLDOWN_MS);
        lastSignalMap.set(c.symbol, Date.now() + SYMBOL_REENTRY_COOLDOWN_MS); // extra harde 1 uur
