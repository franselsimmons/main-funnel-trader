...
function getRsiEdgeMeta({ isBull, rsiSignal, rsiZone }) {
  ...
  const sniperDiscount =
    rank >= 3 ? 8
    : rank >= 2 ? 5
    : rank >= 1 ? 3
    : rank === 0 && continuationOk ? 2
    : 0;
...
async function getSafeConfluence({
  ...
  const externalScore = Number(rawScore || 0);
  const fallbackScore = Number(fallback || 0);
  ...
  c._debugConfluenceBlendUsed = fallbackScore > externalScore;
