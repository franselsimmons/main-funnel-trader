...
// Bootstrap voor huidige week.
// Moet lager zijn dan MICRO_ROTATION_MIN_COMPLETED, anders duurt bootstrap te lang.
const MICRO_ROTATION_BOOTSTRAP_MIN_COMPLETED = Number(
  process.env.MICRO_ROTATION_BOOTSTRAP_MIN_COMPLETED || 3
);
...
const MAX_DURABLE_NOTIFY_ROWS = Number(process.env.TS_MAX_DURABLE_NOTIFY_ROWS || 2000);
const MAX_DURABLE_COOLDOWN_ROWS = Number(process.env.TS_MAX_DURABLE_COOLDOWN_ROWS || 2000);
const MAX_DURABLE_LAST_SIGNAL_ROWS = Number(process.env.TS_MAX_DURABLE_LAST_SIGNAL_ROWS || 1500);
const MAX_DURABLE_POSITION_PATH_ROWS = Number(process.env.TS_MAX_DURABLE_POSITION_PATH_ROWS || 12);
...
function compactPositionForStore(pos) {
  ...
  if (gate) {
    out.rotationGate = compactGateResult(gate);
    out.rotation = compactGateResult(gate);
  }
...
async function saveDurableRuntimeState(_runtimeState = runtimeState, options = {}) {
  ...
  const writeState = async ({
    ...
    const coreBytes = await redisSetJson(RUNTIME_CORE_KEY, core);
...
function buildAnalyzerMicroIdentities(row = {}) {
  const exact = buildAnalyzerMicroIdentity(row);
  if (!exact?.microFamilyId) return [];

  const side = analyzerSide(row);
  const parentFamilyId = exact.parentFamilyId;

  const setupClass = analyzerSetupClass(row);
  const stage = analyzerNormalizeStage(row);
  const rsi = analyzerNormalizeRsi(row);
  const ob = analyzerNormalizeOb(row);
  const flow = analyzerNormalizeFlow(row);
  const btc = analyzerNormalizeBtc(row);
  const scoreBucket = analyzerBucketScore(row.score ?? row.moveScore ?? row.entryScore, "SCORE");
  ...
  return Array.from(new Set(definitions))
    .map(definition => {
      ...
      return {
        microFamilyId: `MICRO_${side}_${parentFamilyId}_${hash}`,
        parentFamilyId,
        definition,
        definitionParts: definition.split(" | "),
        spreadBps: exact.spreadBps
      };
    });
}
...
function getRowMicroFamilyIds(row) {
  ...
  const computedIds = uniqIds([
    analyzerRow?.analyzerMicroFamilyId,
    analyzerRow?.analyzerMicroFamilyIds,
...
function ensureActiveMicroRotation(now = Date.now()) {
  ...
  const existing = auditState.microLearning.activeRotation;
  const existingIds = extractRotationMicroFamilyIds(existing);
  ...
  const refreshed = auditState.microLearning.activeRotation;
  const refreshedIds = extractRotationMicroFamilyIds(refreshed);
  ...
  if (currentWeekRotation?.allowedMicroFamilyIds?.length) {
    auditState.microLearning.activeRotation = currentWeekRotation;
    auditState.microLearning.updatedAt = now;
...
function sendAnalysisActionsNonBlocking(actions, context) {
...
async function loadWeeklyRotationForGate(options = {}) {
  ...
  if (explicit) {
    const explicitIds = extractRotationMicroFamilyIds(explicit);
  ...
  try {
    ...
    if (
      status.gateEnabled === false ||
      status.enabled === false ||
      status.usable === false
    ) {
      console.warn("TS_EXTERNAL_WEEKLY_ROTATION_IGNORED:", JSON.stringify({
        reason: "DISABLED_OR_UNUSABLE",
        enabled: status.enabled ?? null,
        gateEnabled: status.gateEnabled ?? null,
        usable: status.usable ?? null,
        ids: ids.length,
        rotationId: status.rotationId || status.activeRotationId || null
      }));
...
// ================= HELPER FUNCTIONS VOOR ROTATION SELECTOR =================
