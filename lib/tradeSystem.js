...
async function saveDurableRuntimeState(runtimeState = {}, options = {}) {
  ...
  return withDurableRuntimeLock({
    strategyVersion,
    runId,
    task: async ({ lockKey, lockOwner }) => {
      ...
      console.log("TRADE SYSTEM DURABLE SAVE SIZE:", JSON.stringify({
        strategyVersion,
        runId,
        lockKey,
        lockOwner,
      ...
      await redisSetJsonSafe(`${strategyVersion}:runtime:core`, core);
      ...
      if (shadowRows.length > 0) {
        await redisSetJsonSafe(`${strategyVersion}:runtime:shadowRows`, {
          strategyVersion,
          runId,
      ...
      if (featureRows.length > 0) {
        await redisSetJsonSafe(`${strategyVersion}:runtime:featureRows`, {
          strategyVersion,
          runId,
          savedAt: core.savedAt,
          rows: featureRows,
      ...
      return {
        ok: true,
        key: `${strategyVersion}:runtime:core`,
  ...
  try {
    ...
    try {
      ...
      if (shouldSaveRuntime) {
        const saveResult = await saveDurableRuntimeState(runtimeState, { strategyVersion: STRATEGY_VERSION, runId });
        if (!saveResult.ok) {
