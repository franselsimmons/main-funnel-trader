...
// ================= NEUTRAL OB TUNING =================
// Kleine versoepeling zodat niet alles stukloopt op OB_NEUTRAL_LOW_CONF,
// maar nog steeds alleen sterke setups doorlaat.
const NEUTRAL_OB_ENTRY_A_MIN_CONF = 74;
const NEUTRAL_OB_ENTRY_A_MIN_SNIPER = 72;

const NEUTRAL_OB_ENTRY_B_MIN_CONF = 78;
...
const NEUTRAL_OB_ALMOST_MIN_RR = 1.0;
  ...
  for(const originalCoin of candidates){
    ...
    if(prev){
      ...
      if(hitPartial && pos.sizeLeft === 1){
        ...
        await logAction(partialPayload, regime.level, btcState, shouldLog);
      ...
      if(hitTP || hitSL){
        ...
        if(shouldLog){
          ...
          await logAction(exitPayload, regime.level, btcState, true);
        }
      ...
      await logAction(runningPayload, regime.level, btcState, shouldLog);
    ...
    if(getOpenTradeCount() >= MAX_OPEN_TRADES){
      ...
      await logAction(waitPayload, regime.level, btcState, shouldLog);
    ...
    // ================= NEUTRAL OB GUARD =================
    // Voorheen blokkeerde dit bijna alles.
    // Nu laten we alleen sterke entry/almost uitzonderingen door.
    if(
      ob.bias === "NEUTRAL" &&
    ...
    if(
      ...
      if(notify){
        ...
        try{
          ...
          await logAction(entryPayload, regime.level, btcState, shouldLog);
