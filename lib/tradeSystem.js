...
// Nieuwe rotation gate – namespace import voorkomt named-export crashes
import * as microRotationGate from "./tradesystem/microRotationGate.js";

const importedAttachMicroRotationKeys =
  microRotationGate.attachMicroRotationKeys ||
  microRotationGate.default?.attachMicroRotationKeys ||
  null;

const importedCheckTradeSignalAgainstRotation =
  microRotationGate.checkTradeSignalAgainstRotation ||
  microRotationGate.default?.checkTradeSignalAgainstRotation ||
  (typeof microRotationGate.default === "function" ? microRotationGate.default : null);

if (typeof importedCheckTradeSignalAgainstRotation !== "function") {
  throw new Error(
    `microRotationGate_missing_check_export exports=${Object.keys(microRotationGate || {}).join(",")}`
  );
}

function attachMicroRotationKeys(signal = {}, options = {}) {
  if (typeof importedAttachMicroRotationKeys === "function") {
    return importedAttachMicroRotationKeys(signal, options);
  }
  ...
  const tradeSide =
    ["bull", "buy", "long"].includes(rawSide)
      ? "LONG"
      : ["bear", "sell", "short"].includes(rawSide)
        ? "SHORT"
        : String(signal.tradeSide || signal.rotationSide || "UNKNOWN").toUpperCase();
  ...
  return {
    ...
    microFamilyId: signal.microFamilyId || primaryMicroFamilyId,
    rotationMicroFamilyId: signal.rotationMicroFamilyId || primaryMicroFamilyId,
    analyzerMicroFamilyId: signal.analyzerMicroFamilyId || primaryMicroFamilyId,

    microFamilyIds: Array.isArray(signal.microFamilyIds) && signal.microFamilyIds.length
      ? signal.microFamilyIds
      : microFamilyIds,

    microFamilies: Array.isArray(signal.microFamilies) && signal.microFamilies.length
      ? signal.microFamilies
      : microFamilyIds,
    ...
    rotationCandidate: {
      ...
      weekKey: options.weekKey || signal.weekKey || null
    }
  };
}

async function checkTradeSignalAgainstRotation(signal = {}, options = {}) {
  return importedCheckTradeSignalAgainstRotation(signal, options);
}
// Import for analyze family classification - use namespace import to avoid missing export errors
import * as familyMicroAnalyzer from "./familyMicroAnalyzer.js";

// ================= STRATEGY VERSION =================
const STRATEGY_VERSION = "TS_V12_8_WEEKLY_MICRO_ROTATION_CLEAN";

// Moet boven createRuntimeState/createAuditState staan.
const MICRO_FAMILY_SCHEMA_VERSION = "MF_V4_ANALYZE";
...
const ENABLE_EXTERNAL_WEEKLY_ROTATION =
  String(process.env.TS_ENABLE_EXTERNAL_WEEKLY_ROTATION ?? "true").toLowerCase() !== "false";
...
const MICRO_ROTATION_TOP_N_PER_SIDE = Number(
  process.env.MICRO_ROTATION_TOP_N_PER_SIDE || (DISCOVERY_MODE ? 10 : 5)
);

// Niet te laag zetten. Winrate met 3-5 trades is ruis.
const MICRO_ROTATION_MIN_COMPLETED = Number(
  process.env.MICRO_ROTATION_MIN_COMPLETED || (DISCOVERY_MODE ? 3 : 10)
);
...
const MICRO_ROTATION_BOOTSTRAP_MIN_COMPLETED = Number(
  process.env.MICRO_ROTATION_BOOTSTRAP_MIN_COMPLETED || (DISCOVERY_MODE ? 3 : 10)
);
...
const MICRO_ROTATION_PARENT_FALLBACK = false; // changed to false
...
const MICRO_ROTATION_USE_CURRENT_WEEK_BOOTSTRAP =
  String(
    process.env.TS_MICRO_ROTATION_USE_CURRENT_WEEK_BOOTSTRAP ??
    process.env.MICRO_ROTATION_USE_CURRENT_WEEK_BOOTSTRAP ??
    "false"
  ).toLowerCase() === "true";
...
// ================= NIEUWE MICRO-LEARNING RUNTIME BACKFILL CONSTANTS =================
const MICRO_LEARNING_RUNTIME_BACKFILL_ENABLED =
  process.env.MICRO_LEARNING_RUNTIME_BACKFILL_ENABLED !== "false";

const MICRO_LEARNING_RUNTIME_BACKFILL_MAX_CLOSED = Number(
  process.env.MICRO_LEARNING_RUNTIME_BACKFILL_MAX_CLOSED || 1500
);

const MICRO_LEARNING_RUNTIME_BACKFILL_MAX_SHADOW = Number(
  process.env.MICRO_LEARNING_RUNTIME_BACKFILL_MAX_SHADOW || 1000
);

const MICRO_LEARNING_RUNTIME_BACKFILL_MIN_INTERVAL_MS = Number(
  process.env.MICRO_LEARNING_RUNTIME_BACKFILL_MIN_INTERVAL_MS || 5 * 60 * 1000
);

const MICRO_ROTATION_HEALTH_TOP_FAMILIES = Number(
  process.env.MICRO_ROTATION_HEALTH_TOP_FAMILIES || 8
);
...
const MAX_CLOSED_TRADE_AUDIT_ROWS = Number(
  process.env.TS_MAX_CLOSED_TRADE_AUDIT_ROWS || 650
);
const MAX_FEATURE_STORE_ROWS = ENABLE_RAW_FEATURE_ROWS ? 1_000 : 0;
const MAX_SHADOW_OUTCOME_ROWS = Number(
  process.env.TS_MAX_SHADOW_OUTCOME_ROWS || MAX_OPEN_SHADOW_OUTCOME_ROWS || 1500
);
...
const MAX_OPEN_POSITIONS_TOTAL = Number(
  process.env.TS_MAX_OPEN_POSITIONS_TOTAL || (DISCOVERY_MODE ? 80 : 30)
);

const MAX_OPEN_POSITIONS_SAME_SIDE = Number(
  process.env.TS_MAX_OPEN_POSITIONS_SAME_SIDE || (DISCOVERY_MODE ? 45 : 15)
);

const MAX_COUNTER_BTC_OPEN_POSITIONS = Number(
  process.env.TS_MAX_COUNTER_BTC_OPEN_POSITIONS || (DISCOVERY_MODE ? 20 : 5)
);
...
function trimAuditArrays() {
  ...
  auditState.microLearning = pruneMicroLearningRuntimeState(
    auditState.microLearning
  );
}
...
const RUNTIME_CORE_TARGET_BYTES = Number(process.env.TS_RUNTIME_CORE_TARGET_BYTES || 5_500_000);
...
function compactActiveRotationForStore(rotation = {}, mode = "compact") {
  ...
  const allowedMicroFamilyIds = uniqRotationMicroIds([
...
function compactMicroLearningForStore(value = {}, mode = "compact") {
  ...
  return {
    ...
    schemaVersion: state.schemaVersion || MICRO_FAMILY_SCHEMA_VERSION,
...
function compactPositionForStore(pos) {
  ...
  const out = {
    ...
    analyzeFamilyId:
      normalizeAnalyzeFamilyId(
        r.analyzeFamilyId ||
          r.analysisFamilyId ||
          r.parentFamilyId ||
          r.analyzerParentFamilyId ||
          r.familyId ||
          extractParentFamilyIdFromMicroId(microFamilyIds[0])
      ) || null,
    ...
    familyIds: normalizeAnalyzeFamilyId(
      r.analyzeFamilyId ||
        r.analysisFamilyId ||
        r.parentFamilyId ||
        r.analyzerParentFamilyId ||
    ...
    families: normalizeAnalyzeFamilyId(
      r.analyzeFamilyId ||
        r.analysisFamilyId ||
        r.parentFamilyId ||
        r.analyzerParentFamilyId ||
...
function compactRuntimeCorePayload(core = {}, mode = "compact") {
  const c = durableObject(core);
  const audit = durableObject(c.auditCounters);

  const memoryLimit = mode === "ultra"
    ? Math.floor(MAX_DURABLE_MEMORY_ROWS / 3)
    : MAX_DURABLE_MEMORY_ROWS;

  const notifyLimit = mode === "ultra"
    ? Math.floor(MAX_DURABLE_NOTIFY_ROWS / 3)
    : MAX_DURABLE_NOTIFY_ROWS;

  const cooldownLimit = mode === "ultra"
    ? Math.floor(MAX_DURABLE_COOLDOWN_ROWS / 3)
    : MAX_DURABLE_COOLDOWN_ROWS;

  const lastSignalLimit = mode === "ultra"
    ? Math.floor(MAX_DURABLE_LAST_SIGNAL_ROWS / 3)
    : MAX_DURABLE_LAST_SIGNAL_ROWS;

  const memoryRows = durableTail(c.memory, memoryLimit)
    .map(row => {
  ...
  return {
    ...
    notifyState: durableTail(c.notifyState, notifyLimit),
    cooldownMap: durableTail(c.cooldownMap, cooldownLimit),
    symbolCooldownMap: durableTail(c.symbolCooldownMap, cooldownLimit),
    lastSignalMap: durableTail(c.lastSignalMap, lastSignalLimit),

    auditCounters: {
      strategyVersion: STRATEGY_VERSION,
      startedAt: audit.startedAt || Date.now(),
      runs: Number(audit.runs || 0),

      entries: Number(audit.entries || 0),
      exits: Number(audit.exits || 0),
      wins: Number(audit.wins || 0),
      losses: Number(audit.losses || 0),

      rTotal: Number(Number(audit.rTotal || 0).toFixed(4)),
      pnlPctTotal: Number(Number(audit.pnlPctTotal || 0).toFixed(4)),

      entryReasonCounts: compactCounterObject(audit.entryReasonCounts || {}),
      entrySetupClassCounts: compactCounterObject(audit.entrySetupClassCounts || {}),
      exitReasonCounts: compactCounterObject(audit.exitReasonCounts || {}),

      lastSnapshotAt: Number(audit.lastSnapshotAt || 0),

      microLearning: compactMicroLearningForStore(
        audit.microLearning,
        mode
      )
    },
    ...
    coreStats: {
      memoryRows: memoryRows.length,
      memoryTotal: Number(c.coreStats?.memoryTotal || memoryRows.length),
      notifyRows: durableArray(c.notifyState).length,
      cooldownRows: durableArray(c.cooldownMap).length,
      symbolCooldownRows: durableArray(c.symbolCooldownMap).length,
      lastSignalRows: durableArray(c.lastSignalMap).length
    }
  };
}
...
function compactShadowOutcomeForStore(row = {}) {
  const r = durableObject(row);
  ...
  return {
    ...
    id: r.id,
    shadowDedupeKey: r.shadowDedupeKey,
    ...
    exitR: Number.isFinite(Number(r.exitR)) ? Number(r.exitR) : null,
    ...
    hitTP: Boolean(r.hitTP),
    hitSL: Boolean(r.hitSL),

    mfeR: Number(r.mfeR || 0),
    maeR: Number(r.maeR || 0),
    maxPnlPct: Number(r.maxPnlPct || 0),
    minPnlPct: Number(r.minPnlPct || 0),

    microOutcomeRecorded: Boolean(r.microOutcomeRecorded)
  };
}
    ...
    const shadowOutcomes =
      ENABLE_SHADOW_OUTCOMES
        ? durableTail(auditState.shadowOutcomes, MAX_SHADOW_OUTCOME_ROWS)
            .map(compactShadowOutcomeForStore)
        : [];
...
// ================= LOGGING (gefilterd) =================
const TS_DEBUG_LOGS = process.env.TS_DEBUG_LOGS === "true";
const TS_LOG_WAITS = process.env.TS_LOG_WAITS === "true";

function logImportant(tag, payload = {}) {
...
async function logAction(actionPayload, regimeLevel, btcState, shouldLog) {
  ...
  const action = String(actionPayload.action || "").toUpperCase();

  // Geen WAIT/HOLD spam, tenzij expliciet debug.
  if (!["ENTRY", "EXIT"].includes(action) && !TS_LOG_WAITS) {
    return;
  }
...
function collectScannerFunnelCandidates(funnelSide = {}) {
  const stages = [
    "god",
    "a",
    "b",
...
function recordScanObservation(row) {
  ...
  // Alleen observation tellen.
  // Geen shadow outcome maken vanuit SCAN_OBSERVATION,
  // want setupClass is hier nog niet betrouwbaar.
...
function buildOutcomeRowFromClosedTrade(t) {
  const row = {
    ...
    analyzeFamilyId: t.analyzeFamilyId || t.parentFamilyId || null,
    analysisFamilyId: t.analysisFamilyId || t.analyzeFamilyId || t.parentFamilyId || null,
    parentFamilyId: t.parentFamilyId || t.analyzeFamilyId || null,
    analyzerParentFamilyId: t.analyzerParentFamilyId || t.parentFamilyId || t.analyzeFamilyId || null,

    rotationMicroFamilyId: t.rotationMicroFamilyId || t.microFamilyId || t.analyzerMicroFamilyId || null,
    analyzerMicroFamilyId: t.analyzerMicroFamilyId || t.microFamilyId || null,
...
// ================= NIEUWE CONSTANTEN VOOR DISCOVERY LABELS =================
const DISCOVERY_GOD_LABEL_MIN_RR = Number(process.env.TS_DISCOVERY_GOD_LABEL_MIN_RR || 1.20);
const DISCOVERY_GOD_LABEL_MIN_SNIPER = Number(process.env.TS_DISCOVERY_GOD_LABEL_MIN_SNIPER || 85);
const DISCOVERY_GOD_LABEL_MIN_CONFLUENCE = Number(process.env.TS_DISCOVERY_GOD_LABEL_MIN_CONFLUENCE || 85);

const DISCOVERY_A_LABEL_MIN_RR = Number(process.env.TS_DISCOVERY_A_LABEL_MIN_RR || 0.80);
const DISCOVERY_A_LABEL_MIN_SNIPER = Number(process.env.TS_DISCOVERY_A_LABEL_MIN_SNIPER || 65);
const DISCOVERY_A_LABEL_MIN_CONFLUENCE = Number(process.env.TS_DISCOVERY_A_LABEL_MIN_CONFLUENCE || 65);
...
function inferSetupClass({
  ...
  if (DISCOVERY_MODE) {
    ...
    if (
      rr >= DISCOVERY_GOD_LABEL_MIN_RR &&
      sniperScore >= DISCOVERY_GOD_LABEL_MIN_SNIPER &&
      confluence >= DISCOVERY_GOD_LABEL_MIN_CONFLUENCE
    ) {
      return {
        setupClass: "GOD",
    ...
    if (
      rr >= DISCOVERY_A_LABEL_MIN_RR &&
      sniperScore >= DISCOVERY_A_LABEL_MIN_SNIPER &&
      confluence >= DISCOVERY_A_LABEL_MIN_CONFLUENCE
    ) {
      return {
        setupClass: "A",
        entryReason: "A_ENTRY",
    ...
    if (ENABLE_B_ENTRIES && rr >= MIN_PRE_TP_GEOMETRY_RR) {
      return {
        setupClass: "B",
        entryReason: "B_ENTRY",
...
function recordEntry(entryPayload, position) {
  ...
  auditState.recentEntries.push({
    ...
    analyzeFamilyId: position.analyzeFamilyId || position.parentFamilyId || null,
    analysisFamilyId: position.analysisFamilyId || position.analyzeFamilyId || position.parentFamilyId || null,
    parentFamilyId: position.parentFamilyId || position.analyzeFamilyId || null,
    analyzerParentFamilyId: position.analyzerParentFamilyId || position.parentFamilyId || position.analyzeFamilyId || null,

    familyId: position.familyId || position.analyzeFamilyId || position.parentFamilyId || null,
    familyIds: Array.isArray(position.familyIds) ? position.familyIds : [],

    microFamilyId: position.microFamilyId || null,
    rotationMicroFamilyId: position.rotationMicroFamilyId || position.microFamilyId || null,
    analyzerMicroFamilyId: position.analyzerMicroFamilyId || position.microFamilyId || null,
...
function recordExit(exitPayload, pos, exitPrice, isBull) {
  ...
  const exitAnalyzeFamilyId = normalizeAnalyzeFamilyId(
    pos.analyzeFamilyId ||
      pos.analysisFamilyId ||
      pos.parentFamilyId ||
      pos.analyzerParentFamilyId ||
      pos.familyId ||
      extractParentFamilyIdFromMicroId(pos.microFamilyId)
  );

  const exitMicroFamilyId =
    normalizeMicroFamilyId(pos.microFamilyId || pos.analyzerMicroFamilyId || pos.rotationMicroFamilyId) ||
    buildCoreMicroIdFromAnalyzeFamilyId(exitAnalyzeFamilyId);

  Object.assign(exitPayload, {
    analyzeFamilyId: exitAnalyzeFamilyId,
    analysisFamilyId: exitAnalyzeFamilyId,
    parentFamilyId: exitAnalyzeFamilyId,
    analyzerParentFamilyId: exitAnalyzeFamilyId,
    familyId: exitAnalyzeFamilyId,
    familyIds: exitAnalyzeFamilyId ? [exitAnalyzeFamilyId] : [],

    microFamilyId: exitMicroFamilyId,
    rotationMicroFamilyId: exitMicroFamilyId,
    analyzerMicroFamilyId: exitMicroFamilyId,
    microFamilyIds: exitMicroFamilyId ? [exitMicroFamilyId] : [],
  });
  ...
  const canonicalClosedTrade = enforceCanonicalCoreMicroIdentity(closedTrade);

  auditState.closedTrades.push(canonicalClosedTrade);

  recordMicroOutcome(canonicalClosedTrade, "REAL");
...
function getPreviousIsoWeekKey(ts = Date.now()) {
  const date = new Date(ts);
  date.setUTCDate(date.getUTCDate() - 7);
  return getIsoWeekKey(date.getTime());
}

function getMicroOutcomeWeekKey(row = {}, fallbackTs = Date.now()) {
  const ts = getClosedTradeMicroTs(row, fallbackTs);
  return getIsoWeekKey(ts);
}

function isPreviousCompletedWeekRow(row, now = Date.now()) {
  return getMicroOutcomeWeekKey(row, now) === getPreviousIsoWeekKey(now);
}
  ...
  return {
    ...
    backfill: {
      runtime: {
        ...
        lastRowsAccepted: 0,
        lastFamilies: 0,
        lastEligibleFamilies: 0,
        lastActiveIds: 0,
        lastReason: "NEVER_RUN",
        lastSource: null
      }
    },
...
function normalizeMicroLearningState(value) {
  ...
  state.backfill.runtime = state.backfill.runtime && typeof state.backfill.runtime === "object"
    ? state.backfill.runtime
    : {
        lastAt: 0,
        lastRowsScanned: 0,
        lastRowsAccepted: 0,
...
// Source of truth: familyMicroAnalyzer.js
// Analyze families: LONG_1..LONG_50 / SHORT_1..SHORT_50
// Rotation micro id: MICRO_LONG_LONG_37_MF_V4_ANALYZE_<hash>
...
function normalizeAnalyzeFamilyId(raw) {
  ...
  const parentFromMicro = extractParentFamilyIdFromMicroId(direct);

  if (ANALYZE_FAMILY_ID_RE.test(parentFromMicro || "")) {
    return parentFromMicro;
  }

  return null;
}

function buildCoreMicroIdFromAnalyzeFamilyId(familyId) {
...
let analyzeClassifierMissingWarned = false;

function getAnalyzeClassifier() {
  const candidates = [
    familyMicroAnalyzer.classifyAnalyzeEvent,
    familyMicroAnalyzer.classifyFamilyMicroEvent,
    familyMicroAnalyzer.classifyTradeEvent,
    familyMicroAnalyzer.classifyEvent,
    familyMicroAnalyzer.classifyFamily,
    familyMicroAnalyzer.getAnalyzeFamily,
    familyMicroAnalyzer.getAnalyzeFamilyId,
    familyMicroAnalyzer.getFamilyIdForEvent,

    familyMicroAnalyzer.default?.classifyAnalyzeEvent,
    familyMicroAnalyzer.default?.classifyFamilyMicroEvent,
    familyMicroAnalyzer.default?.classifyTradeEvent,
    familyMicroAnalyzer.default?.classifyEvent,
    familyMicroAnalyzer.default?.classifyFamily,
    familyMicroAnalyzer.default?.getAnalyzeFamily,
    familyMicroAnalyzer.default?.getAnalyzeFamilyId,
    familyMicroAnalyzer.default?.getFamilyIdForEvent,

    typeof familyMicroAnalyzer.default === "function"
      ? familyMicroAnalyzer.default
      : null
  ];
...
function extractAnalyzeFamilyIdFromClassifierResult(result) {
  return normalizeAnalyzeFamilyId(result) ||
    normalizeAnalyzeFamilyId(result?.familyId) ||
    normalizeAnalyzeFamilyId(result?.analyzeFamilyId) ||
    normalizeAnalyzeFamilyId(result?.analysisFamilyId) ||
    normalizeAnalyzeFamilyId(result?.parentFamilyId) ||
    normalizeAnalyzeFamilyId(result?.mainFamilyId) ||
    normalizeAnalyzeFamilyId(result?.data?.familyId) ||
    normalizeAnalyzeFamilyId(result?.payload?.familyId) ||
    null;
}

function extractAnalyzeFamilyIdFromRow(row = {}) {
  const candidates = [
    row.analyzeFamilyId,
    ...
    row.entryEvent?.familyId,
    row.entryEvent?.analyzeFamilyId,
    row.entryEvent?.analysisFamilyId,
    row.entryEvent?.filterSnapshot?.familyId,
    row.entryEvent?.filterSnapshot?.analyzeFamilyId,
  ...
  for (const candidate of candidates) {
    const familyId = normalizeAnalyzeFamilyId(candidate);
    if (familyId) return familyId;
  }

  try {
    const classifier = getAnalyzeClassifier();

    if (!classifier) {
      if (!analyzeClassifierMissingWarned) {
        analyzeClassifierMissingWarned = true;

        console.warn("TS_ANALYZE_CLASSIFIER_MISSING:", JSON.stringify({
          availableExports: Object.keys(familyMicroAnalyzer || {}),
          defaultExports:
            familyMicroAnalyzer.default && typeof familyMicroAnalyzer.default === "object"
              ? Object.keys(familyMicroAnalyzer.default)
              : [],
          ts: Date.now()
        }));
      }
    ...
    const safeRow = { ...row };
    const directFamilyId = normalizeAnalyzeFamilyId(safeRow.familyId);

    // Belangrijk: MICRO_* mag analyze classifier niet vervuilen.
    if (!directFamilyId) {
      delete safeRow.familyId;
    }

    const classified = classifier(safeRow);
    const familyId = extractAnalyzeFamilyIdFromClassifierResult(classified);

    if (familyId) return familyId;
  } catch (error) {
    if (process.env.TS_DEBUG_MICRO_GATE === "true") {
      console.warn("TS_ANALYZE_CLASSIFY_FAILED:", JSON.stringify({
...
function buildAnalyzerCoreMicroIdentity(row = {}) {
  const analyzeFamilyId = extractAnalyzeFamilyIdFromRow(row);
  if (!analyzeFamilyId) return null;

  const side = analyzeFamilyId.startsWith("LONG_") ? "LONG" : "SHORT";
  const rowSide = analyzerSide(row);

  if (rowSide && rowSide !== side) {
    return null;
  }

  const microFamilyId = buildCoreMicroIdFromAnalyzeFamilyId(analyzeFamilyId);
  ...
  return {
    microFamilyId,
    parentFamilyId: analyzeFamilyId,
    analyzeFamilyId,
    analysisFamilyId: analyzeFamilyId,
    definition,
    definitionParts,
    familyLevel: "CORE_ANALYZE",
    spreadBps: analyzerSpreadBps(row)
  };
}

function buildAnalyzerExactMicroIdentity() {
  // Exact micro families zijn uitgeschakeld.
  // Eén canonical core-id per analyze family voorkomt gate-mismatch.
  return null;
}
...
function buildAnalyzerMicroIdentities(row = {}) {
  ...
  return [core]
    ...
    .map(item => ({
      ...item,
      familyLevel: item.familyLevel || "CORE_ANALYZE"
    }));
}
...
function normalizeRotationMicroId(raw) {
...
function uniqRotationMicroIds(values) {
  return Array.from(
    new Set(
      values
        .flat(Infinity)
...
// ================= PATCH: CANONICAL CORE MICRO IDENTITY ENFORCEMENT =================
function enforceCanonicalCoreMicroIdentity(row = {}) {
  if (!row?.symbol || !row?.side) return row;

  const analyzerRow = enrichRowWithAnalyzerMicroFamily(row);
  const coreId = extractCoreMicroFamilyIdFromRow(analyzerRow);

  const analyzeFamilyId = normalizeAnalyzeFamilyId(
    analyzerRow.analyzeFamilyId ||
      analyzerRow.analysisFamilyId ||
      analyzerRow.parentFamilyId ||
      analyzerRow.analyzerParentFamilyId ||
      analyzerRow.familyId ||
      extractParentFamilyIdFromMicroId(coreId)
  );

  if (!coreId || !analyzeFamilyId) {
    return analyzerRow;
  }
  ...
  return {
    ...
    analysisFamilyId: analyzeFamilyId,
    parentFamilyId: analyzeFamilyId,
    analyzerParentFamilyId: analyzeFamilyId,
    familyId: analyzeFamilyId,
    familyIds: [analyzeFamilyId],
    families: [analyzeFamilyId],

    // Rotation contract.
    rotationMicroFamilyId: coreId,
    analyzerMicroFamilyId: coreId,
    microFamilyId: coreId,
    microFamily: coreId,
    microFamilyIds: [coreId],
    microFamilies: [coreId],

    microFamilySchemaVersion: MICRO_FAMILY_SCHEMA_VERSION,

    rotationCandidate: {
      ...(analyzerRow.rotationCandidate || {}),
...
function extractCoreMicroFamilyIdFromRow(row = {}) {
  return uniqIds([
    row?.analyzerMicroFamilyId,
    row?.rotationMicroFamilyId,
    row?.microFamilyId,
    ...
    row?.rotationCandidate?.microFamilyIds,
    row?.rotationCandidate?.microFamilies,
  ]).find(isCoreMicroFamilyId) || null;
}

function getCanonicalMicroRotationMatch({ signal = {}, rotationIds = [] }) {
  const checkedMicroFamilyIds = uniqIds([
    signal.analyzerMicroFamilyId,
    signal.rotationMicroFamilyId,
    signal.microFamilyId,
    signal.familyId,
    ...
    .slice(0, Math.max(1, MICRO_LEARNING_MAX_FAMILIES_PER_ROW));

  const activeMicroFamilyIds = uniqRotationMicroIds(rotationIds)
    .filter(isCoreMicroFamilyId);
...
function buildCanonicalRotationDecision({
  rotationCheck = {},
  canonicalMatch = {},
  selectedRotationForGate = {},
  rotation = null
}) {
  const rawDecision = rotationCheck?.decision || {};
  const selectedRotation = selectedRotationForGate?.rotation || rotation || {};

  const activeMicroFamilyIds = uniqRotationMicroIds([
    canonicalMatch.activeMicroFamilyIds,
    selectedRotationForGate.ids,
    rawDecision.activeMicroFamilyIds,
    rawDecision.realActiveMicroFamilyIds,
    selectedRotation.allowedMicroFamilyIds,
    selectedRotation.activeMicroFamilyIds,
    selectedRotation.familyIds,
    selectedRotation.activeFamilyIds,
    selectedRotation.allowlist
  ]).filter(isCoreMicroFamilyId);

  const checkedMicroFamilyIds = uniqIds([
    canonicalMatch.checkedMicroFamilyIds,
    rawDecision.checkedMicroFamilyIds,
    rotationCheck?.signal?.analyzerMicroFamilyId,
    rotationCheck?.signal?.rotationMicroFamilyId,
    rotationCheck?.signal?.microFamilyId,
    rotationCheck?.signal?.familyId,
    rotationCheck?.signal?.microFamilyIds,
    rotationCheck?.signal?.familyIds
  ])
    .filter(isCoreMicroFamilyId)
    .slice(0, Math.max(1, MICRO_LEARNING_MAX_FAMILIES_PER_ROW));
  ...
  const matchedMicroFamilyId =
    canonicalMatch.matchedMicroFamilyId ||
    checkedMicroFamilyIds.find(id => activeSet.has(id)) ||
    rawDecision.matchedMicroFamilyId ||
    null;

  const rotationEmpty = activeMicroFamilyIds.length === 0;

  const rawAllowed = Boolean(
    rotationCheck?.allowed ||
    rotationCheck?.ok ||
    rotationCheck?.pass ||
    rawDecision?.allowed ||
    rawDecision?.ok ||
  ...
  const emptyBootstrapAllowed =
    rotationEmpty &&
    MICRO_ROTATION_ALLOW_EMPTY_BOOTSTRAP;

  const allowed = MICRO_ROTATION_STRICT_ENTRY_GATE
    ? rotationEmpty
      ? emptyBootstrapAllowed
      : Boolean(matchedMicroFamilyId)
    : true;

  const rawReason =
    rotationCheck?.waitReason ||
    rotationCheck?.reason ||
    rawDecision?.waitReason ||
    rawDecision?.reason ||
    rawDecision?.gateReason;
    ...
    ? matchedMicroFamilyId
      ...
      : !MICRO_ROTATION_STRICT_ENTRY_GATE
        ...
        : emptyBootstrapAllowed
          ...
          : rawReason || "WEEKLY_ROTATION_ALLOWED"
    : !checkedMicroFamilyIds.length
  ...
  const rotationId =
    rawDecision.activeRotationId ||
    rawDecision.rotationId ||
    selectedRotation.activeRotationId ||
  ...
  return {
    ...
    source:
      rawDecision.source ||
      selectedRotationForGate.source ||
      selectedRotation.source ||
      null,

    weekKey:
      rawDecision.weekKey ||
      selectedRotation.weekKey ||
      selectedRotation.sourceWeekKey ||
      null,
    ...
    microFamilyId:
      matchedMicroFamilyId ||
      checkedMicroFamilyIds[0] ||
      rawDecision.microFamilyId ||
      null,
    ...
    hasRealMicroAllowlist: activeMicroFamilyIds.length > 0,

    strictGate: MICRO_ROTATION_STRICT_ENTRY_GATE,
    emptyBootstrapAllowed,
    rotationEmpty,
...
function enrichRowWithAnalyzerMicroFamily(row = {}) {
  ...
  const primary = identities.find(item => item.familyLevel === "CORE_ANALYZE") || identities[0];

  if (!primary?.microFamilyId) return row;

  const analyzeFamilyId = normalizeAnalyzeFamilyId(
    primary.analyzeFamilyId ||
      primary.analysisFamilyId ||
      primary.parentFamilyId
  );

  if (!analyzeFamilyId) return row;

  const microFamilyId = primary.microFamilyId;

  return {
    ...row,

    // Analyze-compatible identity.
    analyzeFamilyId,
    analysisFamilyId: analyzeFamilyId,
    parentFamilyId: analyzeFamilyId,
    analyzerParentFamilyId: analyzeFamilyId,

    // Belangrijk: analyze ziet `familyId` eerst.
    // Daarom MOET dit LONG_1..50 / SHORT_1..50 zijn, niet MICRO_*.
    familyId: analyzeFamilyId,
    familyIds: [analyzeFamilyId],
    families: [analyzeFamilyId],

    // Rotation-compatible identity.
    rotationMicroFamilyId: microFamilyId,
    analyzerMicroFamilyId: microFamilyId,
    analyzerExactMicroFamilyId: null,
    microFamilySchemaVersion: MICRO_FAMILY_SCHEMA_VERSION,

    microFamilyId,
    microFamily: microFamilyId,
    microFamilyIds: [microFamilyId],
    microFamilies: [microFamilyId],
...
function getClosedTradeMicroTs(row, fallback = Date.now()) {
  const raw =
    row?.completedAt ??
    row?.exitedAt ??
    row?.closedAt ??
...
function microLearningHasAnyFamilies(state = {}) {
  const familyRows = [
    ...Object.values(state?.daily?.families || {}),
    ...Object.values(state?.week?.families || {}),
    ...Object.values(state?.lastCompletedWeek?.families || {})
  ];

  const hasCompletedFamilies = familyRows.some(row =>
    Number(row?.completed || 0) > 0
  );

  const hasActiveRotation =
    extractRotationMicroFamilyIds(state?.activeRotation).length > 0;

  return hasCompletedFamilies || hasActiveRotation;
}

function addClosedTradeToMicroSummary(summary, trade, dayKey) {
  if (!summary || !trade) return 0;

  const row = buildOutcomeRowFromClosedTrade(trade);
  const familyIds = getRowMicroFamilyIds(row)
    .slice(0, MICRO_LEARNING_MAX_FAMILIES_PER_ROW);
  ...
  for (const familyId of familyIds) {
    const stats = getOrCreateFamilyStats(summary, familyId, row.side);
    if (!stats) continue;
    ...
    if (stats.examples.length < 12) {
      stats.examples.push(
        `BACKFILL_${row.symbol}_${row.side}_${status}_R=${Number(exitR || 0).toFixed(2)}`
      );
    }
  ...
  if (dayKey) {
    summary.days[dayKey] = {
      ...
      updatedAt: Date.now(),
      familyCount: Object.keys(summary.families || {}).length
    };
  }
...
function seedMicroLearningFromClosedTradesIfEmpty(now = Date.now()) {
  auditState.microLearning = normalizeMicroLearningState(auditState.microLearning);

  const state = auditState.microLearning;

  if (microLearningHasAnyFamilies(state)) {
    return false;
  }

  const closedTrades = durableArray(auditState.closedTrades)
    .filter(row => row?.symbol && row?.side);

  if (!closedTrades.length) {
    return false;
  }

  const activeWeekKey = state.activeWeekKey || getIsoWeekKey(now);
  const weeks = {};

  let tradesScanned = 0;
  let familyRowsAdded = 0;

  for (const trade of closedTrades) {
    const ts = getClosedTradeMicroTs(trade, now);
    const weekKey = getIsoWeekKey(ts);
    const dayKey = getUtcDayKey(ts);

    if (!weeks[weekKey]) {
      weeks[weekKey] = createMicroWeekSummary(weekKey);
    }

    tradesScanned++;
    familyRowsAdded += addClosedTradeToMicroSummary(
      weeks[weekKey],
      trade,
      dayKey
    );
  ...
  const completedWeekKeys = weekKeys.filter(weekKey =>
    weekKey < activeWeekKey
  );

  const lastCompletedWeekKey =
    completedWeekKeys[completedWeekKeys.length - 1] || null;

  if (lastCompletedWeekKey) {
    state.lastCompletedWeek = weeks[lastCompletedWeekKey];
  }

  if (weeks[activeWeekKey]) {
    state.week = weeks[activeWeekKey];
  }

  state.updatedAt = now;

  console.log("TS_MICRO_LEARNING_BACKFILLED_FROM_CLOSED_TRADES:", JSON.stringify({
    tradesScanned,
    familyRowsAdded,
    activeWeekKey,
    weekKeys,
    lastCompletedWeekKey,
    currentWeekFamilies: Object.keys(state.week?.families || {}).length,
    lastCompletedWeekFamilies: Object.keys(state.lastCompletedWeek?.families || {}).length
  }));

  return familyRowsAdded > 0;
}
...
// (wordt later vervangen door de nieuwe backfill logica, maar deze functie blijft bestaan zoals in de patch)
function ensureActiveMicroRotation(now = Date.now()) {
  auditState.microLearning = normalizeMicroLearningState(auditState.microLearning);

  const state = auditState.microLearning;

  const activeWeekKey = getIsoWeekKey(now);
  const sourceWeekKey = getPreviousIsoWeekKey(now);

  const completedWeek = state.lastCompletedWeek;
  const completedWeekKey = completedWeek?.weekKey || null;

  const hasValidPreviousWeek =
    completedWeekKey === sourceWeekKey &&
    completedWeek?.families &&
    Object.keys(completedWeek.families).length > 0;

  console.log("TS_WEEKLY_MICRO_ROTATION_WEEK_SELECTED", JSON.stringify({
    activeWeekKey,
    sourceWeekKey,
    storedCompletedWeekKey: completedWeekKey,
    rule: "ACTIVE_WEEK_USES_PREVIOUS_COMPLETED_WEEK",
    currentWeekBootstrap: MICRO_ROTATION_USE_CURRENT_WEEK_BOOTSTRAP,
    ts: now
  }));

  if (hasValidPreviousWeek) {
    const activeRotation = buildActiveRotationFromWeek(completedWeek, {
      minCompleted: MICRO_ROTATION_MIN_COMPLETED
    });

    const ids = extractRotationMicroFamilyIds(activeRotation);
    ...
    if (ids.length > 0) {
      state.activeRotation = {
        ...
        sourceWeekKey,
        activeWeekKey,
        source: "TRADE_SYSTEM_PREVIOUS_WEEK_MICRO_ROTATION",
        bootstrap: false,
        rankingMode: "WINRATE_WILSON_BAYES"
      };
      ...
      logImportant("TS_MICRO_ROTATION_ACTIVE", {
        activeWeekKey,
        sourceWeekKey,
        rotationId: state.activeRotation.rotationId,
        activeIds: ids.length,
        longFamilies: state.activeRotation.longFamilies?.map(row => ({
          familyId: row.familyId,
          completed: row.completed,
          wins: row.wins,
          losses: row.losses,
          fairWinrate: row.fairWinrateNum,
          avgR: row.avgR,
          rotationScore: row.rotationScore
        })) || [],
        shortFamilies: state.activeRotation.shortFamilies?.map(row => ({
          familyId: row.familyId,
          completed: row.completed,
          wins: row.wins,
          losses: row.losses,
      ...
      return state.activeRotation;
    }

    state.activeRotation = null;
    state.updatedAt = now;

    console.warn("TS_WEEKLY_MICRO_ROTATION_EMPTY_AFTER_RANKING", JSON.stringify({
      activeWeekKey,
      sourceWeekKey,
      completedWeekKey,
      minCompleted: MICRO_ROTATION_MIN_COMPLETED,
      families: Object.keys(completedWeek.families || {}).length,
      ts: now
    }));
  ...
  if (!MICRO_ROTATION_USE_CURRENT_WEEK_BOOTSTRAP) {
    ...
    console.warn("TS_WEEKLY_MICRO_ROTATION_NOT_READY", JSON.stringify({
      activeWeekKey,
      sourceWeekKey,
      storedCompletedWeekKey: completedWeekKey,
      reason: "NO_PREVIOUS_COMPLETED_WEEK_ROTATION",
      currentWeekBootstrap: false,
      ts: now
    }));
  ...
  const currentIds = extractRotationMicroFamilyIds(currentWeekRotation);

  if (currentIds.length > 0) {
    state.activeRotation = {
      ...currentWeekRotation,
      activeWeekKey,
      sourceWeekKey: state.week?.weekKey || activeWeekKey,
      source: "TRADE_SYSTEM_CURRENT_WEEK_BOOTSTRAP_ROTATION",
      bootstrap: true,
      refreshedAt: now
    };
    ...
    console.warn("TS_WEEKLY_MICRO_ROTATION_CURRENT_WEEK_BOOTSTRAP_USED", JSON.stringify({
      activeWeekKey,
      sourceWeekKey: state.activeRotation.sourceWeekKey,
      activeIds: currentIds.length,
      ts: now
    }));
...
// ================= NIEUWE MICRO-RUNTIME BACKFILL + HEALTH LOGS (toegevoegd vóór ensureActiveMicroRotation) =================
function getMicroOutcomeR(row = {}) {
  const value =
    row.exitR ??
    row.resultR ??
    row.pnlR ??
...
function isCompletedMicroOutcomeRow(row = {}) {
  ...
  if (Number.isFinite(Number(getMicroOutcomeR(row)))) return true;
...
function normalizeOutcomeSide(row = {}) {
  const side = String(row.side || row.direction || row.tradeSide || "").toLowerCase();

  if (["bull", "buy", "long"].includes(side)) return "bull";
  if (["bear", "sell", "short"].includes(side)) return "bear";

  return "";
}

function getMicroOutcomeStatus(row = {}) {
  return String(
    row.exitReason ||
      row.status ||
      row.reason ||
...
function recordMicroOutcomeIntoSummary(summary, rawRow, source = "RUNTIME_BACKFILL") {
  if (!summary || typeof summary !== "object") {
    return {
      accepted: false,
  ...
  const side = normalizeOutcomeSide(rawRow);
  ...
  if (!isCompletedMicroOutcomeRow(rawRow)) {
    return {
      accepted: false,
      reason: "NOT_COMPLETED_OUTCOME"
    };
  }

  const row = enrichRowWithMicroRotation({
    ...rawRow,
    symbol: normalizeBaseSymbol(rawRow.symbol),
    side,
    status: rawRow.status || "CLOSED",
    action: rawRow.action || "ENTRY",
    reason: rawRow.entryReason || rawRow.reason || "RUNTIME_BACKFILL",
    entryReason: rawRow.entryReason || rawRow.reason || "RUNTIME_BACKFILL"
  });
  ...
  if (!ids.length) {
    return {
      accepted: false,
      reason: "NO_CORE_MICRO_FAMILY_ID"
    };
  }

  const exitR = getMicroOutcomeR(row);
  const hasExitR = Number.isFinite(Number(exitR));
  const status = getMicroOutcomeStatus(row);

  const win = hasExitR
    ? Number(exitR) > MICRO_OUTCOME_WIN_R_THRESHOLD
    : Boolean(row.win) || status === "TP" || status === "HIT_TP";

  const loss = hasExitR
    ? Number(exitR) < -MICRO_OUTCOME_LOSS_R_THRESHOLD
    : Boolean(row.loss) || status === "SL" || status === "BE_SL" || status === "HIT_SL";
  ...
  for (const familyId of ids) {
    const stats = getOrCreateFamilyStats(summary, familyId, side);
    if (!stats) continue;

    stats.seen++;
    stats.completed++;
    stats.totalR += hasExitR ? Number(exitR) : 0;
    ...
    if (stats.examples.length < 12) {
      stats.examples.push(
        `${source}_${row.symbol}_${row.side}_${status}_R=${Number(exitR || 0).toFixed(2)}`
      );
    }
  ...
  return {
    accepted: written > 0,
    reason: written > 0 ? "OK" : "NO_STATS_WRITTEN",
    ids,
    written
  };
}

function getEligibleMicroFamiliesFromWeek(week = {}, minCompleted = MICRO_ROTATION_MIN_COMPLETED) {
  return Object.values(week?.families || {})
    .filter(family => isCoreMicroFamilyId(family?.familyId))
    .map(family => {
      const refreshed = refreshFamilyDerivedStats({ ...family });
      ...
      return {
        ...refreshed,
        rotationScore: scoreMicroFamily(refreshed)
      };
    })
    .filter(family => Number(family.completed || 0) >= Number(minCompleted || 0))
    .filter(family => Number(family.rotationScore || 0) >= MICRO_ROTATION_MIN_SCORE)
    .sort((a, b) => Number(b.rotationScore || 0) - Number(a.rotationScore || 0));
}

function getTopMicroFamilyHealthRows(week = {}, limit = MICRO_ROTATION_HEALTH_TOP_FAMILIES) {
  return Object.values(week?.families || {})
    .filter(family => isCoreMicroFamilyId(family?.familyId))
    .map(family => {
      const refreshed = refreshFamilyDerivedStats({ ...family });
      ...
      return {
        ...
        side: refreshed.side,
        completed: Number(refreshed.completed || 0),
        wins: Number(refreshed.wins || 0),
        losses: Number(refreshed.losses || 0),
        winrate: Number(Number(refreshed.winrateNum || 0).toFixed(4)),
        fairWinrate: Number(Number(refreshed.fairWinrateNum || 0).toFixed(4)),
        wilson: Number(Number(refreshed.winrateLowerBoundNum || 0).toFixed(4)),
        bayes: Number(Number(refreshed.bayesianWinrateNum || 0).toFixed(4)),
        avgR: Number(Number(refreshed.avgR || 0).toFixed(4)),
        rotationScore: scoreMicroFamily(refreshed)
      };
    })
    .sort((a, b) => Number(b.rotationScore || 0) - Number(a.rotationScore || 0))
...
function buildMicroLearningHealthSnapshot(label = "MICRO_HEALTH") {
  auditState.microLearning = normalizeMicroLearningState(auditState.microLearning);

  const state = auditState.microLearning;
  const activeIds = extractRotationMicroFamilyIds(state.activeRotation);
  const lastCompletedWeek = state.lastCompletedWeek || {};
  const currentWeek = state.week || {};

  const lastCompletedFamilies = Object.keys(lastCompletedWeek.families || {}).length;
  const currentWeekFamilies = Object.keys(currentWeek.families || {}).length;

  const eligibleLastCompleted = getEligibleMicroFamiliesFromWeek(
    lastCompletedWeek,
    MICRO_ROTATION_MIN_COMPLETED
  );

  const eligibleCurrent = getEligibleMicroFamiliesFromWeek(
    currentWeek,
    MICRO_ROTATION_BOOTSTRAP_MIN_COMPLETED
  );

  let reason = "READY";

  if (!activeIds.length && !lastCompletedFamilies && !currentWeekFamilies) {
    reason = "NO_MICRO_FAMILIES_BUILT";
  } else if (!activeIds.length && lastCompletedFamilies && !eligibleLastCompleted.length) {
    reason = "NO_LAST_WEEK_FAMILY_MEETS_MIN_COMPLETED";
  } else if (!activeIds.length && currentWeekFamilies && !MICRO_ROTATION_USE_CURRENT_WEEK_BOOTSTRAP) {
    reason = "CURRENT_WEEK_BOOTSTRAP_DISABLED";
  } else if (!activeIds.length) {
    reason = "ACTIVE_ROTATION_EMPTY";
  }

  return {
    label,
    ready: activeIds.length > 0,
    reason,
    ...
    activeRotationSource: state.activeRotation?.source || null,
    activeRotationIds: activeIds.length,

    lastCompletedWeekKey: lastCompletedWeek.weekKey || null,
    lastCompletedFamilies,
    eligibleLastCompletedFamilies: eligibleLastCompleted.length,

    currentWeekKey: currentWeek.weekKey || null,
    currentWeekFamilies,
    eligibleCurrentWeekFamilies: eligibleCurrent.length,

    minCompleted: MICRO_ROTATION_MIN_COMPLETED,
    bootstrapMinCompleted: MICRO_ROTATION_BOOTSTRAP_MIN_COMPLETED,
    topFamilies: getTopMicroFamilyHealthRows(lastCompletedFamilies ? lastCompletedWeek : currentWeek),

    backfill: state.backfill?.runtime || null
  };
}

function logMicroLearningHealth(label, extra = {}) {
  const snapshot = buildMicroLearningHealthSnapshot(label);
...
async function backfillMicroLearningFromRuntimeOutcomes({
  runId,
  force = false,
  now = Date.now()
} = {}) {
  ...
  const state = auditState.microLearning;
  const runtimeBackfill = state.backfill.runtime;

  const activeWeekKey = getIsoWeekKey(now);
  const sourceWeekKey = getPreviousIsoWeekKey(now);

  if (!MICRO_LEARNING_RUNTIME_BACKFILL_ENABLED) {
    runtimeBackfill.lastAt = now;
    runtimeBackfill.lastReason = "RUNTIME_BACKFILL_DISABLED";
    runtimeBackfill.lastSource = "RUNTIME";

    return {
      ok: false,
      skipped: true,
      reason: "RUNTIME_BACKFILL_DISABLED"
    };
  }

  const activeIdsBefore = extractRotationMicroFamilyIds(state.activeRotation);
  const activeRotationSourceWeek =
    state.activeRotation?.sourceWeekKey ||
    state.activeRotation?.weekKey ||
    null;

  if (
    !force &&
    activeIdsBefore.length > 0 &&
    activeRotationSourceWeek === sourceWeekKey
  ) {
    return {
      ok: true,
      skipped: true,
      reason: "ACTIVE_PREVIOUS_WEEK_ROTATION_ALREADY_READY",
      activeWeekKey,
      sourceWeekKey,
      activeIds: activeIdsBefore.length
    };
  }

  const sinceLast = now - Number(runtimeBackfill.lastAt || 0);

  if (
    !force &&
    runtimeBackfill.lastAt > 0 &&
    sinceLast < MICRO_LEARNING_RUNTIME_BACKFILL_MIN_INTERVAL_MS
  ) {
    return {
      ok: false,
      skipped: true,
      reason: "RUNTIME_BACKFILL_INTERVAL_ACTIVE",
      sinceLast,
      minIntervalMs: MICRO_LEARNING_RUNTIME_BACKFILL_MIN_INTERVAL_MS,
      activeWeekKey,
      sourceWeekKey
    };
  }

  const closedRows = durableTail(
    auditState.closedTrades,
    MICRO_LEARNING_RUNTIME_BACKFILL_MAX_CLOSED
  ).filter(isCompletedMicroOutcomeRow);

  const shadowRows = durableTail(
    auditState.shadowOutcomes,
    MICRO_LEARNING_RUNTIME_BACKFILL_MAX_SHADOW
  )
    .filter(row => String(row?.status || "OPEN").toUpperCase() !== "OPEN")
    .filter(isCompletedMicroOutcomeRow);

  const allRows = [
    ...closedRows.map(row => ({ row, source: "RUNTIME_CLOSED" })),
    ...shadowRows.map(row => ({ row, source: "RUNTIME_SHADOW" }))
  ];

  const previousWeekRows = allRows.filter(item =>
    getMicroOutcomeWeekKey(item.row, now) === sourceWeekKey
  );

  const availableWeeks = Array.from(
    new Set(allRows.map(item => getMicroOutcomeWeekKey(item.row, now)))
  ).sort();

  if (!previousWeekRows.length) {
    runtimeBackfill.lastAt = now;
    runtimeBackfill.lastRowsScanned = allRows.length;
    runtimeBackfill.lastRowsAccepted = 0;
    runtimeBackfill.lastFamilies = 0;
    runtimeBackfill.lastEligibleFamilies = 0;
    runtimeBackfill.lastActiveIds = 0;
    runtimeBackfill.lastReason = "NO_PREVIOUS_WEEK_RUNTIME_OUTCOME_ROWS";
    runtimeBackfill.lastSource = "RUNTIME";

    console.warn("TS_MICRO_RUNTIME_BACKFILL_NO_PREVIOUS_WEEK_ROWS", JSON.stringify({
      runId,
      activeWeekKey,
      sourceWeekKey,
    ...
    return {
      ok: false,
      skipped: true,
      reason: "NO_PREVIOUS_WEEK_RUNTIME_OUTCOME_ROWS",
      activeWeekKey,
      sourceWeekKey,
      rowsScanned: allRows.length,
      availableWeeks
  ...
  const backfillWeek = createMicroWeekSummary(sourceWeekKey);
  backfillWeek.source = "TRADE_SYSTEM_PREVIOUS_WEEK_RUNTIME_OUTCOME_BACKFILL";
  backfillWeek.generatedAt = now;
  backfillWeek.backfill = true;
  backfillWeek.activeWeekKey = activeWeekKey;

  const rejectCounts = {};
  let accepted = 0;
  let written = 0;

  for (const item of previousWeekRows) {
    const result = recordMicroOutcomeIntoSummary(
      backfillWeek,
      item.row,
      item.source
    );
    ...
    if (!result.accepted) {
      ...
      rejectCounts[reason] = Number(rejectCounts[reason] || 0) + 1;
      continue;
    }
  ...
  backfillWeek.updatedAt = now;

  const familyCount = Object.keys(backfillWeek.families || {}).length;

  const eligibleFamilies = getEligibleMicroFamiliesFromWeek(
    backfillWeek,
    MICRO_ROTATION_MIN_COMPLETED
  );

  const activeRotation = buildActiveRotationFromWeek(backfillWeek, {
    minCompleted: MICRO_ROTATION_MIN_COMPLETED
  });

  const activeIds = extractRotationMicroFamilyIds(activeRotation);

  state.lastCompletedWeek = backfillWeek;

  state.activeRotation = activeIds.length
    ? {
        ...activeRotation,
        source: "TRADE_SYSTEM_PREVIOUS_WEEK_RUNTIME_BACKFILL_ROTATION",
        sourceWeekKey,
        activeWeekKey,
        refreshedAt: now,
        bootstrap: false
  ...
  runtimeBackfill.lastAt = now;
  runtimeBackfill.lastRowsScanned = previousWeekRows.length;
  runtimeBackfill.lastRowsAccepted = accepted;
  runtimeBackfill.lastFamilies = familyCount;
  runtimeBackfill.lastEligibleFamilies = eligibleFamilies.length;
  runtimeBackfill.lastActiveIds = activeIds.length;
  runtimeBackfill.lastReason =
    activeIds.length > 0
      ? "PREVIOUS_WEEK_RUNTIME_BACKFILL_ROTATION_READY"
      : familyCount === 0
        ? "NO_CORE_MICRO_FAMILIES_BUILT"
        : eligibleFamilies.length === 0
          ? "NO_FAMILY_MEETS_MIN_COMPLETED"
          : "ROTATION_EMPTY_AFTER_BACKFILL";
  runtimeBackfill.lastSource = "RUNTIME_PREVIOUS_WEEK";

  const result = {
    ok: activeIds.length > 0,
    skipped: false,
    reason: runtimeBackfill.lastReason,

    runId,
    activeWeekKey,
    sourceWeekKey,

    rowsScanned: previousWeekRows.length,
    totalRuntimeRows: allRows.length,
    closedRows: previousWeekRows.filter(item => item.source === "RUNTIME_CLOSED").length,
    shadowRows: previousWeekRows.filter(item => item.source === "RUNTIME_SHADOW").length,
    ...
    minCompleted: MICRO_ROTATION_MIN_COMPLETED,
    availableWeeks,
    topFamilies: getTopMicroFamilyHealthRows(backfillWeek)
  };

  console.log("TS_MICRO_RUNTIME_BACKFILL_RESULT:", JSON.stringify({
    ...result,
    ts: now
  }));
...
// ================= MICRO-LEARNING RUNTIME PRUNING CONSTANTS =================
const MAX_RUNTIME_MICRO_DAILY_FAMILIES = Number(
  process.env.TS_MAX_RUNTIME_MICRO_DAILY_FAMILIES || 900
);

const MAX_RUNTIME_MICRO_WEEK_FAMILIES = Number(
  process.env.TS_MAX_RUNTIME_MICRO_WEEK_FAMILIES || 1600
);

const MAX_RUNTIME_MICRO_LAST_WEEK_FAMILIES = Number(
  process.env.TS_MAX_RUNTIME_MICRO_LAST_WEEK_FAMILIES || 1200
);

const MAX_RUNTIME_MICRO_EXAMPLES = Number(
  process.env.TS_MAX_RUNTIME_MICRO_EXAMPLES || 3
);

function pruneFamilyStatsForRuntime(family = {}) {
  family.entryReasons = compactCounterObject(family.entryReasons || {});
  family.setupClasses = compactCounterObject(family.setupClasses || {});
  family.rsiZones = compactCounterObject(family.rsiZones || {});
  family.obBiases = compactCounterObject(family.obBiases || {});

  family.examples = durableTail(
    family.examples,
    MAX_RUNTIME_MICRO_EXAMPLES
  );

  return family;
}

function pruneMicroFamilyMapInPlace(families = {}, maxFamilies = 1000) {
  const entries = Object.values(durableObject(families))
    .filter(row => row?.familyId)
    .map(pruneFamilyStatsForRuntime)
    .sort((a, b) => microFamilySortScore(b) - microFamilySortScore(a))
    .slice(0, Math.max(0, Number(maxFamilies || 0)));
...
function pruneMicroLearningRuntimeState(state = {}) {
  const s = normalizeMicroLearningState(state);

  if (s.daily?.families) {
    pruneMicroFamilyMapInPlace(
      s.daily.families,
      MAX_RUNTIME_MICRO_DAILY_FAMILIES
    );
  }
  ...
  if (s.week?.families) {
    pruneMicroFamilyMapInPlace(
      s.week.families,
      MAX_RUNTIME_MICRO_WEEK_FAMILIES
    );
  }

  if (s.lastCompletedWeek?.families) {
    pruneMicroFamilyMapInPlace(
      s.lastCompletedWeek.families,
      MAX_RUNTIME_MICRO_LAST_WEEK_FAMILIES
    );
  }

  if (s.week?.days) {
    s.week.days = Object.fromEntries(
      Object.entries(s.week.days).slice(-14)
    );
  }
...
function enrichRowWithMicroRotation(row) {
  ...
  // Stap 1: analyzer bepaalt de canonical core-ID.
  const canonicalBase = enforceCanonicalCoreMicroIdentity(row);

  // Stap 2: attach mag rotation metadata toevoegen, maar niet de ID bepalen.
  const attached = attachMicroRotationKeys(canonicalBase, {
    weekKey: auditState?.microLearning?.activeWeekKey || getIsoWeekKey()
  });

  // Stap 3: na attach opnieuw forceren. Dit sloopt oude EXACT ids uit arrays.
  return enforceCanonicalCoreMicroIdentity({
    ...attached,

    analyzerMicroFamilyId: canonicalBase.analyzerMicroFamilyId,
    rotationMicroFamilyId: canonicalBase.rotationMicroFamilyId,

    parentFamilyId: canonicalBase.parentFamilyId,
    analyzerParentFamilyId: canonicalBase.analyzerParentFamilyId,

    analyzerDefinition: canonicalBase.analyzerDefinition,
    analyzerDefinitionParts: canonicalBase.analyzerDefinitionParts,
    definition: canonicalBase.definition,
    definitionParts: canonicalBase.definitionParts,

    spreadBps: canonicalBase.spreadBps
  });
}
...
function getRotationSources(rotation = {}) {
  ...
  return [
    ...
    // Normalized gate/status wrappers.
    rotation.gate,
    rotation.status,
    rotation.activeWeek,
    rotation.nextWeek,
    ...
    // Some stores return { data: { ... } } or { result: { ... } }.
    rotation.data,
    rotation.result,
    rotation.payload,
...
async function loadWeeklyRotationForGate(options = {}) {
  ...
  try {
    const mod = await import("./rotation/getActiveWeeklyGate.js");

    const getActiveWeeklyGateFn =
      mod.getActiveWeeklyGate ||
      mod.default;

    if (typeof getActiveWeeklyGateFn !== "function") {
      throw new Error("getActiveWeeklyGate_export_missing");
    }

    const status = await getActiveWeeklyGateFn();
    ...
    const ids = extractRotationMicroFamilyIds(normalized);
    const disabled = isRotationDisabledOrUnusable(normalized);

    if (disabled || ids.length === 0) {
      console.warn("TS_EXTERNAL_WEEKLY_ROTATION_IGNORED:", JSON.stringify({
        reason: disabled ? "DISABLED_OR_UNUSABLE" : "NO_MICRO_FAMILY_IDS",
        ...getRotationLogMeta(normalized),
        rawKeys: Object.keys(status || {}).slice(0, 25),
        normalizedKeys: Object.keys(normalized || {}).slice(0, 25),
        ts: Date.now()
      }));
    ...
    console.log("TS_EXTERNAL_WEEKLY_ROTATION_LOADED:", JSON.stringify({
      rotationId:
        normalized.rotationId ||
        normalized.activeRotationId ||
        normalized.id ||
        null,
      weekKey:
        normalized.weekKey ||
        normalized.activeWeekKey ||
        normalized.sourceWeekKey ||
        null,
      ids: ids.length,
...
const ENABLE_PROCESS_TRADES_ANALYSIS_WEBHOOK =
  process.env.TS_PROCESS_TRADES_ANALYSIS_WEBHOOK === "true";
...
async function sendAnalysisActionsNonBlocking(actions, context) {
  if (!ENABLE_PROCESS_TRADES_ANALYSIS_WEBHOOK) {
    return {
      ok: true,
      skipped: true,
      reason: "PROCESS_TRADES_ANALYSIS_WEBHOOK_DISABLED",
      sent: 0,
      total: 0
    };
  }
...
async function finalizeResult(actions, candidates, btcState, runId, options = {}) {
  ...
  const WEBHOOK_MAX_ANALYSIS_ROWS = Number(process.env.WEBHOOK_MAX_ANALYSIS_ROWS ?? 5);

  const webhookActions = ENABLE_PROCESS_TRADES_ANALYSIS_WEBHOOK
    ? finalActions
        .filter(action => {
          const actionType = String(action.action || "").toUpperCase();

          // Geen WAIT/HOLD naar analysis webhook.
          // De trade-funnel route doet zelf al de analyse append.
          return actionType === "ENTRY" || actionType === "EXIT";
        })
        .sort((a, b) => {
          const priorityDiff = getActionPriority(b) - getActionPriority(a);
          if (priorityDiff !== 0) return priorityDiff;
          ...
          const scoreA =
            Number(a.score ?? 0) +
            Number(a.confluence ?? 0) +
            Number(a.sniperScore ?? 0);

          const scoreB =
            Number(b.score ?? 0) +
            Number(b.confluence ?? 0) +
            Number(b.sniperScore ?? 0);
...
function getRsiAdjustedRequiredSniper({ requiredSniper, rsiEdge }) {
  ...
  const minAllowed = base <= 0 ? 0 : 30;

  return clamp(Math.round(adjusted), minAllowed, 95);
}
...
function extractRotationMicroFamilyIds(rotation = {}) {
  ...
  for (const source of sources) {
    ...
    const familyRows = [
      ...
      ...(Array.isArray(source.selectedLongFamilies) ? source.selectedLongFamilies : []),
      ...(Array.isArray(source.selectedShortFamilies) ? source.selectedShortFamilies : []),
  ...
  return uniqRotationMicroIds(values);
}
...
function selectWeeklyRotationForGate({
  ...
  // Hard filter: alleen canonical MF_V4_ANALYZE ids mogen de gate in.
  const externalCoreIds = externalIds.filter(isCoreMicroFamilyId);
  const learnedCoreIds = learnedIds.filter(isCoreMicroFamilyId);

  const externalUsable =
    externalCoreIds.length > 0 &&
    !isRotationDisabledOrUnusable(externalRotation);

  const learnedUsable =
    learnedCoreIds.length > 0 &&
    !isRotationDisabledOrUnusable(learnedRotation);

  if (externalUsable) {
    return {
      rotation: externalRotation,
      source: "EXTERNAL_ROTATION_CORE",
      ids: externalCoreIds
    };
  }
  ...
  if (learnedUsable) {
    return {
      ...
      source: "TRADE_SYSTEM_LEARNED_ROTATION_CORE",
      ids: learnedCoreIds
    };
  }
...
// ================= WEBHOOK TIMEOUT HELPERS =================
export const sendWebhookWithTimeout = async ({
  url,
  payload,
  timeoutMs = 1500,
}) => {
  ...
  try {
    ...
    return {
      ok: false,
      error: err?.name === 'AbortError' ? 'WEBHOOK_TIMEOUT' : err?.message,
    };
  } finally {
    clearTimeout(timer);
  }
...
export const sendWebhookBatch = async ({
  jobs,
  concurrency = 3,
  maxJobs = 10,
  timeoutMs = 1500,
  ...
  const worker = async () => {
    while (index < queue.length) {
      ...
      const result = await sendWebhookWithTimeout({
        url: job.url,
        payload: job.payload,
        timeoutMs,
      });
...
// ================= ROTATION BLOCK SUMMARIZER =================
export const summarizeRotationBlocks = blocks => {
  const byReason = {};
  const byParent = {};
  const examples = {};
  ...
  for (const block of blocks) {
    ...
    if (block.matchedParentFamilyId) {
      byParent[block.matchedParentFamilyId] =
        (byParent[block.matchedParentFamilyId] ?? 0) + 1;
    }

    if (!examples[block.reason]) {
      examples[block.reason] = {
        symbol: block.symbol,
        side: block.side,
        microFamilyId: block.microFamilyId,
        matchedParentFamilyId: block.matchedParentFamilyId,
      };
    }
  }
...
// ================= EXTRA HELPERS =================
const DISCOVERY_MEMORY_TIME_STOP_ENABLED =
  process.env.TS_DISCOVERY_MEMORY_TIME_STOP_ENABLED !== "false";

const DISCOVERY_MEMORY_MAX_AGE_MS = Number(
  process.env.TS_DISCOVERY_MEMORY_MAX_AGE_MS || 12 * 60 * 60 * 1000
);

const DISCOVERY_MAX_TIME_STOP_EXITS_PER_RUN = Number(
  process.env.TS_DISCOVERY_MAX_TIME_STOP_EXITS_PER_RUN || 80
);

function getMemoryPositionAgeMs(pos, now = Date.now()) {
  return now - Number(pos?.createdAt || pos?.openedAt || pos?.entryTs || now);
}

function getDiscoveryMemoryExitReason(pos, now = Date.now()) {
  if (!DISCOVERY_MODE) return null;
  if (!DISCOVERY_MEMORY_TIME_STOP_ENABLED) return null;

  const ageMs = getMemoryPositionAgeMs(pos, now);

  if (ageMs >= DISCOVERY_MEMORY_MAX_AGE_MS) {
    return "DISCOVERY_TIME_STOP";
  }
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    // Belangrijk na schema-reset:
    // closedTrades bestaan nog, maar microLearning kan leeg zijn.
    // Bouw dan eerst microLearning opnieuw op uit gesloten trades.
    seedMicroLearningFromClosedTradesIfEmpty(Date.now());
    ...
    // ===== Nieuwe micro backfill en health logging vóór ensureActiveMicroRotation =====
    logMicroLearningHealth("TS_MICRO_LEARNING_HEALTH_BEFORE_BACKFILL", {
      runId
    });

    const runtimeBackfillResult = await backfillMicroLearningFromRuntimeOutcomes({
      runId
    });

    console.log("TS_MICRO_BACKFILL_GATE_INPUT:", JSON.stringify({
      runId,
      ok: Boolean(runtimeBackfillResult?.ok),
      skipped: Boolean(runtimeBackfillResult?.skipped),
      reason: runtimeBackfillResult?.reason || null,
      rowsScanned: Number(runtimeBackfillResult?.rowsScanned || 0),
      rowsAccepted: Number(runtimeBackfillResult?.rowsAccepted || 0),
      families: Number(runtimeBackfillResult?.familyCount || 0),
      eligibleFamilies: Number(runtimeBackfillResult?.eligibleFamilies || 0),
      activeIds: Number(runtimeBackfillResult?.activeIds || 0),
      ts: Date.now()
    }));

    logMicroLearningHealth("TS_MICRO_LEARNING_HEALTH_AFTER_BACKFILL", {
      runId
    });
    ...
    logMicroLearningHealth("TS_MICRO_ROTATION_HEALTH_AFTER_ENSURE", {
      runId,
      learnedRotationOk: Boolean(learnedRotationForGate),
      learnedRotationIds: extractRotationMicroFamilyIds(learnedRotationForGate).length,
      learnedRotationId: learnedRotationForGate?.rotationId || null
    });
    ...
    if (Array.isArray(input)) {
      ...
      // Collect all stages from both sides
      candidatesRaw = [
        ...collectScannerFunnelCandidates(input?.funnel?.bull),
        ...collectScannerFunnelCandidates(input?.funnel?.bear)
      ];
    ...
    let discoveryTimeStopExitsThisRun = 0;
    ...
    // ===== EENMALIGE SELECTIE VAN DE ROTATION (buiten de candidate-loop) =====
    ...
    const selectedRotationHealth = {
      runId,
      source: selectedRotationForGate.source,
      selectedIds: selectedRotationForGate.ids.length,

      strictGate: MICRO_ROTATION_STRICT_ENTRY_GATE,
      allowEmptyBootstrap: MICRO_ROTATION_ALLOW_EMPTY_BOOTSTRAP,
      ...
      learnedIds: extractRotationMicroFamilyIds(learnedRotationForGate).length,
      externalIds: extractRotationMicroFamilyIds(weeklyRotationForGate).length,

      topFamilies: getTopMicroFamilyHealthRows(
        auditState.microLearning?.lastCompletedWeek ||
          auditState.microLearning?.week ||
          {}
      ),
    ...
    logImportant("TS_MICRO_GATE_ACTIVE", selectedRotationHealth);
    ...
    const rotationBlocks = []; // collectie van blocked entries voor summary
    ...
    for (const originalCoin of candidates) {
      ...
      if (prev) {
        const pos = enforceCanonicalCoreMicroIdentity({
          ...prev,
          symbol: c.symbol,
          side: c.side
        });
        ...
        const discoveryExitReason = getDiscoveryMemoryExitReason(pos);

        if (
          discoveryExitReason &&
          discoveryTimeStopExitsThisRun < DISCOVERY_MAX_TIME_STOP_EXITS_PER_RUN
        ) {
          discoveryTimeStopExitsThisRun++;

          const executionPrice = Number(c.price);

          const exitPayload = {
            ...buildCommonPayload(c, flow, null, funding, obData),

            action: "EXIT",
            reason: discoveryExitReason,
          ...
          console.log("TS_DISCOVERY_MEMORY_TIME_STOP_EXIT:", JSON.stringify({
            symbol: c.symbol,
            side: c.side,
            ageMs: getMemoryPositionAgeMs(pos),
            exitR: exitPayload.exitR,
            pnlPct: exitPayload.pnlPct,
            memoryRemaining: memory.size,
            ts: Date.now()
          }));
      ...
      try {
        ...
        // Gebruik de al geselecteerde rotation (selectedRotationForGate, learnedWeeklyRotation)
        const analyzerEntryPayload = enforceCanonicalCoreMicroIdentity(
          enrichRowWithMicroRotation(entryPayload)
        );

        const rotationReadySignal = enforceCanonicalCoreMicroIdentity(
          attachMicroRotationKeys(analyzerEntryPayload, {
        ...
        if (process.env.TS_DEBUG_MICRO_GATE === "true") {
          console.log("TS_MICRO_GATE_CANONICAL_DEBUG:", JSON.stringify({
            ...
            parentFamilyId: rotationReadySignal.parentFamilyId,
            isCore: isCoreMicroFamilyId(rotationReadySignal.microFamilyId),
            selectedRotationSource: selectedRotationForGate.source,
            selectedRotationIds: selectedRotationForGate.ids.length,
            selectedRotationSample: selectedRotationForGate.ids.slice(0, 6),
            ts: Date.now()
          }));
        }

        logDebug("TS_ROTATION_SIGNAL_READY", {
          symbol: rotationReadySignal.symbol,
          side: rotationReadySignal.side,
        ...
        const rotationCheckRaw = await checkTradeSignalAgainstRotation(rotationReadySignal, {
          weeklyRotation: learnedWeeklyRotation,
          rotation: learnedWeeklyRotation,
          rotationState: learnedWeeklyRotation,
          marketState: { btcState, regime: regimeLevel },
        ...
        const signalForAction = enforceCanonicalCoreMicroIdentity({
          ...rotationReadySignal,
          ...(rotationCheckRaw.signal || {})
        });

        const canonicalRotationMatch = getCanonicalMicroRotationMatch({
          signal: signalForAction,
          rotationIds: selectedRotationForGate.ids
        });

        const rotationDecision = buildCanonicalRotationDecision({
          rotationCheck: rotationCheckRaw,
          canonicalMatch: canonicalRotationMatch,
          selectedRotationForGate,
          rotation: learnedWeeklyRotation
        });
        ...
        const rotationCheck = {
          ...rotationCheckRaw,
          allowed: Boolean(rotationDecision.allowed),
          ok: Boolean(rotationDecision.allowed),
          pass: Boolean(rotationDecision.allowed),
          reason: rotationDecision.reason,
          waitReason: rotationDecision.waitReason,
          decision: rotationDecision,
          signal: signalForAction
        };

        // Parent fallback blijft bewust uit. Alleen exact canonical core match telt.
        const parentRotationMatch = {
          allowed: false,
          matchedParentFamilyId: null,
          checkedParentFamilyIds: [],
          activeParentFamilyIds: []
        };

        const hardRotationPass = !MICRO_ROTATION_STRICT_ENTRY_GATE
          ? true
          : selectedRotationForGate.ids.length === 0
            ? MICRO_ROTATION_ALLOW_EMPTY_BOOTSTRAP
            : Boolean(canonicalRotationMatch.allowed);

        if (!hardRotationPass) {
          const hardReason = !signalForAction.microFamilyId
            ? "CORE_MICRO_FAMILY_MISSING"
            : selectedRotationForGate.ids.length === 0
              ? "STRICT_GATE_WITH_ZERO_ACTIVE_MICRO_FAMILIES"
              : "REAL_MICRO_FAMILY_NOT_IN_ACTIVE_ROTATION";

          rotationCheck.allowed = false;
          rotationCheck.ok = false;
          rotationCheck.pass = false;
          rotationCheck.reason = hardReason;
          rotationCheck.waitReason = hardReason;
          rotationCheck.decision = {
            ...
            allowed: false,
            reason: hardReason,
            waitReason: hardReason,
            gateReason: hardReason,
            matchedMicroFamilyId: null,
            activeMicroFamilyIds: selectedRotationForGate.ids,
            realActiveMicroFamilyIds: selectedRotationForGate.ids,
            checkedMicroFamilyIds: canonicalRotationMatch.checkedMicroFamilyIds,
            strictGate: MICRO_ROTATION_STRICT_ENTRY_GATE,
        ...
        if (!rotationCheck.allowed) {
          ...
          logImportant("TS_ENTRY_BLOCKED_BY_WEEKLY_ROTATION", {
            symbol: c.symbol,
            side: c.side,
            scannerStage: c.scannerStage || c.stage,
            setupClass: setup.setupClass,
            ...
            activeMicroFamilyIds: selectedRotationForGate.ids?.slice?.(0, 4) || []
          });

          // Voeg block toe voor summary
          rotationBlocks.push({
        ...
        const canonicalEntryMicroFamilyId =
          rotationCheck.decision?.matchedMicroFamilyId ||
          signalForAction.microFamilyId ||
          null;

        const canonicalEntryAnalyzeFamilyId = normalizeAnalyzeFamilyId(
          signalForAction.analyzeFamilyId ||
            signalForAction.analysisFamilyId ||
            signalForAction.parentFamilyId ||
            signalForAction.analyzerParentFamilyId ||
            signalForAction.familyId ||
            extractParentFamilyIdFromMicroId(canonicalEntryMicroFamilyId)
        );

        Object.assign(entryPayload, {
          ...signalForAction,

          rotation: rotationCheck.decision,
          rotationGate: rotationCheck.decision,

          // Analyze identity.
          analyzeFamilyId: canonicalEntryAnalyzeFamilyId,
          analysisFamilyId: canonicalEntryAnalyzeFamilyId,
          parentFamilyId: canonicalEntryAnalyzeFamilyId,
          analyzerParentFamilyId: canonicalEntryAnalyzeFamilyId,
          familyId: canonicalEntryAnalyzeFamilyId,
          familyIds: canonicalEntryAnalyzeFamilyId ? [canonicalEntryAnalyzeFamilyId] : [],
          families: canonicalEntryAnalyzeFamilyId ? [canonicalEntryAnalyzeFamilyId] : [],

          // Rotation identity.
          microFamilyId: canonicalEntryMicroFamilyId,
          rotationMicroFamilyId: canonicalEntryMicroFamilyId,
          analyzerMicroFamilyId: canonicalEntryMicroFamilyId,
          microFamilyIds: canonicalEntryMicroFamilyId ? [canonicalEntryMicroFamilyId] : [],
          microFamilies: canonicalEntryMicroFamilyId ? [canonicalEntryMicroFamilyId] : [],

          liveEligible: true,
          shadowOnly: false
        });

        position.rotation = rotationCheck.decision;
        ...
        // Analyze identity.
        position.analyzeFamilyId = entryPayload.analyzeFamilyId;
        position.analysisFamilyId = entryPayload.analysisFamilyId;
        position.parentFamilyId = entryPayload.parentFamilyId;
        position.analyzerParentFamilyId = entryPayload.analyzerParentFamilyId;
        ...
        position.rotationMicroFamilyId = entryPayload.rotationMicroFamilyId;
        position.analyzerMicroFamilyId = entryPayload.analyzerMicroFamilyId;
        ...
        logImportant("TS_ENTRY_OPENED", {
          ...
          activeRotationMatched: Boolean(canonicalRotationMatch.allowed),
          strictGateDisabledPass:
            !MICRO_ROTATION_STRICT_ENTRY_GATE &&
            !canonicalRotationMatch.allowed,
          rotationPassReason: rotationCheck.reason,
    ...
    // Log rotation block summary
    if (rotationBlocks.length > 0) {
      logImportant("TS_ROTATION_BLOCK_SUMMARY", summarizeRotationBlocks(rotationBlocks));
    }
