...
// ================= STRATEGY VERSION =================
const STRATEGY_VERSION = "TS_V12_3_RR_EDGE_BTC_STRUCTURE";
...
const MAX_SPREAD_PCT = 0.0010;
const MID_BULL_MAX_SPREAD_PCT = 0.0008;
const MIN_DEPTH_USD_1P = 120000;

const MIN_RR_FLOOR = 1.25;

const GRADE_A_MIN_RR_FLOOR = 1.45;
const GRADE_B_MIN_RR_FLOOR = 1.70;
const GRADE_C_MIN_RR_FLOOR = 2.00;

const A_ENTRY_MIN_RR = 1.45;
const B_ENTRY_MIN_RR = 1.70;
const GOD_ENTRY_MIN_RR = 1.60;

const A_MIN_SNIPER = 72;
const A_MIN_CONFLUENCE = 78;

const B_MIN_SNIPER = 78;
const B_MIN_CONFLUENCE = 82;

const GOD_MIN_SNIPER = 82;
const GOD_MIN_CONFLUENCE = 85;

const MID_RSI_MIN_CONFLUENCE = 64;

const TREND_CONTINUATION_MIN_CONFLUENCE = 72;
const TREND_CONTINUATION_MIN_SNIPER = 70;
const TREND_CONTINUATION_MIN_RR = 1.40;

const A_FINAL_MIN_RR = 1.45;
const A_GOD_MAX_TP_REWARD_MULTIPLIER = 1.25;

const ENABLE_B_ENTRIES = false;
const ENABLE_BULLISH_MID_TREND_PROBES = false;

const MIN_DEPTH_USD_1P_ABSOLUTE = 50000;
const A_MIN_DEPTH_USD_1P = 75000;
const BULL_TREND_MIN_DEPTH_USD_1P = 100000;

const REQUIRE_BULL_TREND_PULLBACK = true;
const MAX_BULL_DISTANCE_FROM_LOCAL_HIGH_PCT = 0.010;

const ENABLE_BTC_BULLISH_BEAR_EXCEPTION = false;

const BTC_BULLISH_BEAR_EXCEPTION_MIN_DEPTH_USD_1P = 150000;
const BTC_BULLISH_BEAR_EXCEPTION_MAX_SPREAD_PCT = 0.0007;
const BTC_BULLISH_BEAR_EXCEPTION_MIN_RR = 1.80;
const BTC_BULLISH_BEAR_EXCEPTION_MIN_CONF = 88;
const BTC_BULLISH_BEAR_EXCEPTION_MAX_CONF = 100;
const BTC_BULLISH_BEAR_EXCEPTION_MIN_SNIPER = 82;

const SHORT_BLOCKED_RSI_ZONES = ["LOWER_2", "LOWER_3"];

const COUNTERTREND_MIN_RR_FLOOR = 1.80;
const BUILDUP_MIN_RR_FLOOR = 1.60;

const CANDIDATE_MIN_SCORE = 52;
const BTC_BEARISH_LONG_MIN_SCORE = 68;
const BTC_NEUTRAL_MIN_SCORE = 58;

const EARLY_RSI_MIN_SNIPER = 78;

const SHORT_LOWER1_CONTINUATION_MIN_CONFLUENCE = 82;
const SHORT_LOWER1_CONTINUATION_MIN_SNIPER = 78;
const SHORT_LOWER1_CONTINUATION_MIN_RR = 1.70;
...
const ELITE_MOMENTUM_MIN_CONFLUENCE = 82;
const ELITE_MOMENTUM_MIN_1H_MOVE_PCT = 0.10;

const LOW_VOL_MIN_CONFLUENCE = 62;
const NO_FLOW_MIN_CONFLUENCE = 62;

const TF_MIN_STRENGTH = 0;
const GLOBAL_MIN_CONFLUENCE = 60;

const OB_AGAINST_MIN_CONFLUENCE = 82;
const BAD_MARKET_QUALITY_MIN_CONFLUENCE = 78;
const OB_NEUTRAL_MIN_CONFLUENCE = 78;

const MID_BULL_SPREAD_EXCEPTION_MIN_CONFLUENCE = 86;
const MID_BULL_SPREAD_EXCEPTION_MIN_SNIPER = 80;

const EXTREME_FUNDING_ABS_MAX = 0.010;
const BULL_CROWDED_FUNDING_MAX = 0.006;
const BEAR_CROWDED_FUNDING_MIN = -0.006;
const CROWDED_FUNDING_MIN_CONFLUENCE = 86;

const SETUP_GRADE_A_MIN_POINTS = 9;
const SETUP_GRADE_B_MIN_POINTS = 8;

const NEUTRAL_OB_A_EXCEPTION_MIN_CONFLUENCE = 86;
const NEUTRAL_OB_A_EXCEPTION_MIN_RR = 1.45;
const NEUTRAL_OB_A_EXCEPTION_MIN_SNIPER = 82;

const NEUTRAL_OB_B_EXCEPTION_MIN_CONFLUENCE = 84;
const NEUTRAL_OB_B_EXCEPTION_MIN_RR = 1.70;
const NEUTRAL_OB_B_EXCEPTION_MIN_SNIPER = 80;
const NEUTRAL_OB_B_EXCEPTION_MIN_SCORE = 82;

const MAX_DEEP_CANDIDATES = 30;

// ================= DURABLE / RUNTIME STORE CONSTANTS =================
function envInt(name, fallback) {
  const n = Number(process.env[name]);
  return Number.isFinite(n) && n > 0 ? n : fallback;
}

const DURABLE_KEY_PREFIX =
  process.env.TRADE_SYSTEM_RUNTIME_KEY_PREFIX ||
  `tradeSystem:${STRATEGY_VERSION}`;

const RUNTIME_STORE_KEY = `${DURABLE_KEY_PREFIX}:legacy`;
const RUNTIME_CORE_KEY = `${DURABLE_KEY_PREFIX}:core`;
const RUNTIME_RECENT_KEY = `${DURABLE_KEY_PREFIX}:recentEntries`;

const RUNTIME_CLOSED_META_KEY = `${DURABLE_KEY_PREFIX}:closedTrades:meta`;
const RUNTIME_CLOSED_CHUNK_PREFIX = `${DURABLE_KEY_PREFIX}:closedTrades:chunk:`;

const RUNTIME_FEATURE_META_KEY = `${DURABLE_KEY_PREFIX}:featureStore:meta`;
const RUNTIME_FEATURE_CHUNK_PREFIX = `${DURABLE_KEY_PREFIX}:featureStore:chunk:`;

const RUNTIME_SHADOW_META_KEY = `${DURABLE_KEY_PREFIX}:shadowOutcomes:meta`;
const RUNTIME_SHADOW_CHUNK_PREFIX = `${DURABLE_KEY_PREFIX}:shadowOutcomes:chunk:`;

const RUNTIME_LOCK_KEY = `${DURABLE_KEY_PREFIX}:runtimeLock`;

const DURABLE_LOCK_ATTEMPTS = envInt("TS_DURABLE_LOCK_ATTEMPTS", 6);
const DURABLE_LOCK_RETRY_MS = envInt("TS_DURABLE_LOCK_RETRY_MS", 750);
const DURABLE_LOCK_TTL_MS = envInt("TS_DURABLE_LOCK_TTL_MS", 180000);

const DURABLE_MAX_CHUNK_BYTES = envInt("TS_DURABLE_MAX_CHUNK_BYTES", 650000);

// ================= AUDIT / STORE LIMITS =================
const MAX_RECENT_ENTRY_AUDIT_ROWS = envInt("TS_MAX_RECENT_ENTRY_AUDIT_ROWS", 150);
const MAX_CLOSED_TRADE_AUDIT_ROWS = envInt("TS_MAX_CLOSED_TRADE_AUDIT_ROWS", 1200);
const MAX_FEATURE_STORE_ROWS = envInt("TS_MAX_FEATURE_STORE_ROWS", 3000);
const MAX_SHADOW_OUTCOME_ROWS = envInt("TS_MAX_SHADOW_OUTCOME_ROWS", 3000);

// ================= DATA FETCH / FALLBACKS =================
const DATA_FETCH_CONCURRENCY = envInt("TS_DATA_FETCH_CONCURRENCY", 6);
const ENABLE_FALLBACK_RISK_GEOMETRY = true;
const MIN_EXTERNAL_SCORE_HEALTH = 10;
...
// ================= RSI / CONTINUATION EXTRA CONSTANTS =================
const SHORT_LOWER1_ALLOWED_BTC_STATES = [
  "BEARISH",
  "STRONG_BEAR",
  "NEUTRAL"
];

const LONG_LOWER2_MAX_1H_CHANGE = 1.20;
const SHORT_UPPER2_MIN_1H_CHANGE = -1.20;

// ================= BULLISH MID TREND PROBE CONSTANTS =================
const BULLISH_MID_TREND_PROBE_MIN_CONFLUENCE = 84;
const BULLISH_MID_TREND_PROBE_MIN_SNIPER = 80;
const BULLISH_MID_TREND_PROBE_MIN_RR = 1.45;
const BULLISH_MID_TREND_PROBE_MAX_SPREAD_PCT = 0.0008;
const BULLISH_MID_TREND_PROBE_MIN_DEPTH_USD_1P = 100000;
const BULLISH_MID_TREND_PROBE_REQUIRE_BTC_BULLISH = true;
const BULLISH_MID_TREND_PROBE_MIN_1H_MOVE_PCT = 0.15;
const BULLISH_MID_TREND_PROBE_MIN_24H_MOVE_PCT = 1.20;
const BULLISH_MID_TREND_PROBE_MIN_PULLBACK_FROM_HIGH_PCT = 0.0025;

// ================= PATH / MFE / MAE CONSTANTS =================
const MAX_PRICE_PATH_SAMPLES = envInt("TS_MAX_PRICE_PATH_SAMPLES", 60);

const HALF_R_LEVEL = 0.50;
const ONE_R_LEVEL = 1.00;
const NEAR_TP_PROGRESS = 0.80;
const DIRECT_SL_MFE_LIMIT_R = 0.15;

// ================= BREAK EVEN CONSTANTS =================
const ENABLE_BREAK_EVEN_RULE = false;
const BREAK_EVEN_TRIGGER_R = 0.50;
const BREAK_EVEN_LOCK_R = 0.02;

// ================= POST EXIT MONITOR CONSTANTS =================
const POST_EXIT_MONITOR_MS = envInt("TS_POST_EXIT_MONITOR_MS", 6 * 60 * 60 * 1000);
const POST_EXIT_MAX_MONITOR_ROWS = envInt("TS_POST_EXIT_MAX_MONITOR_ROWS", 300);
const POST_EXIT_MAX_MONITOR_PER_RUN = envInt("TS_POST_EXIT_MAX_MONITOR_PER_RUN", 20);

const TP_FOLLOW_THROUGH_R = 0.35;
const TP_BIG_FOLLOW_THROUGH_R = 0.75;

const SL_RECOVERY_HALF_R = 0.50;
const SL_RECOVERY_ONE_R = 1.00;
const SL_DEEP_ADVERSE_R = -1.25;

// ================= SHADOW OUTCOME CONSTANTS =================
const SHADOW_DUPLICATE_COOLDOWN_MS = envInt(
  "TS_SHADOW_DUPLICATE_COOLDOWN_MS",
  6 * 60 * 60 * 1000
);

const SHADOW_MONITOR_MS = envInt("TS_SHADOW_MONITOR_MS", 6 * 60 * 60 * 1000);
const SHADOW_DIRECTIONAL_WIN_PCT = 0.30;
const SHADOW_DIRECTIONAL_LOSS_PCT = -0.30;

// ================= FEATURE OPTIMIZER CONSTANTS =================
const BEST_SETUP_MIN_SAMPLE_LOW = 5;
const BEST_SETUP_MIN_SAMPLE_MEDIUM = 12;
const BEST_SETUP_MIN_SAMPLE_HIGH = 30;

const BEST_SETUP_MIN_WINRATE = 0.52;
const BEST_SETUP_MIN_AVG_R = 0.05;

// ================= FINAL FILTER DECISION CONSTANTS =================
const FINAL_DECISION_MIN_COMPLETED = 20;
const FINAL_DECISION_TARGET_COMPLETED = 100;
const FINAL_DECISION_TOP_N = 20;

// ================= NEW EXPOSURE/CORRELATION CAPS =================
const MAX_OPEN_POSITIONS_TOTAL = 5;
const MAX_OPEN_POSITIONS_SAME_SIDE = 3;
const MAX_COUNTER_BTC_OPEN_POSITIONS = 1;

// ================= PATCH 3: nieuwe constante voor MID RSI continuation discount =================
const MID_RSI_CONTINUATION_RR_DISCOUNT = 0.05;

// ================= V12 QUALITY GATES =================
const ENABLE_ENTRY_QUALITY_GATE_V12 = false;   // keep false for discovery
...
const ENABLE_EARLY_FAILURE_EXIT = false;      // was true -> false
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    // Use classifyBtcState if raw BTC data is available, otherwise fallback to old logic
