...
async function finalizeResult(actions, candidates, btcState, runId, options = {}) {
  ...
  const webhookActions = finalActions
    .filter(action => {
      ...
      if (reason === "WEEKLY_ROTATION_ENTRY_MICRO_FAMILY_MISSING") return false;
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        // Verrijk het signaal met micro‑families (geen parent/sub fallback)
        const rotationReadySignal = attachMicroRotationKeys(entryPayload, {
          weeklyRotation: options.weeklyRotation,
          rotation: options.rotation,
          rotationState: options.rotationState,
