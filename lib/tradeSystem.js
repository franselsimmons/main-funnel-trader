...
function normalizeBitgetSymbol(raw){
  ...
  // zorg dat het eindigt op USDT of USDC
...
async function fetchCandles(symbol, timeframe = "1h", limit = 150){
  ...
  // ---- BITGET (primary) ----
  const bitgetUrl = `https://api.bitget.com/api/v2/mix/market/candles?symbol=${clean}&productType=USDT-FUTURES&granularity=${granularity}&limit=${limit}`;

  try{
    const res = await fetch(bitgetUrl);

    if(res.status === 429){
      // rate limit -> fallback naar Binance
    ...
    // bitget error (incl 429) -> fallback
    console.log("⚠️ Bitget fallback → Binance:", clean, e.message);
  }

  // ---- BINANCE FALLBACK (Futures) ----
  try{
    const binanceTf = timeframe === "1h" ? "1h" : (timeframe === "5m" ? "5m" : "15m");
    const url = `https://fapi.binance.com/fapi/v1/klines?symbol=${clean}&interval=${binanceTf}&limit=${limit}`;
    ...
    console.log("❌ Binance fallback fail:", clean, e.message);
  }
...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}){
  ...
  for(let i = 0; i < candidates.length; i += BATCH_SIZE){
    ...
    await Promise.all(batch.map(async (c) => {
      ...
      // candles (RSI + structure)
      try{
        const rawSymbol = c.rawBitgetSymbol || symbol;
        const pair = normalizeBitgetSymbol(rawSymbol);
        ...
        if(c15m.length === 0 || c1h.length === 0){
          console.log("🚨 EMPTY CANDLES:", pair);
        }

        rsiMap[symbol] = {
          m15: getAdvancedRSIContext(c15m),
          h1: getAdvancedRSIContext(c1h),
          structure: getStructureState(c5m)
        };
      }catch(e){
        console.log("❌ RSI MAP FAIL:", symbol, e.message);
        rsiMap[symbol] = null;
      }
    }));
