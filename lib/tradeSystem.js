...
// ================= STRATEGY VERSION =================
const STRATEGY_VERSION = "TS_V12_0_LOSER_FILTER_QUALITY_GATE";
...
// ================= EINDE DEEL 2/3 =================
// ================= ENTRY / EXIT ACTION BUILDERS =================
function buildWaitAction(c, reason, extra = {}) {
  return {
    symbol: c?.symbol || "UNKNOWN",
    rawBitgetSymbol: c?.rawBitgetSymbol || c?.contractSymbol || null,
    side: c?.side || "unknown",
    ...
    score: Number(c?.moveScore || c?.score || 0),
...
function buildHoldAction(position, price, reason, extra = {}) {
  return {
    symbol: position.symbol,
    rawBitgetSymbol: position.rawBitgetSymbol || position.symbol,
    side: position.side,
    ...
    setupClass: position.setupClass || "UNKNOWN",
    entryReason: position.entryReason || position.reason || "UNKNOWN",

    entry: Number(position.entry || 0),
    sl: Number(position.sl || 0),
    initialSl: Number(position.initialSl || position.sl || 0),
    tp: Number(position.tp || 0),
    price: Number(price || 0),

    rr: Number(position.rr || 0),
    currentR: Number(Number(position.currentR || 0).toFixed(3)),
    mfeR: Number(Number(position.mfeR || 0).toFixed(3)),
    maeR: Number(Number(position.maeR || 0).toFixed(3)),

    maxTpProgress: Number(Number(position.maxTpProgress || 0).toFixed(3)),
    maxSlProgress: Number(Number(position.maxSlProgress || 0).toFixed(3)),
    ...
    breakEvenSl: position.breakEvenSl ?? null,

    ticksObserved: Number(position.ticksObserved || 0),
    ...
    ts: Date.now(),
    strategyVersion: position.strategyVersion || STRATEGY_VERSION,

    ...extra
  };
}

function buildExitAction(position, reason, price, extra = {}) {
  return {
    symbol: position.symbol,
    rawBitgetSymbol: position.rawBitgetSymbol || position.symbol,
    side: position.side,
...
function buildEntryAction(position, extra = {}) {
  return {
    symbol: position.symbol,
    rawBitgetSymbol: position.rawBitgetSymbol || position.symbol,
    side: position.side,
    ...
    action: "ENTRY",
    reason: position.entryReason || position.reason || "UNKNOWN",

    setupClass: position.setupClass || "UNKNOWN",
    grade: position.grade || "N/A",
    gradePoints: Number(position.gradePoints || 0),
    recommendedRisk: position.recommendedRisk || "watch",
    ...
    rr: Number(position.rr || 0),
    baseRR: Number(position.baseRR || 0),
    finalRr: Number(position.rr || 0),
    plannedRR: Number(position.rr || 0),
    tpRewardMultiplier: Number(position.tpRewardMultiplier || 1),

    score: Number(position.score || position.moveScore || 0),
    moveScore: Number(position.score || position.moveScore || 0),
    confluence: Number(position.confluence || 0),

    sniper: position.sniper || "UNKNOWN",
    sniperScore: Number(position.sniperScore || 0),
...
function getPositionKey(symbol, side) {
  return `${normalizeBaseSymbol(symbol)}_${String(side || "").toLowerCase()}`;
}

function getCandidatePositionKey(c) {
  return getPositionKey(c?.symbol, c?.side);
}

function getPositionByCandidate(c) {
  return memory.get(getCandidatePositionKey(c));
}

function setPosition(position) {
  if (!position?.symbol || !position?.side) return null;
...
function deletePosition(position) {
  if (!position?.symbol || !position?.side) return false;

  const key = getPositionKey(position.symbol, position.side);
  return memory.delete(key);
}

function hasOpenPositionForCandidate(c) {
  return memory.has(getCandidatePositionKey(c));
}
...
// ================= SAFE MARKET DATA WRAPPERS =================
async function fetchSafeOrderBookAnalysis(contractSymbol) {
  const symbol = normalizeBitgetSymbol(contractSymbol);
...
async function fetchSafeFunding(contractSymbol) {
  const symbol = normalizeBitgetSymbol(contractSymbol);
  if (!symbol) return { rate: 0, fetchFailed: true };
  ...
  try {
    ...
    if (Number.isFinite(Number(raw?.rate))) {
      return {
        ...raw,
...
async function fetchSafeRsiData(contractSymbol) {
  const symbol = normalizeBitgetSymbol(contractSymbol);
  if (!symbol) return null;
    ...
    const cached = await cachedFetch(
      ...
      async () => {
        const [candles15m, candles1h, candles4h] = await Promise.all([
          fetchCandles(symbol, "15m", RSI_CANDLE_LIMIT),
          fetchCandles(symbol, "1H", RSI_CANDLE_LIMIT),
          fetchCandles(symbol, "4H", RSI_CANDLE_LIMIT)
        ]);
        ...
        let externalMtf = null;

        try {
          if (typeof getMTFRSI === "function") {
            externalMtf = await getMTFRSI(symbol, {
              candles15m,
              candles1h,
              candles4h
            });
        ...
        const mtf = mergeMtfRsi(externalMtf, fallbackMtf);
...
async function fetchSafeFlow(c, contractSymbol) {
  try {
    const value = await callFirstValid(
      "detectFlow",
      [
        () => detectFlow(c, contractSymbol),
        () => detectFlow(c),
        () => detectMarketFlow(c),
        () => detectMarketFlow(contractSymbol)
      ],
    ...
    if (value && typeof value === "object") {
      return {
        type: String(value.type || value.flow || c.flow || "UNKNOWN").toUpperCase(),
        score: Number(value.score || value.flowScore || 0),
        ...value
      };
    }

    return {
      type: String(value || c.flow || "UNKNOWN").toUpperCase(),
      score: Number(c.flowScore || 0)
    };
  } catch {
    return {
      type: String(c?.flow || "UNKNOWN").toUpperCase(),
      score: Number(c?.flowScore || 0)
    };
  }
}

async function fetchSafeLiquidationData(contractSymbol, c) {
  try {
    const liquidation = await callFirstValid(
      "getLiquidationData",
      [
        () => getLiquidationData(contractSymbol, c.price, c.side),
        () => getLiquidationData(contractSymbol, c.price),
        () => getLiquidationData(c.symbol, c.price),
        () => getLiquidationData(c)
      ],
      value => value !== null && value !== undefined,
      null
    );
...
function resolveContractSymbol(c) {
  return normalizeBitgetSymbol(
    c?.rawBitgetSymbol ||
    c?.contractSymbol ||
    c?.bitgetSymbol ||
    c?.symbol
...
// ================= EXIT ENGINE =================
function getExitTrigger(position, price, isBull) {
  const currentPrice = Number(price || 0);
  const sl = Number(position?.sl || 0);
  const tp = Number(position?.tp || 0);

  if (!currentPrice || !sl || !tp) {
    return null;
  }
  ...
  if (slHit) {
    return {
      reason: position?.breakEvenActivated ? "BE_SL" : "SL",
      triggerPrice: sl,
      executionPrice: currentPrice
    };
  }
...
function shouldForceTimeExit(position) {
  if (!ENABLE_MAX_HOLD_TIME_EXIT) return false;

  const createdAt = Number(position?.createdAt || 0);
  if (!createdAt) return false;

  const ageMs = Date.now() - createdAt;
  if (ageMs < MAX_HOLD_TIME_MS) return false;

  const currentR = Number(position?.currentR || 0);
  const mfeR = Number(position?.mfeR || 0);

  if (currentR >= MAX_HOLD_TIME_MIN_R_KEEP) return false;
  if (mfeR >= MAX_HOLD_TIME_MIN_MFE_KEEP) return false;

  return true;
}

function shouldTrailAfterNearTp(position, isBull) {
  if (!ENABLE_NEAR_TP_TRAIL) return false;
  if (!position?.nearTpSeen) return false;
  if (position?.nearTpTrailActivated) return false;

  const entry = Number(position.entry || 0);
  const initialSl = getInitialRiskSl(position);
  const currentSl = Number(position.sl || 0);

  if (!entry || !initialSl || !currentSl) return false;

  const riskDist = Math.abs(entry - initialSl);
  if (!riskDist) return false;

  const trailSl = isBull
    ? entry + riskDist * NEAR_TP_TRAIL_LOCK_R
    : entry - riskDist * NEAR_TP_TRAIL_LOCK_R;
  ...
  position.nearTpTrailActivated = true;
  position.nearTpTrailAt = Date.now();
  position.slBeforeNearTpTrail = currentSl;
  position.nearTpTrailSl = Number(trailSl);
  position.sl = Number(trailSl);

  return true;
}

async function processOpenPosition(position) {
  const isBull = String(position.side || "").toLowerCase() === "bull";
  const contractSymbol = resolveContractSymbol(position);

  const ob = await fetchSafeOrderBookAnalysis(contractSymbol);
  const price = Number(ob?.mid || position.lastPrice || position.entry || 0);

  if (!price) {
    return buildHoldAction(position, 0, "NO_PRICE_FOR_OPEN_POSITION");
  }

  updatePositionPathMetrics(position, price, isBull);
  applyBreakEvenRule(position, isBull);
  shouldTrailAfterNearTp(position, isBull);

  const earlyFailure = evaluateEarlyFailureExit(position, ob);

  if (earlyFailure.exit) {
    const exitPayload = buildExitAction(position, earlyFailure.reason, price, {
      triggerPrice: price,
      executionPrice: price,
      obBias: ob?.bias || "UNKNOWN",
      spreadPct: normalizeSpread(ob?.spreadPct),
      depthMinUsd1p: Number(ob?.depthMinUsd1p || 0)
    });

    recordExit(exitPayload, position, price, isBull);
    deletePosition(position);

    return exitPayload;
  }

  const trigger = getExitTrigger(position, price, isBull);

  if (trigger) {
    const exitPayload = buildExitAction(position, trigger.reason, trigger.executionPrice, {
    ...
    recordExit(exitPayload, position, trigger.executionPrice, isBull);
    deletePosition(position);

    return exitPayload;
  }

  if (shouldForceTimeExit(position)) {
    const exitPayload = buildExitAction(position, "TIME_EXIT_NO_FOLLOW_THROUGH", price, {
      triggerPrice: price,
      executionPrice: price,
      obBias: ob?.bias || "UNKNOWN",
      spreadPct: normalizeSpread(ob?.spreadPct),
  ...
  return buildHoldAction(position, price, "POSITION_OPEN", {
    obBias: ob?.bias || position.obBias || "UNKNOWN",
    spreadPct: normalizeSpread(ob?.spreadPct || position.spreadPct),
    depthMinUsd1p: Number(ob?.depthMinUsd1p || position.depthMinUsd1p || 0)
  });
}

// ================= ENTRY FUNNEL =================
function validateCandidateBase(c, btcState) {
  if (!c?.symbol) {
    return {
      ok: false,
      reason: "NO_SYMBOL"
  ...
  const score = Number(c.moveScore || c.score || 0);

  if (score < CANDIDATE_MIN_SCORE) {
    return {
      ok: false,
      reason: "LOW_CANDIDATE_SCORE",
      requiredScore: CANDIDATE_MIN_SCORE,
      score
    };
  }
  ...
  if (
    side === "bull" &&
    ["BEARISH", "STRONG_BEAR"].includes(String(btcState || "").toUpperCase()) &&
    score < BTC_BEARISH_LONG_MIN_SCORE
  ) {
    return {
      ok: false,
      reason: "BTC_BEARISH_LONG_LOW_SCORE",
      requiredScore: BTC_BEARISH_LONG_MIN_SCORE,
      score
    };
  }

  if (
    String(btcState || "").toUpperCase() === "NEUTRAL" &&
    score < BTC_NEUTRAL_MIN_SCORE
  ) {
    return {
      ok: false,
      reason: "BTC_NEUTRAL_LOW_SCORE",
      requiredScore: BTC_NEUTRAL_MIN_SCORE,
      score
    };
  }

  if (Number(c.tfStrength || 0) < TF_MIN_STRENGTH) {
    return {
      ok: false,
      reason: "LOW_TF_STRENGTH",
      requiredTfStrength: TF_MIN_STRENGTH,
      tfStrength: Number(c.tfStrength || 0)
    };
  }
...
function validateMarketQuality({
  c,
  ob,
  funding,
  confluence,
  ...
  if (ob?.spoof) {
    return {
      ok: false,
      reason: "ORDERBOOK_SPOOF_DETECTED"
    };
  }
  ...
  if (depthMinUsd1p > 0 && depthMinUsd1p < MIN_DEPTH_USD_1P_ABSOLUTE) {
    return {
      ok: false,
      reason: "DEPTH_BELOW_ABSOLUTE_MIN",
      depthMinUsd1p,
      requiredDepth: MIN_DEPTH_USD_1P_ABSOLUTE
    };
  }

  if (depthMinUsd1p > 0 && depthMinUsd1p < MIN_DEPTH_USD_1P) {
    const aDepthException =
      depthMinUsd1p >= A_MIN_DEPTH_USD_1P &&
      Number(confluence || 0) >= A_MIN_CONFLUENCE &&
      Number(sniperScore || 0) >= A_MIN_SNIPER;
    ...
    if (!aDepthException) {
      return {
        ...
        depthMinUsd1p,
        requiredDepth: MIN_DEPTH_USD_1P
      };
    }
  }
  ...
  if (
    side === "bull" &&
    ["TREND", "RUNNING", "BREAKOUT"].includes(String(c.flow || "").toUpperCase()) &&
    depthMinUsd1p > 0 &&
    depthMinUsd1p < BULL_TREND_MIN_DEPTH_USD_1P
  ) {
    return {
      ok: false,
      reason: "BULL_TREND_DEPTH_TOO_LOW",
      depthMinUsd1p,
      requiredDepth: BULL_TREND_MIN_DEPTH_USD_1P
    };
  }

  if (
    Math.abs(Number(funding?.rate || 0)) > EXTREME_FUNDING_ABS_MAX
  ) {
    return {
      ok: false,
  ...
  if (
    ...
    Number(confluence || 0) < CROWDED_FUNDING_MIN_CONFLUENCE
  ) {
    return {
      ok: false,
      reason: "BULL_CROWDED_FUNDING"
    };
  }
  ...
  if (obAgainst && Number(confluence || 0) < OB_AGAINST_MIN_CONFLUENCE) {
    return {
      ok: false,
      reason: "OB_AGAINST_LOW_CONFLUENCE",
      obBias,
      requiredConfluence: OB_AGAINST_MIN_CONFLUENCE,
      confluence
    };
  }

  if (
    (obBias === "NEUTRAL" || obBias === "UNKNOWN") &&
    setupClass === "A" &&
    (
      Number(confluence || 0) < NEUTRAL_OB_A_EXCEPTION_MIN_CONFLUENCE ||
      Number(sniperScore || 0) < NEUTRAL_OB_A_EXCEPTION_MIN_SNIPER ||
      Number(rr || 0) < NEUTRAL_OB_A_EXCEPTION_MIN_RR
    )
  ) {
    return {
      ok: false,
      reason: "NEUTRAL_OB_A_EXCEPTION_FAILED"
    };
  }
  ...
  if (
    ...
    setupClass === "B" &&
    (
      Number(confluence || 0) < NEUTRAL_OB_B_EXCEPTION_MIN_CONFLUENCE ||
      Number(sniperScore || 0) < NEUTRAL_OB_B_EXCEPTION_MIN_SNIPER ||
      Number(rr || 0) < NEUTRAL_OB_B_EXCEPTION_MIN_RR ||
      Number(c.moveScore || 0) < NEUTRAL_OB_B_EXCEPTION_MIN_SCORE
    )
  ) {
    return {
      ok: false,
      reason: "NEUTRAL_OB_B_EXCEPTION_FAILED"
    };
  }

  return {
    ok: true,
    reason: "MARKET_QUALITY_OK"
  };
}

function validateRsiEntry({
  c,
  rsiSignal,
  rsiZone,
  rsiEdge,
  ...
  if (side === "bull" && String(rsiZone).startsWith("UPPER")) {
    return {
      ok: false,
      reason: "RSI_LONG_TOO_HIGH"
    };
  }

  if (
    side === "bear" &&
    SHORT_BLOCKED_RSI_ZONES.has?.(String(rsiZone).toUpperCase())
  ) {
    return {
      ok: false,
      reason: "RSI_SHORT_TOO_LOW_A_ONLY"
    };
  }

  if (
    side === "bear" &&
    String(rsiZone).toUpperCase() === "LOWER_1" &&
    !SHORT_LOWER1_ALLOWED_BTC_STATES.has?.(String(btcState || "").toUpperCase())
  ) {
    return {
      ok: false,
      reason: "RSI_SHORT_LOWER1_BTC_STATE_BLOCK"
    };
  }
  ...
  if (
    ...
    String(rsiZone).toUpperCase() === "LOWER_1" &&
    (
      Number(confluence || 0) < SHORT_LOWER1_CONTINUATION_MIN_CONFLUENCE ||
      Number(sniperScore || 0) < SHORT_LOWER1_CONTINUATION_MIN_SNIPER ||
      Number(rr || 0) < SHORT_LOWER1_CONTINUATION_MIN_RR
    )
  ) {
    return {
      ok: false,
      reason: "RSI_SHORT_LOWER1_WEAK_CONTINUATION"
    };
  }

  if (
    side === "bull" &&
    String(rsiZone).toUpperCase() === "LOWER_2" &&
    Number(c.change1h || 0) > LONG_LOWER2_MAX_1H_CHANGE
  ) {
    return {
      ok: false,
      reason: "LONG_LOWER2_TOO_EXTENDED_1H"
    };
  }

  if (
    side === "bear" &&
    String(rsiZone).toUpperCase() === "UPPER_2" &&
    Number(c.change1h || 0) < SHORT_UPPER2_MIN_1H_CHANGE
  ) {
    return {
      ok: false,
      reason: "SHORT_UPPER2_NOT_WEAK_ENOUGH_1H"
    };
  }

  if (
    String(rsiZone).toUpperCase() === "MID" &&
    Number(confluence || 0) < MID_RSI_MIN_CONFLUENCE
  ) {
    return {
      ok: false,
      reason: side === "bull" ? "RSI_LONG_NO_EDGE" : "RSI_SHORT_NO_EDGE",
      requiredConfluence: MID_RSI_MIN_CONFLUENCE,
      confluence
    };
  }

  if (Number(rsiEdge?.rank || 0) < 0) {
    return {
      ok: false,
      reason: side === "bull" ? "RSI_LONG_NO_EDGE" : "RSI_SHORT_NO_EDGE"
    };
...
function buildBullishMidTrendProbe({
  c,
  btcState,
  rsiZone,
  confluence,
  ...
  if (!ENABLE_BULLISH_MID_TREND_PROBES) {
    return {
      ok: false,
      reason: "BULLISH_MID_TREND_PROBE_DISABLED"
    };
  }

  if (String(c.side || "").toLowerCase() !== "bull") {
    return {
      ok: false,
      reason: "NOT_BULL"
    };
  ...
  if (String(rsiZone || "").toUpperCase() !== "MID") {
    return {
      ok: false,
      reason: "RSI_NOT_MID"
    };
  ...
  if (
    BULLISH_MID_TREND_PROBE_REQUIRE_BTC_BULLISH &&
    !["BULLISH", "STRONG_BULL"].includes(String(btcState || "").toUpperCase())
  ) {
    return {
      ok: false,
  ...
  if (String(c.flow || "").toUpperCase() !== "TREND") {
    return {
      ok: false,
      reason: "FLOW_NOT_TREND"
    };
  }

  if (Number(confluence || 0) < BULLISH_MID_TREND_PROBE_MIN_CONFLUENCE) {
    return {
      ok: false,
      reason: "PROBE_LOW_CONFLUENCE"
    };
  }

  if (Number(sniperScore || 0) < BULLISH_MID_TREND_PROBE_MIN_SNIPER) {
    return {
      ok: false,
      reason: "PROBE_LOW_SNIPER"
    };
  }

  if (Number(rr || 0) < BULLISH_MID_TREND_PROBE_MIN_RR) {
    return {
      ok: false,
      reason: "PROBE_LOW_RR"
    };
  ...
  if (normalizeSpread(ob?.spreadPct) > BULLISH_MID_TREND_PROBE_MAX_SPREAD_PCT) {
    return {
      ok: false,
      reason: "PROBE_SPREAD_TOO_WIDE"
    };
  }

  if (Number(ob?.depthMinUsd1p || 0) < BULLISH_MID_TREND_PROBE_MIN_DEPTH_USD_1P) {
    return {
      ok: false,
      reason: "PROBE_DEPTH_TOO_LOW"
    };
  }

  if (Number(c.change1h || 0) < BULLISH_MID_TREND_PROBE_MIN_1H_MOVE_PCT) {
    return {
      ok: false,
      reason: "PROBE_1H_MOVE_TOO_LOW"
    };
  }

  if (Number(c.change24 || 0) < BULLISH_MID_TREND_PROBE_MIN_24H_MOVE_PCT) {
    return {
      ok: false,
      reason: "PROBE_24H_MOVE_TOO_LOW"
    };
  }

  if (
    Number(c.distanceFromLocalHighPct || c.pullbackFromHighPct || 0) <
    BULLISH_MID_TREND_PROBE_MIN_PULLBACK_FROM_HIGH_PCT
  ) {
    return {
      ok: false,
      reason: "PROBE_PULLBACK_TOO_SMALL"
    };
  }

  return {
    ok: true,
    reason: "BULLISH_MID_TREND_PROBE_OK"
  };
}

function buildBtcBullishBearException({
  c,
  btcState,
  confluence,
  sniperScore,
  ...
  if (!ENABLE_BTC_BULLISH_BEAR_EXCEPTION) {
    return {
      ok: false,
      reason: "BTC_BULLISH_BEAR_EXCEPTION_DISABLED"
    };
  }

  if (String(c.side || "").toLowerCase() !== "bear") {
    return {
      ok: false,
      reason: "NOT_BEAR"
    };
  ...
  if (!["BULLISH", "STRONG_BULL"].includes(String(btcState || "").toUpperCase())) {
    return {
      ok: false,
      reason: "BTC_NOT_BULLISH"
    };
  ...
  if (Number(ob?.depthMinUsd1p || 0) < BTC_BULLISH_BEAR_EXCEPTION_MIN_DEPTH_USD_1P) {
    return {
      ok: false,
      reason: "BTC_BULL_BEAR_DEPTH_TOO_LOW"
    };
  }

  if (normalizeSpread(ob?.spreadPct) > BTC_BULLISH_BEAR_EXCEPTION_MAX_SPREAD_PCT) {
    return {
      ok: false,
      reason: "BTC_BULL_BEAR_SPREAD_TOO_WIDE"
    };
  }

  if (Number(rr || 0) < BTC_BULLISH_BEAR_EXCEPTION_MIN_RR) {
    return {
      ok: false,
      reason: "BTC_BULL_BEAR_RR_TOO_LOW"
    };
  }

  if (
    Number(confluence || 0) < BTC_BULLISH_BEAR_EXCEPTION_MIN_CONF ||
    Number(confluence || 0) > BTC_BULLISH_BEAR_EXCEPTION_MAX_CONF
  ) {
    return {
      ok: false,
      reason: "BTC_BULL_BEAR_CONF_OUT_OF_RANGE"
    };
  }

  if (Number(sniperScore || 0) < BTC_BULLISH_BEAR_EXCEPTION_MIN_SNIPER) {
    return {
      ok: false,
      reason: "BTC_BULL_BEAR_SNIPER_TOO_LOW"
    };
  }
...
function getRsiEdge({ side, rsi, rsiHTF, rsiZone, rsiSignal }) {
  const s = String(side || "").toLowerCase();
  const zone = String(rsiZone || "UNKNOWN").toUpperCase();

  let rank = 0;

  if (s === "bull") {
    if (zone === "LOWER_3") rank = 3;
    else if (zone === "LOWER_2") rank = 2;
    else if (zone === "LOWER_1") rank = 1;
    else if (zone === "MID") rank = rsiSignal?.continuationScore > 0 ? 0 : -1;
    else rank = -2;
  }

  if (s === "bear") {
    if (zone === "UPPER_3") rank = 3;
    else if (zone === "UPPER_2") rank = 2;
    else if (zone === "UPPER_1") rank = 1;
    else if (zone === "MID") rank = rsiSignal?.continuationScore > 0 ? 0 : -1;
    else rank = -2;
  }

  const slope = Number(rsiSignal?.slope3 || 0);

  const continuationOk =
    s === "bull"
      ? Number(rsi || 0) >= 45 && Number(rsiHTF || 0) >= 45 && slope >= 0
      : Number(rsi || 0) <= 55 && Number(rsiHTF || 0) <= 55 && slope <= 0;

  if (rank === 0 && !continuationOk) {
    rank = -1;
  }
...
function getRsiZoneFromSignal(rsiSignal, isBull) {
  if (typeof classifyRsiZone === "function") {
    try {
      return classifyRsiZone(rsiSignal.rsi, isBull, rsiSignal.zones);
    } catch {}
  }

  const rsi = Number(rsiSignal?.rsi || 50);
  const zones = rsiSignal?.zones || getDefaultRsiZones();

  if (rsi <= Number(zones.L3 || 25)) return "LOWER_3";
  if (rsi <= Number(zones.L2 || 35)) return "LOWER_2";
  if (rsi <= Number(zones.L1 || 45)) return "LOWER_1";
  if (rsi >= Number(zones.U3 || 75)) return "UPPER_3";
  if (rsi >= Number(zones.U2 || 65)) return "UPPER_2";
  if (rsi >= Number(zones.U1 || 55)) return "UPPER_1";

  return "MID";
}

function calculateFinalTpAndRr({ risk, isBull, setupClass }) {
  const entry = Number(risk.entry || 0);
  const sl = Number(risk.sl || 0);
  const tp = Number(risk.tp || 0);

  const baseRR = calculateRRFromPrices(entry, sl, tp, isBull);

  if (!entry || !sl || !tp || !baseRR) {
    return {
      finalTp: tp,
      finalRr: 0,
      baseRR: 0,
  ...
  if (setupClass === "GOD" || setupClass === "A") {
    rewardMultiplier = A_GOD_MAX_TP_REWARD_MULTIPLIER;
  }

  if (setupClass === "B" || setupClass === "B_TREND_PROBE") {
    rewardMultiplier = B_MAX_TP_REWARD_MULTIPLIER || 1;
  }

  const cappedRr = Math.min(baseRR * rewardMultiplier, baseRR);

  const finalTp = isBull
    ? entry + riskDist * cappedRr
    : entry - riskDist * cappedRr;

  return {
    finalTp: Number(finalTp),
    finalRr: Number(cappedRr.toFixed(3)),
    baseRR: Number(baseRR.toFixed(3)),
    tpRewardMultiplier: Number(rewardMultiplier)
  };
}

// ================= CANDIDATE DEEP PROCESSOR =================
async function processCandidate(c, context) {
  const btcState = context.btcState;
  const regimeLevel = context.regimeLevel || c.regime || "UNKNOWN";
  const runId = context.runId;

  const base = validateCandidateBase(c, btcState);

  if (!base.ok) {
    return buildWaitAction(c, base.reason, base);
  }

  if (hasOpenPositionForCandidate(c)) {
    const pos = getPositionByCandidate(c);
    return processOpenPosition(pos);
  }

  if (!ALLOW_OPPOSITE_SIDE_SAME_SYMBOL && hasAnyOpenPositionForSymbol(c.symbol)) {
    return buildWaitAction(c, "SYMBOL_ALREADY_HAS_OPEN_POSITION");
  }

  const side = String(c.side || "").toLowerCase();
  const isBull = side === "bull";
  const contractSymbol = resolveContractSymbol(c);

  const [ob, funding, rsiData, flow, liquidity, liquidation] = await Promise.all([
    fetchSafeOrderBookAnalysis(contractSymbol),
    fetchSafeFunding(contractSymbol),
    fetchSafeRsiData(contractSymbol),
    fetchSafeFlow(c, contractSymbol),
    getSafeLiquidityZones({ contractSymbol, c }),
    fetchSafeLiquidationData(contractSymbol, c)
  ]);

  c.flow = flow?.type || c.flow || "UNKNOWN";
  ...
  const rsi = Number(rsiSignal.rsi || 50);
  const rsiHTF = Number(rsiSignal.rsiHTF || rsi);
  const rsiZone = getRsiZoneFromSignal(rsiSignal, isBull);

  const rsiEdge = getRsiEdge({
    side,
    rsi,
    rsiHTF,
    rsiZone,
  ...
  if (!isValidRiskGeometry(risk, isBull)) {
    return buildWaitAction(c, "INVALID_RISK_GEOMETRY", {
      rsi,
      rsiHTF,
      rsiZone,
      obBias: ob?.bias || "UNKNOWN",
      ...
      depthMinUsd1p: Number(ob?.depthMinUsd1p || 0),
      sniperScore: Number(sniper?.score || 0),
      error: "risk geometry invalid"
    });
  }

  const rrRaw = calculateRRFromPrices(
    Number(risk.entry),
    Number(risk.sl),
    Number(risk.tp),
    isBull
  ...
  const rsiRrCredit = getRsiSetupRrCredit(rsiEdge);
  const rr = Number((rrRaw + rsiRrCredit).toFixed(3));

  const confluence = await getSafeConfluence({
    c,
    flow,
    sniper,
    ob,
  ...
  const rsiValidation = validateRsiEntry({
    c,
    rsiSignal,
    rsiZone,
    rsiEdge,
  ...
  const probe = buildBullishMidTrendProbe({
    c,
    btcState,
    rsiZone,
    confluence,
  ...
  const btcBullishBearException = buildBtcBullishBearException({
    c,
    btcState,
    confluence,
    sniperScore,
  ...
  c._debugBullishMidTrendProbe = probe.ok;
  c._debugBullishMidTrendProbeReason = probe.reason;

  c._debugBtcBullishBearException = btcBullishBearException.ok;
  c._debugBtcBullishBearExceptionReason = btcBullishBearException.reason;

  if (!rsiValidation.ok && !probe.ok && !btcBullishBearException.ok) {
    return buildWaitAction(c, rsiValidation.reason, {
      ...rsiValidation,
      rsi,
      rsiHTF,
      rsiZone,
      rsiEdge,
      confluence,
      rawConfluence: c._debugExternalConfluence,
      externalConfluence: c._debugExternalConfluence,
      fallbackConfluence: c._debugFallbackConfluence,
      confluenceBlendUsed: c._debugConfluenceBlendUsed,
      confluenceSource: c._debugConfluenceSource,
      sniperScore,
      rawSniperScore: sniper?.rawSniperScore,
      fallbackSniperScore: sniper?.fallbackSniperScore,
      sniperBlendUsed: sniper?.sniperBlendUsed,
      rr,
      plannedRR: rr,
      obBias: ob?.bias || "UNKNOWN",
      spreadPct: normalizeSpread(ob?.spreadPct),
  ...
  const setup = inferSetupClass({
    ...
    rr,
    rsiEdge,
    bullishMidTrendProbe: probe.ok,
    btcBullishBearException: btcBullishBearException.ok
  });

  if (!setup) {
    return buildWaitAction(c, "NO_SETUP_CLASS", {
      rsi,
      rsiHTF,
      rsiZone,
      rsiEdge,
  ...
  const marketQuality = validateMarketQuality({
    c,
    ob,
    funding,
    confluence,
  ...
  const finalGeometry = calculateFinalTpAndRr({
    risk,
    isBull,
    setupClass: setup.setupClass
  });

  if (finalGeometry.finalRr < setup.requiredRR) {
    return buildWaitAction(c, "LOW_FINAL_RR", {
      setupClass: setup.setupClass,
      requiredRR: setup.requiredRR,
      rr,
      finalRr: finalGeometry.finalRr,
      baseRR: finalGeometry.baseRR,
      rsi,
      rsiHTF,
      rsiZone,
      confluence,
  ...
  const setupGrade = typeof gradeSetup === "function"
    ? gradeSetup({
        c,
        setupClass: setup.setupClass,
        confluence,
        sniperScore,
        rr: finalGeometry.finalRr,
        ob,
        rsiZone,
        btcState,
        flow
    ...
    : {
        grade: setup.setupClass === "GOD" || setup.setupClass === "A" ? "A" : "B",
        points: confluence,
        recommendedRisk: setup.setupClass === "GOD" ? "high" : "normal"
      };

  const qualityGate = validateEntryQualityGateV12({
    ...c,
    setupClass: setup.setupClass,
    rsi,
    rsiHTF,
    rsiZone,
    plannedRR: finalGeometry.finalRr,
    confluence,
    sniperScore,
    obBias: ob?.bias || "UNKNOWN",
    spreadPct: normalizeSpread(ob?.spreadPct),
  ...
  if (!qualityGate.ok) {
    return buildWaitAction(c, qualityGate.reason, {
      setupClass: setup.setupClass,
      rsi,
      rsiHTF,
      rsiZone,
  ...
  const confirmationKey = `${normalizeBaseSymbol(c.symbol)}_${side}_${setup.setupClass}`;

  if (requiresEntryConfirmation({
    setupClass: setup.setupClass,
    rsiZone,
    obBias: ob?.bias || "UNKNOWN",
    confluence,
    ...
    if (!hasRecentEntryConfirmation(confirmationKey)) {
      markEntryConfirmation(confirmationKey);

      return buildWaitAction(c, "ENTRY_CONFIRMATION_REQUIRED", {
        setupClass: setup.setupClass,
        rsi,
        rsiHTF,
        rsiZone,
  ...
  const position = buildEntryPosition({
    c,
    risk,
    finalTp: finalGeometry.finalTp,
    finalRr: finalGeometry.finalRr,
    baseRR: finalGeometry.baseRR,
    tpRewardMultiplier: finalGeometry.tpRewardMultiplier,
    setupClass: setup.setupClass,
  ...
  const action = buildEntryAction(position, {
    requiredConfluence: setup.requiredConfluence,
    requiredSniper: setup.requiredSniper,
    requiredRR: setup.requiredRR,
  ...
  recordEntry(action, position);

  return action;
}

// ================= OPEN POSITION REFRESH =================
async function processAllOpenPositions() {
  const positions = Array.from(memory.values());
  ...
  return mapConcurrent(
    positions,
    OPEN_POSITION_CONCURRENCY || 6,
    async position => processOpenPosition(position)
  );
}

// ================= SCAN OBSERVATION SHADOW ROWS =================
function maybeRecordScanObservation({
  c,
  flow,
  sniper,
  confluence,
  ...
  try {
    const row = buildScanObservationRow({
      c,
      flow,
      sniper,
      confluence,
...
async function ensureRuntimeLoaded() {
  ...
  try {
    ...
    console.warn("TS_RUNTIME_LOAD_FAILED", e.message);
  }
}

async function persistRuntimeAfterRun() {
  try {
    trimAuditArrays();
    await saveRuntimeState();
  } catch (e) {
    console.warn("TS_RUNTIME_SAVE_FAILED", e.message);
  }
}

// ================= MAIN TRADE SYSTEM =================
async function analyzeTradeSystem(candidates = [], options = {}) {
  await ensureRuntimeLoaded();

  const runId = options.runId || `ts_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  const btcState = String(options.btcState || options.marketState || "UNKNOWN").toUpperCase();
  const regimeLevel = String(options.regimeLevel || options.regime || "UNKNOWN").toUpperCase();

  const rows = capDeepCandidates(
  ...
  const openActions = await processAllOpenPositions();

  const freshCandidates = rows.filter(c => {
    if (!c?.symbol) return false;

    const key = getCandidatePositionKey(c);
    if (memory.has(key)) return false;

    return true;
  });

  const candidateActions = await mapConcurrent(
    freshCandidates,
    TRADE_SYSTEM_CONCURRENCY || 8,
    async c => {
      try {
        return await processCandidate(c, {
          btcState,
          regimeLevel,
          runId
        });
      } catch (e) {
        return buildWaitAction(c, "PROCESS_CANDIDATE_ERROR", {
          error: e.message
        });
      }
    }
  ...
  await persistRuntimeAfterRun();

  return result;
}

// ================= OPTIONAL ALIASES =================
async function runTradeSystem(candidates = [], options = {}) {
  return analyzeTradeSystem(candidates, options);
}

async function tradeSystem(candidates = [], options = {}) {
  return analyzeTradeSystem(candidates, options);
}
...
module.exports = {
  ...
  processCandidate,
  processOpenPosition,
  processAllOpenPositions,

  buildOptimizerReport,
  buildMfeMaeReport,
  buildPostExitReport,
  buildBestSetupAdvice,
  buildFinalFilterDecision,
  buildAnalyzePayload,

  buildOpenPositionRows,

  getPositionKey,
  getCandidatePositionKey,
