...
function attachRotationFamilyKeys(entryPayload, btcState, runId) {
  ...
  const rotationSide = normalizeRotationSide(
    entryPayload.rotationSide ||
    entryPayload.tradeSide ||
    entryPayload.side
  );

  const microFamilyId = extractMicroFamilyIdFromSignal(entryPayload);
  ...
  return {
    ...
    // Alleen echte microfamily.
    microFamilyId,
    microFamily: microFamilyId,
    microFamilyIds,
    microFamilies: microFamilyIds,

    // Aliases blijven gevuld voor audit/analyse, maar bevatten dezelfde micro-key.
