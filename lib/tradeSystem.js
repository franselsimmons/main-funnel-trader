...
function logAction(actionPayload, regimeLevel, btcState, shouldLog){
  ...
  for(const originalCoin of candidates){
    ...
    if(prev){
      ...
      if(hitPartial && pos.sizeLeft === 1){
        ...
        logAction(partialPayload, regime.level, btcState, shouldLog);
      ...
      if(hitTP || hitSL){
        ...
        if(shouldLog){
          ...
          logAction(exitPayload, regime.level, btcState, true);
        }
      ...
      logAction(runningPayload, regime.level, btcState, shouldLog);
    ...
    if(getOpenTradeCount() >= MAX_OPEN_TRADES){
      ...
      logAction(waitPayload, regime.level, btcState, shouldLog);
    ...
    if(
      ...
      if(notify){
        ...
        try{
          ...
          logAction(entryPayload, regime.level, btcState, shouldLog);
