...
// ================= EXECUTION QUALITY =================
const MAX_SPREAD_PCT = 0.0035; // 0.35%
const MIN_DEPTH_USD_1P = 100000;
const MIN_RR_FLOOR = 0.85;
...
// ================= ORDERBOOK BATCH & RETRY =================
const ORDERBOOK_BATCH_SIZE = 6;
const ORDERBOOK_RETRY_COUNT = 1;
const ORDERBOOK_RETRY_DELAY_MS = 250;

// ================= DEFAULT OB (aangepast met foutcodes) =================
const DEFAULT_OB = {
  mid: 0,
  spreadPct: 0.05,
  depthMinUsd1p: 0,
...
// ================= NIEUWE HELPERS (batchen/retry) =================
function sleep(ms){
  return new Promise(resolve => setTimeout(resolve, ms));
}

function resolveMarketSymbol(c){
  return String(
    c?.exchangeSymbol ||
    c?.marketSymbol ||
    c?.symbol ||
...
function toLinearUsdtSymbol(symbol){
...
function parseOrderbookError(err){
  const message = String(err?.message || "orderbook_fetch_failed");
  const upper = message.toUpperCase();

  let code = "ORDERBOOK_FETCH_FAILED";

  if(upper.includes("429")) code = "HTTP_429";
  else if(upper.includes("404")) code = "HTTP_404";
  else if(upper.includes("401")) code = "HTTP_401";
  else if(upper.includes("403")) code = "HTTP_403";
  else if(upper.includes("EMPTY_BOOK")) code = "EMPTY_BOOK";
  else if(
...
async function fetchOrderBookWithRetry(symbol){
  let lastError = null;

  for(let attempt = 0; attempt <= ORDERBOOK_RETRY_COUNT; attempt++){
    try{
      return await fetchOrderBook(symbol);
    }catch(err){
      lastError = err;

      if(attempt < ORDERBOOK_RETRY_COUNT){
        await sleep(ORDERBOOK_RETRY_DELAY_MS * (attempt + 1));
      }
    }
  }
...
function buildCommonPayload(c, flow, sniper, funding, ob){
  return {
    ...
    obFetchErrorCode: ob?.fetchErrorCode || null,
    obFetchErrorMessage: ob?.fetchErrorMessage || "",
...
export async function processTrades(
  ...
  // ================= BATCHED ORDERBOOK FETCH (aangepast) =================
  await runInBatches(candidates, ORDERBOOK_BATCH_SIZE, async (candidate) => {
    const symbol = String(candidate.symbol || "").toUpperCase();
    const marketSymbol = resolveMarketSymbol(candidate);
    const linearSymbol = toLinearUsdtSymbol(marketSymbol || symbol);

    try{
      const raw = await fetchOrderBookWithRetry(marketSymbol || symbol);
      ...
      const parsed = parseOrderbookError(err);

      obMap[symbol] = {
        ...DEFAULT_OB,
        fetchFailed: true,
        fetchErrorCode: parsed.code,
        fetchErrorMessage: parsed.message
      };
    }

    try{
      fundingMap[symbol] = await fetchFunding(linearSymbol);
    }catch{
  ...
  for(const originalCoin of candidates){
    ...
    // Nieuw: linear symbol voor liquidations
    const linearMarketSymbol = toLinearUsdtSymbol(resolveMarketSymbol(c) || c.symbol);
    ...
    if(ob.fetchFailed){
      const waitPayload = buildWait(
        c,
        `ORDERBOOK_FETCH_FAILED:${ob.fetchErrorCode || "unknown"}`,
        flow,
        sniper,
        0,
        0,
    ...
    // =====================================================
    // MANAGE EXISTING POSITION FIRST (onveranderd)
    // =====================================================
    if(prev){
    ...
    // ================= ENTRY FILTERS (onveranderd) =================
    // ... (de rest van de entry filters blijft hetzelfde; alleen de variabele linearMarketSymbol is toegevoegd)
    // Om de leesbaarheid te behouden, is de rest van de code hieronder identiek aan het origineel,
    // behalve dat we nu linearMarketSymbol gebruiken in de liquidation call (al gebeurd) en
    // de buildWait / entryPayload maken gebruik van de nieuwe buildCommonPayload (met foutcodes).
    // Ik kopieer de bestaande logica vanaf hier.

    if(getOpenTradeCount() >= MAX_OPEN_TRADES){
      const waitPayload = buildWait(c, "MAX_OPEN_TRADES", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    const buildupOK =
      ...
      directionalOb &&
      setupGrade.grade === "A" &&
      Number(c.moveScore || 0) >= BUILDUP_ELITE_MIN_SCORE &&
      c.tfStrength >= BUILDUP_ELITE_MIN_TF &&
      rr >= BUILDUP_ELITE_MIN_RR &&
      confluence >= BUILDUP_ELITE_MIN_CONF &&
      sniper?.valid &&
      getSniperScore(sniper) >= BUILDUP_ELITE_MIN_SNIPER;
...
// Helper functions die nog ontbraken in de originele snippet maar die wel gebruikt worden
