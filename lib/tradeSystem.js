...
const MICRO_FAMILY_SCHEMA_VERSION = "MF_V3_CORE";
...
function enforceCanonicalCoreMicroIdentity(row = {}) {
  ...
  return {
    ...
    // Belangrijk: gate mag alleen deze core-ID zien.
...
function getCanonicalMicroRotationMatch({ signal = {}, rotationIds = [] }) {
  ...
  const activeMicroFamilyIds = uniqIds(rotationIds)
    .filter(isCoreMicroFamilyId);
...
function buildCanonicalRotationDecision({
  ...
  const activeMicroFamilyIds = uniqIds([
...
function enrichRowWithAnalyzerMicroFamily(row = {}) {
  ...
  // Only use the core microFamilyId, no exact or other extras.
  const microFamilyIds = primary?.microFamilyId
    ? [primary.microFamilyId]
    : [];
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        const canonicalEntryFamilyId =
          rotationCheck.decision?.matchedMicroFamilyId ||
          signalForAction.microFamilyId ||
          null;

        // Gebruik het verrijkte signaal voor de verdere bouw.
        // In strict mode moet de opgeslagen entry altijd de matched rotation family dragen.
        Object.assign(entryPayload, {
          ...
          familyId: canonicalEntryFamilyId,
          microFamilyId: canonicalEntryFamilyId,
          analyzerMicroFamilyId: signalForAction.analyzerMicroFamilyId || canonicalEntryFamilyId,

          familyIds: canonicalEntryFamilyId ? [canonicalEntryFamilyId] : [],
          microFamilyIds: canonicalEntryFamilyId ? [canonicalEntryFamilyId] : [],
          families: canonicalEntryFamilyId ? [canonicalEntryFamilyId] : [],
          microFamilies: canonicalEntryFamilyId ? [canonicalEntryFamilyId] : [],
