...
const MIN_RR_FLOOR = 0.80; // verlaagd van 0.85 naar 0.80
...
// ================= NIEUWE HELPERS (batchen/retry/resolve) =================
function sleep(ms){
...
async function fetchOrderBookWithRetry(symbol, productType = "USDT-FUTURES"){
  let lastError = null;

  for(let attempt = 0; attempt <= ORDERBOOK_RETRY_COUNT; attempt++){
    try{
      return await fetchOrderBook(symbol, productType);
    }catch(err){
      lastError = err;
...
export async function processTrades(
  ...
  // ================= BATCHED ORDERBOOK/FUNDING FETCH =================
  await runInBatches(candidates, ORDERBOOK_BATCH_SIZE, async (candidate) => {
    const symbol = String(candidate.symbol || "").toUpperCase();
    const bitgetSymbol = candidate.bitgetSymbol || symbol;
    const productType = candidate.productType || "USDT-FUTURES";
    const linearSymbol = toLinearUsdtSymbol(bitgetSymbol);

    try{
      const raw = await fetchOrderBookWithRetry(bitgetSymbol, productType);
  ...
  for(const originalCoin of candidates){
    ...
    const linearMarketSymbol = toLinearUsdtSymbol(c.bitgetSymbol || c.symbol);
