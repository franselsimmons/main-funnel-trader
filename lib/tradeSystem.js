...
import {
  assertTradeAllowedByMicroRotation
} from "./tradesystem/microRotationGate.js";

// ================= STRATEGY VERSION =================
const STRATEGY_VERSION = "TS_V12_7_MICRO_ROTATION_GATE";
...
const DURABLE_LOCK_ATTEMPTS = 2;
const DURABLE_LOCK_RETRY_MS = 500;
const DURABLE_LOCK_TTL_MS = 20 * 1000;
...
function buildTradeCandidates(candidatesRaw) {
  ...
  for (const [key, pos] of memory.entries()) {
    ...
    prefilterStats.openPositionInjection++;
...
function normalizeMicroFamilyId(raw) {
  ...
  // Hard block: parent/sub/unknown mag nooit rotation-key worden.
  ...
  if (id === "LONG_UNKNOWN" || id === "SHORT_UNKNOWN") return null;
...
// ================= ROTATION HELPERS: MICRO FAMILY ONLY =================
...
function normalizeFamilyToken(value, fallback = "NA") {
...
function bucketRotationValue(value, step, prefix, decimals = 0) {
  const n = Number(value);
  ...
  const cleanLower = lower.toFixed(decimals).replace(".", "P").replace("-", "M");
  const cleanUpper = upper.toFixed(decimals).replace(".", "P").replace("-", "M");

  return `${prefix}_${cleanLower}_${cleanUpper}`;
}

function getRotationObRelation(side, obBias) {
  const s = String(side || "").toLowerCase();
  const ob = String(obBias || "NEUTRAL").toUpperCase();
...
function buildMicroFamilyIdFromFeatureRow(row = {}) {
  const rotationSide = normalizeRotationSide(row.side);

  const setupClass = normalizeFamilyToken(row.setupClass || "NONE");
  const entryReason = normalizeFamilyToken(row.entryReason || row.reason || "UNKNOWN");
  const flow = normalizeFamilyToken(row.flow || "UNKNOWN");
  const rsiZone = normalizeFamilyToken(row.rsiZone || "UNKNOWN");
  const obRelation = normalizeFamilyToken(
    row.obSideRelation || getRotationObRelation(row.side, row.obBias)
  );
  const btcState = normalizeFamilyToken(row.btcState || "UNKNOWN");
  const regime = normalizeFamilyToken(row.regime || "UNKNOWN");

  const confBucket = bucketRotationValue(row.confluence, 10, "CONF", 0);
  const sniperBucket = bucketRotationValue(row.sniperScore, 10, "SNIPER", 0);
  const rrBucket = bucketRotationValue(
    row.plannedRR || row.finalRr || row.rr,
    0.25,
    "RR",
    2
  );
...
function attachRotationFamilyKeys(entryPayload, btcState, runId) {
  const featureRow = buildFeatureRowFromAction(entryPayload, btcState, runId);
  const rotationSide = normalizeRotationSide(entryPayload.side);

  const microFamilyId = buildMicroFamilyIdFromFeatureRow({
    ...featureRow,
    side: entryPayload.side,
    setupClass: entryPayload.setupClass,
    entryReason: entryPayload.entryReason || entryPayload.reason,
    reason: entryPayload.reason,
    flow: entryPayload.flow,
    rsiZone: entryPayload.rsiZone,
    obBias: entryPayload.obBias,
    btcState,
    regime: entryPayload.regime,
    confluence: entryPayload.confluence,
    sniperScore: entryPayload.sniperScore,
    plannedRR: entryPayload.plannedRR || entryPayload.finalRr || entryPayload.rr
  });

  const legacyCohorts = getFeatureCohortKeys(featureRow).filter(Boolean);
  const parentFamilyId = `${rotationSide}_${normalizeFamilyToken(entryPayload.setupClass || "NONE")}`;

  const familyIds = [microFamilyId, ...legacyCohorts];
  ...
  return {
    ...
    // BELANGRIJK: scalar microFamilyId voor rotation gate.
    microFamilyId,
    familyId: microFamilyId,

    // Arrays blijven voor logging/analyse.
...
function normalizeMicroRotationResult(result, signal) {
  if (!result) {
    return {
      allowed: true,
      reason: "MICRO_ROTATION_ALLOWED",
      decision: null,
      signal
    };
  }
  ...
  if (result.allowed === false) {
    return {
      allowed: false,
      reason: result.reason || "MICRO_ROTATION_BLOCKED",
      decision: result.decision || result.rotationGate || result.rotation || null,
  ...
  return {
    allowed: true,
    reason: result.reason || "MICRO_ROTATION_ALLOWED",
    decision: result.decision || result.rotationGate || result.rotation || result,
    signal: result.signal || signal
  };
}

async function checkEntryAgainstMicroRotation(signal, options = {}) {
  if (options.enabled === false) {
    return {
      allowed: true,
      reason: "MICRO_ROTATION_GATE_DISABLED",
      decision: null,
      signal
    };
  }

  const microFamilyId = extractMicroFamilyIdFromSignal(signal);
  ...
  const normalizedSignal = {
    ...signal,
    side: rotationSide,
    direction: rotationSide,
    rotationSide,
    tradeSide: rotationSide,
    microFamilyId,
    microFamily: microFamilyId,
    microFamilyIds: microFamilyId ? [microFamilyId] : [],
    microFamilies: microFamilyId ? [microFamilyId] : []
  };
  ...
  try {
    const result = await assertTradeAllowedByMicroRotation(normalizedSignal, {
      ...options,
      requireSideMatch: true,
      microOnly: true,
      allowParentFallback: false,
      allowSubFallback: false
    });

    return normalizeMicroRotationResult(result, normalizedSignal);
  } catch (error) {
    return {
      allowed: false,
      reason: error?.code || error?.reason || error?.message || "MICRO_ROTATION_BLOCKED",
      decision: error?.decision || error?.rotationGate || {
        allowed: false,
        reason: error?.code || error?.message || "MICRO_ROTATION_BLOCKED",
        rotationSide,
        microFamilyId
      },
      signal: normalizedSignal
...
// ================= BITGET CANDLE FETCH FUNCTIONS (replacing placeholder) =================
...
function buildOpenPositionRows() {
  return Array.from(memory.values()).map(pos => ({
    ...
    shadowOnly: Boolean(pos.shadowOnly),
    rotationSide: pos.rotationSide || null,
    rotationId: pos.rotation?.activeRotationId || pos.rotation?.rotationId || null,
    familyId: pos.familyId || null,
    microFamilyId: pos.microFamilyId || null
  }));
}
...
async function finalizeResult(actions, candidates, btcState, runId, options = {}) {
  ...
  const webhookActions = finalActions
    .filter(action => {
      ...
      // Rotation-missing spam niet naar webhook sturen.
      if (reason === "WEEKLY_ROTATION_ENTRY_MICRO_FAMILY_MISSING") {
        return false;
      }
  ...
  const webhookResult = await sendAnalysisActions(webhookActions, {
    runId,
    btcState,
    strategyVersion: STRATEGY_VERSION,
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        // ---------------- ROTATION GATE START ----------------
        const rotationAwareEntryPayload = attachRotationFamilyKeys(
          entryPayload,
          btcState,
          runId
        );

        // Debug log vóór de rotation check
        console.log("TS_ROTATION_SIGNAL_READY:", JSON.stringify({
          symbol: c.symbol,
          side: c.side,
          rotationSide: rotationAwareEntryPayload.rotationSide,
          setupClass: setup.setupClass,
          reason: setup.entryReason,
          microFamilyId: rotationAwareEntryPayload.microFamilyId,
          microFamilyIds: rotationAwareEntryPayload.microFamilyIds,
          familyId: rotationAwareEntryPayload.familyId,
          parentFamilyId: rotationAwareEntryPayload.parentFamilyId
        }));

        const rotationCheck = await checkEntryAgainstMicroRotation(
          rotationAwareEntryPayload,
          {
            enabled: process.env.WEEKLY_ROTATION_LIVE_GATE !== "0",
            requireSideMatch: true,

            // Default: geen actieve rotation mag discovery/live niet killen.
            // Hard blokkeren: WEEKLY_ROTATION_EMPTY_POLICY=DENY_ALL
            allowWhenNoRotation:
        ...
        if (!rotationCheck.allowed) {
          const blockedPayload = buildWait(
            c,
            `WEEKLY_ROTATION_${rotationCheck.reason || "BLOCKED"}`,
            flow,
            sniper,
            confluence,
            finalRr,
          ...
          blockedPayload.rotationSide = rotationAwareEntryPayload.rotationSide;
          blockedPayload.tradeSide = rotationAwareEntryPayload.tradeSide;

          blockedPayload.familyIds = rotationAwareEntryPayload.familyIds;
          blockedPayload.families = rotationAwareEntryPayload.families;
          blockedPayload.microFamilyIds = rotationAwareEntryPayload.microFamilyIds;
          blockedPayload.microFamilies = rotationAwareEntryPayload.microFamilies;
          blockedPayload.familyId = rotationAwareEntryPayload.familyId;
          blockedPayload.microFamilyId = rotationAwareEntryPayload.microFamilyId;
          blockedPayload.rotationCandidate = rotationAwareEntryPayload.rotationCandidate;
          ...
          console.log("TS_ENTRY_BLOCKED_BY_WEEKLY_ROTATION:", JSON.stringify({
            ...
            microFamilyId: rotationAwareEntryPayload.microFamilyId,
            microFamilyIds: rotationAwareEntryPayload.microFamilyIds?.slice?.(0, 5) || []
          }));

          continue;
        }

        Object.assign(entryPayload, rotationCheck.signal || rotationAwareEntryPayload);
        ...
        // Cooldown not used for entry, but we can add if needed.
        ...
        console.log("TS_ENTRY_OPENED:", JSON.stringify({
          ...
          setupClass: setup.setupClass,
          reason: setup.entryReason,
          rotationId: entryPayload.rotation?.activeRotationId || entryPayload.rotation?.rotationId || null,
