...
export async function processTrades(
  ...
  const certaintyMode = options.certaintyMode || "aggressive"; // "safe" of "aggressive" (alleen gebruikt voor stage en sniper drempel)
  ...
  for(const originalCoin of candidates){
    ...
    // ================= RSI waarde voor timing =================
    const rsi = rsiData?.m15?.rsi ?? 50;

    // ================= STAGE, SNIPER, GRADE (aangepast op basis van certaintyMode) =================
    let allowedStages = ["entry"];
    if(certaintyMode !== "safe") allowedStages = ["entry", "almost"];

    const stageOK = allowedStages.includes(c.stage);

    let sniperThreshold = 55;
    if(certaintyMode === "safe") sniperThreshold = 60;

    const sniperOK =
      (sniper?.valid && getSniperScore(sniper) >= sniperThreshold) ||
      confluence >= 85;
    ...
    const gradeOK =
      ...
      (setupGrade.grade === "B" &&
       confluence >= 68 &&
       (ob.bias !== "NEUTRAL" || isNeutralObEntryException({c, flow, sniper, confluence, rr, setupGrade, counterTrend: (btcState === "BULLISH" && !isBull) || (btcState === "BEARISH" && isBull)})) &&
       getSniperScore(sniper) >= 68 &&
       c.tfStrength >= 1 &&
       c.stage !== "buildup");

    let minRrFloor = getDynamicMinRrFloor({
      c,
      setupGrade,
      flow,
      sniper,
      confluence,
      counterTrend: (btcState === "BULLISH" && !isBull) || (btcState === "BEARISH" && isBull)
    });

    if(!isBull && btcState === "BEARISH"){
      minRrFloor = Math.max(0.90, minRrFloor - 0.05);
    }
    ...
    let state = notifyState.get(key) || {
      ...
      hold: false,
      exit: false,
      phase: "IDLE"   // IDLE → WAIT → READY → ENTRY (na entry weer IDLE)
    };
    ...
    // ================= ENTRY FILTERS (alleen cooldowns + basis condities) =================
    ...
    // ================= LOW VOL FILTER =================
    let lowVolConfluenceLimit = 55;
    if(certaintyMode === "safe") lowVolConfluenceLimit = 60;

    if(vol === "LOW" && confluence < lowVolConfluenceLimit){
    ...
    let marketQualityConfluenceLimit = 65;
    if(certaintyMode === "safe") marketQualityConfluenceLimit = 75;

    if((badSpread || badDepth) && confluence < marketQualityConfluenceLimit){
    ...
    if(
      ob.bias === "NEUTRAL" &&
      confluence < 50 &&
      !isNeutralObEntryException({c, flow, sniper, confluence, rr, setupGrade, counterTrend: (btcState === "BULLISH" && !isBull) || (btcState === "BEARISH" && isBull)})
    ){
    ...
    // ================= FASE LOGICA =================
    // setupValid is de basisconditie voor een geldige setup (zonder timing)
    const setupValid =
      stageOK &&
      sniperOK &&
      gradeOK &&
    ...
    // === FASE 1: IDLE -> WAIT (setup gevonden)
    ...
    // === FASE 2: WAIT -> READY (RSI timing)
    if(state.phase === "WAIT"){
      const goodLongTiming = isBull && rsi <= 40;
      const goodShortTiming = !isBull && rsi >= 60;
      ...
      // RSI timing is goed → ga naar READY
      state.phase = "READY";
      notifyState.set(key, state);
    }

    // ================= ENTRY ALLEEN IN READY FASE =================
    if(
      state.phase === "READY" &&
      setupValid &&
      !ob.spoof &&
      ...
      // TP aanpassing (safe mode vs aggressive)
      let finalTp = riskBase.tp;
      if(certaintyMode === "safe"){
        finalTp = finalTp * 0.95;
      } else {
        // aggressive mode: dynamische TP op basis van RSI (extra boost)
        if((isBull && rsi < 30) || (!isBull && rsi > 70)) finalTp = finalTp * 1.1;
        else if((isBull && rsi > 50) || (!isBull && rsi < 50)) finalTp = finalTp * 0.9;
      }
    ...
    // Geen entry, blijf in huidige fase (WAIT of IDLE) en log als WAIT
    if(state.phase === "WAIT"){
      const waitPayload = buildWait(
        c,
        "WAIT_FOR_RSI_PULLBACK",
