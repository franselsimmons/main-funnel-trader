...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // 🔒 HARD BLOCK 1: als er al een positie open staat voor exact deze kant → skip volledig
    if (memory.has(key)) {
      continue;
    }

    // 🔒 HARD BLOCK 2: blokkeer ook de andere kant (geen long+short tegelijk)
    ...
    // ========== DIRECT ENTRY ==========
    // 🔒 HARD BLOCK 3: alleen entry als er nog geen positie is voor deze kant (dubbele check)
    // 🔒 HARD BLOCK 4: extra veiligheid tegen race conditions (laatste entry < 2 min)
