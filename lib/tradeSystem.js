...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  cleanExpiredGuards();
  let candidates = dedupeCandidates(coins);
  candidates = candidates.slice(0, 4); // max 4 coins
  ...
  // ========== FETCH ALLE DATA PER COIN (met caching) ==========
  ...
  await Promise.all(candidates.map(async (c) => {
    ...
    try {
      funding = await cachedFetch(
        `fund_${symbol}`,
        () => fetchFunding(symbol + "USDT"),
        30000
      );
    } catch {}

    // Candles (cached, 20s)
    const candles5m = await cachedFetch(
      `c5_${cleanPair}`,
      () => fetchCandles(cleanPair, "5m", 100),
      20000
    );
    ...
    // Optioneel: 4h candles (voor MTF)
    const candles4h = await cachedFetch(
      `c4h_${cleanPair}`,
  ...
  for (const originalCoin of candidates) {
    ...
    const confluence = calculateConfluence(c, ob, liquidity, funding, regimeForConfluence, hasLiquidationData ? liquidation : null, null); // oude RSI niet meer nodig
    ...
    // 🔥 SNIPER OK is nu score >= 70 (niet meer de oude threshold)
    const sniperOK = sniper.score >= 70;
    // fallback voor compatibiliteit (wordt zelden gebruikt)
    const sniperThreshold = 55;
    const sniperOK_legacy = (sniper?.valid && getSniperScore(sniper) >= sniperThreshold) || confluence >= 85;
    ...
    // Setup valid (geen RSI nodig als harde eis)
    const setupValid = stageOK && finalSniperOK && gradeOK && !ob.spoof && rr >= minRrFloor;

    // 🔥 ELITE ENTRY MET SNIPER SCORE
    // Versie: sniper score >= 80 OF (score >= 72 én confluence >= 70)
    const eliteEntry = setupValid &&
                        rsiSignal.valid &&
                        (
                          sniper.score >= 80 ||
                          (sniper.score >= 72 && confluence >= 70)
                        );

    // ========== POSITION MANAGEMENT (HOLD / EXIT) ==========
    if (prev) {
      const pos = { ...prev };
    ...
    // ========== ENTRY FILTERS (voordat we entry doen) ==========
