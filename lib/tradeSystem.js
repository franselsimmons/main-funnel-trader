  ...
  return finalizeResult([], [], btcState, runId, options);
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        if (!setup) {
          const payload = buildWait(c, "LOW_ENTRY_QUALITY", flow, sniper, confluence, baseRR, funding, obData, risk, setupGrade, Math.min(A_MIN_CONFLUENCE, B_MIN_CONFLUENCE), Math.min(A_ENTRY_MIN_RR, B_ENTRY_MIN_RR));
          payload.rsi = rsi; payload.rsiHTF = rsiHTF; payload.rsiZone = rsiZone;
          payload.plannedRR = baseRR;
          actions.push(payload);
          continue;
