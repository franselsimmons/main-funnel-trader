...
// ================= ANALYZER TRADE WEBHOOK =================
const ANALYZER_WEBHOOK_SECRET =
...
const ANALYZER_WEBHOOK_URL =
  ...
  "https://crypto-voorspeller.vercel.app/api/webhooks/tradesystem?secret=090117";
  
const ANALYZER_WEBHOOK_TIMEOUT_MS = 2500;

function toAnalyzerDirection(side) {
...
function buildAnalyzerTradeEvent(payload, meta = {}) {
  const action = String(payload?.action || payload?.eventType || "").toUpperCase();

  const tradeId =
    payload.tradeId ||
    `${STRATEGY_VERSION}_${normalizeBaseSymbol(payload.symbol)}_${payload.side}_${payload.entry || payload.ts || Date.now()}`;

  const eventId =
    payload.eventId ||
    `${tradeId}_${action}_${payload.reason || payload.entryReason || "UNKNOWN"}_${Date.now()}`;
  ...
  return {
    ...
    symbol: normalizeBaseSymbol(payload.symbol),

    // Analyzer krijgt beide varianten.
    side: toAnalyzerDirection(payload.side),
    direction: toAnalyzerDirection(payload.side),
    tradeSystemSide: payload.side,

    runId: meta.runId || payload.runId || null,
    btcState: meta.btcState || payload.btcState || "UNKNOWN",
    regime: meta.regimeLevel || payload.regime || "UNKNOWN",
...
async function sendAnalyzerTradeEventSafe(payload, meta = {}) {
  const action = String(payload?.action || payload?.eventType || "").toUpperCase();
  ...
  if (!["ENTRY", "EXIT"].includes(action)) {
    return {
      ok: false,
      skipped: true,
      reason: "NOT_ENTRY_OR_EXIT"
    };
  }

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), ANALYZER_WEBHOOK_TIMEOUT_MS);

  const event = buildAnalyzerTradeEvent(payload, meta);

  try {
    const res = await fetch(ANALYZER_WEBHOOK_URL, {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
    "x-webhook-secret": ANALYZER_WEBHOOK_SECRET
  },
  body: JSON.stringify({
    runId: meta.runId || event.runId || null,
    btcState: meta.btcState || event.btcState || "UNKNOWN",
    strategyVersion: STRATEGY_VERSION,
  ...
  signal: controller.signal
    ...
    console.log("TS_ANALYZER_TRADE_EVENT_RESULT:", JSON.stringify({
      ok: res.ok,
      status: res.status,
      action: event.action,
    ...
    console.error("TS_ANALYZER_TRADE_EVENT_FAILED:", JSON.stringify({
      action: event.action,
      tradeId: event.tradeId,
      symbol: event.symbol,
      side: event.side,
...
// ================= STRATEGY VERSION =================
const STRATEGY_VERSION = "TS_V12_4_RR_GATE_CONFIRM_SHADOW_FIX";
...
const DURABLE_LOCK_ATTEMPTS = 4;
const DURABLE_LOCK_RETRY_MS = 750;
const DURABLE_LOCK_TTL_MS = 120 * 1000;

// Audit / feature-store caps
const MAX_RECENT_ENTRY_AUDIT_ROWS = 150;
const MAX_CLOSED_TRADE_AUDIT_ROWS = 1_000;
const MAX_FEATURE_STORE_ROWS = 5_000;
const MAX_SHADOW_OUTCOME_ROWS = 5_000;
...
const A_GOD_MAX_TP_REWARD_MULTIPLIER = 1.00;
...
function inferTsSetupFromPreset(row, preset) {
  ...
  const isGod =
    rr >= safeNumber(preset.GOD_PRE_TP_MIN_BASE_RR, GOD_PRE_TP_MIN_BASE_RR) &&
  ...
  const isA =
    rr >= safeNumber(preset.A_PRE_TP_MIN_BASE_RR, A_PRE_TP_MIN_BASE_RR) &&
  ...
  const isB =
    ...
    rr >= safeNumber(preset.B_PRE_TP_MIN_BASE_RR, B_PRE_TP_MIN_BASE_RR) &&
...
// ================= PATCH 2: FUNCTION TO ENFORCE MINIMUM BASE RR =================
...
// ================= NEW SNAPSHOT FUNCTIONS FOR FILTERS =================
function snapshotBucketByStep(value, step, label, decimals = 0) {
  const n = Number(value);
  if (!Number.isFinite(n)) return `${label}_NA`;
  const lower = Math.floor(n / step) * step;
  const upper = lower + step;
  return `${label}_${lower.toFixed(decimals)}_${upper.toFixed(decimals)}`
    .replace(/\./g, "P")
    .replace(/-/g, "M")
    .toUpperCase();
}

function snapshotScoreBucket(value, label) {
  const n = Number(value);
  if (!Number.isFinite(n)) return `${label}_NA`;
...
function snapshotRrBucket(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return "RR_NA";
...
function snapshotSpreadBucket(spreadPct) {
  const bps = normalizeSpread(spreadPct) * 10000;
  if (!Number.isFinite(bps)) return "SPREAD_NA";
  if (bps <= 5) return "SPREAD_LE_5BPS";
...
function snapshotDepthBucket(depth) {
  const d = Number(depth || 0);
  if (!Number.isFinite(d)) return "DEPTH_NA";
  if (d < 50_000) return "DEPTH_LT_50K";
  if (d < 100_000) return "DEPTH_50K_100K";
  if (d < 200_000) return "DEPTH_100K_200K";
  if (d < 500_000) return "DEPTH_200K_500K";
  if (d < 1_000_000) return "DEPTH_500K_1M";
  return "DEPTH_GTE_1M";
}

function snapshotFundingBucket(rate) {
  const r = Number(rate || 0);
  if (r <= -0.015) return "FUNDING_NEG_EXTREME";
  if (r <= -0.008) return "FUNDING_NEG_HIGH";
  if (r < -0.002) return "FUNDING_NEG";
...
function snapshotObRelation(side, obBias) {
  const s = String(side || "").toLowerCase();
  const ob = String(obBias || "NEUTRAL").toUpperCase();
  if (ob === "NEUTRAL" || ob === "UNKNOWN") return "NEUTRAL";
  if ((s === "bull" && ob === "BULLISH") || (s === "bear" && ob === "BEARISH")) return "WITH";
  if ((s === "bull" && ob === "BEARISH") || (s === "bear" && ob === "BULLISH")) return "AGAINST";
  return "NEUTRAL";
}

function snapshotBool(value) {
  return Boolean(value) ? "YES" : "NO";
}

function buildEntryFilterSnapshot({
  c,
  setup,
  setupGrade,
  flow,
  ...
  const side = String(c.side || "").toLowerCase();
  const obBias = String(obData?.bias || "NEUTRAL").toUpperCase();
  const obRelation = snapshotObRelation(side, obBias);
  const spreadPct = normalizeSpread(obData?.spreadPct);
  const depthMinUsd1p = Number(obData?.depthMinUsd1p || 0);
  const fundingRate = Number(funding?.rate || 0);
  const sniperScore = Number(sniper?.score || 0);
  const scannerScore = Number(c.moveScore || 0);
  const counterBtc = isCounterBtcSide(side, btcState);
  ...
  return {
    ...
    scanner: {
      score: scannerScore,
      scoreBucket: snapshotScoreBucket(scannerScore, "SCORE"),
      candidateMinScore: CANDIDATE_MIN_SCORE,
      btcBearishLongMinScore: BTC_BEARISH_LONG_MIN_SCORE,
      btcNeutralMinScore: BTC_NEUTRAL_MIN_SCORE
    },

    market: {
      btcState: String(btcState || "UNKNOWN").toUpperCase(),
      counterBtc,
      counterBtcLabel: snapshotBool(counterBtc),
      regime: String(regimeLevel || "UNKNOWN").toUpperCase(),
      flow: String(flow?.type || "UNKNOWN").toUpperCase(),
      funding: fundingRate,
      fundingBucket: snapshotFundingBucket(fundingRate)
    },

    rsi: {
      rsi: Number(rsi || 0),
      rsiHTF: Number(rsiHTF || 0),
      rsiValueBucket: snapshotBucketByStep(rsi, 5, "RSI", 0),
      rsiZone: String(rsiZone || "UNKNOWN").toUpperCase(),
      rsiEdge: rsiEdge?.label || "UNKNOWN",
      rsiEdgeRank: Number(rsiEdge?.rank ?? 0),
      rsiContinuationOk: Boolean(rsiEdge?.continuationOk),
      rsiContinuationOkLabel: snapshotBool(rsiEdge?.continuationOk),
    ...
    confluence: {
      ...
      effectiveConfluence: Number(confluence || 0),
      externalConfluence: c._debugExternalConfluence ?? null,
      fallbackConfluence: c._debugFallbackConfluence ?? null,
      confluenceSource: c._debugConfluenceSource || null,
      confluenceBucket: snapshotScoreBucket(confluence, "CONF")
    },
    ...
    sniper: {
      ...
      rawSniperScore: Number(sniper?.rawSniperScore || 0),
      fallbackSniperScore: Number(sniper?.fallbackSniperScore || 0),
      sniperBlendUsed: Boolean(sniper?.sniperBlendUsed),
      sniperBlendUsedLabel: snapshotBool(sniper?.sniperBlendUsed),
      sniperBucket: snapshotScoreBucket(sniperScore, "SNIPER")
    },

    rr: {
      baseRR: Number(baseRR || 0),
      finalRr: Number(finalRr || 0),
      plannedRR: Number(finalRr || 0),
      requiredRRRaw: Number(rawRequiredRR || 0),
      requiredRRFinal: Number(requiredRR || 0),
      finalRequiredRR: Number(finalRequiredRR || 0),
      requiredSniper: Number(requiredSniper || 0),
      tpRewardMultiplier: Number(tpRewardMultiplier || 1),
      baseRRBucket: snapshotRrBucket(baseRR),
      finalRRBucket: snapshotRrBucket(finalRr)
    },
    ...
    orderbook: {
      ...
      obRelation,
      spreadPct,
      spreadBps: Number((spreadPct * 10000).toFixed(3)),
      maxSpreadAllowed: Number(maxSpread || 0),
      spreadBucket: snapshotSpreadBucket(spreadPct),
      spreadPass: spread <= Number(maxSpread || 0),
      spreadPassLabel: snapshotBool(spread <= Number(maxSpread || 0)),
      depthMinUsd1p,
      depthBucket: snapshotDepthBucket(depthMinUsd1p),
      spoof: Boolean(obData?.spoof),
      spoofLabel: snapshotBool(obData?.spoof),
      obFetchFailed: Boolean(obData?.fetchFailed),
      obFetchFailedLabel: snapshotBool(obData?.fetchFailed)
    },
    ...
    timeframe: {
      ...
      tfStrength: Number(c?.tfStrength || 0),
      tfAlignment: String(c?.tfAlignment || "UNKNOWN").toUpperCase(),
      tfStrengthBucket: snapshotBucketByStep(c?.tfStrength, 0.5, "TF", 1),
      change1h: Number(c?.change1h || 0),
      change24: Number(c?.change24 || 0),
      change1hBucket: snapshotBucketByStep(c?.change1h, 1, "CH1H", 0),
      change24hBucket: snapshotBucketByStep(c?.change24, 2, "CH24H", 0)
    },
    ...
    structure: {
      ...
      pullbackConfirmedLabel: snapshotBool(bullPullbackMeta?.pullbackConfirmed),
      sweepConfirmedLabel: snapshotBool(bullPullbackMeta?.sweepConfirmed),
      retestConfirmedLabel: snapshotBool(bullPullbackMeta?.retestConfirmed),
      fakeBreakoutConfirmedLabel: snapshotBool(c.fakeBreakoutConfirmed || c.fakeBreakout),
      distanceFromLocalHighPct: Number(bullPullbackMeta?.distanceFromLocalHighPct || 0),
      distanceFromLocalHighBucket: snapshotBucketByStep(
        Number(bullPullbackMeta?.distanceFromLocalHighPct || 0) * 100,
        0.5,
        "DIST_HIGH_PCT",
        1
      ),
      hasLiquidationData: Boolean(hasLiquidationData),
      hasLiquidationDataLabel: snapshotBool(hasLiquidationData)
    },

    exceptions: {
      bullishMidTrendProbe: Boolean(bullishMidTrendProbe?.ok),
      bullishMidTrendProbeLabel: snapshotBool(bullishMidTrendProbe?.ok),
      bullishMidTrendProbeReason: bullishMidTrendProbe?.reason || null,
      btcBullishBearException: Boolean(btcBullishBearException?.exception),
      btcBullishBearExceptionLabel: snapshotBool(btcBullishBearException?.exception),
      btcBullishBearExceptionReason: btcBullishBearException?.reason || null,
      neutralObExceptionOk: Boolean(neutralObExceptionOk),
      neutralObExceptionOkLabel: snapshotBool(neutralObExceptionOk),
      qualityGateOk: Boolean(entryQualityGate?.ok),
      qualityGateOkLabel: snapshotBool(entryQualityGate?.ok),
      qualityGateReason: entryQualityGate?.reason || "UNKNOWN",
      finalDepthOk: Boolean(finalDepthValidation?.ok),
      finalDepthOkLabel: snapshotBool(finalDepthValidation?.ok),
      finalDepthReason: finalDepthValidation?.reason || "UNKNOWN",
      bullAntiChaseOk: Boolean(bullAntiChase?.ok),
      bullAntiChaseOkLabel: snapshotBool(bullAntiChase?.ok),
      bullAntiChaseReason: bullAntiChase?.reason || "UNKNOWN",
      confirmationRequired: Boolean(confirmationRequired),
      confirmationRequiredLabel: snapshotBool(confirmationRequired),
      confirmationSeen: Boolean(confirmationSeen),
      confirmationSeenLabel: snapshotBool(confirmationSeen)
    },
    ...
    riskGeometry: {
      ...
      sl: Number(risk?.sl || 0),
      tp: Number(risk?.tp || 0),
      riskPct: Number(risk?.entry || 0)
        ? Number((Math.abs(Number(risk?.entry || 0) - Number(risk?.sl || 0)) / Number(risk?.entry || 0)).toFixed(6))
        : 0,
      slSource: risk?.slSource || "UNKNOWN",
      tpSource: risk?.tpSource || "UNKNOWN"
    },

    liveConstants: getDiscordFilterValuesSnapshot()
  };
}

// ================= BUILD ENTRY POSITION (MODIFIED) =================
...
function recordEntry(entryPayload, position) {
  ...
  auditState.recentEntries.push({
    ...
    filterSnapshot: position.filterSnapshot || entryPayload.filterSnapshot || null
  });
...
function recordExit(exitPayload, pos, exitPrice, isBull) {
  ...
  const filterSnapshot = pos.filterSnapshot || exitPayload.filterSnapshot || null;
  exitPayload.filterSnapshot = filterSnapshot;
...
async function finalizeResult(actions, candidates, btcState, runId, options = {}) {
  ...
  const webhookResult = await sendAnalysisActions(finalActions, {
    runId,
    btcState,
    strategyVersion: STRATEGY_VERSION,
    discoveryMode: DISCOVERY_MODE,
    ...
    for (const originalCoin of candidates) {
      ...
      if (prev) {
        ...
        if (hitTP || hitSL) {
          ...
          // ==== ANALYZER TRADE EVENT PUSH (EXIT) ====
          await sendAnalyzerTradeEventSafe(exitPayload, {
            runId,
            btcState,
            regimeLevel: pos.regime || getRegimeKey(scanRegime, options.regime)
          });
          // ==== END ANALYZER PUSH ====
        ...
        if (earlyFailure.exit) {
          ...
          // ==== ANALYZER TRADE EVENT PUSH (EARLY EXIT) ====
          await sendAnalyzerTradeEventSafe(exitPayload, {
            runId,
            btcState,
            regimeLevel: pos.regime || getRegimeKey(scanRegime, options.regime)
      ...
      try {
        ...
        // Build filter snapshot
        const filterSnapshot = buildEntryFilterSnapshot({
          c,
          setup,
          setupGrade,
          flow,
        ...
        // ==== ANALYZER TRADE EVENT PUSH (ENTRY) ====
        await sendAnalyzerTradeEventSafe(entryPayload, {
          runId,
          btcState,
          regimeLevel
        });
