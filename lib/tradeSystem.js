...
// ================= CORE =================
export async function processTrades(scan, options = {}) {
  const notify = options.notify !== false;
  const shouldLog = options.log !== false;
  const certaintyMode = options.certaintyMode || "aggressive";

  // Verzamel coins uit entry + almost (beide kanten)
  const candidatesRaw = [
    ...(scan?.funnel?.bull?.entry || []),
    ...(scan?.funnel?.bear?.entry || []),
    ...(scan?.funnel?.bull?.almost || []),
    ...(scan?.funnel?.bear?.almost || [])
  ];

  // Sorteer op moveScore (hoog naar laag) en neem max 8
  ...
  const btcState = scan?.btc?.state || market?.trend || "NEUTRAL";
  ...
  for (const originalCoin of candidates) {
    ...
    const regimeLevel = getRegimeKey(regime, scan?.regime);
    const regimeForConfluence = getRegimeValueForConfluence(regime, scan?.regime);
