...
const MICRO_ROTATION_MIN_COMPLETED = Number(process.env.MICRO_ROTATION_MIN_COMPLETED || 5);
const MICRO_ROTATION_MIN_SCORE = Number(process.env.MICRO_ROTATION_MIN_SCORE || -999);
...
function normalizeMicroFamilyId(raw) {
  ...
  // Nieuwe gate gebruikt MF_...
  if (!id.startsWith("MICRO_") && !id.startsWith("MF_")) return null;
  ...
  if (id === "MF_UNKNOWN" || id === "MICRO_UNKNOWN") return null;
...
function refreshFamilyDerivedStats(stats) {
  ...
  stats.winrateNum = completed ? Number((wins / completed).toFixed(4)) : 0;
...
function scoreMicroFamily(family) {
  const completed = Number(family.completed || 0);
  const seen = Number(family.seen || 0);
  const avgR = Number(family.avgR || 0);
  const winrate = Number(family.winrateNum || 0);
  const directSLPct = completed ? Number(family.directSL || 0) / completed : 0;
  const sampleConfidence = Math.min(1, completed / 30);
  const observationConfidence = Math.min(1, seen / 100);
...
function buildActiveRotationFromWeek(week) {
  const families = Object.values(week?.families || {})
    .map(family => ({
      ...refreshFamilyDerivedStats({ ...family }),
      rotationScore: scoreMicroFamily(family)
    }))
...
function recordMicroOutcome(row, source = "UNKNOWN") {
  ...
  const exitR = Number(row.exitR ?? row.resultR ?? row.pnlR ?? row.rMultiple ?? 0);
  ...
  for (const familyId of ids) {
    ...
    stats.completed++;
    stats.totalR += Number.isFinite(exitR) ? exitR : 0;
...
function enrichRowWithMicroRotation(row) {
  ...
  return {
    ...row,
    ...enriched,
    familyIds: Array.isArray(enriched.microFamilyIds) ? enriched.microFamilyIds : [],
    families: Array.isArray(enriched.microFamilyIds) ? enriched.microFamilyIds : [],
    microFamilies: Array.isArray(enriched.microFamilyIds) ? enriched.microFamilyIds : []
  };
}
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        const rotationReadySignal = attachMicroRotationKeys(entryPayload, {
        ...
        const rotationCheck = await checkTradeSignalAgainstRotation(rotationReadySignal, {
          ...
          // Kwaliteitsdrempels (discovery mode is relaxed)
          minEntryScore: 55,
          minAlmostScore: 75,
          minConfluence: 60,
          minSniperScore: 60,
