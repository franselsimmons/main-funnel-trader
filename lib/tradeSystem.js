...
const MAX_SPREAD_PCT = 0.0022;
const MID_BULL_MAX_SPREAD_PCT = 0.00154;      // aangescherpt
...
const A_ENTRY_MIN_RR = 1.05;                 // verlaagd
const B_ENTRY_MIN_RR = 1.20;
const GOD_ENTRY_MIN_RR = 1.25;

const A_MIN_SNIPER = 48;                     // verlaagd
const A_MIN_CONFLUENCE = 60;                 // verlaagd
const B_MIN_SNIPER = 72;
const B_MIN_CONFLUENCE = 75;

const GOD_MIN_SNIPER = 75;
const GOD_MIN_CONFLUENCE = 75;

const MID_RSI_MIN_CONFLUENCE = 58;           // verlaagd
const TREND_CONTINUATION_MIN_CONFLUENCE = 58;
const TREND_CONTINUATION_MIN_SNIPER = 55;

const A_FINAL_MIN_RR = 1.15;
...
const MIN_DEPTH_USD_1P_ABSOLUTE = 15_000;
const A_MIN_DEPTH_USD_1P = 40_250;
const BULL_TREND_MIN_DEPTH_USD_1P = 49_000;
...
const REQUIRE_LIVE_EXECUTION =
  String(process.env.REQUIRE_LIVE_EXECUTION || "true").toLowerCase() === "true";

const EXECUTION_BASE_URL = String(process.env.EXECUTION_BASE_URL || "")
  .replace(/\/+$/, "");

const EXECUTION_ROUTE_SECRET = String(
  process.env.EXECUTION_ROUTE_SECRET ||
  process.env.WEBHOOK_SECRET ||
  ""
);

const TRADE_MARGIN_USDT = Number(process.env.TRADE_MARGIN_USDT || 5);
const TRADE_LEVERAGE = Number(process.env.TRADE_LEVERAGE || 10);
const ORDER_EXECUTION_TIMEOUT_MS = 8000;
const FORCE_FIRST_TS_ENTRY =
  String(process.env.FORCE_FIRST_TS_ENTRY || "false").toLowerCase() === "true";

// ================= LIVE FAST MODE / OPTIMIZER THROTTLE =================
const TS_HEAVY_OPTIMIZER_ENABLED =
  String(process.env.TS_HEAVY_OPTIMIZER_ENABLED || "false").toLowerCase() === "true";

const TS_HEAVY_OPTIMIZER_INTERVAL_MS = parseIntEnv(
  "TS_HEAVY_OPTIMIZER_INTERVAL_MS",
  15 * 60 * 1000,
  60 * 1000,
  24 * 60 * 60 * 1000
);

const TS_HYDRATE_DATALAKE_ON_LIVE_RUN =
  String(process.env.TS_HYDRATE_DATALAKE_ON_LIVE_RUN || "false").toLowerCase() === "true";

const TS_OPTIMIZER_CACHE_KEY = "__TRADE_SYSTEM_OPTIMIZER_CACHE__";

function getTsOptimizerCache() {
  if (!globalThis[TS_OPTIMIZER_CACHE_KEY]) {
    globalThis[TS_OPTIMIZER_CACHE_KEY] = {
      lastRunAt: 0,
      lastResult: null
    };
  }

  return globalThis[TS_OPTIMIZER_CACHE_KEY];
}

function shouldRunHeavyOptimizer(options = {}) {
  if (options.analyze === true) return true;
  if (!TS_HEAVY_OPTIMIZER_ENABLED) return false;

  const cache = getTsOptimizerCache();
  return Date.now() - Number(cache.lastRunAt || 0) >= TS_HEAVY_OPTIMIZER_INTERVAL_MS;
}

function buildSkippedTsMasterLog({ btcState, runId, reason = "SKIPPED_FAST_LIVE_RUN" }) {
  const cache = getTsOptimizerCache();
  ...
  return {
    ...
    cached: Boolean(cache.lastResult),
    cachedAt: cache.lastRunAt || 0,
    ...
    recommendedLiveAfstelling: null,
    patchLines: [],
    coverage: {
      coverageOk: true,
      missingFilters: []
    ...
    lastResult: cache.lastResult
      ? {
          decision: cache.lastResult.decision,
          ts: cache.lastResult.ts,
          sample: cache.lastResult.sample,
          expectedPerformance: cache.lastResult.expectedPerformance,
          moneyProjection: cache.lastResult.moneyProjection
        }
      : null
  };
}
...
function createAuditState() {
  return {
    ...
    // Loaded from append-only Redis data lake.
    // Niet opslaan in runtime snapshot; alleen gebruiken voor analyse.
    dataLakeOutcomeRows: [],
    dataLakeWindow: null,
...
// Onafhankelijk van STRATEGY_VERSION.
// Deze keys blijven bestaan over strategy resets heen.
const DATA_LAKE_PROFILE = "MAIN";

const DATA_LAKE_SCAN_KEY = `tradeSystem:dataLake:${DATA_LAKE_PROFILE}:scanRows`;
const DATA_LAKE_OUTCOME_KEY = `tradeSystem:dataLake:${DATA_LAKE_PROFILE}:outcomes`;
const DATA_LAKE_OPTIMIZER_KEY = `tradeSystem:dataLake:${DATA_LAKE_PROFILE}:optimizerResults`;

const DATA_LAKE_ENABLED = String(process.env.TS_DATA_LAKE_ENABLED ?? "true").toLowerCase() === "true";
const DATA_LAKE_RETENTION_DAYS = parseIntEnv("TS_DATA_LAKE_RETENTION_DAYS", 30, 1, 180);
const DATA_LAKE_ANALYZE_LOOKBACK_DAYS = parseIntEnv("TS_ANALYZE_LOOKBACK_DAYS", 14, 1, 60);

// Max totaal te laden rows voor optimizer/analyse.
// Mag hoog blijven, want laden gebeurt vanaf nu gepagineerd.
const DATA_LAKE_MAX_LOAD_ROWS = parseIntEnv("TS_DATA_LAKE_MAX_LOAD_ROWS", 10000, 100, 50000);

// Belangrijk: Upstash max response is 10 MB.
// Laad dus kleine ZRANGEBYSCORE pages.
const DATA_LAKE_LOAD_PAGE_ROWS = parseIntEnv("TS_DATA_LAKE_LOAD_PAGE_ROWS", 250, 25, 1000);

// Soft cap op totale client-side load per run.
// Voorkomt dat optimizer onnodig zwaar wordt.
const DATA_LAKE_LOAD_MAX_BYTES = parseIntEnv("TS_DATA_LAKE_LOAD_MAX_BYTES", 8_000_000, 1_000_000, 9_500_000);

const DATA_LAKE_RAW_SCAN_MAX_PER_RUN = parseIntEnv("TS_DATA_LAKE_RAW_SCAN_MAX_PER_RUN", 300, 0, 1000);

function parseIntEnv(name, fallback, min = -Infinity, max = Infinity) {
...
const dataLakePendingWrites = [];

function dataLakeEnabled() {
  return DATA_LAKE_ENABLED && hasRedis();
}

function getDataLakeKey(type) {
  const t = String(type || "").toUpperCase();

  if (t === "SCAN_ROW") return DATA_LAKE_SCAN_KEY;
  if (t === "OUTCOME") return DATA_LAKE_OUTCOME_KEY;
  if (t === "OPTIMIZER_RESULT") return DATA_LAKE_OPTIMIZER_KEY;

  return null;
}

function compactDataLakeScanRow(row) {
  return {
    type: "SCAN_ROW",
    profile: DATA_LAKE_PROFILE,
    ts: Number(row.ts || Date.now()),

    runId: row.runId || null,
    strategyVersion: row.strategyVersion || STRATEGY_VERSION,
    source: String(row.source || "UNKNOWN").toUpperCase(),
    scanStage: String(row.scanStage || "UNKNOWN").toUpperCase(),

    symbol: normalizeBaseSymbol(row.symbol),
    side: String(row.side || "").toLowerCase(),

    action: String(row.action || "UNKNOWN").toUpperCase(),
    reason: String(row.reason || row.oldReason || "UNKNOWN").toUpperCase(),

    setupClass: String(row.setupClass || "UNKNOWN").toUpperCase(),
    liveGrade: String(row.liveGrade || row.grade || row.setupClass || "UNKNOWN").toUpperCase(),
    entryType: String(row.entryType || row.runnerEntryType || "UNKNOWN").toUpperCase(),

    stage: String(row.stage || "unknown").toLowerCase(),
    scannerStage: String(row.scannerStage || row.stage || "unknown").toLowerCase(),

    score: Number(row.score ?? row.moveScore ?? 0),
    confluence: Number(row.confluence || 0),
    sniperScore: Number(row.sniperScore || 0),

    rr: Number(row.rr || row.plannedRR || 0),
    plannedRR: Number(row.plannedRR || row.rr || 0),
    finalRr: Number(row.finalRr || row.rr || row.plannedRR || 0),

    entry: Number(row.entry || row.price || 0),
    sl: Number(row.sl || row.stopLoss || 0),
    tp: Number(row.tp || row.takeProfit || 0),
...
function compactDataLakeOutcome(row) {
  return {
    type: "OUTCOME",
    profile: DATA_LAKE_PROFILE,
    ts: Number(row.completedAt || row.exitedAt || row.ts || Date.now()),
    ...
    action: String(row.action || "ENTRY").toUpperCase(),
    reason: String(row.reason || row.entryReason || "UNKNOWN").toUpperCase(),
    entryReason: String(row.entryReason || row.reason || "UNKNOWN").toUpperCase(),
    ...
    rr: Number(row.rr || row.plannedRR || 0),
    plannedRR: Number(row.plannedRR || row.rr || 0),
    baseRR: Number(row.baseRR || row.rr || row.plannedRR || 0),

    entry: Number(row.entry || 0),
    sl: Number(row.sl || row.initialSl || 0),
    initialSl: Number(row.initialSl || row.sl || 0),
    tp: Number(row.tp || 0),
    ...
    breakEvenStop: Boolean(row.breakEvenStop),
    ...
    completedAt: Number(row.completedAt || row.exitedAt || Date.now()),
  };
}

function compactDataLakeOptimizerResult(row) {
  return {
    ...
    expectedPerformance: row.expectedPerformance || null,
    moneyProjection: row.moneyProjection || null,

    bestA: row.bestA || null,
    bestB: row.bestB || null,
    bestCombined: row.bestCombined || null,

    recommendedLiveAfstelling: row.recommendedLiveAfstelling || null,
    coverage: row.coverage || null,
  };
}

function compactDataLakeEvent(event) {
  const type = String(event?.type || "").toUpperCase();

  if (type === "SCAN_ROW") return compactDataLakeScanRow(event);
  if (type === "OUTCOME") return compactDataLakeOutcome(event);
  if (type === "OPTIMIZER_RESULT") return compactDataLakeOptimizerResult(event);
...
async function redisZAddJson(key, payload, scoreTs) {
  const serialized = JSON.stringify(payload);
  await redisCommand(["ZADD", key, String(scoreTs), serialized]);
  return jsonByteLength(serialized);
}

async function redisZRangeByScoreJson(key, fromTs, toTs, limit = DATA_LAKE_MAX_LOAD_ROWS) {
  const maxRows = Math.max(0, Number(limit || DATA_LAKE_MAX_LOAD_ROWS));
  if (!maxRows) return [];

  const rows = [];
  let offset = 0;
  let pageRows = DATA_LAKE_LOAD_PAGE_ROWS;
  let loadedBytes = 0;
  let skippedOversizedRows = 0;

  while (rows.length < maxRows) {
    const count = Math.min(pageRows, maxRows - rows.length);
    ...
    try {
      ...
      if (/max request size exceeded/i.test(message) && pageRows > 1) {
        pageRows = Math.max(1, Math.floor(pageRows / 2));

        console.warn("DATA_LAKE_PAGE_SIZE_REDUCED:", JSON.stringify({
          key,
          offset,
          nextPageRows: pageRows,
          error: message,
        }));
      ...
      if (/max request size exceeded/i.test(message) && pageRows <= 1) {
        ...
        console.warn("DATA_LAKE_OVERSIZED_ROW_SKIPPED:", JSON.stringify({
          key,
          offset,
          skippedOversizedRows,
          error: message,
        ...
        if (skippedOversizedRows >= 25) {
          console.warn("DATA_LAKE_LOAD_ABORTED_TOO_MANY_OVERSIZED_ROWS:", JSON.stringify({
            key,
            skippedOversizedRows,
    ...
    for (const item of result) {
      ...
      const itemBytes = jsonByteLength(itemText);

      if (rows.length > 0 && loadedBytes + itemBytes > DATA_LAKE_LOAD_MAX_BYTES) {
        console.warn("DATA_LAKE_LOAD_BYTE_CAP_REACHED:", JSON.stringify({
          key,
          loadedRows: rows.length,
          loadedBytes,
          maxBytes: DATA_LAKE_LOAD_MAX_BYTES,
  ...
  console.log("DATA_LAKE_ZRANGE_PAGED_LOAD:", JSON.stringify({
    key,
    fromTs,
    toTs,
    loadedRows: rows.length,
...
async function pruneDataLakeKey(key) {
  const cutoff = Date.now() - DATA_LAKE_RETENTION_DAYS * 24 * 60 * 60 * 1000;
  await redisCommand(["ZREMRANGEBYSCORE", key, "-inf", String(cutoff)]);
}

async function appendDataLakeEvent(event) {
  if (!dataLakeEnabled()) return false;

  const type = String(event?.type || "").toUpperCase();
  const key = getDataLakeKey(type);

  if (!key) return false;

  const payload = compactDataLakeEvent(event);
  const ts = Number(payload.ts || Date.now());

  await redisZAddJson(key, payload, ts);

  // Lage kans om prune te doen; voorkomt extra latency op elke row.
  if (Math.random() < 0.01) {
    await pruneDataLakeKey(key).catch(() => false);
  }

  return true;
}

function queueDataLakeAppend(event) {
  if (!dataLakeEnabled()) return;

  const task = appendDataLakeEvent(event).catch(error => {
    console.warn("DATA_LAKE_APPEND_FAILED:", error.message);
    return false;
  });

  dataLakePendingWrites.push(task);
}

async function flushDataLakeWrites() {
  if (!dataLakePendingWrites.length) return;

  const writes = dataLakePendingWrites.splice(0, dataLakePendingWrites.length);
  await Promise.allSettled(writes);
}

async function loadDataLakeEvents({ type, fromTs, toTs = Date.now(), limit = DATA_LAKE_MAX_LOAD_ROWS }) {
  if (!dataLakeEnabled()) return [];

  const key = getDataLakeKey(type);
  if (!key) return [];

  return redisZRangeByScoreJson(key, fromTs, toTs, limit);
}

function recordRawScanRowsToDataLake({ rows, runId, btcState, scanRegime }) {
  if (!dataLakeEnabled()) return;
  if (!Array.isArray(rows) || rows.length === 0) return;

  const maxRows = Math.max(0, Number(DATA_LAKE_RAW_SCAN_MAX_PER_RUN || 0));
  if (!maxRows) return;

  const now = Date.now();
  const normalizedBtcState = String(btcState || "UNKNOWN").toUpperCase();
  const normalizedRegime = String(scanRegime?.level || scanRegime || "UNKNOWN").toUpperCase();

  for (const raw of rows.slice(0, maxRows)) {
    ...
    try {
      ...
      const score = Number(raw.moveScore ?? raw.score ?? 0);
      const price = Number(raw.price ?? raw.lastPrice ?? raw.markPrice ?? 0);
      ...
      queueDataLakeAppend({
        ...
        stage: String(raw.stage || "unknown").toLowerCase(),
        scannerStage: String(raw.scannerStage || raw.stage || "unknown").toLowerCase(),
        stageSource: raw.stageSource || "scanner",

        score,
        moveScore: score,
        price,

        flow: String(raw.flow || raw.scannerFlow || "UNKNOWN").toUpperCase(),
        scannerFlow: String(raw.scannerFlow || raw.flow || "UNKNOWN").toUpperCase(),

        btcState: normalizedBtcState,
        regime: normalizedRegime,

        change1h: Number(raw.change1h || raw.change1hPct || 0),
        change24: Number(raw.change24 || raw.change24h || raw.change24hPct || 0),

        setupClass: String(raw.setupClass || "UNKNOWN").toUpperCase(),
        liveGrade: String(raw.liveGrade || raw.grade || raw.setupClass || "UNKNOWN").toUpperCase(),
        entryType: String(raw.entryType || raw.runnerEntryType || "UNKNOWN").toUpperCase(),

        confluence: Number(raw.confluence || 0),
        sniperScore: Number(raw.sniperScore || 0),

        rr: Number(raw.rr || raw.plannedRR || 0),
        plannedRR: Number(raw.plannedRR || raw.rr || 0),
        finalRr: Number(raw.finalRr || raw.rr || raw.plannedRR || 0),

        entry: Number(raw.entry || price || 0),
        sl: Number(raw.sl || raw.stopLoss || 0),
        tp: Number(raw.tp || raw.takeProfit || 0),

        rsi: Number(raw.rsi || 0),
        rsiHTF: Number(raw.rsiHTF || raw.mean1h || 0),
        rsiZone: String(raw.rsiZone || "UNKNOWN").toUpperCase(),

        obBias: String(raw.obBias || "UNKNOWN").toUpperCase(),
        spreadPct: Number(raw.spreadPct || 0),
        depthMinUsd1p: Number(raw.depthMinUsd1p || 0),

        funding: Number(raw.funding || raw.fundingRate || 0),

        tfScore: Number(raw.tfScore || 0),
        tfStrength: Number(raw.tfStrength || 0),
        tfAlignment: String(raw.tfAlignment || "UNKNOWN").toUpperCase(),

        hasRiskGeometry: Boolean(raw.entry && raw.sl && raw.tp),

        liveEligible: false,
        shadowOnly: true,
        scannerHot: score >= CANDIDATE_MIN_SCORE,
      });
    } catch (error) {
      console.warn("DATA_LAKE_RAW_SCAN_ROW_QUEUE_FAILED:", JSON.stringify({
...
async function hydrateDataLakeOutcomeRows() {
  if (!dataLakeEnabled()) {
    auditState.dataLakeOutcomeRows = [];
    auditState.dataLakeWindow = null;
    return [];
  }

  const toTs = Date.now();
  const fromTs = toTs - DATA_LAKE_ANALYZE_LOOKBACK_DAYS * 24 * 60 * 60 * 1000;

  try {
    await pruneDataLakeKey(DATA_LAKE_OUTCOME_KEY).catch(error => {
      console.warn("TS_DATA_LAKE_OUTCOME_PRUNE_SKIPPED:", error.message);
      return false;
    });

    const rows = await loadDataLakeEvents({
      type: "OUTCOME",
      fromTs,
      toTs,
      limit: DATA_LAKE_MAX_LOAD_ROWS,
    });

    const normalized = rows
      .map(row => normalizeTsOptimizerRow(row, row.source || "DATA_LAKE"))
      .filter(isCompletedTsOutcomeRow)
      .filter(row => row.symbol)
      .sort((a, b) => safeNumber(a.ts, 0) - safeNumber(b.ts, 0));

    auditState.dataLakeOutcomeRows = normalized;
    auditState.dataLakeWindow = {
      enabled: true,
      fromTs,
      toTs,
      lookbackDays: DATA_LAKE_ANALYZE_LOOKBACK_DAYS,
      loadedRows: normalized.length,
      rawRows: rows.length,
      pageRows: DATA_LAKE_LOAD_PAGE_ROWS,
      maxBytes: DATA_LAKE_LOAD_MAX_BYTES,
      error: null,
    };

    console.log("TS_DATA_LAKE_OUTCOMES_LOADED", JSON.stringify(auditState.dataLakeWindow));
    ...
    console.warn("TS_DATA_LAKE_OUTCOMES_LOAD_FAILED_SOFT:", JSON.stringify(auditState.dataLakeWindow));
...
function trimAuditArrays() {
  ...
  if (!Array.isArray(auditState.dataLakeOutcomeRows)) {
    auditState.dataLakeOutcomeRows = [];
  }
  ...
  if (auditState.dataLakeOutcomeRows.length > DATA_LAKE_MAX_LOAD_ROWS) {
    auditState.dataLakeOutcomeRows = auditState.dataLakeOutcomeRows.slice(-DATA_LAKE_MAX_LOAD_ROWS);
  }
}
...
// ================= FIX: createShadowOutcome met source parameter =================
...
function completeShadowOutcome(row, status, price, r, pnlPct) {
  ...
  const outcomeRow = buildOutcomeRowFromShadow(row);
...
// Zelfde architectuur als RUNNER master optimizer.
// Gebruikt alle completed REAL + SHADOW + SCAN_SHADOW rows.
// Doel: sterke balans tussen winrate, totalPnlPct, totalR en voldoende sample.
...
function buildTsOptimizerUniverse() {
  ...
  const dataLakeRows = (auditState.dataLakeOutcomeRows || [])
    .map(row => normalizeTsOptimizerRow(row, row?.source || "DATA_LAKE"))
    .filter(isCompletedTsOutcomeRow);

  const dedupe = new Map();

  for (const row of [...realRows, ...shadowRows, ...dataLakeRows]) {
    const key = [
      row.source,
      row.symbol,
      row.side,
...
function tsRowPassesBasePreset(row, preset) {
  ...
  // ================= FIX: gebruik listIncludes voor arrays in voorspellingsengine =================
...
function scoreTsMasterStats(row, keepRatio) {
  ...
  // Winrate-heavy, maar PnL/R en sample blijven zwaar genoeg.
  const raw =
    winrate * 70 +
    totalPnlPct * 2.4 +
    avgPnlPct * 28 +
...
function mergeTsRecommendedLiveAfstelling({ combined, bestA, bestB }) {
  ...
  // Niet automatisch onder absolute liquidity floor.
...
// ================= FIX: buildTsPatchLinesFromPreset met arrays (geen Set) =================
...
function buildTsMasterBestAfstellingLog({ btcState, runId }) {
  ...
  return {
    ...
    sample: {
      ...
      dataLakeRows: auditState.dataLakeOutcomeRows?.length || 0,
      dataLakeWindow: auditState.dataLakeWindow || null,
      confidence:
        rows.length >= 1000
          ? "HIGH"
          : rows.length >= 300
...
function recordExit(exitPayload, pos, exitPrice, isBull) {
  ...
  queueDataLakeAppend({
    type: "OUTCOME",
    profile: DATA_LAKE_PROFILE,
    ...buildOutcomeRowFromClosedTrade(closedTrade),
  });
...
function logOptimizerReport({ reasonTable, btcState, runId, runHeavyOptimizer = false }) {
  if (!runHeavyOptimizer) {
    const skipped = buildSkippedTsMasterLog({
      btcState,
      runId,
      reason: "HEAVY_OPTIMIZER_SKIPPED_LIVE_PATH"
    });

    console.log("TS_MASTER_BEST_AFSTELLING_SKIPPED", JSON.stringify({
      runId,
      btcState,
      reason: skipped.decision,
      cached: skipped.cached,
      cachedAt: skipped.cachedAt
    }));
  ...
  const masterLog = buildTsMasterBestAfstellingLog({
    btcState,
    runId
  });

  masterLog.runtimeMs = Date.now() - startedAt;

  const cache = getTsOptimizerCache();
  cache.lastRunAt = Date.now();
  cache.lastResult = masterLog;

  console.log("TS_MASTER_BEST_AFSTELLING", JSON.stringify({
    decision: masterLog.decision,
    runtimeMs: masterLog.runtimeMs,
    sample: masterLog.sample,
    expectedPerformance: masterLog.expectedPerformance
  }));
...
function logTradeSystemAudit({ candidates, actions, btcState, runId, runHeavyOptimizer = false }) {
  const rows = Array.isArray(actions) ? actions : [];
...
function buildAnalyzePayload({ auditResult, candidates, actions, btcState, runId }) {
  ...
  return {
    ...
    masterBestAfstelling: auditResult?.masterBestAfstelling || buildTsMasterBestAfstellingLog({
      btcState,
      runId
    }),

    finalFilterDecision: buildFinalFilterDecision(),

    optimizerReport: buildOptimizerReport({
      reasonTable,
      btcState,
      runId
    }),
...
function finalizeResult(actions, candidates, btcState, runId, options = {}) {
  ...
  const auditResult = logTradeSystemAudit({
    ...
    btcState,
    runId,
    runHeavyOptimizer: Boolean(options.runHeavyOptimizer)
  });
...
async function postExecutionRoute(path, payload) {
  if (!LIVE_TRADING_ENABLED) {
    const result = {
      ok: !REQUIRE_LIVE_EXECUTION,
      skipped: true,
      reason: "LIVE_TRADING_DISABLED",
      requireLive: REQUIRE_LIVE_EXECUTION
    };

    console.error("TS_EXECUTION_DISABLED", JSON.stringify(result));
    return result;
  }
  ...
  if (!EXECUTION_BASE_URL) {
    return {
      ok: false,
      reason: "EXECUTION_BASE_URL_MISSING"
    };
  }

  if (!EXECUTION_ROUTE_SECRET) {
    return {
      ok: false,
      reason: "EXECUTION_ROUTE_SECRET_MISSING"
    };
  }

  const cleanPath = `/${String(path || "").replace(/^\/+/, "")}`;
  const url = new URL(`${EXECUTION_BASE_URL}${cleanPath}`);

  // Compatibel met jouw /api/position melding:
  // ?secret=JOUW_SECRET
  url.searchParams.set("secret", EXECUTION_ROUTE_SECRET);

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), ORDER_EXECUTION_TIMEOUT_MS);
  ...
  try {
    const res = await fetch(url.toString(), {
      ...
      headers: {
        ...
        // Alle gangbare secret formats meesturen.
        // Je endpoint hoeft er maar één van te accepteren.
        Authorization: `Bearer ${EXECUTION_ROUTE_SECRET}`,
        "x-execution-secret": EXECUTION_ROUTE_SECRET,
        "x-webhook-secret": EXECUTION_ROUTE_SECRET,
        "x-route-secret": EXECUTION_ROUTE_SECRET
      },
      body: JSON.stringify({
        ...payload,
        secret: EXECUTION_ROUTE_SECRET
      }),
    ...
    if (!res.ok) {
      return {
        ok: false,
        status: res.status,
        reason: "EXECUTION_ROUTE_HTTP_ERROR",
        response: json
      };
    }
    ...
    if (json?.ok === false) {
      return {
        ok: false,
        status: res.status,
        reason: json.reason || json.message || "EXECUTION_ROUTE_REJECTED",
        response: json
      };
    }

    console.log("TS_EXECUTION_ROUTE_OK", JSON.stringify({
      path: cleanPath,
      status: res.status,
      response: json
    }));
    ...
    return {
      ...
      reason: error.name === "AbortError"
        ? "EXECUTION_ROUTE_TIMEOUT"
        : "EXECUTION_ROUTE_THROWN",
      error: error.message
    };
  } finally {
...
async function executeTradeSystemEntryViaSignalRoute({
  c,
  position,
  entryPayload,
  setupClass,
  ...
  const payload = {
    ...
    action: "ENTRY",
    symbol: normalizeBitgetSymbol(position.rawBitgetSymbol || position.symbol),
    baseSymbol: normalizeBaseSymbol(position.symbol),
    ...
    rsi: Number(position.rsi || 0),
    rsiHTF: Number(position.rsiHTF || 0),
    rsiZone: position.rsiZone || "UNKNOWN",
    ...
    clientOrderId: [
      "TS",
      STRATEGY_VERSION.replace(/[^A-Z0-9_]/gi, "").slice(0, 20),
      normalizeBaseSymbol(position.symbol),
      side.toUpperCase(),
      Date.now()
    ].join("_"),
  ...
  const result = await postExecutionRoute("/api/signal", payload);

  if (result.ok) {
    console.log("TS_ENTRY_EXECUTION_OK", JSON.stringify({
      symbol: position.symbol,
      side: position.side,
      setupClass,
      reason: reasonEntry,
    ...
    console.error("TS_ENTRY_EXECUTION_FAILED", JSON.stringify({
      symbol: position.symbol,
      side: position.side,
      setupClass,
      reason: reasonEntry,
...
async function executeTradeSystemExitViaCloseRoute({
  c,
  pos,
  reason,
  executionPrice,
  ...
  const payload = {
    ...
    action: "EXIT",
    symbol: normalizeBitgetSymbol(pos.rawBitgetSymbol || pos.symbol),
    baseSymbol: normalizeBaseSymbol(pos.symbol),
    ...
    clientCloseId: [
      "TS_CLOSE",
      normalizeBaseSymbol(pos.symbol),
      side.toUpperCase(),
      Date.now()
    ].join("_"),
  ...
  const result = await postExecutionRoute("/api/close", payload);

  if (result.ok) {
    console.log("TS_EXIT_EXECUTION_OK", JSON.stringify({
      symbol: pos.symbol,
      side: pos.side,
      reason,
      response: result.response
    ...
    console.error("TS_EXIT_EXECUTION_FAILED", JSON.stringify({
      symbol: pos.symbol,
      side: pos.side,
      reason,
      result
...
export async function processTrades(input, options = {}) {
  ...
  const runHeavyOptimizer = shouldRunHeavyOptimizer(options);
  ...
  try {
    ...
    if (runHeavyOptimizer || TS_HYDRATE_DATALAKE_ON_LIVE_RUN || options.analyze === true) {
      await hydrateDataLakeOutcomeRows();
    } else {
      auditState.dataLakeOutcomeRows = [];
      auditState.dataLakeWindow = {
        enabled: dataLakeEnabled(),
        skipped: true,
        reason: "DATALAKE_HYDRATE_SKIPPED_FAST_LIVE_RUN",
        loadedRows: 0,
        rawRows: 0,
        ts: Date.now()
      };
    ...
    try {
      recordRawScanRowsToDataLake({
        rows: candidatesRaw,
        runId,
        btcState: scanBtc?.state || scanBtc || null,
        scanRegime,
      });
    } catch (error) {
      console.warn("DATA_LAKE_RAW_SCAN_RECORD_FAILED_SOFT:", JSON.stringify({
        runId,
        rows: Array.isArray(candidatesRaw) ? candidatesRaw.length : 0,
        error: error.message,
      }));
    }
    ...
    for (const originalCoin of candidates) {
      ...
      if (prev) {
        ...
        if (hitTP || hitSL) {
          ...
          // Execute exit on the exchange
          const closeExecutionResult = await executeTradeSystemExitViaCloseRoute({
            c,
            pos,
            reason,
            executionPrice,
          ...
          if (!closeExecutionResult.ok && REQUIRE_LIVE_EXECUTION) {
            const failedExitPayload = {
              ...exitPayload,
              action: "HOLD",
              reason: "EXIT_EXECUTION_FAILED",
              closeExecutionResult
            };

            actions.push(failedExitPayload);
          ...
          exitPayload.closeExecution = closeExecutionResult;
      ...
      // ================= FORCE FIRST TRADE SYSTEM ENTRY =================
// Noodschakelaar: eerste accepted TradeSystem candidate direct naar /api/signal.
// Bypasst de strategy filters, maar blijft via dezelfde execution bridge lopen.
if (
  FORCE_FIRST_TS_ENTRY &&
  !forceEntryUsed &&
  LIVE_TRADING_ENABLED &&
  !hasAnyOpenPositionForSymbol(c.symbol) &&
  !processingLocks.has(symbolLockKey)
) {
  processingLocks.add(symbolLockKey);

  try {
    const forceRegime = getVolatilityRegime(c);
    const forceRegimeLevel = getRegimeKey(forceRegime, scanRegime);
    const forceRegimeForConfluence = getRegimeValueForConfluence(forceRegime, scanRegime);

    const forceLiquidity = getLiquidityZones(c, obData);

    const forceHasLiquidationData =
      Array.isArray(liquidation?.clusters) &&
      liquidation.clusters.length > 0;

    const forceRsiSignal = rsiData?.mtf
      ? getRSISignal(rsiData.mtf, c.side)
      : null;

    const forceRsi = Number.isFinite(Number(forceRsiSignal?.rsi))
      ? Number(forceRsiSignal.rsi)
      : 50;

    const forceRsiHTF = Number.isFinite(Number(forceRsiSignal?.mean1h))
      ? Number(forceRsiSignal.mean1h)
      : null;

    const forceRsiZone = forceRsiSignal?.zones
      ? getRsiZone(forceRsiSignal)
      : "UNKNOWN";

    const forceRsiContext =
      forceRsiSignal?.zones && Number.isFinite(forceRsi)
        ? {
            valid: true,
            rsi: forceRsi,
            zones: forceRsiSignal.zones
          }
        : null;

    const forceConfluence = Number(
      calculateConfluence(
        c,
        obData,
        forceLiquidity,
        funding,
        forceRegimeForConfluence,
        forceHasLiquidationData ? liquidation : null,
        forceRsiContext
      ) || 0
    );
    ...
    const forceSniperScore = Number(forceSniper?.score || 0);

    let forceRisk = null;

    try {
      forceRisk = await calculateRisk(
        c,
        obData,
        forceLiquidity,
        forceHasLiquidationData ? liquidation : null
    ...
    const fallbackRiskPct = 0.006;
    const fallbackRewardPct = 0.012;

    const fallbackSl = isBull
      ? Number(c.price) * (1 - fallbackRiskPct)
      : Number(c.price) * (1 + fallbackRiskPct);

    const fallbackTp = isBull
      ? Number(c.price) * (1 + fallbackRewardPct)
      : Number(c.price) * (1 - fallbackRewardPct);

    if (
      !forceRisk ||
      !Number(forceRisk.sl || 0) ||
      !Number(forceRisk.tp || 0)
    ) {
      forceRisk = {
        entry: Number(c.price),
        sl: fallbackSl,
        tp: fallbackTp,
        rr: calculateRRFromPrices(Number(c.price), fallbackSl, fallbackTp, isBull),
        slSource: "force-fallback",
        tpSource: "force-fallback"
      };
    }

    const forceRR =
      calculateRRFromPrices(
        Number(c.price),
        Number(forceRisk.sl),
        Number(forceRisk.tp),
        isBull
      ) || Number(forceRisk.rr || 0) || 2;

    const setupClass = "FORCE";
    const reasonEntry = "FORCE_FIRST_TS_ENTRY";
    ...
    const position = {
      ...
      stage: c.stage,
      scannerStage: c.scannerStage || c.stage,
      stageSource: c.stageSource || "force",
      uiOnly: Boolean(c.uiOnly),
      ...
      entry: Number(c.price),
      entries: [Number(c.price)],
      maxEntries: 1,
      lastEntryAt: Date.now(),

      sl: Number(forceRisk.sl),
      initialSl: Number(forceRisk.sl),
      tp: Number(forceRisk.tp),
      rr: Number(forceRR),
      baseRR: Number(forceRR),
      tpRewardMultiplier: 1,

      grade: "FORCE",
      gradePoints: 0,
      recommendedRisk: "force",

      confluence: forceConfluence,
      sniper: forceSniper?.type || "FORCE",
      sniperScore: forceSniperScore,

      slSource: forceRisk.slSource || "force",
      tpSource: forceRisk.tpSource || "force",
    ...
    const entryPayload = {
      ...buildCommonPayload(c, flow, forceSniper, funding, obData),
      action: "ENTRY",
      reason: reasonEntry,
      setupClass,
    ...
    const executionResult = await executeTradeSystemEntryViaSignalRoute({
      c,
      position,
      entryPayload,
      setupClass,
    ...
    if (!executionResult.ok) {
      const failedPayload = {
        ...entryPayload,
        action: "WAIT",
        reason: "FORCE_ENTRY_EXECUTION_FAILED",
        executionResult
      };

      actions.push(failedPayload);
    ...
    position.execution = executionResult;
    ...
    await logAction(entryPayload, forceRegimeLevel, btcState, shouldLog);
    ...
    processingLocks.delete(symbolLockKey);
      ...
      try {
        ...
        if (!executionResult.ok && REQUIRE_LIVE_EXECUTION) {
          const failedPayload = {
            ...entryPayload,
            action: "WAIT",
