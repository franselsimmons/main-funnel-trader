...
// ================= STRATEGY VERSION =================
// Reset bewust: A-only tuning, B uit, short-LOWER-RSI fix en schone R-audit.
const STRATEGY_VERSION = "TS_V11_1_A_FOCUS_SOFT_GATES_RESET";
...
const TRADE_SYSTEM_FILTERS = Object.freeze([
  ...
  },
  {
    phase: "ENTRY",
    filter: "A/GOD ONLY",
    rule: "GOD or A setup only during A-only tuning. B disabled. No C_ENTRY. No scaling.",
    blockReason: "SETUP_NOT_READY / B_DISABLED_A_ONLY"
  }
]);
  ...
  return {
    ...
    answers: {
      ...
      breakEvenStops: {
        ...
        examples: breakEvenStops.slice(-10).map(t => `${t.symbol}_${t.side}_exitR=${t.exitR}_mfe=${t.mfeR}_mae=${t.maeR}`)
      },
