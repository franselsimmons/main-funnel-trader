// tradeSystem.js - Definitieve gebalanceerde versie
// - C entry verwijderd
// - A entry: sniper>=70, conf>=75, rr>=1.1
// - B entry: sniper>=60, conf>=65, rr>=1.05
// - MID RSI alleen blok bij conf<60
// - Shorts ook in LOWER_1/LOWER_2
// - Momentum: change1h>0.1 of change24>1.2 of flow=TREND
// - Fake breakout alleen bij NEUTRAL+conf<65
// - Long UPPER alleen UPPER_2/3 blok bij conf<75
// - RSI check: alleen actief bij conf<60
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // ❗ Long in UPPER zones: alleen UPPER_2/3 blokkeren bij lage confluence (UPPER_1 mag door)
    if (isBull && ["UPPER_2", "UPPER_3"].includes(rsiZone) && confluence < 75) {
      continue;
    }
    ...
    // FAKE BREAKOUT (alleen blok bij NEUTRAL + lage confluence)
    ...
    // ========== RSI-LOGICA MET CONFLUENCE DREMPEL ==========
    ...
    if (isBull) {
      ...
      if (!rsiOK && confluence < 60 && !strongSetup) {
    ...
    // MID filter (alleen bij lage confluence)
    ...
    // ========== MOMENTUM (VERSOEPELD) ==========
    const hasMomentum =
      Math.abs(Number(c.change1h || 0)) > 0.1 ||
      Math.abs(Number(c.change24 || 0)) > 1.2 ||
      flow.type === "TREND";

    if (!hasMomentum && confluence < 70) {
      const waitPayload = buildWait(c, "NO_MOMENTUM", flow, sniper, confluence, rr, funding, ob, riskBase, null, null, null);
      actions.push(waitPayload);
      continue;
    }

    // ========== FAKE BREAKOUT (ALLEEN BIJ NEUTRAL + LAGE CONFLUENCE) ==========
    if (!isValidFakeBreakout && flow.type === "NEUTRAL" && confluence < 65) {
    ...
    // ========== POSITION MANAGEMENT (zelfde als vorige versie) ==========
    if (prev) {
      const pos = { ...prev };
