...
// ================= EXECUTION QUALITY (EXTREME WINRATE) =================
const MAX_SPREAD_PCT = 0.0025; 
const MIN_DEPTH_USD_1P = 250000; 
const MIN_RR_FLOOR = 1.15; // RR verhoogd naar 1.15

// ================= DYNAMIC RR FLOOR =================
const GRADE_A_MIN_RR_FLOOR = 1.15; // RR verhoogd
const GRADE_B_MIN_RR_FLOOR = 1.25; // RR verhoogd
const GRADE_C_MIN_RR_FLOOR = 1.40; // RR verhoogd
const COUNTERTREND_MIN_RR_FLOOR = 1.60; // RR verhoogd
const BUILDUP_MIN_RR_FLOOR = 1.30; // RR verhoogd

// ================= NEUTRAL OB TUNING =================
const NEUTRAL_OB_ENTRY_A_MIN_CONF = 82;
const NEUTRAL_OB_ENTRY_A_MIN_SNIPER = 80;
const NEUTRAL_OB_ENTRY_A_MIN_RR = 1.20; // RR verhoogd

const NEUTRAL_OB_ENTRY_B_MIN_CONF = 85;
const NEUTRAL_OB_ENTRY_B_MIN_SNIPER = 82;
const NEUTRAL_OB_ENTRY_B_MIN_SCORE = 85;
const NEUTRAL_OB_ENTRY_B_MIN_RR = 1.30; // RR verhoogd

const NEUTRAL_OB_ALMOST_A_MIN_CONF = 88;
const NEUTRAL_OB_ALMOST_A_MIN_SNIPER = 86;
const NEUTRAL_OB_ALMOST_MIN_RR = 1.30; // RR verhoogd

// ================= BUILDUP ELITE ENTRY =================
const BUILDUP_ELITE_MIN_CONF = 92;
const BUILDUP_ELITE_MIN_SNIPER = 85;
const BUILDUP_ELITE_MIN_SCORE = 80;
const BUILDUP_ELITE_MIN_RR = 1.30; // RR verhoogd
const BUILDUP_ELITE_MIN_TF = 3;
...
function getSetupGrade({
  ...
  if(spread <= 0.0025 && depth >= 250000) points += 1;
  ...
  // Punten voor RR aangepast
  if(rr >= 1.5) points += 1;
  else if(rr < 1.0) points -= 1;
...
function getDynamicMinRrFloor({
  ...
  if(
    ...
    floor = Math.min(floor, 1.10); // Verhoogd, mag net iets onder de base duiken voor perfecte trades
  }

  return clamp(floor, 1.10, 1.80); // Clamp waarden strakker gezet
}
...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    // 1. HARDE BTC ALIGNMENT: Traden tegen de algehele markt wordt VOLLEDIG geblokkeerd
    if(isBull && btcState !== "BULLISH"){
      const waitPayload = buildWait(c, "BTC_NOT_BULLISH", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if(!isBull && btcState !== "BEARISH"){
      const waitPayload = buildWait(c, "BTC_NOT_BEARISH", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }

    // 2. ANTI-CHASING FILTER: Blokkeer trades die al gigantisch gestegen/gedaald zijn
    if(sniper?.type === "LATE_CONTINUATION" || sniper?.type === "LATE_MOVE" || sniper?.type === "OVEREXTENDED"){
      const waitPayload = buildWait(c, "CHASING_LATE_MOVE", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade);
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    }
    ...
    if(
      (c.stage === "entry" && c.tfStrength < 2) ||
      (c.stage === "almost" && c.tfStrength < 2) ||
    ...
    // 3. MINIMALE CONFLUENCE VERDER VERHOOGD (70 -> 75)
    const minConf = 75;
    ...
    if(!hasLiquidationRoom && confluence < 82){
    ...
    if((badSpread || badDepth) && confluence < 88){
    ...
    if(Math.abs(fundingRate) > 0.015 && confluence < 88){
    ...
    if(isBull && fundingRate > 0.012 && confluence < 88){
