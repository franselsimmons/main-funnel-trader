...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // ========== LIQUIDATION (geen Binance, of zwaar throttled) ==========
    let liquidation = null;
    // Slechts 30% kans om Binance liquidation endpoint te gebruiken (ter voorkoming van 451)
    if (Math.random() > 0.7) {
      try {
        liquidation = await getLiquidationZones(c.symbol + "USDT", c.price);
      } catch (e) {
        // negeer fout (451 of timeout)
      }
    }
    const hasLiquidationData = Array.isArray(liquidation?.clusters) && liquidation.clusters.length > 0;

    // Markt context voor risk manager
    const marketCtx = {
