// tradeSystem.js - Aangepast met versoepelingen volgens pro-model
...
// ================= STATE =================
const memory = new Map();           // open posities
const notifyState = new Map();      // notificatie flags
const cooldownMap = new Map();      // cooldown per key (symbool_side)
const symbolCooldownMap = new Map(); // cooldown per symbool (alle kanten)
const processingLocks = new Set();
const lastSignalMap = new Map();    // extra anti‑spam (1 uur per symbool)
...
// ========== AANGEPASTE getSetupGrade (ruimer A/B + downgrade bij lage confluence) ==========
function getSetupGrade({ c, ob, flow, sniper, confluence, rr, hasLiquidationData, isBull }) {
  ...
  // A is nu vanaf 9 punten (was 9), B vanaf 7 punten (was 8)
  if (points >= 9) { grade = "A"; recommendedRisk = "normal"; }
  else if (points >= 7) { grade = "B"; recommendedRisk = "small"; }
  
  // Extra filter: downgrade A naar B als confluence < 70
...
export async function processTrades(input, options = {}) {
  ...
  if (Array.isArray(input)) {
    ...
    for (const c of candidates) {
      c.analysisType = deepCandidates.includes(c) ? "DEEP" : "LIGHT";
    }
  ...
  const fetchCoinData = async (c) => {
    ...
    try {
      const raw = await cachedFetch(
        ...
        async () => {
          ...
          for (let i = 0; i < 2; i++) {
            try {
              data = await fetchOrderBook(symbol + "USDT");
              if (data) break;
            } catch {}
      ...
      if (raw) {
        ...
        ob = { ...DEFAULT_OB, ...(analyzed || {}), fetchFailed: false };
    ...
    let funding = { rate: 0 };
    try {
      funding = await cachedFetch(`fund_${symbol}`, () => fetchFunding(symbol + "USDT"), 120000);
    } catch {}
    ...
    let candles4h = null;
    if (c.tfStrength >= 2) {
      candles4h = await cachedFetch(`c4h_${cleanPair}`, () => fetchCandles(cleanPair, "4h", 100), 30000).catch(() => null);
    }
    ...
    try {
      liquidation = await getLiquidationZones(symbol);
    } catch (e) {
  ...
  for (const originalCoin of candidates) {
    ...
    // 🔒 HARD BLOCK 1: globale lastSignalMap (1 uur)
    if (lastSignalMap.has(c.symbol) && Date.now() < lastSignalMap.get(c.symbol)) {
      continue;
    }

    // 🔒 HARD BLOCK 2: symbolCooldownMap (trade system cooldown)
    if (symbolCooldownMap.has(c.symbol) && Date.now() < symbolCooldownMap.get(c.symbol)) {
      continue;
    }

    // 🔒 HARD BLOCK 3: positie open voor exact deze kant
    if (memory.has(key)) {
      continue;
    }

    // 🔒 HARD BLOCK 4: andere kant open (geen long+short tegelijk)
    if (memory.has(`${c.symbol}_bull`) || memory.has(`${c.symbol}_bear`)) {
      continue;
    }
    ...
    // RSI data valid check (geen stille fallback naar 50)
    const rsi = Number.isFinite(rsiSignal?.rsi) ? rsiSignal.rsi : null;
    if (rsi === null) {
      const waitPayload = buildWait(c, "RSI_DATA_INVALID", flow, rsiSignal, 0, 0, funding, ob, null, { grade: "C", points: 0, recommendedRisk: "watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    // RSI zones + killer filters
    const rsiZone = rsiSignal?.zones
      ? (rsi <= rsiSignal.zones.L3 ? "LOWER_3"
        : rsi <= rsiSignal.zones.L2 ? "LOWER_2"
        : rsi <= rsiSignal.zones.L1 ? "LOWER_1"
        : rsi >= rsiSignal.zones.U3 ? "UPPER_3"
        : rsi >= rsiSignal.zones.U2 ? "UPPER_2"
        : rsi >= rsiSignal.zones.U1 ? "UPPER_1"
        : "MID")
      : "MID";

    if (rsiZone === "MID") {
      const waitPayload = buildWait(c, "RSI_MID_NO_EDGE", flow, rsiSignal, 0, 0, funding, ob, null, { grade: "C", points: 0, recommendedRisk: "watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    // AANGEPAST: early RSI toestaan bij LOWER_1/UPPER_1 met hoge sniper score
    const earlyRSI =
      (isBull && rsiZone === "LOWER_1" && getSniperScore(sniper) >= 80) ||
      (!isBull && rsiZone === "UPPER_1" && getSniperScore(sniper) >= 80);

    if (isBull && !["LOWER_2", "LOWER_3"].includes(rsiZone) && !earlyRSI) {
      const waitPayload = buildWait(c, "RSI_NOT_OVERSOLD_ENOUGH", flow, rsiSignal, 0, 0, funding, ob, null, { grade: "C", points: 0, recommendedRisk: "watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    // Extra pro: LOWER_2 alleen als prijs nog daalt (blijft behouden)
    if (isBull && rsiZone === "LOWER_2" && c.change1h > -0.2) {
      const waitPayload = buildWait(c, "RSI_NOT_DEEP_ENOUGH", flow, rsiSignal, 0, 0, funding, ob, null, { grade: "C", points: 0, recommendedRisk: "watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    if (!isBull && !["UPPER_2", "UPPER_3"].includes(rsiZone) && !earlyRSI) {
      const waitPayload = buildWait(c, "RSI_NOT_OVERBOUGHT_ENOUGH", flow, rsiSignal, 0, 0, funding, ob, null, { grade: "C", points: 0, recommendedRisk: "watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }

    if (!isBull && rsiZone === "UPPER_2" && c.change1h < 0.2) {
      const waitPayload = buildWait(c, "RSI_NOT_HIGH_ENOUGH", flow, rsiSignal, 0, 0, funding, ob, null, { grade: "C", points: 0, recommendedRisk: "watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    // Confluence en sniper (let op: sniper is nog niet gedeclareerd, moet na rsiSignal)
    // We verschuiven sniper naar hier, na rsiSignal en earlyRSI.
    ...
    // ================= AANGEPAST MOMENTUM (zachter voor early moves) =================
    const strongMomentum =
      Math.abs(Number(c.change1h || 0)) > 0.35 &&
      Math.abs(Number(c.change24 || 0)) > 3 &&
    ...
    if (!strongMomentum && !softMomentum) {
      const waitPayload = buildWait(c, "NO_MOMENTUM", flow, sniper, confluence, rr, funding, ob, riskBase, null, null, null);
      actions.push(waitPayload);
      continue;
    }

    // ================= AANGEPAST FAKE BREAKOUT (niet altijd verplicht) =================
    ...
    if (candles15m.length >= 20) {
      ...
      if (isBull) {
        if (c.price > recentLow && c.price < recentLow * 1.01) candleFakeBreakout = true;
      } else {
        if (c.price < recentHigh && c.price > recentHigh * 0.99) candleFakeBreakout = true;
      }
    }

    const isValidFakeBreakout = fakeBreakout || candleFakeBreakout;
    
    const allowWithoutFakeBreakout =
      confluence >= 80 &&
      sniper.score >= 75 &&
      flow.type === "TREND";

    if (!isValidFakeBreakout && !allowWithoutFakeBreakout) {
      const waitPayload = buildWait(c, "NO_FAKE_BREAKOUT", flow, sniper, confluence, rr, funding, ob, riskBase, null, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    const sniperOK = sniper.score >= 71;
    const sniperOK_legacy = (sniper?.valid && getSniperScore(sniper) >= 55) || confluence >= 85;
    ...
    const gradeOK = setupGrade.grade === "A" || setupGrade.grade === "B";
    ...
    const godModeEntry = eliteEntry && sniper.score >= 85 && confluence >= 85 && rr >= 1.2;
    const shouldEnter = eliteEntry || bEntry;
    const reasonEntry = godModeEntry ? "GOD_MODE" : eliteEntry ? "ELITE_ENTRY" : bEntry ? "B_ENTRY" : "NONE";

    console.log(`🔍 ${c.symbol} (${isDeep ? "DEEP" : "LIGHT"}): sniper=${sniper.score}, conf=${confluence}, rr=${rr.toFixed(2)}, grade=${setupGrade.grade}, elite=${eliteEntry}, b=${bEntry}, godmode=${godModeEntry}, rsiZone=${rsiZone}, fakeBreakout=${isValidFakeBreakout}`);

    // ========== POSITION MANAGEMENT (met scaling) ==========
    if (prev) {
      const pos = { ...prev };
    ...
    if (vol === "LOW" && confluence < lowVolConfluenceLimit && isDeep) {
      actions.push(buildWait(c, "LOW_VOL", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }
    if (flow.type === "NEUTRAL" && confluence < 58 && isDeep) {
      actions.push(buildWait(c, "NO_FLOW", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }
    
    // AANGEPAST RR check met override voor top setups
    const rrOverride = confluence >= 85 && sniper.score >= 80;
    if (rr < minRrFloor && !rrOverride && isDeep) {
    ...
    if ((c.stage === "entry" && c.tfStrength < 1) || (c.stage === "almost" && c.tfStrength < 1) || (c.stage === "buildup" && c.tfStrength < BUILDUP_ELITE_MIN_TF)) {
      if (isDeep) {
        actions.push(buildWait(c, "ENTRY_FILTERED", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
        continue;
      } else {
        actions.push(buildWait(c, "LIGHT_NO_ENTRY", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
        continue;
      }
    }
    ...
    if (confluence < minConf && isDeep) {
      actions.push(buildWait(c, "LOW_CONFLUENCE", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, minConf, null));
      continue;
    }

    const obAgainst = isObAgainstSide(ob, isBull);
    const hasLiquidationRoom = true; // wordt elders gebruikt
    if (obAgainst && confluence < 75 && isDeep) {
      actions.push(buildWait(c, "OB_AGAINST", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }
    if (!hasLiquidationRoom && confluence < 75 && isDeep) {
      actions.push(buildWait(c, "NO_LIQUIDATION_ROOM", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }
    ...
    if ((badSpread || badDepth) && confluence < marketQualityConfluenceLimit && isDeep) {
      actions.push(buildWait(c, "BAD_MARKET_QUALITY", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }
    if (ob.bias === "NEUTRAL" && confluence < 50 && !isNeutralObEntryException({ c, flow, sniper, confluence, rr, setupGrade, counterTrend: (btcState === "BULLISH" && !isBull) || (btcState === "BEARISH" && isBull) }) && isDeep) {
      actions.push(buildWait(c, "OB_NEUTRAL_LOW_CONF", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }

    const fundingRate = Number(funding?.rate || 0);
    if (Math.abs(fundingRate) > 0.015 && confluence < 85 && isDeep) {
      actions.push(buildWait(c, "EXTREME_FUNDING", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade, null, null));
      continue;
    }
    if (isBull && fundingRate > 0.012 && confluence < 85 && isDeep) {
    ...
    if (shouldEnter && !hasAnyOpenPositionForSymbol(c.symbol)) {
      ...
      if (certaintyMode === "safe") {
        ...
        if (sniper.score >= 90) finalTp *= 0.9;
        else if (sniper.score >= 80) finalTp *= 0.95;
        if (isBull) {
          if (rsi < 30) finalTp *= 1.12;
          else if (rsi < 45) finalTp *= 1.06;
        } else {
          if (rsi > 70) finalTp *= 0.88;
          else if (rsi > 55) finalTp *= 0.94;
        }
      }
      const position = {
  ...
  if (actions.length === 0 && candidates.length > 0) {
    ...
    const fallbackActions = candidates.map(c => ({
      symbol: c.symbol,
      side: c.side,
      action: "WAIT",
