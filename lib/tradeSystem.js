...
// Import for analyze family classification
...
function extractAnalyzeFamilyIdFromRow(row = {}) {
  ...
  try {
    if (typeof classifyAnalyzeEvent === "function") {
      // Belangrijk:
      // Verwijder invalid MICRO familyId voordat classifyAnalyzeEvent draait,
      // anders pakt analyze familyId niet door de `familyId || analyzeFamilyId` chain.
      const safeRow = { ...row };
      ...
      const classified = classifyAnalyzeEvent(safeRow);
      const familyId = normalizeAnalyzeFamilyId(classified?.familyId);
