...
const DURABLE_LOCK_RETRY_MS = Number(
  process.env.TRADE_SYSTEM_LOCK_RETRY_MS || 1000
);

const DURABLE_LOCK_WAIT_MS = Number(
  process.env.TRADE_SYSTEM_LOCK_WAIT_MS || 180_000
);

const DURABLE_LOCK_ATTEMPTS = Math.max(
  1,
  Math.ceil(DURABLE_LOCK_WAIT_MS / DURABLE_LOCK_RETRY_MS)
);

const DURABLE_LOCK_TTL_MS = Number(
  process.env.TRADE_SYSTEM_LOCK_TTL_MS || 120_000
);

// Audit / feature-store caps
const MAX_RECENT_ENTRY_AUDIT_ROWS = 150;
const MAX_CLOSED_TRADE_AUDIT_ROWS = Number(
  process.env.TS_MAX_CLOSED_TRADE_AUDIT_ROWS || 500
);
const MAX_FEATURE_STORE_ROWS = Number(
  process.env.TS_MAX_FEATURE_STORE_ROWS || 1_000
);
const MAX_SHADOW_OUTCOME_ROWS = Number(
  process.env.TS_MAX_SHADOW_OUTCOME_ROWS || 1_000
);
...
async function acquireRuntimeLock(owner) {
  ...
  const deadline = startedAt + DURABLE_LOCK_WAIT_MS;
  ...
  while (Date.now() <= deadline) {
    ...
    try {
      ...
      if (result === "OK") {
        console.log("TRADE SYSTEM LOCK ACQUIRED:", JSON.stringify({
          attempt,
      ...
      if (ttlMs === -1) {
        console.warn("TRADE SYSTEM LOCK WITHOUT TTL DURING WAIT, CLEARING:", JSON.stringify({
          attempt,
          key: RUNTIME_LOCK_KEY
        }));

        await redisCommand(["DEL", RUNTIME_LOCK_KEY]).catch(() => null);
        continue;
      }

      console.log("TRADE SYSTEM LOCK WAITING:", JSON.stringify({
        attempt,
        ttlMs,
        waitedMs: Date.now() - startedAt,
        maxWaitMs: DURABLE_LOCK_WAIT_MS,
        key: RUNTIME_LOCK_KEY
      }));

      const sleepMs =
        Number.isFinite(Number(ttlMs)) && Number(ttlMs) > 0
          ? Math.min(DURABLE_LOCK_RETRY_MS, Number(ttlMs) + 50)
          : DURABLE_LOCK_RETRY_MS;
  ...
  console.error("TRADE SYSTEM LOCK WAIT TIMEOUT:", JSON.stringify({
    attempts: attempt,
    waitedMs: Date.now() - startedAt,
    maxWaitMs: DURABLE_LOCK_WAIT_MS,
    key: RUNTIME_LOCK_KEY
...
// ================= MAX DEEP CANDIDATES =================
const MAX_DEEP_CANDIDATES = Number(
  process.env.TS_MAX_DEEP_CANDIDATES || 30
);
...
export async function processTrades(input, options = {}) {
  ...
  if (!lockAcquired) {
    console.error("TRADE_SYSTEM_LOCK_WAIT_TIMEOUT:", JSON.stringify({
      runId,
      lockKey: RUNTIME_LOCK_KEY,
      waitedMs: DURABLE_LOCK_WAIT_MS,
      reason: "PREVIOUS_RUN_STILL_ACTIVE_AFTER_WAIT",
      ts: Date.now()
    }));

    throw new Error("TRADE_SYSTEM_DURABLE_LOCK_WAIT_TIMEOUT");
  }
