// tradeSystem.js
// - Geen C_ENTRY
// - Geen MAX_DEEP/MAX_LIGHT cap
// - Geen scaling / bijkopen
// - Open posities altijd opnieuw checken voor TP/SL
// - Position management vóór entry-blocks
// - Confluence vóór sniper
// - Fake breakout via fakeBreakoutEngine
// - BTC/regime via options
// - Scanner-entry = candidate, geen echte entry
...
function buildCommonPayload(c, flow, sniper, funding, ob) {
  return {
    ...
    qualityScore: Number(ob?.qualityScore || 0),
    ...
    fakeBreakout: c?._debugFakeBreakout || "NONE",
    ts: Date.now()
  };
}
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    const candleClose = latestClose(rsiData?.candles15m || []);
    ...
    const fakeBreakoutResult = detectFakeBreakout(
      c,
      hasLiquidationData ? liquidation : null,
      liquidity,
      rsiData?.candles15m || []
    ...
    const isValidFakeBreakout = Boolean(fakeBreakoutResult.valid);
    c._debugFakeBreakout = fakeBreakoutResult.type;

    console.log(
      `📊 DATA: ${c.symbol} | hasRSI=${!!rsiContext} | conf=${confluence} | sniper=${sniperScore} | rr=${formatRR(rr)} | fake=${fakeBreakoutResult.type}:${fakeBreakoutResult.score}`
    );
    ...
    console.log(
      `🔍 ${c.symbol} (${c.analysisType || "DEEP"}): sniper=${sniperScore}, conf=${confluence}, rr=${formatRR(rr)}, grade=${setupGrade.grade}, elite=${eliteEntry}, b=${bEntry}, godmode=${godModeEntry}, rsiZone=${rsiZone}, fakeBreakout=${fakeBreakoutResult.type}:${fakeBreakoutResult.score}`
    );
    ...
    const position = {
      ...
      fakeBreakout: fakeBreakoutResult.type,
      fakeBreakoutScore: fakeBreakoutResult.score,
    ...
    const entryPayload = {
      ...
      fakeBreakout: position.fakeBreakout,
      fakeBreakoutScore: position.fakeBreakoutScore
    };
