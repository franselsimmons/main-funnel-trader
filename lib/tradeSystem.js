...
const STRATEGY_VERSION = "TS_V11_3_DEPTH_ANTI_CHASE_BTC_BEAR_EXCEPTION";
...
const MID_BULL_MAX_SPREAD_PCT = 0.00135;
...
const TRADE_SYSTEM_FILTERS = Object.freeze([
  ...
  },
  {
    ...
    blockReason: "BULL_RSI_EXTENSION_BLOCK / BULL_TREND_NO_PULLBACK / BULL_TOO_FAR_FROM_PULLBACK_ZONE"
  },
  {
    phase: "ENTRY",
  ...
  },
  {
    phase: "ENTRY",
    filter: "A/GOD/B+ ONLY",
    rule: "GOD, A or strict B+ setup only. No C_ENTRY. No scaling.",
...
function validateBullAntiChase(candidate) {
  ...
  if (!hasPullback) {
    return { ok: false, reason: "BULL_TREND_NO_PULLBACK" };
  }

  if (Number(candidate.distanceFromLocalHighPct ?? 0) > MAX_BULL_DISTANCE_FROM_LOCAL_HIGH_PCT) {
...
function logOptimizerReport({ reasonTable, btcState, runId }) {
  ...
  const postExitReport = buildPostExitReport();
  const bestSetupAdvice = buildBestSetupAdvice();
  const finalFilterDecision = buildFinalFilterDecision();
  ...
  console.log("TS_FINAL_FILTER_DECISION", JSON.stringify(finalFilterDecision));
  ...
  console.log("TS_BEST_SETUP_ADVICE", JSON.stringify(bestSetupAdvice));

  console.log("TS_FEATURE_COHORTS_TOP", JSON.stringify({
    tag: "TS_FEATURE_COHORTS_TOP",
    strategyVersion: STRATEGY_VERSION,
    runId,
    btcState,
    ts: Date.now(),
    sample: bestSetupAdvice.sample,
    bestSetups: bestSetupAdvice.bestSetups.slice(0, 10),
    badSetups: bestSetupAdvice.badSetups.slice(0, 10),
    blockedButWouldHaveWon: bestSetupAdvice.blockedButWouldHaveWon.slice(0, 10),
    allowedButLost: bestSetupAdvice.allowedButLost.slice(0, 10)
  }));

  console.log("TS_FEATURE_THRESHOLD_OPTIMIZER", JSON.stringify({
    tag: "TS_FEATURE_THRESHOLD_OPTIMIZER",
    strategyVersion: STRATEGY_VERSION,
    runId,
    btcState,
    ts: Date.now(),
    sample: bestSetupAdvice.sample,
    recommendedFilterValues: bestSetupAdvice.recommendedFilterValues
  }));
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      if (btcState === "NEUTRAL" && Number(c.moveScore || 0) < 65) {
      ...
      // ================= SHORT LOWER RSI BLOCK WITH EXCEPTION =================
      const allowLowerOneShortContinuation =
        ...
        sniperScore >= 75 &&
        rr >= 1.25 &&
        ["BEARISH", "STRONG_BEAR", "NEUTRAL"].includes(btcState);
      ...
      const eliteMomentumBypass =
        ...
        rr >= A_ENTRY_MIN_RR &&
        Math.abs(Number(c.change1h || 0)) >= 0.10;

      if (!strongMomentum && !softMomentum && !eliteMomentumBypass) {
      ...
      const tpRewardMultiplier = getTpRewardMultiplier({
        setupClass: setupClass === "A_SHORT_EXCEPTION" ? "A" : setupClass,
        certaintyMode,
        sniperScore,
        rsi,
        isBull
      ...
      const effectiveGrade = setupClass === "A_SHORT_EXCEPTION"
        ? "A_SHORT_EXCEPTION"
        : setupGrade.grade;

      const effectiveGradePoints = setupClass === "A_SHORT_EXCEPTION"
        ? setupGrade.points + 1
        : setupGrade.points;

      const effectiveRecommendedRisk = setupClass === "A_SHORT_EXCEPTION"
        ? "reduced"
