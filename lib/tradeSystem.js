...
// ================= V12 SIDE EXPOSURE / RSI TIMING HELPERS =================
...
function countOpenPositionsBySide(side) {
  ...
  for (const pos of memory.values()) {
    if (String(pos?.side || "").toLowerCase() === s) {
      count++;
    }
  }
...
function countRecentLossesBySide(side, lookbackMs = SIDE_LOSS_LOOKBACK_MS) {
  const s = String(side || "").toLowerCase();
...
function validateSideExposureGateV12(candidate) {
  if (!ENABLE_V12_SIDE_EXPOSURE_GATE) {
    return { ok: true, reason: "SIDE_EXPOSURE_DISABLED" };
  }
  ...
  const totalOpen = countOpenPositionsTotal();
  const sideOpen = countOpenPositionsBySide(side);
  const recentSideLosses = countRecentLossesBySide(side);

  const elite = isEliteAQuality({
    confluence,
    sniperScore
  });

  if (totalOpen >= MAX_OPEN_POSITIONS_TOTAL && !elite) {
    return {
      ok: false,
      reason: "V12_MAX_TOTAL_EXPOSURE"
    };
  }

  if (sideOpen >= MAX_OPEN_POSITIONS_PER_SIDE && !elite) {
    return {
      ok: false,
      reason: "V12_MAX_SIDE_EXPOSURE"
    };
  }

  if (
    recentSideLosses >= SIDE_LOSS_THROTTLE_MIN_LOSSES &&
    !elite
  ) {
    return {
      ok: false,
      reason: "V12_SIDE_LOSS_THROTTLE"
    };
  }

  return {
    ok: true,
    reason: "V12_SIDE_EXPOSURE_OK"
  };
}
