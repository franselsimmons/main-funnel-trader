// tradeSystem.js - RSI versoepeld + strongSetup override
// - C entry verwijderd
// - A entry: sniper>=70, conf>=75, rr>=1.1
// - B entry: sniper>=60, conf>=65, rr>=1.05
// - MID RSI block alleen bij conf < 60
// - Shorts toegestaan in LOWER_1 en LOWER_2
// - Sterke setups (conf>=65, TREND, rr>=1.0) mogen RSI bypassen
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // ========== RSI-LOGICA (VERSOEPELD) ==========
    // Sterke setup override: als confluence >=65, TREND en rr >=1.0, dan RSI minder zwaar
    const strongSetup = confluence >= 65 && flow.type === "TREND" && rr >= 1.0;
    
    // Trend continuation alleen voor LOWER_1/UPPER_1 met hogere eisen
    const trendContinuationRSI =
      ...
      sniperScore >= 65 &&
      (
        (isBull && rsiZone === "LOWER_1") ||
        (!isBull && rsiZone === "UPPER_1")
      );

    // Shorts nu ook toegestaan in LOWER_1 en LOWER_2 (was alleen UPPER)
    if (isBull) {
      const rsiOK =
    ...
    if (!isBull) {
      // SHORT: UPPER_2, UPPER_3 + LOWER_1 + LOWER_2 (nieuw)
      const rsiOK =
        ["UPPER_2", "UPPER_3", "UPPER_1"].includes(rsiZone) ||
        earlyRSI ||
        trendContinuationRSI ||
        ["LOWER_1", "LOWER_2"].includes(rsiZone);
    ...
    // 🔥 MID filter nu alleen bij lage confluence
    if (rsiZone === "MID" && confluence < 60 && !strongSetup) {
      const waitPayload = buildWait(c, "RSI_MID_LOW_CONF", flow, sniper, confluence, rr, funding, ob, riskBase, { grade:"C", points:0, recommendedRisk:"watch" }, null, null);
      actions.push(waitPayload);
      continue;
    }
    ...
    // Momentum (aangescherpt)
    const strongMomentum = Math.abs(Number(c.change1h||0)) > 0.3 && Math.abs(Number(c.change24||0)) > 3 && (flow.type === "TREND" || flow.type === "BUILDING");
    const softMomentum = Math.abs(Number(c.change1h||0)) > 0.2 && Math.abs(Number(c.change24||0)) > 2 && (flow.type === "TREND" || flow.type === "BUILDING");
    ...
    // A en B entry (C verwijderd)
    const aSetupValid = stageOK && finalSniperOK && setupGrade.grade === "A" && !ob.spoof && rr >= minRrFloor;
    const eliteEntry = aSetupValid && sniperScore >= 70 && confluence >= 75 && rr >= 1.1 && c.tfStrength >= 1;
    const bSetupValid = stageOK && setupGrade.grade === "B" && !ob.spoof && rr >= 1.05;
    const bEntry = !eliteEntry && bSetupValid && sniperScore >= 60 && confluence >= 65 && rr >= 1.05 && c.tfStrength >= 1;
