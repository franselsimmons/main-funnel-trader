...
function getDynamicMinRrFloor({ c, setupGrade, flow, sniper, confluence, counterTrend }) {
  ...
  // 🔥 VERDER VERSOEPELD: minimale RR nu 0.8
...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    if (flow.type === "NEUTRAL" && confluence < 60) {
    ...
    // 🔥 VERSOEPELDE RSI (long ≤52, short ≥48)
    const rsiAligned = (isBull && rsi <= 52) || (!isBull && rsi >= 48);
    const setupValid = stageOK && sniperOK && gradeOK && !ob.spoof && rr >= minRrFloor && rsiAligned;

    // 🔥 FASE LOGICA: bij IDLE, ga direct naar READY als RSI al goed is
    ...
    if (state.phase === "WAIT") {
      const goodLongTiming = isBull && rsi <= 52;
      const goodShortTiming = !isBull && rsi >= 48;
    ...
    // 🔥 ELITE ENTRY (versoepeld: confluentie 62, sniper 65, tf 1)
    const elite = confluence >= 62 && (sniper?.score || 0) >= 65 && c.tfStrength >= 1;
