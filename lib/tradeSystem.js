...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // 🔥 VERZOEPELD: grade A of B is voldoende (C wordt nog steeds geblokkeerd)
    ...
    // 🔥 TIERED ENTRIES
    const eliteEntry = setupValid && sniper.score >= 75 && confluence >= 75 && rr >= 1.1;
    const normalEntry = setupValid && sniper.score >= 65 && confluence >= 60 && rr >= 1.1;
    const godModeEntry = eliteEntry && sniper.score >= 85 && confluence >= 85 && rr >= 1.2;

    // Voor entry gebruiken we normalEntry of eliteEntry (elite valt ook onder normal)
    const shouldEnter = normalEntry || eliteEntry;

    // Debug log
    console.log(`🔍 ${c.symbol} (${isDeep ? "DEEP" : "LIGHT"}): sniper=${sniper.score}, conf=${confluence}, rr=${rr.toFixed(2)}, grade=${setupGrade.grade}, elite=${eliteEntry}, normal=${normalEntry}, godmode=${godModeEntry}`);
    ...
    const minConf = Math.max(45, minConfDynamic); // 🔥 VERLAAGD van 50 naar 45
    ...
    // ========== TIERED ENTRY ==========
    if (shouldEnter) {
      const reasonEntry = godModeEntry ? "GOD_MODE" : (eliteEntry ? "ELITE_ENTRY" : "NORMAL_ENTRY");
