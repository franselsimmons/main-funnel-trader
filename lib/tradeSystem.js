...
import { getLiquidityZones } from "./liquidityEngine.js"; // ✅ FIXED IMPORT
import { getLiquidationZones } from "./liquidationEngine.js"; // ✅ FIXED ENGINE
...
export async function processTrades(coins){
  ...
  for(const originalCoin of coins){
    ...
    // ================= BTC BIAS (SOFT FIX) =================
    if(market?.trend === "BULLISH" && !isBull){
      if(c.moveScore < 80) continue;
    }

    if(market?.trend === "BEARISH" && isBull){
      if(c.moveScore < 80) continue;
    }

    // ================= DYNAMIC CONFLUENCE =================
    const minConf = isBull ? 65 : 60;

    if(confluence < minConf) continue;

    // ================= DYNAMIC RR =================
    const minRR = regime.level === "HIGH" ? 1.1 : 1.2;
