...
function getDynamicMinConf({ c, ob, flow, vol, isBull }) {
  ...
  if (flow.type === "TREND" && isObWithSide(ob, isBull) && sniper?.valid && getSniperScore(sniper) >= 80)
...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // 🔥 Eerst alle benodigde variabelen voor confluence definiëren
    ...
    // 🔥 Dan pas de confluence berekenen
    ...
    // 🔥 Dan sniper (die heeft confluence nodig)
    const sniper = getSniperEntry(c, ob, rsiSignal);

    // Optionele debug (tijdelijk)
    if (!confluence || isNaN(confluence)) {
      console.warn(`⚠️ BAD CONFLUENCE for ${c.symbol}:`, { confluence, liquidity, regimeForConfluence });
    }
    ...
    const sniperOK = sniper.score >= 65;
    ...
    // Elite entry: sniper valid + score >= 70 (RSI niet verplicht)
    const eliteEntry = setupValid && sniper.score >= 70;
  ...
  // Fallback als er geen acties zijn (mag niet gebeuren door LIGHT WAIT, maar voor de zekerheid)
