...
const MIN_RR_FLOOR = 1.1; // Aangepast: 1.1

// ================= DYNAMIC RR FLOOR =================
const GRADE_A_MIN_RR_FLOOR = 1.2; // Aangepast
const GRADE_B_MIN_RR_FLOOR = 1.15; // Aangepast
const GRADE_C_MIN_RR_FLOOR = 1.1; // Aangepast
const COUNTERTREND_MIN_RR_FLOOR = 1.4; // Aangepast
const BUILDUP_MIN_RR_FLOOR = 1.15; // Aangepast

// ================= NEUTRAL OB TUNING =================
const NEUTRAL_OB_ENTRY_A_MIN_CONF = 82;
const NEUTRAL_OB_ENTRY_A_MIN_SNIPER = 80;
const NEUTRAL_OB_ENTRY_A_MIN_RR = 1.15; // Aangepast

const NEUTRAL_OB_ENTRY_B_MIN_CONF = 85;
const NEUTRAL_OB_ENTRY_B_MIN_SNIPER = 82;
const NEUTRAL_OB_ENTRY_B_MIN_SCORE = 85;
const NEUTRAL_OB_ENTRY_B_MIN_RR = 1.2; // Aangepast

const NEUTRAL_OB_ALMOST_A_MIN_CONF = 88;
const NEUTRAL_OB_ALMOST_A_MIN_SNIPER = 86;
const NEUTRAL_OB_ALMOST_MIN_RR = 1.2; // Aangepast

// ================= BUILDUP ELITE ENTRY =================
const BUILDUP_ELITE_MIN_CONF = 92;
const BUILDUP_ELITE_MIN_SNIPER = 85;
const BUILDUP_ELITE_MIN_SCORE = 80;
const BUILDUP_ELITE_MIN_RR = 1.2; // Aangepast
...
function getDynamicMinRrFloor({
  ...
  if(
    ...
    floor = Math.min(floor, 1.0); // Aangepast van 1.10 naar 1.0
  }

  return clamp(floor, 1.0, 1.6); // Aangepast
}
...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    // TF filter aangepast: entry/almost nu >=1 ipv 2, buildup >=2
    if(
      (c.stage === "entry" && c.tfStrength < 1) ||
      (c.stage === "almost" && c.tfStrength < 1) ||
      (c.stage === "buildup" && c.tfStrength < 2)
    ){
    ...
    // 3. MINIMALE CONFLUENCE VERLAAGD naar 70
    const minConf = 70;
    ...
    // Sniper score drempel verlaagd naar 70 (was 75)
    const sniperOK =
      (
        sniper?.valid &&
        getSniperScore(sniper) >= 70
      ) ||
      confluence >= 88;
