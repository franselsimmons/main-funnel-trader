...
// ========== DYNAMISCHE MIN CONF (AANGEPAST: basis 42, alleen NEUTRAL/BUILDING) ==========
function getDynamicMinConf({ c, ob, flow, vol, isBull }) {
  let minConf = 42; // WAS 45

  if(ob.bias === "NEUTRAL") minConf += 1;
  if(flow.type === "BUILDING") minConf += 1;
  // tfStrength en vol tellen niet meer mee
  // if(c.tfStrength < 2) minConf += 0;
  // if(vol === "HIGH") minConf += 0;
...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    // ========== BEAR BOOST (shorts krijgen edge in bear market) ==========
    ...
    // ================= 🔥 RSI BONUS (soft i.p.v. hard gate) =================
    let rsiBonus = 0;
    if(rsiData){
      if(isType1RSIEntry(rsiData.m15, c.side)) rsiBonus += 1;
      if(isType1RSIEntry(rsiData.h1, c.side)) rsiBonus += 1;
    }
    c.tfStrength += rsiBonus * 0.3;
    ...
    // ================= 🔥 HARD FILTER: ALLEEN entry + almost =================
    if(!["entry", "almost"].includes(c.stage)){
      const waitPayload = buildWait(
        c,
        "ONLY_SCANNER_ENTRY_OR_ALMOST_ALLOWED",
        flow,
        sniper,
        confluence,
        rr,
    ...
    // ========== EXTRA: shorts in bear markt mogen iets lagere RR ==========
    if(!isBull && btcState === "BEARISH"){
    ...
    // ========== LOW VOL FILTER MINDER STRICT ==========
    if(vol === "LOW" && confluence < 55){
    ...
    if((badSpread || badDepth) && confluence < 65){
    ...
    // Stage OK: entry of almost
    const stageOK = ["entry", "almost"].includes(c.stage);

    // SNIPER drempel verlaagd naar 55 (was 60)
    const sniperOK =
      (
        sniper?.valid &&
        getSniperScore(sniper) >= 55
      ) ||
      confluence >= 85;
    ...
    // ================= RSI IS NIET MEER EEN HARD FILTER =================
    // (de bonus is al toegevoegd aan c.tfStrength)

    // ================= OPTIONELE DEBUG (uitgecomment) =================
    // console.log({
    //   symbol: c.symbol,
    //   stage: c.stage,
    //   rr,
    ...
    // Let op: rsiOk is verwijderd uit de conditie
    if(
      stageOK &&
      sniperOK &&
      gradeOK &&
