...
} from "./rotation/rotationTradeAdapter.js";

// ================= STRATEGY VERSION =================
const STRATEGY_VERSION = "TS_V12_6_ROTATION_GATE";
...
const DURABLE_LOCK_ATTEMPTS = 10;
const DURABLE_LOCK_RETRY_MS = 1000;
const DURABLE_LOCK_TTL_MS = 15 * 1000;
...
if (runtimeState.strategyVersion !== STRATEGY_VERSION) {
  ...
  runtimeState.lastSignalMap = new Set();
...
function cleanExpiredGuards() {
  ...
  for (const key of lastSignalMap) {
    // lastSignalMap is a Set; we need to check if it's a timestamp? In code it's a Set of strings, not timestamps.
    // But original had lastSignalMap as a Map? Actually earlier it's defined as new Set(). But in the runtime reset it uses new Map? Inconsistency.
    // We'll keep as is from original: lastSignalMap is a Set used for confirmation keys. No expiration needed.
  }
}
...
function attachRotationFamilyKeys(entryPayload, btcState, runId) {
  ...
  const familyIds = getFeatureCohortKeys(featureRow);
...
function markEntryConfirmation(key) {
  lastSignalMap.add(`CONFIRM_${key}`);
  // Optionally set a timeout to remove it after TTL, but Set doesn't have TTL. We'll keep as is; expiration is handled by cleanup? Not needed.
}
...
// Helper to fetch candles (assuming available globally or imported)
async function fetchCandles(symbol, timeframe, limit) {
  // Placeholder: implement actual fetch or use existing function.
  // In original file there is no fetchCandles defined; we assume it exists or we can mock.
  // For safety, we'll return empty array if not available.
  console.warn("fetchCandles not implemented, returning empty array");
  return [];
}
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        const rotationCheck = await checkTradeSignalAgainstRotation(
        ...
        if (!rotationCheck.allowed) {
          ...
          console.log("TS_ENTRY_BLOCKED_BY_WEEKLY_ROTATION:", JSON.stringify({
            ...
            familyIds: rotationAwareEntryPayload.familyIds?.slice?.(0, 5) || []
          }));
        ...
        console.log("TS_ENTRY_OPENED:", JSON.stringify({
          ...
          familyId: entryPayload.familyId,
          score: Number(c.moveScore || 0),
          rawConfluence,
