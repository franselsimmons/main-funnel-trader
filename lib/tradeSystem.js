...
// - RSI continuation verbeterd via continuationScore/slope fallback
...
const DATA_FETCH_CONCURRENCY = 3;
...
function getRsiContinuationScore({ rsiSignal, isBull, rsiZone, flow, confluence, sniperScore, rr }) {
  if (Number.isFinite(Number(rsiSignal?.continuationScore))) {
    return Number(rsiSignal.continuationScore);
  }

  let score = 0;

  if (flow?.type === "TREND") score += 2;
  if (rr >= 1.05) score += 1;
  if (confluence >= 72) score += 2;
  if (confluence >= 78) score += 1;
  if (sniperScore >= 50) score += 1;
  if (sniperScore >= 60) score += 1;

  const slope3 = Number(rsiSignal?.slope3 || 0);

  if (isBull) {
    if (rsiZone === "MID") score += 1;
    if (rsiZone === "LOWER_1") score += 2;
    if (slope3 >= -0.25) score += 1;
    if (slope3 > 0.5) score += 1;
  } else {
    if (rsiZone === "MID") score += 1;
    if (rsiZone === "UPPER_1") score += 2;
    if (slope3 <= 0.25) score += 1;
    if (slope3 < -0.5) score += 1;
  }
...
function isTrendContinuationRSI({ isBull, rsiZone, rsiSignal, flow, confluence, sniperScore, rr }) {
  if (flow?.type !== "TREND") return false;
  if (rr < 1.05) return false;

  const continuationScore = getRsiContinuationScore({
    rsiSignal,
    isBull,
    rsiZone,
    flow,
  ...
  if (isBull) {
    if (!["MID", "LOWER_1"].includes(rsiZone)) return false;

    return (
      (confluence >= 72 && sniperScore >= 50 && continuationScore >= 5) ||
      (confluence >= 78 && sniperScore >= 45)
    );
  }

  if (!["MID", "UPPER_1"].includes(rsiZone)) return false;
...
function isEarlyRSI({ isBull, rsiZone, sniperScore, rsiSignal }) {
  if (isBull) {
    return (
      rsiZone === "LOWER_1" &&
      (
        sniperScore >= 72 ||
        Boolean(rsiSignal?.rising)
      )
    );
  }
  ...
  return (
    ...
    (
      sniperScore >= 72 ||
      Boolean(rsiSignal?.falling)
    )
  );
}

function isHardRsiExhaustionAgainstSide({ isBull, rsiZone }) {
  if (isBull) {
    return ["UPPER_2", "UPPER_3"].includes(rsiZone);
  }

  return ["LOWER_2", "LOWER_3"].includes(rsiZone);
}
...
export async function processTrades(input, options = {}) {
  ...
  const chunks = chunkArray(candidates, DATA_FETCH_CONCURRENCY);
  ...
  for (const originalCoin of candidates) {
    ...
    if (isHardRsiExhaustionAgainstSide({ isBull, rsiZone })) {
      actions.push(buildWait(
        c,
        isBull ? "RSI_LONG_TOO_HIGH" : "RSI_SHORT_TOO_LOW",
        flow,
        null,
        0,
        0,
    ...
    // ================= RSI LOGIC =================
    const trendContinuationRSI = isTrendContinuationRSI({
      isBull,
      rsiZone,
      rsiSignal,
      flow,
    ...
    if (!isBull) {
      const rsiOK =
        ...
        earlyRSI ||
        trendContinuationRSI ||
        (rsiZone === "LOWER_1" && flow.type === "TREND" && confluence >= 78 && sniperScore >= 55);
    ...
    const rsiMomentumContinuation =
      flow.type === "TREND" &&
      trendContinuationRSI &&
      confluence >= 72 &&
      rr >= 1.05;

    if (!strongMomentum && !softMomentum && !rsiMomentumContinuation) {
    ...
    const position = {
      ...
      rsiZone,
      rsiContinuationScore: getRsiContinuationScore({
        rsiSignal,
        isBull,
        rsiZone,
        flow,
    ...
    const entryPayload = {
      ...
      rsiContinuationScore: position.rsiContinuationScore
    };
