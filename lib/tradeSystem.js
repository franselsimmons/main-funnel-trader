...
const ABSOLUTE_MIN_RR = 0.70;  // vervangt MIN_RR_FLOOR
...
const PREFETCH_BATCH_PAUSE_MS = 700;
...
function getExecutionSymbol(c){
  return String(c?.contractSymbol || `${c?.symbol || ""}USDT`).toUpperCase();
}

async function prefetchMarketData(candidates){
  const obMap = {};
  const fundingMap = {};
  ...
  for(const c of candidates){
    ...
    if(!uniqueBySymbol.has(symbol)){
      uniqueBySymbol.set(symbol, c);
    }
  }

  const uniqueCandidates = Array.from(uniqueBySymbol.values());

  for(let i = 0; i < uniqueCandidates.length; i += PREFETCH_BATCH_SIZE){
    const chunk = uniqueCandidates.slice(i, i + PREFETCH_BATCH_SIZE);

    await Promise.all(
      chunk.map(async (c) => {
        const symbol = String(c.symbol || "").toUpperCase();
        const executionSymbol = getExecutionSymbol(c);

        try{
          const raw = await fetchOrderBook(executionSymbol);
        ...
        try{
          fundingMap[symbol] = await fetchFunding(executionSymbol);
        }catch{
    ...
    if(i + PREFETCH_BATCH_SIZE < uniqueCandidates.length){
      await sleep(PREFETCH_BATCH_PAUSE_MS);
    }
  }
...
// ================= NIEUWE HELPER: DYNAMISCHE RR EIS =================
function getRequiredRR({
  c,
  setupGrade,
  flow,
  confluence,
  ...
  const tfStrength = Number(c?.tfStrength || 0);
  const grade = String(setupGrade?.grade || "C").toUpperCase();
...
export async function processTrades(
  ...
  const {
    obMap,
    fundingMap
  } = await prefetchMarketData(candidates);
  ...
  for(const originalCoin of candidates){
    ...
    // OUDE STATISCHE RR CHECK VERWIJDERD
    // if(rr < MIN_RR_FLOOR){ ... }
    ...
    // ================= NIEUWE RR CHECK (DYNAMISCH) =================
    if(rr < Math.max(requiredRR, ABSOLUTE_MIN_RR)){
      const waitPayload = {
        ...buildWait(c, "LOW_RR", flow, sniper, confluence, rr, funding, ob, riskBase, setupGrade),
        requiredRR: Number(requiredRR).toFixed(2)
      };
      await logAction(waitPayload, regimeLevel, btcState, shouldLog);
      actions.push(waitPayload);
      continue;
    ...
    if(
      ...
      gradeOK &&
      !ob.spoof &&
      rr >= Math.max(requiredRR, ABSOLUTE_MIN_RR)
    ){
      const reasonEntry =
        c.stage === "buildup"
          ? "BUILDUP_ELITE_ENTRY"
