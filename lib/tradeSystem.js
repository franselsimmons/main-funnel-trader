...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  // 🔥 LIMIET: max 6 coins om rate limit te voorkomen
  ...
  // ========== FETCH ALLE DATA PER COIN (met retries en throttling) ==========
  const obMap = {};
  const fundingMap = {};
  const rsiMap = {};
  ...
  await Promise.all(candidates.map(async (c) => {
    ...
    // ---- Orderbook met retry ----
    let orderbookRaw = null;
    for (let retry = 0; retry < 2; retry++) {
      try {
        orderbookRaw = await fetchOrderBook(symbol + "USDT");
        if (orderbookRaw) break;
      } catch {}
      await new Promise(r => setTimeout(r, 200));
    }
    if (orderbookRaw) {
      const analyzed = analyzeOrderBookAdvanced(orderbookRaw);
    ...
    // ---- Candles + RSI + structure (bewaar ook candles voor marketCtx) ----
    try {
      const pair = c.rawBitgetSymbol || symbol;
      const [c15m, c1h, c5m] = await Promise.all([
        fetchCandles(pair, "15m", 150),
        fetchCandles(pair, "1h", 150),
        fetchCandles(pair, "5m", 150)
      ]);

      // RSI context fallback bij lege candles
      const rsiContextM15 = c15m.length ? getAdvancedRSIContext(c15m) : { rsi: 50 };
      const rsiContextH1 = c1h.length ? getAdvancedRSIContext(c1h) : { rsi: 50 };
      // STRUCTURE: UNKNOWN vervangen door NEUTRAL zodat trade niet geblokkeerd wordt
      const structureState = c5m.length ? getStructureState(c5m) : { trend: "NEUTRAL" };
      ...
      console.warn(`RSI/structure mislukt voor ${symbol}:`, e.message);
      rsiMap[symbol] = null;
    }
  }));
