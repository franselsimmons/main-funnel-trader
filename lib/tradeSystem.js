...
function sanitizePrimitive(value) {
  ...
  // For arrays/objects, return null to avoid nested parsing errors
  return null;
}
...
function buildAnalysisWebhookEvent(action, { runId, btcState, index }) {
  ...
  return {
    eventId: `ts_${STRATEGY_VERSION}_${runId}_${index}_${eventType}_${normalizeBaseSymbol(action?.symbol)}_${String(action?.side || "x")}_${String(action?.reason || action?.action || "x")}`,
    eventType,
    source: "TRADE_SYSTEM",
    strategyVersion: STRATEGY_VERSION,
    runId,
  ...
  try {
    const res = await fetch(ANALYSIS_WEBHOOK_URL, {
      ...
      headers: {
        ...
        // Support meerdere checks aan analyse-kant
        ...
        // Optional fallback als je endpoint Bearer auth gebruikt
    ...
    console.warn("TS_ANALYSIS_WEBHOOK_ERROR:", JSON.stringify({
      error: isAbort
