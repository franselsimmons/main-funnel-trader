...
function ensureActiveMicroRotation(now = Date.now()) {
  ...
  if (hasValidPreviousWeek) {
    ...
    if (ids.length > 0) {
      ...
      console.log("TS_WEEKLY_MICRO_ROTATION_READY", JSON.stringify({
        ok: true,
        activeWeekKey,
        sourceWeekKey,
        rotationId: state.activeRotation.rotationId,
        activeIds: ids.length,
        longFamilies: state.activeRotation.longFamilies?.length || 0,
        shortFamilies: state.activeRotation.shortFamilies?.length || 0,
        minCompleted: MICRO_ROTATION_MIN_COMPLETED,
        top: [
          ...(state.activeRotation.longFamilies || []),
          ...(state.activeRotation.shortFamilies || [])
        ].slice(0, MICRO_ROTATION_HEALTH_TOP_FAMILIES).map(row => ({
          familyId: row.familyId,
          side: row.side,
          completed: row.completed,
          wins: row.wins,
          losses: row.losses,
          winrate: row.winrateNum,
          fairWinrate: row.fairWinrateNum,
          wilson: row.winrateLowerBoundNum,
          bayes: row.bayesianWinrateNum,
          avgR: row.avgR,
...
// ================= NIEUWE FUNCTIES VOOR PARENT FALLBACK EN STABLE MICRO FAMILY =================
import crypto from 'crypto';

const MICRO_RE = /^MICRO_(LONG|SHORT)_(LONG|SHORT)_(\d+)(?:_|$)/;

export const getParentFamilyId = (familyId = '') => {
  const match = String(familyId).match(MICRO_RE);
  if (!match) return null;

  const [, rotationSide, , code] = match;
  return `${rotationSide}_${code}`;
};

export const getSignalFamilyIds = (signal = {}) => {
  return [
    signal.microFamilyId,
    signal.familyId,
...
export const hasScoreBucketAtLeast = (familyIds, minBucket = 95) => {
  if (minBucket <= 65) return familyIds.some(id => /SCORE_(65|75|85|95)_PLUS/.test(id));
  if (minBucket <= 75) return familyIds.some(id => /SCORE_(75|85|95)_PLUS/.test(id));
  if (minBucket <= 85) return familyIds.some(id => /SCORE_(85|95)_PLUS/.test(id));
  return familyIds.some(id => id.includes('SCORE_95_PLUS'));
};

export const isParentFallbackEligible = ({ signal, candidate, familyIds }) => {
  const setupClass = signal?.setupClass ?? candidate?.setupClass;
  const reason = signal?.reason ?? candidate?.reason;
  const score = Number(candidate?.score ?? signal?.score ?? 0);

  if (setupClass !== 'GOD') return false;
  if (reason !== 'GOD_ENTRY') return false;

  if (score > 0) return score >= 95;

  return hasScoreBucketAtLeast(familyIds, 95);
};

export const resolveWeeklyMicroRotationGate = ({
  signal,
  candidate,
  rotation,
  config = {},
  ...
  if (!strictGate) {
    return { allowed: true, reason: 'ROTATION_GATE_DISABLED' };
  }

  if (!rotation?.canOpenNewEntries) {
    return {
      allowed: false,
      reason: rotation?.blockReason ?? 'ROTATION_BLOCKED',
    };
  }

  const activeMicroFamilyIds = new Set(
    rotation.activeMicroFamilyIds ??
    rotation.allowedMicroFamilyIds ??
    rotation.selectedIds ??
    []
  );

  if (activeMicroFamilyIds.size === 0) {
    return {
      allowed: Boolean(allowEmptyBootstrap),
      reason: allowEmptyBootstrap
        ? 'EMPTY_ROTATION_BOOTSTRAP_ALLOWED'
        : 'EMPTY_ROTATION_BLOCKED',
    };
  }

  const familyIds = getSignalFamilyIds(signal);
  const exactMatch = familyIds.find(id => activeMicroFamilyIds.has(id));

  if (exactMatch) {
    return {
      allowed: true,
      reason: 'MICRO_FAMILY_EXACT_MATCH',
      matchedFamilyId: exactMatch,
    };
  }

  const signalParents = new Set(
    familyIds.map(getParentFamilyId).filter(Boolean)
  );

  const activeParents = new Set(
    [...activeMicroFamilyIds].map(getParentFamilyId).filter(Boolean)
  );

  const parentMatch = [...signalParents].find(parentId => activeParents.has(parentId));

  if (!parentMatch) {
    return {
      allowed: false,
      reason: 'NO_PARENT_FAMILY_MATCH',
      checkedFamilyIds: familyIds.slice(0, 8),
      activeMicroFamilyIds: [...activeMicroFamilyIds],
    };
  }
  ...
  if (!parentFallback) {
    return {
      allowed: false,
      reason: 'REAL_MICRO_FAMILY_NOT_IN_ACTIVE_ROTATION',
      matchedParentFamilyId: parentMatch,
      parentFallbackEnabled: false,
      checkedFamilyIds: familyIds.slice(0, 8),
      activeMicroFamilyIds: [...activeMicroFamilyIds],
    };
  }

  if (!isParentFallbackEligible({ signal, candidate, familyIds })) {
    return {
      allowed: false,
      reason: 'PARENT_MATCH_BUT_SIGNAL_TOO_WEAK',
      matchedParentFamilyId: parentMatch,
      checkedFamilyIds: familyIds.slice(0, 8),
    };
  }
  ...
  return {
    allowed: true,
    reason: 'PARENT_FALLBACK_HIGH_QUALITY_GOD_ENTRY',
    matchedParentFamilyId: parentMatch,
  };
};

export const WEEKLY_MICRO_ROTATION_CONFIG = {
  strictGate: true,
  parentFallback: true,
  allowEmptyBootstrap: false,
};
...
const bucketSide = side => side === 'bear' ? 'SHORT' : 'LONG';

const bucketScore = score => {
  const n = Number(score ?? 0);
  if (n >= 95) return 'SCORE_95_PLUS';
  if (n >= 85) return 'SCORE_85_PLUS';
  if (n >= 75) return 'SCORE_75_PLUS';
  if (n >= 65) return 'SCORE_65_PLUS';
  return 'SCORE_LOW';
};

const bucketLiquidity = ({ spreadBucket, depthBucket }) => {
  if (spreadBucket === 'TIGHT' && depthBucket === 'DEEP') return 'LIQ_STRONG';
  if (depthBucket === 'BAD') return 'LIQ_BAD';
  if (spreadBucket === 'WIDE') return 'LIQ_WEAK';
  return 'LIQ_OK';
};

export const buildStableMicroFamilyId = ({
  side,
  parentCode,
  setupClass,
  reason,
  ...
  const parts = [
    ...
    setupClass ?? 'NA',
    reason ?? 'NA',
    rsiRelationBucket ?? 'RSI_NA',
    marketRegimeBucket ?? 'REGIME_NA',
    bucketLiquidity(liquidity ?? {}),
    bucketScore(score),
  ];

  const suffix = stableHash(parts.join('|'));

  return `MICRO_${dir}_${dir}_${parentCode}_MF_V2_CORE_${suffix}`;
};
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    console.log("TS_WEEKLY_ROTATION_SELECTED_FOR_GATE:", JSON.stringify(selectedRotationHealth));
    console.log("TS_MICRO_ROTATION_GATE_HEALTH:", JSON.stringify(selectedRotationHealth));
    ...
    if (rotationBlocks.length > 0) {
      console.log("TS_ROTATION_BLOCK_SUMMARY", JSON.stringify(
        summarizeRotationBlocks(rotationBlocks)
      ));
    }
