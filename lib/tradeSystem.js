...
const GRADE_B_MIN_RR_FLOOR = 1.10;   // AANGEPAST van 1.15
const GRADE_C_MIN_RR_FLOOR = 1.20;   // AANGEPAST van 1.25
...
function getDynamicMinConf({ c, ob, flow, vol, isBull }) {
  let minConf = 45; // AANGEPAST van 48/52

  if(ob.bias === "NEUTRAL") minConf += 1;
  if(flow.type === "BUILDING") minConf += 1;
  if(c.tfStrength < 2) minConf += 0;   // AANGEPAST (was +1)
  if(vol === "HIGH") minConf += 0;     // AANGEPAST (was +1)
...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    // AANGEPAST: OB NEUTRAL blokkeert alleen nog bij lage confluence
    if(
      ob.bias === "NEUTRAL" &&
      confluence < 50 &&
    ...
    // AANGEPAST: && -> || en default true (was false)
    const rsiOk = rsiData
      ? (
