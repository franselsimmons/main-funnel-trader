...
function normalizeBitgetSymbol(symbol) {
  ...
  // Bitget USDT perpetual expects _UMCBL suffix
...
async function fetchCandles(symbol, timeframe = "1h", limit = 150) {
  ...
  try {
    ...
    if (res.status === 429) {
      console.warn("⚠️ BITGET RATE LIMIT:", clean);
      return [];
    }
...
export async function processTrades(coins, btc = null, mode = "auto", scannerRegime = null, options = {}) {
  ...
  // LIMIT COINS (performance)
  candidates = candidates.slice(0, 6);

  const actions = [];
  const market = await getMarketContext();

  // ========== CENTRALE MARKET DATA (1x per coin) ==========
  const marketDataMap = new Map();

  const CHUNK_SIZE = 3; // max 3 coins tegelijk
  for (let i = 0; i < candidates.length; i += CHUNK_SIZE) {
    const chunk = candidates.slice(i, i + CHUNK_SIZE);
    await Promise.all(chunk.map(async (c) => {
      const symbol = c.symbol.toUpperCase();
      const rawBitgetSymbol = c.rawBitgetSymbol || symbol;
      const pair = rawBitgetSymbol; // wordt genormaliseerd in fetchCandles

      // Haal alles parallel met caching
      const [candles5m, candles15m, candles1h, obRaw, funding] = await Promise.all([
        cachedFetch(`c5_${pair}`, () => fetchCandles(pair, "5m"), 30000),
        cachedFetch(`c15_${pair}`, () => fetchCandles(pair, "15m"), 30000),
        cachedFetch(`c1h_${pair}`, () => fetchCandles(pair, "1h"), 30000),
        cachedFetch(`ob_${symbol}`, async () => {
          const raw = await fetchOrderBook(symbol + "USDT");
          return analyzeOrderBookAdvanced(raw);
        }, 5000),
        cachedFetch(`fund_${symbol}`, () => fetchFunding(symbol + "USDT"), 30000)
      ]);

      const ob = { ...DEFAULT_OB, ...(obRaw || {}), fetchFailed: !obRaw };
      const rsiContextM15 = getAdvancedRSIContext(candles15m);
      const rsiContextH1 = getAdvancedRSIContext(candles1h);
      const structureState = getStructureState(candles5m);
      ...
      marketDataMap.set(symbol, {
        ...
        ob,
        funding,
        rsi: { m15: rsiContextM15, h1: rsiContextH1 },
        structure: structureState
      });
    }));
    // Pauze tussen chunks om rate limit te voorkomen
    if (i + CHUNK_SIZE < candidates.length) await new Promise(r => setTimeout(r, 400));
  }
  ...
  for (const originalCoin of candidates) {
    ...
    const marketData = marketDataMap.get(c.symbol);
    if (!marketData) continue;

    const ob = marketData.ob;
    ...
    // Markt context voor risk manager (geen extra fetches)
    const marketCtx = {
      candles5m: marketData.candles5m,
      candles15m: marketData.candles15m,
      candles1h: marketData.candles1h
    };
    const riskBase = await calculateRisk(c, ob, liquidity, liquidation, marketCtx);
    ...
    const structure = marketData.structure || { trend: "UNKNOWN" };
...
// ================= CONSTANTEN (moeten bovenaan staan na imports) =================
