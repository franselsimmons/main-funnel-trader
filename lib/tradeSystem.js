...
function tsRowPassesBasePreset(row, preset) {
  ...
  if (
    ...
    preset.SHORT_BLOCKED_RSI_ZONES.includes(row.rsiZone)
  ) {
    return false;
  }
  ...
  if (
    row.side === "bear" &&
    row.rsiZone === "LOWER_1" &&
    !preset.SHORT_LOWER1_ALLOWED_BTC_STATES.includes(row.btcState)
  ) {
    return false;
  }
...
function buildTsPatchLinesFromPreset(preset) {
  return tsMasterFilterKeys().map(key => {
    if (key === "SHORT_BLOCKED_RSI_ZONES") {
      return `const SHORT_BLOCKED_RSI_ZONES = new Set(${JSON.stringify(preset[key])});`;
    }

    if (key === "SHORT_LOWER1_ALLOWED_BTC_STATES") {
      return `const SHORT_LOWER1_ALLOWED_BTC_STATES = new Set(${JSON.stringify(preset[key])});`;
    }
