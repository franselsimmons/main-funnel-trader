...
export async function processTrades(input, options = {}) {
  ...
  for (const originalCoin of candidates) {
    ...
    // 🔒 HARD BLOCK: cooldown check helemaal bovenaan
    if (Date.now() < (symbolCooldownMap.get(c.symbol) || 0)) {
      continue;
    }

    // 🔒 HARD BLOCK: als er al een positie open staat voor exact deze kant → skip volledig
    if (memory.has(key)) {
      continue;
    }

    // 🔒 HARD BLOCK: blokkeer ook de andere kant (geen long+short tegelijk)
    ...
    // 1. STRONG trend → volledig blokkeren tegenrichting
    if (btcState === "STRONG_BULL" && !isBull) {
      continue; // GEEN SHORTS
    }
    if (btcState === "STRONG_BEAR" && isBull) {
      continue; // GEEN LONGS
    }

    // 2. Normale BULL/BEAR → alleen hoge score laten door
    if (btcState === "BULLISH" && !isBull && c.moveScore < 75) {
      continue;
    }
    if (btcState === "BEARISH" && isBull && c.moveScore < 75) {
      continue;
    }

    // 3. (Optioneel) NEUTRAL BTC → alleen super setups
    if (btcState === "NEUTRAL" && c.moveScore < 80) {
      continue;
    }
    // =================================================
    ...
    // ===== B (vangt alles op wat geen A is) =====
    const bSetupValid =
      stageOK &&
      setupGrade.grade === "B" &&
      !ob.spoof &&
      rr >= 1.05;   // minimale basis

    const bEntry =
      !eliteEntry &&               // 👈 sleutel: alleen als het geen A is
      bSetupValid &&
      sniper.score >= 62 &&
      confluence >= 66 &&
      rr >= 1.05 &&
    ...
    console.log(`🔍 ${c.symbol} (${isDeep ? "DEEP" : "LIGHT"}): sniper=${sniper.score}, conf=${confluence}, rr=${rr.toFixed(2)}, grade=${setupGrade.grade}, elite=${eliteEntry}, b=${bEntry}, godmode=${godModeEntry}`);
    ...
    if (prev) {
      ...
      // HOLD verwijderd – geen Discord melding
      memory.set(key, pos);
    ...
    // ❌ OUDE symbolCooldown CHECK VERWIJDERD (nu bovenaan afgehandeld)
    ...
    // ❌ OUDE BTC FILTERS VERWIJDERD (nu bovenaan hard afgehandeld)
    // Hier stonden de regels met btcState === "BULLISH" && !isBull && c.moveScore < 70 etc.
    ...
    // ========== DIRECT ENTRY ==========
    // 🔒 HARD BLOCK: alleen entry als er nog geen positie is voor deze kant (dubbele check)
    // 🔒 HARD BLOCK: extra veiligheid tegen race conditions (laatste entry < 2 min)
    if (shouldEnter && !memory.has(key)) {
      const lastEntry = memory.get(key);
      if (lastEntry && (Date.now() - lastEntry.createdAt) < 120000) {
        continue;
      }
