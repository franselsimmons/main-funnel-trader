// ================= TRADESYSTEM.JS (FULLY PATCHED V11.6) =================
...
// ================= STRATEGY VERSION =================
const STRATEGY_VERSION = "TS_V11_6_RSI_EDGE_SOFT_MTF";
...
async function getSafeRsiSignal({ c, rsiData, isBull, btcState }) {
  ...
  const raw = await callFirstValid(
    "getRSISignal",
    [
      // Correcte native signature
      () => getRSISignal(rsiData?.mtf, c.side, btcState),

      // Backward-compatible object signature
      () => getRSISignal({
        mtf: rsiData?.mtf,
        side: c.side,
        coin: c,
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        // Hard block via RSI-edge (UPPER_3 of LOWER_3 zonder continuation)
        ...
        // Oude harde blocks voor UPPER_2/3 long zonder continuation (edge vangt al het meeste)
        ...
        // Setup classificatie met RR credit
