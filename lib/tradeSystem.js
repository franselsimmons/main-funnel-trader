...
function getDynamicMinRrFloor({
  ...
  if(
    ...
    floor = Math.min(floor, 1.05); // Kleine korting voor de allerbeste trades
  }

  return clamp(floor, 1.05, 1.6);
}
...
export async function processTrades(
  ...
  for(const originalCoin of candidates){
    ...
    // WINRATE EIS: Timeframe Strength moet nu weer strak op minimaal 2 staan voor de bevestiging.
    if(
    ...
    // WINRATE EIS: Confluence absolute bodem is keihard 75
    const minConf = 75;
    ...
    // WINRATE EIS: Sniper validatie keihard 75
    const sniperOK =
      (
        sniper?.valid &&
