...
const MICRO_LEARNING_MAX_FAMILIES_PER_ROW = 5;

const MICRO_ROTATION_TOP_N_PER_SIDE = Number(process.env.MICRO_ROTATION_TOP_N_PER_SIDE || 8);
...
const REDIS_SAFE_MAX_BYTES = 9_000_000;
...
function buildRuntimeCorePayload() {
  ...
  return {
    ...
    auditCounters: {
      ...
      microLearning: auditState.microLearning
    }
  };
}
...
async function saveDurableRuntimeState(runtimeState = {}, options = {}) {
  if (!hasRedis()) return false;

  const strategyVersion =
    options.strategyVersion ??
    runtimeState.strategyVersion ??
    (typeof STRATEGY_VERSION !== "undefined" ? STRATEGY_VERSION : "TS_RUNTIME");

  const baseKey = `${strategyVersion}:runtime`;

  const core = buildDurableCoreState({
    ...runtimeState,
    strategyVersion,
  });

  const debugSizes = {
    core: durableBytes(core),
    openPositions: durableBytes(core.openPositions),
    entries: durableBytes(core.entries),
    exits: durableBytes(core.exits),
    recentEntries: durableBytes(core.recentEntries),
    closedTrades: durableBytes(core.closedTrades),
  };

  console.log("TRADE SYSTEM DURABLE SAVE SIZE:", JSON.stringify(debugSizes));

  await redisSetJsonSafe(`${baseKey}:core`, core);

  // Optioneel apart bewaren, maar nooit in :core.
  const shadowRows = durableTail(
    runtimeState.shadowRows,
    durableNumber(options.maxShadowRows, 250)
  ).map(compactTradeRow);

  if (shadowRows.length > 0) {
    await redisSetJsonSafe(`${baseKey}:shadowRows`, {
      strategyVersion,
  ...
  const featureRows = durableTail(
    runtimeState.featureRows,
    durableNumber(options.maxFeatureRows, 250)
  );

  if (featureRows.length > 0) {
    await redisSetJsonSafe(`${baseKey}:featureRows`, {
      strategyVersion,
      savedAt: core.savedAt,
      rows: featureRows,
  ...
  console.log("TRADE SYSTEM DURABLE SAVE SPLIT:", JSON.stringify({
    strategyVersion,
    openPositions: core.openPositions.length,
    entries: core.entries.length,
    exits: core.exits.length,
    recentEntries: core.recentEntries.length,
    closedTrades: core.closedTrades.length,
    shadowRows: shadowRows.length,
    featureRows: featureRows.length,
    coreBytes: debugSizes.core
  }));

  return {
    ok: true,
    key: `${baseKey}:core`,
    bytes: debugSizes.core,
    counts: core.meta.originalCounts,
  };
}
...
function buildAnalyzerMicroIdentity(row = {}) {
  const side = analyzerSide(row);
...
function enrichRowWithAnalyzerMicroFamily(row = {}) {
  ...
  const identity = buildAnalyzerMicroIdentity(row);
  if (!identity?.microFamilyId) return row;
  ...
  return {
    ...
    analyzerMicroFamilyId: identity.microFamilyId,
    microFamilyId: identity.microFamilyId,
    familyId: identity.microFamilyId,
    ...
    parentFamilyId: identity.parentFamilyId,
    analyzerParentFamilyId: identity.parentFamilyId,

    analyzerDefinition: identity.definition,
    analyzerDefinitionParts: identity.definitionParts,
    definition: row.definition || identity.definition,
    definitionParts: row.definitionParts || identity.definitionParts,

    spreadBps: identity.spreadBps
  };
}
...
function getRowMicroFamilyIds(row) {
  ...
  const existing = [
    ...
    analyzerRow?.microFamilyId,
    analyzerRow?.familyId,
    ...(Array.isArray(analyzerRow?.microFamilyIds) ? analyzerRow.microFamilyIds : []),
    ...(Array.isArray(analyzerRow?.familyIds) ? analyzerRow.familyIds : [])
  ];
  ...
  if (normalizedExisting.length) {
    return Array.from(new Set(normalizedExisting)).slice(0, MICRO_LEARNING_MAX_FAMILIES_PER_ROW);
  }
  ...
  return Array.from(new Set(enriched.microFamilyIds || []))
    .map(normalizeMicroFamilyId)
    .filter(Boolean)
    .slice(0, MICRO_LEARNING_MAX_FAMILIES_PER_ROW);
}

function getOrCreateFamilyStats(summary, familyId, side) {
  if (!summary.families) summary.families = {};
...
function scoreMicroFamily(family) {
  ...
  // avgR is alleen tie-breaker, geen PnL-optimalisatie.
  const score =
    Number(refreshed.fairWinrateNum || 0) * 1000 +
    Number(refreshed.sampleReliability || 0) * 40 +
...
function buildActiveRotationFromWeek(week) {
  const families = Object.values(week?.families || {})
    ...
    .filter(family => Number(family.completed || 0) >= MICRO_ROTATION_MIN_COMPLETED)
    .filter(family => Number(family.rotationScore || 0) >= MICRO_ROTATION_MIN_SCORE)
    .sort((a, b) => {
      const scoreDiff = Number(b.rotationScore || 0) - Number(a.rotationScore || 0);
...
async function finalizeResult(actions, candidates, btcState, runId, options = {}) {
  ...
  const WEBHOOK_MAX_ANALYSIS_ROWS = Number(process.env.WEBHOOK_MAX_ANALYSIS_ROWS ?? 35);
...
async function loadWeeklyRotationForGate(options = {}) {
  ...
  try {
    ...
    if (status.gateEnabled === false) return null;
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    // Herbouw bestaande weekly rotation met de nieuwe fair-winrate ranking.
    refreshActiveMicroRotationFromCompletedWeek(Date.now());
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        const learnedWeeklyRotation =
          weeklyRotationForGate ??
          auditState.microLearning?.activeRotation ??
          null;

        const analyzerEntryPayload = enrichRowWithAnalyzerMicroFamily(entryPayload);
        ...
        const rotationReadySignal = attachMicroRotationKeys(analyzerEntryPayload, {
          ...
          // Sta entries toe zolang er nog geen actieve rotation-lijst is.
          strictWeeklyRotation: false,
          allowBootstrapWhenRotationEmpty: true,

          // Belangrijk: GOD mag de learned winrate allowlist niet omzeilen.
        ...
        if (signalForAction.analyzerMicroFamilyId) {
          signalForAction.microFamilyId = signalForAction.analyzerMicroFamilyId;
          signalForAction.familyId = signalForAction.analyzerMicroFamilyId;
    ...
    try {
      ...
      if (shouldSaveRuntime) {
        await saveDurableRuntimeState(runtimeState, options);
