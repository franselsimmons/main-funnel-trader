...
export async function processTrades(input, options = {}) {
  ...
  // 🔥 ALLEEN LIMITEREN ALS HET EEN SCAN-OBJECT IS (oude fallback)
  // Bij directe array vertrouwen we volledig op de smart selector
  let candidates;
  if (Array.isArray(input)) {
    candidates = candidatesRaw;  // geen slice – selector is al leidend
  } else {
  ...
  if (candidates.length === 0) {
    return { actions: [], message: "Geen entry/almost coins beschikbaar" };
  }
