...
const DEFAULTS = {
  // TradeSystem geeft deze meestal expliciet mee.
  // Defaults staan relaxed zodat deze gate niet onbedoeld je TS-quality funnel overneemt.
  minEntryScore: 0,
  minAlmostScore: 0,
  minConfluence: 0,
  minSniperScore: 0,
  ...
  // true = harde weekly-rotation filtering.
  strictWeeklyRotation: true,

  // false = geen entries wanneer er nog geen actieve micro-family allowlist is.
  allowBootstrapWhenRotationEmpty: false,

  // false = GOD mag actieve weekly micro-family lijst niet omzeilen.
  allowGodSoftPass: false,

  // false = disabled/lege rotation mag door naar bootstrap-logica.
  // true = disabled rotation blokt hard.
  blockWhenRotationDisabled: false,

  // Exact analyzer micro IDs:
  // 1 fine-grained + 4 coarse variants = 5.
  maxFamilyIdsChecked: 5,
...
const getParentFamilyId = signal => {
  const direct = firstDefined(signal, [
    ...
    "filterSnapshot.analyzerParentFamilyId",
...
// Belangrijk:
// Dit bootst de analyzer/tradeSystem microfamily-space na:
// - 1 exacte fine-grained definitie
// - 4 grovere analyzer varianten
// Daardoor kan weekly learning exact matchen zonder parent fallback.
const buildAnalyzerMicroFamilyIdentities = signal => {
  const exact = buildAnalyzerMicroFamilyId(signal);
  if (!exact?.id) return [];

  const side = normalizeRotationSide(signal.rotationSide, normalizeSide(signal.side));
  const parentFamilyId = exact.parentFamilyId;

  const setupClass = normalizeSetupClass(signal.setupClass, signal);
  const stage = normalizeMicroStage(signal);
  const rsi = normalizeRsi(signal);
  const ob = normalizeOb(signal);
  const flow = normalizeFlow(signal);
  const btc = normalizeBtc(signal);

  const scoreBucket = bucketScoreFine(
    firstDefined(signal, [
      "score",
      "moveScore",
      "entryScore",
...
export const getMicroFamilyCandidates = signal => {
  const existing = [
    ...
    signal?.filterSnapshot?.analyzerMicroFamilyId,

    ...(Array.isArray(signal?.analyzerMicroFamilyIds) ? signal.analyzerMicroFamilyIds : []),
    ...
    signal?.rotationCandidate?.primaryFamilyId,
    signal?.rotationCandidate?.analyzerMicroFamilyId,

    ...(Array.isArray(signal?.rotationCandidate?.microFamilyIds) ? signal.rotationCandidate.microFamilyIds : []),
    ...(Array.isArray(signal?.rotationCandidate?.familyIds) ? signal.rotationCandidate.familyIds : []),
  ];
  ...
  const analyzerIdentities = buildAnalyzerMicroFamilyIdentities({
    ...signal,
    side,
    rotationSide,
    stage,
  ...
  const analyzerIds = analyzerIdentities
    .map(item => item.id)
    .filter(Boolean);
...
export const attachMicroRotationKeys = (signal = {}, context = {}) => {
  ...
  const primaryAnalyzer = analyzerIdentities[0] || null;
  const analyzerMicroFamilyIds = analyzerIdentities
    .map(item => item.id)
    .filter(Boolean);

  const analyzerMicroFamilyId = primaryAnalyzer?.id || null;
  ...
  const enrichedBase = {
    ...
    parentFamilyId: signal.parentFamilyId ?? primaryAnalyzer?.parentFamilyId,
    analyzerParentFamilyId: signal.analyzerParentFamilyId ?? primaryAnalyzer?.parentFamilyId,
    ...
    microDefinition: primaryAnalyzer?.definition,
    microDefinitionParts: primaryAnalyzer?.definitionParts,
    analyzerDefinition: primaryAnalyzer?.definition,
    analyzerDefinitionParts: primaryAnalyzer?.definitionParts,
  };
  ...
  const microFamilyId = isRealMicroFamilyId(analyzerMicroFamilyId)
    ? cleanKey(analyzerMicroFamilyId)
    : (
  ...
  const weekKey = cleanKey(
    context.weekKey ??
      ...
      context.rotation?.weekKey ??
      context.rotationState?.weekKey ??
  ...
  return {
    ...
    rotationCandidate: {
      ...
      setupClass,
      reason,
      parentFamilyId: primaryAnalyzer?.parentFamilyId,
      analyzerParentFamilyId: primaryAnalyzer?.parentFamilyId,
      analyzerMicroFamilyId,
      analyzerMicroFamilyIds,
      microFamilyId,
      microFamilyIds,
...
const flattenFamilyIds = (value, wantedRotationSide = "UNKNOWN") => {
  ...
  const direct = [
    ...
    ...(Array.isArray(value.rotationMicroFamilyIds) ? value.rotationMicroFamilyIds : []),
    ...
    ...(Array.isArray(value.analyzerMicroFamilyIds) ? value.analyzerMicroFamilyIds : []),
...
export const extractRotationMicroFamilyIds = (rotation = {}, side = "unknown") => {
...
const getRotationCheckFamilyIds = ({
  enriched,
  hasRealMicroAllowlist,
  maxFamilyIdsChecked,
}) => {
  const candidates = uniq([
    ...
    enriched.familyId,
    enriched.primaryFamilyId,

    ...(Array.isArray(enriched.analyzerMicroFamilyIds) ? enriched.analyzerMicroFamilyIds : []),
    ...(Array.isArray(enriched.microFamilyIds) ? enriched.microFamilyIds : []),
    ...(Array.isArray(enriched.microFamilies) ? enriched.microFamilies : []),
    ...(Array.isArray(enriched.familyIds) ? enriched.familyIds : []),
    ...(Array.isArray(enriched.families) ? enriched.families : []),

    enriched.rotationCandidate?.microFamilyId,
    enriched.rotationCandidate?.analyzerMicroFamilyId,
    enriched.rotationCandidate?.familyId,
    enriched.rotationCandidate?.primaryFamilyId,

    ...(Array.isArray(enriched.rotationCandidate?.analyzerMicroFamilyIds)
      ? enriched.rotationCandidate.analyzerMicroFamilyIds
      : []),

    ...(Array.isArray(enriched.rotationCandidate?.microFamilyIds)
      ? enriched.rotationCandidate.microFamilyIds
      : []),

    ...(Array.isArray(enriched.rotationCandidate?.familyIds)
      ? enriched.rotationCandidate.familyIds
      : []),
  ]);

  const filtered = hasRealMicroAllowlist
    ? candidates.filter(isRealMicroFamilyId)
    : candidates.filter(id => !isBroadFallbackFamilyId(id));

  return filtered.slice(0, maxFamilyIdsChecked);
};
...
export const checkTradeSignalAgainstRotation = async (signal = {}, context = {}) => {
  ...
  const checkedMicroFamilyIds = getRotationCheckFamilyIds({
    enriched,
    hasRealMicroAllowlist,
    maxFamilyIdsChecked,
  });

  const matchedMicroFamilyId = checkedMicroFamilyIds.find(id =>
    activeSet.has(cleanKey(id))
  );
  ...
  if (!options.strictWeeklyRotation) {
    return allow("WEEKLY_ROTATION_SOFT_ALLOW_NOT_STRICT", enriched, {
      activeMicroFamilyIds: limitArray(activeMicroFamilyIds, options.maxActiveFamilyIdsReturned),
      realActiveMicroFamilyIds: limitArray(realActiveMicroFamilyIds, options.maxActiveFamilyIdsReturned),
      checkedMicroFamilyIds,
      quality,
