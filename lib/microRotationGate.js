...
const getParentFamilyId = signal => {
  ...
  const directKey = cleanKey(direct ?? "");

  if (/\b(LONG|SHORT)_\d{1,3}\b/.test(directKey)) {
    return directKey.match(/\b(LONG|SHORT)_\d{1,3}\b/)?.[0] || directKey;
  }

  const familyKey = cleanKey(signal?.familyId ?? signal?.filterSnapshot?.familyId ?? "");

  if (/\b(LONG|SHORT)_\d{1,3}\b/.test(familyKey)) {
    return familyKey.match(/\b(LONG|SHORT)_\d{1,3}\b/)?.[0] || familyKey;
  }
...
const getRotationMeta = rotation => {
  ...
  return {
    ...
    bootstrap: Boolean(active.bootstrap),
...
export const checkTradeSignalAgainstRotation = async (signal = {}, context = {}) => {
  ...
  // Cruciaal:
  // Zodra de analyzer echte MICRO_* families actief heeft, wordt MF_* fallback genegeerd.
  ...
  const checkedMicroFamilyIds = Array.isArray(enriched.microFamilyIds)
    ? enriched.microFamilyIds
        .filter(id => !hasRealMicroAllowlist || isRealMicroFamilyId(id))
  ...
  if (matchedMicroFamilyId) {
    return allow(
      ...
      {
        matchedMicroFamilyId,
        activeMicroFamilyIds: activeMicroFamilyIds.slice(0, options.maxActiveFamilyIdsReturned),
        realActiveMicroFamilyIds: realActiveMicroFamilyIds.slice(0, options.maxActiveFamilyIdsReturned),
        checkedMicroFamilyIds,
        quality,
        rotationMeta,
        bootstrap: false,
