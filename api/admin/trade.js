...
function isoWeekKeyUtc(value) {
const ts = normalizeTimestampMs(value, Date.now());
...
const context = temporalAdminContextFromRecord(row, 'entry', fallbackTs);
return {
...context,
temporalContextVersion: TEMPORAL_CONTEXT_VERSION,
marketEventClusterId: context.entryTs === null
? null
: temporalAdminMarketEventClusterId(row, context.entryTs)
};
}
function buildExitTemporalContext(row = {}, fallbackTs = null) {
return temporalAdminContextFromRecord(
row,
'exit',
fallbackTs === null ? Date.now() : fallbackTs
);
}
...
shortRows.map((position) => position.trueMicroFamilyId ||
position.microFamilyId)
);
const discordEligiblePositions = shortRows.filter((position) =>
position.discordAlertEligible);
const selectedMicroFamilyPositions = shortRows.filter((position) =>
position.selectedMicroFamilyAlert);
const invalidRiskShapePositions = shortRows.filter((position) =>
!position.shortRiskShapeValid);
return {
...modeFlags(),
...
|| p.macroFamilyId),
byTrueMicroFamily: countBy(shortRows, (p) => p.trueMicroFamilyId ||
p.microFamilyId),
byMacroFamily: countBy(shortRows, (p) => p.parentTrueMicroFamilyId ||
p.macroFamilyId),
byMicroFamily: countBy(shortRows, (p) => p.trueMicroFamilyId ||
p.microFamilyId),
bySide: {
...
const waitActions = allShortActions.filter((action) => action.action ===
'WAIT');
...
allShortActions.map((action) => action.trueMicroFamilyId ||
...
rotation.macroIds,
shortRows.map((row) => row.parentTrueMicroFamilyId || row.macroFamilyId ||
row.parentMacroFamilyId)
])
...
res.setHeader('X-Admin-Trade-Mode', 'short-only-virtual-learning-75-child-truemicro-v1');
