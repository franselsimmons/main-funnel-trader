...
'SHORT_ADAPTIVE_UI_MARKETWEATHER_CURRENTFIT_MEASUREMENT_GATE_EMPIRICAL_VETO_RANKING_TEMPORAL_V4';
...
function normalizeRequestedTradeSide(value) {
  ...
  if (['SHORT', 'BEAR', 'BEARISH', 'SELL', 'DOWN'].includes(raw)) {
    return TARGET_TRADE_SIDE;
  }
  if (['LONG', 'BULL', 'BULLISH', 'BUY', 'UP'].includes(raw)) {
...
function getProfitFactor(row = {}) {
  ...
  if (winR <= 0 && lossR <= 0) {
    const temporalRoot = row.temporalStats && typeof row.temporalStats === 'object'
      ? row.temporalStats
      : row.temporalFamilyStats && typeof row.temporalFamilyStats === 'object'
        ? row.temporalFamilyStats
        : {};
    const dayType = temporalRoot.dayType && typeof temporalRoot.dayType === 'object'
      ? temporalRoot.dayType
      : row.dayTypeStats && typeof row.dayTypeStats === 'object'
        ? row.dayTypeStats
        : row.contextStats && typeof row.contextStats === 'object'
          ? row.contextStats
          : {};
    const buckets = [dayType.WEEKDAY, dayType.WEEKEND]
      .filter((bucket) => bucket && typeof bucket === 'object');
    for (const bucket of buckets) {
      const lifetime = bucket.lifetimeStats && typeof bucket.lifetimeStats === 'object'
        ? bucket.lifetimeStats
        : bucket;
      winR += Math.max(0, num(lifetime.grossWinR ?? lifetime.positiveR, 0));
      lossR += Math.abs(num(lifetime.grossLossR ?? lifetime.negativeR, 0));
    }
  }
...
const bearishPct = bearishPctDirect ?? ratioCountToPct(
     firstFiniteNumber([
       ...
       nestedBreadth.shortCount,
       nestedBreadth.downCount
     ]),
     universeCount
);
const bullishPct = bullishPctDirect ?? ratioCountToPct(
     firstFiniteNumber([
       ...
       nestedBreadth.longCount,
       nestedBreadth.upCount
     ]),
     universeCount
);
...
function marketWeatherSourcePriority(row = {}) {
    ...
    if (row.sourceKey === SHORT_MARKET_WEATHER_KEY) score += 8;
    if (row.sourceKey === MARKET_WEATHER_KEY) score += 5;
    if (row.sourceKey === SHORT_MARKET_UNIVERSE_KEY) score += 3;
    if (row.sourceKey === MARKET_UNIVERSE_KEY) score += 2;
    if (row.sourceKey === SHORT_TRADE_RUN_META_KEY) score += 1;
    return score;
}
...
if (bullishPct !== null && bullishPct >= 60) {
    score -= 20;
...
'none',
      entry: riskGeometry.entry,
      entryPrice: riskGeometry.entry,
      initialSl: riskGeometry.initialSl,
      sl: riskGeometry.initialSl,
      tp: riskGeometry.tp,
      exitPrice: riskGeometry.exitPrice,
      currentPrice: riskGeometry.currentPrice,
      riskTradeSide: TARGET_TRADE_SIDE,
...
function parseFilters(req) {
    ...
    const side = normalizeRequestedTradeSide(firstQueryValue(requestQuery.side,
TARGET_TRADE_SIDE));
    const familyId = String(firstQueryValue(requestQuery.familyId, '') || '').trim();
    const macroFamilyId = String(firstQueryValue(requestQuery.macroFamilyId, '') ||
'').trim();
    const parentTrueMicroFamilyId =
String(firstQueryValue(requestQuery.parentTrueMicroFamilyId, '') || '').trim();
    const setup = upper(firstQueryValue(requestQuery.setup, ''));
    const regime = upper(firstQueryValue(requestQuery.regime, ''));
    const confirmationProfile =
upper(firstQueryValue(requestQuery.confirmationProfile, ''));
    const q = String(firstQueryValue(requestQuery.q, '') || '').trim().toUpperCase();
    return {
         side,
         familyId,
         macroFamilyId,
         ...
         confirmationProfile,
         q,
         activeOnly: isTrue(firstQueryValue(requestQuery.activeOnly, false)),
         macroActiveOnly: isTrue(firstQueryValue(requestQuery.macroActiveOnly, false)),
         includeEmpty: firstQueryValue(requestQuery.includeEmpty, null) === null
              ? true
              : isTrue(firstQueryValue(requestQuery.includeEmpty, true)),
         minCompleted: num(firstQueryValue(requestQuery.minCompleted, 0), 0),
         minSample: num(firstQueryValue(requestQuery.minSample, 0), 0),
         minSeen: num(firstQueryValue(requestQuery.minSeen, 0), 0),
         tier: String(firstQueryValue(requestQuery.tier, '') || '').trim().toUpperCase(),
         status: String(firstQueryValue(requestQuery.status, '') ||
'').trim().toUpperCase(),
         currentFit: String(firstQueryValue(requestQuery.currentFit, '') ||
'').trim().toUpperCase()
    };
}
...
function forcedShortFallbackRows(activeRotation, existingRows = [], marketWeather
= null) {
...
export default async function handler(req, res) {
 ...
 res.setHeader('X-Admin-Micro-Families-Mode', 'short-only-75-child-current-measurement-currentfit-temporal-v7');
...
PERSISTENT_LEARNING_KEY
     ...
     const requestedMode = String(firstQueryValue(requestQuery.mode, 'adaptive') ||
'adaptive');
     const mode = normalizeMode(requestedMode);
     const requestedLimitRaw = firstQueryValue(requestQuery.limit, DEFAULT_LIMIT);
     ...
     const sideLimit = toSafeLimit(
          firstQueryValue(
               requestQuery.sideLimit,
               firstQueryValue(requestQuery.sideEnsureLimit, DEFAULT_SIDE_LIMIT)
          ),
          DEFAULT_SIDE_LIMIT,
          MAX_SIDE_LIMIT
     );
     const bestLimit = toSafeLimit(
          firstQueryValue(requestQuery.bestLimit, DEFAULT_BEST_LIMIT),
          DEFAULT_BEST_LIMIT,
          MAX_BEST_LIMIT
     );
     const includeActiveRotation =
isTrue(firstQueryValue(requestQuery.includeActiveRotation, false));
   const details = isTrue(firstQueryValue(requestQuery.details, false));
   const compactRaw = firstQueryValue(requestQuery.compact, null);
...
marketWeather);
   ...
   if (usedForcedShortFallback) {
       const fallbackShortRows = forcedShortFallbackRows(activeRotation,
...
compact);
   const normalizedShortRows = normalizeRows(split.shortRows, activeSet,
...
'UNKNOWN'],
     ...
     empiricalVetoPolicy: {
          ...
          appliesTo: 'EXACT_75_CHILD_SHORT_TRUE_MICRO_FAMILIES_ONLY',
          measurementFixVersion: MEASUREMENT_FIX_VERSION,
          minCompleted: EMPIRICAL_VETO_MIN_COMPLETED,
          maxAvgR: EMPIRICAL_VETO_MAX_AVG_R,
          observingRule:
...
'learningQualityRank|adaptiveScore|balancedScore|fairWinrate|totalR|avgR|avgCostR',
          ...
          perf: {
               ...
               path: 'shortOnly75ChildCurrentMeasurementGateTriggerBoundaryFillCurrentFitStrictDiscordRankingTemporalV7',
