...
// ================= ANALYSIS WEBHOOK / DISCOVERY MODE =================
const ENABLE_ANALYSIS_WEBHOOK = true;

const ANALYSIS_WEBHOOK_URL = process.env.ANALYSIS_WEBHOOK_URL || "";
...
const ANALYSIS_WEBHOOK_CONCURRENCY = Number(
  process.env.ANALYSIS_WEBHOOK_CONCURRENCY || 3
);

const ANALYSIS_WEBHOOK_TIMEOUT_MS = Number(
  process.env.ANALYSIS_WEBHOOK_TIMEOUT_MS || 20000
);
// Discovery mode:
...
// ================= WEBHOOK SANITIZATION HELPERS (PATCHED) =================
const ANALYSIS_WEBHOOK_PAYLOAD_MAX_CHARS = Number(
  process.env.ANALYSIS_WEBHOOK_PAYLOAD_MAX_CHARS || 120000
);
...
function sanitizeWebhookPayload(payload) {
  ...
  for (const [key, value] of Object.entries(payload)) {
    out[key] = truncateWebhookString(value);
  }
...
function buildWrappedAnalysisWebhookBody(event) {
  const payload = sanitizeWebhookPayload(event?.payload || {});
  const eventType = safeWebhookString(event?.eventType || payload.eventType || "SNAPSHOT", "SNAPSHOT");
  const reason = getWebhookReason(payload);

  return {
    eventId: safeWebhookString(event?.eventId),
    eventType,
    source: safeWebhookString(event?.source || "TRADE_SYSTEM", "TRADE_SYSTEM"),
    strategyVersion: safeWebhookString(event?.strategyVersion || STRATEGY_VERSION, STRATEGY_VERSION),
    runId: safeWebhookString(event?.runId),
    tradeId: safeWebhookString(event?.tradeId),

    // Flat compatibility fields
    symbol: safeWebhookString(payload.symbol || event?.symbol || "UNKNOWN", "UNKNOWN"),
    side: safeWebhookString(payload.side || event?.side || "unknown", "unknown"),
    action: safeWebhookString(payload.action || event?.action || eventType, eventType),
    reason: safeWebhookString(reason, "UNKNOWN"),

    // Existing wrapped schema, but every nested value is string-safe
    payload
  };
}

function buildFlatAnalysisWebhookBody(event) {
  const wrapped = buildWrappedAnalysisWebhookBody(event);
  ...
  const flat = {
    ...
    strategyVersion: wrapped.strategyVersion,
    runId: wrapped.runId,
    tradeId: wrapped.tradeId,
    ...
    // JSON string, not object, so endpoints that trim all fields will not crash
    payloadJson: truncateWebhookString(payload, ANALYSIS_WEBHOOK_PAYLOAD_MAX_CHARS)
  };

  return sanitizeWebhookPayload(flat);
}

// ================= ANALYSIS WEBHOOK HELPERS =================
...
function buildAnalysisTradeId(action, runId, index) {
  ...
  if (action?.action === "ENTRY") {
    return `${STRATEGY_VERSION}_${symbol}_${side}_${createdAt}`;
  }

  return `${STRATEGY_VERSION}_${symbol}_${side}_${runId}_${index}`;
}

function buildAnalysisWebhookEvent(action, { runId, btcState, index }) {
  const eventType = getAnalysisEventType(action);
  const tradeId = buildAnalysisTradeId(action, runId, index);
  ...
  const rawPayload = {
    ...
    // Objects worden JSON strings via sanitizer
    filterValues: action?.filterValues || getDiscordFilterValuesSnapshot(),
    currentFilterValues: CURRENT_FILTER_VALUES,
    tradeSystemFilters: TRADE_SYSTEM_FILTERS,

    discoveryMode: DISCOVERY_MODE,
    analysisSchema: "TS_DISCOVERY_V1",
  ...
  const reason = safeWebhookString(
    payload.rejectReason || payload.exitReason || payload.entryReason || payload.reason || action?.reason || "UNKNOWN",
    "UNKNOWN"
  );
  ...
  try {
    const res = await fetch(ANALYSIS_WEBHOOK_URL, {
      ...
      headers: {
        ...
        "x-webhook-secret": ANALYSIS_WEBHOOK_SECRET,
        "x-trade-webhook-secret": ANALYSIS_WEBHOOK_SECRET,
        "x-analysis-webhook-secret": ANALYSIS_WEBHOOK_SECRET,

        Authorization: `Bearer ${ANALYSIS_WEBHOOK_SECRET}`
      },
      body,
...
async function sendAnalysisWebhookEvent(event) {
  ...
  const wrappedBody = buildWrappedAnalysisWebhookBody(event);
  let result = await postAnalysisWebhookBody(wrappedBody);

  const trimTypeError =
    result.status === 400 &&
    /trim is not a function/i.test(String(result.text || ""));

  if (!result.ok && trimTypeError) {
    const flatBody = buildFlatAnalysisWebhookBody(event);

    console.warn("TS_ANALYSIS_WEBHOOK_RETRY_FLAT_STRING_BODY:", JSON.stringify({
      status: result.status,
      eventType: wrappedBody.eventType,
      symbol: wrappedBody.symbol,
      side: wrappedBody.side,
      reason: wrappedBody.reason,
      response: String(result.text || "").slice(0, 300)
    }));

    result = await postAnalysisWebhookBody(flatBody);
  }

  const sentBody = result.bodyObj || wrappedBody;
...
async function sendAnalysisActions(actions, { runId, btcState }) {
  ...
  // Filter out HOLD actions unless explicitly enabled via environment variable
  const rows = Array.isArray(actions)
    ? actions.filter(action => {
        ...
        if (a === "HOLD" && process.env.ANALYSIS_WEBHOOK_SEND_HOLDS !== "true") {
          return false;
        }
...
function finalizeResult(actions, candidates, btcState, runId, options = {}) {
  ...
  void sendAnalysisActions(finalActions, {
    runId,
    btcState
  }).catch(e => {
  ...
  try {
    ...
    for (const originalCoin of candidates) {
      ...
      try {
        ...
        // ================= PATCH 1: split baseRR and finalRR gates =================
