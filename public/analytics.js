...
function renderGlobalAdvice(data){
  ...
  if(!global.length){
    ...
    el("globalAdviceContent").innerHTML = `
      ...
      Analyse gebruikt alleen echte filter-coins. UI fallback telt niet mee voor systeemadvies.
    `;
...
// ================= TRADE SYSTEM ANALYSIS FALLBACK =================
function buildTradeSystemFallback(data){
  ...
  }else if(pct(entries.length, total) > 25){
    advice = "Veel entries. Kwaliteit bewaken, eventueel iets strenger.";
  }else if(pct(entries.length, total) < 3 && total >= 10){
  ...
  return {
    ...
    exits: exits.length,
    entryRate: pct(entries.length, total),
    waitRate: pct(waits.length, total),
    avgConfluence: avg("confluence"),
    avgRR: avg("rr"),
    avgScore: avg("score"),
    waitReasons,
    advice
...
function renderTradeSystemAnalysis(data){
  ...
  const rows = waitReasons.length
    ...
    : `
      <tr>
        <td colspan="4">Geen blokkades deze scan.</td>
      </tr>
    `;
  ...
  box.innerHTML = `
    ...
    <div class="metric-grid">
      <div class="metric-box">
        ...
        <strong>${ts.total || 0}</strong>
      </div>

      <div class="metric-box">
        <span>Entries</span>
        <strong>${ts.entries || 0}</strong>
      </div>

      <div class="metric-box">
        <span>Waits</span>
        <strong>${ts.waits || 0}</strong>
      </div>

      <div class="metric-box">
        <span>Entry Rate</span>
        <strong>${ts.entryRate || 0}%</strong>
      </div>

      <div class="metric-box">
        <span>Avg Confluence</span>
        <strong>${ts.avgConfluence || 0}</strong>
      </div>

      <div class="metric-box">
        <span>Avg RR</span>
        <strong>${ts.avgRR || 0}</strong>
      </div>
    </div>
